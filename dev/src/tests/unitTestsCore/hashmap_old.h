...
{
	template< typename K >
	Uint32 genericHashFunction_old( const K & key )
	{
		return GetHash( key );
	}
}

// Hashmap statistics to be retrieved via THashMap_old::GetStats()
struct THashmapStats
{
	Uint32 m_size;					// Number of elements stored in hash map
 ...
 */
template < typename K, typename V, typename EqualFunc = DefaultEqualFunc< K >, EMemoryClass MC_Type = MC_HashMap >
class THashMap_old
{
private:
...
public:
	...
	{
	private:
		friend class THashMap_old;

		THashMap_old*	m_map;
		Uint32		m_bucketIndex;
		TElement*	m_element;

		RED_FORCE_INLINE iterator( THashMap_old* map, Uint32 bucketIndex, TElement* element )
			: m_map( map )
		...
		RED_FORCE_INLINE iterator( THashMap_old* map )
			: m_map( map )
			, m_bucketIndex( 0 )
			, m_element ( nullptr )
		{
	...
	private:
		...
		const THashMap_old*	m_map;
		Uint32			m_bucketIndex;
		const TElement*	m_element;

		RED_FORCE_INLINE const_iterator( const THashMap_old* map, Uint32 bucketIndex, const TElement* element )
			: m_map( map )
			, m_bucketIndex( bucketIndex )
			, m_element ( element )
		{
		...
		RED_FORCE_INLINE const_iterator( const THashMap_old* map )
			: m_map( map )
			, m_bucketIndex( 0 )
			, m_element ( nullptr )
		{
	...
	RED_FORCE_INLINE THashMap_old( Uint32 initialCapacity = 0 )
		: m_buckets( nullptr )
		, m_size( 0 )
		, m_capacity( 0 )
	...
	RED_FORCE_INLINE THashMap_old( const THashMap_old& other )
		: m_buckets( nullptr )
		, m_size( 0 )
		, m_capacity( 0 )
		, m_autoDownsize( false )
	...
	RED_FORCE_INLINE THashMap_old( THashMap_old&& other )
		: m_buckets( nullptr )
		, m_size( 0 )
		, m_capacity( 0 )
		, m_autoDownsize( false )
	...
	THashMap_old& operator = ( const THashMap_old& other )
	{
		if ( this == &other )
		{
			return *this;
	...
	THashMap_old& operator = ( THashMap_old&& other )
	{
		if ( this == &other )
		{
			return *this;
	...
	Bool Insert( const K& key, const V& value )
	{
		...
		const Uint32 hash = genericHashFunction_old( key );
	...
	void Clear()
	{
		if ( m_capacity )
		{
			...
			RED_MEMORY_FREE( MemoryPool_Default, MC_Type, m_pool.GetMemory() );
			m_capacity = 0;
		}
	}
	...
	void Rehash( Uint32 newCapacity )
	{
		...
		void* newMemory = RED_MEMORY_ALLOCATE( MemoryPool_Default, MC_Type, newMemorySize );
...
// 	}
	...
	RED_FORCE_INLINE Bool operator == ( const THashMap_old& other ) const
	{
		if ( Size() != other.Size() )
		{
			return false;
	...
	// Serialization
	friend IFile& operator << ( IFile& file, THashMap_old& map )
	{
		if ( file.IsReader() )
		{
			map.Clear();
	...
	void GetStats( THashmapStats& stats )
	{
		stats.m_size = m_size;
...
private:
	...
	{
		return const_cast< const THashMap_old< K, V, EqualFunc, MC_Type >* >( this )->FindInternal( key, bucketIndexOut, const_cast< const TElement*& >( elementOut ) );
	}
...
template < typename K, typename V, typename EqualFunc, EMemoryClass MC_Type >
typename THashMap_old< K, V, EqualFunc, MC_Type >::iterator begin( THashMap_old< K, V, EqualFunc, MC_Type >& map ) { return map.Begin(); }

template < typename K, typename V, typename EqualFunc, EMemoryClass MC_Type >
typename THashMap_old< K, V, EqualFunc, MC_Type >::iterator end( THashMap_old< K, V, EqualFunc, MC_Type >& map ) { return map.End(); }

template < typename K, typename V, typename EqualFunc, EMemoryClass MC_Type >
typename THashMap_old< K, V, EqualFunc, MC_Type >::const_iterator begin( const THashMap_old< K, V, EqualFunc, MC_Type >& map ) { return map.Begin(); }

template < typename K, typename V, typename EqualFunc, EMemoryClass MC_Type >
typename THashMap_old< K, V, EqualFunc, MC_Type >::const_iterator end( const THashMap_old< K, V, EqualFunc, MC_Type >& map ) { return map.End(); }
