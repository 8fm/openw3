...
class CDrawsIndexedTest : public CBaseTest
{
public:
	CDrawsIndexedTest( String& name )
		: CBaseTest( name )
	{
	}

	virtual ~CDrawsIndexedTest(){};
	...
	virtual Bool Execute( CTestUtils& context )
	{
		...
		for( Uint32 i = 0; i < context.GetObjects().Size(); ++i )
		{
			...
			bufPS.m_surfaceColor = context.GetObjects()[(i+2)%context.GetObjects().Size()].Color;
			...
			GpuApi::DrawIndexedPrimitive( GpuApi::PRIMTYPE_TriangleList, 0, 0, context.GetObjects()[i].NumVertices, 0, context.GetObjects()[i].NumIndices / 3 );
