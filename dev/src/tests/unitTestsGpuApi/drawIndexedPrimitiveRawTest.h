...
#ifndef _DRAW_INDEXED_PRIMITIVE_RAW_TEST_H_
#define _DRAW_INDEXED_PRIMITIVE_RAW_TEST_H_

#include "testEngine.h"

class CDrawIndexedPrimitiveRawTest : public CBaseTest
{
public:
	CDrawIndexedPrimitiveRawTest( String& name )
		: CBaseTest( name )
	{
	}
	virtual ~CDrawIndexedPrimitiveRawTest(){};
	...
	virtual Bool Execute( CTestUtils& context )
	{
		...
		for( Uint32 i = 0; i < context.GetObjects().Size(); ++i )
		{
			...
			buf.m_surfaceColor = context.GetObjects()[(i+3)%context.GetObjects().Size()].Color;
			...
			GpuApi::BindVertexBuffers( 0, 1, &context.GetObjects()[i].VertexBufferIndexed, &m_stride, &m_offset );
			GpuApi::BindIndexBuffer( context.GetObjects()[i].IndexBuffer );

			GpuApi::DrawIndexedPrimitiveRaw( GpuApi::PRIMTYPE_TriangleList, 0, 0, context.GetObjects()[i].NumVertices, 0, context.GetObjects()[i].NumIndices / 3 );
