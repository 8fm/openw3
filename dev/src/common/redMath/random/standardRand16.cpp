...
#if RAND_MAX == RED_STANDARD_RAND_MAX16
...
{
	return RED_STANDARD_RAND_MAX16;
}

template<>
Uint32 StandardRand::Max() const
{
	return ( RED_STANDARD_RAND_MAX16 << 15 ) | RED_STANDARD_RAND_MAX16;
}
...
{
	return ( static_cast< Uint64 >( Max< Int32 >() ) << 30 ) | Max< Int32 >();
}
...
{
	// Since the max value is 0x7ffff, the 16th bit is unused, hence why we only shift by 15
	return ( static_cast< Uint32 >( Get< Uint16 >() ) << 15 ) | Get< Uint16 >();
}
...
{
	// Since the max value is 0x7ffff, the 16th bit is unused, hence why we only shift by 15
	return ( static_cast< Int32 >( Get< Int16 >() ) << 15 ) | Get< Int16 >();
}
...
#endif // RAND_MAX == RED_STANDARD_RAND_MAX16
