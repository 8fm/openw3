...
IMPLEMENT_ENGINE_CLASS( CMaterialParameterTextureArray );

CMaterialParameterTextureArray::CMaterialParameterTextureArray()
{
}

IProperty* CMaterialParameterTextureArray::GetParameterProperty() const
{
...
void CMaterialParameterTextureArray::OnRebuildSockets()
{
...
CodeChunk CMaterialParameterTextureArray::Compile( CMaterialBlockCompiler& compiler, EMaterialShaderTarget shaderTarget, CMaterialOutputSocket* socket ) const
{
	// Create dynamic data that will bind value
	CodeChunk parameterName;

	IMaterialShaderCompiler& shader = compiler.GetPS();
	...
	EMaterialSamplerType samplerType = MST_TextureArray;
