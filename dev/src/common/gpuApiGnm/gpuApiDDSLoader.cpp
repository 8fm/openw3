...
#define ISBITMASK( r,g,b,a ) ( ddpf.RBitMask == r && ddpf.GBitMask == g && ddpf.BBitMask == b && ddpf.ABitMask == a )
	...
	//--------------------------------------------------------------------------------------
	static TextureRef CreateTextureResource( Uint32 resDim,
		size_t width,
		size_t height,
		size_t depth,
		size_t mipCount,
		...
		Bool isCubeMap,
		const TextureInitData* initData )
	{
		...
		case D3D11_RESOURCE_DIMENSION_TEXTURE2D:
			{
				...
				desc.initLevels = static_cast<Uint16>( mipCount );
				desc.sliceNum = static_cast<Uint16>( arraySize );
				desc.format = format;
				desc.usage = TEXUSAGE_Samplable;
				if ( isCubeMap )
				{
					desc.type = TEXTYPE_CUBE;
				}
				else
				{
					desc.type = TEXTYPE_2D;
				}
			}
			break;
		...
		case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
			{
				...
				desc.format = format;
				desc.usage = TEXUSAGE_Samplable;
				desc.type = TEXTYPE_ARRAY;
			}
			break; 
		}

		desc.usage |= TEXUSAGE_Immutable;

		return CreateTexture( desc, TEXG_Generic, initData );
	}


	//--------------------------------------------------------------------------------------

	static TextureRef CreateTextureFromDDS( const DDS_HEADER* header, const Uint8* bitData, size_t bitSize, size_t maxsize )
	{
		Bool result = true;
		...
		if ( result )
		{
			...
			retRef = CreateTextureResource( resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize,
				texFormat, isCubeMap, &initData );
		}
