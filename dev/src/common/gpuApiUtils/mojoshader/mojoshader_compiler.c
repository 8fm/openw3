...
#pragma warning( push )
#pragma warning( disable:4510 ) //'add_type_coercion::<unnamed-tag>' : default constructor could not be generated
#pragma warning( disable:4610 ) //struct 'add_type_coercion::<unnamed-tag>' can never be instantiated - user defined constructor required
#pragma warning( disable:4063 ) // case '(smth)' is not a valid value for switch of enum 'Token'
#pragma warning( disable:4127 ) // conditional expression is constant
#pragma warning( disable:4244 ) //'=' : conversion from 'int64' to 'int', possible loss of data
#pragma warning( disable:4244 ) //'argument' : conversion from 'int64' to 'const int', possible loss of data
#pragma warning( disable:4512 ) //'add_type_coercion::<unnamed-tag>' : assignment operator could not be generated
...
#pragma warning( push )
#pragma warning( disable:4127 ) // conditional expression is constant in while(0)
static MOJOSHADER_astExpression *new_identifier_expr(Context *ctx,
													 const char *string)
{
	NEW_AST_NODE(retval, MOJOSHADER_astExpressionIdentifier, MOJOSHADER_AST_OP_IDENTIFIER);
...
{
	NEW_AST_NODE(retval, MOJOSHADER_astExpressionCallFunction, MOJOSHADER_AST_OP_CALLFUNC);
...
static MOJOSHADER_astExpression *new_deref_struct_expr(Context *ctx,
										...
										const char *member)
{
	NEW_AST_NODE(retval, MOJOSHADER_astExpressionDerefStruct, MOJOSHADER_AST_OP_DEREF_STRUCT);
	retval->datatype = NULL;
											...
											MOJOSHADER_astScalarOrArray *soa)
{
	...
	if (!soa->isarray)
	{
		...
		if (isconst)
		{
			retval->type = (MOJOSHADER_astDataTypeType)((int)retval->type | MOJOSHADER_AST_DATATYPE_CONST); //retval->type |= MOJOSHADER_AST_DATATYPE_CONST;
		}
		else
		{
			retval->type = (MOJOSHADER_astDataTypeType)((int)retval->type & ~MOJOSHADER_AST_DATATYPE_CONST); //retval->type &= ~MOJOSHADER_AST_DATATYPE_CONST;
	...
	if (!ok)
		...
		retval->array.elements = (int)data.value.i;
											...
											const int len)
{
	...
	char buf[32];
	snprintf(buf, sizeof (buf), strlen("%s%d"), "%s%d", typestr, len);
...
static void destroy_context(Context *ctx)
{
	if (ctx != NULL)
	{
		...
		if (ctx->ir != NULL)
		{
			for (int i = 0; i <= ctx->user_func_index; i++)
...
static void init_builtins(Context *ctx)
{
	...
	for (i = 0; i < STATICARRAYLEN(types); i++)
	{
		...
		for (j = 1; j <= 4; j++)
		{
			...
			len = snprintf(buf, sizeof (buf), strlen( "%s%d" ), "%s%d", types[i].str, j);
			push_usertype(ctx, stringcache_len(ctx->strcache, buf, len), dt);
			for (k = 1; k <= 4; k++)
			{
				// "float2x2"
				dt = new_datatype_matrix(ctx, types[i].datatype, j, k);
				len = snprintf(buf, sizeof (buf), strlen("%s%dx%d"), "%s%dx%d", types[i].str,j,k);
				push_usertype(ctx, stringcache_len(ctx->strcache,buf,len), dt);
			} // for
		} // for
	} // for
...
static void *build_ir_internal(Context *ctx, void *_ast)
{
	...
	switch (ast->ast.type)
	{
		...
		case MOJOSHADER_AST_OP_FLOAT_LITERAL:
			return new_ir_constfloat(ctx, (float)ast->floatliteral.value);
