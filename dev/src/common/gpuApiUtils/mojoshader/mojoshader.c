...
static inline char *StrDup(Context *ctx, const char *str)
{
    ...
    if (retval != NULL)
        strcpy_s(retval, strlen(str)+1, str);
    return retval;
} // StrDup
...
static void floatstr(Context *ctx, char *buf, size_t bufsize, float f,
                     int leavedecimal)
{
    const size_t len = snprintf(buf, bufsize, strlen("%f"), "%f", f);
    if ((len+2) >= bufsize)
                                           ...
                                           size_t regnum_size)
{
    ...
    if (has_number)
        snprintf(regnum_str, regnum_size, strlen("%u"), "%u", (uint) regnum);
    else
        regnum_str[0] = '\0';
                                                 ...
                                                 char *buf, size_t buflen)
{
    ...
    snprintf(buf, buflen, strlen("%s%s%s%s%s%s%s%s%s%s"), "%s%s%s%s%s%s%s%s%s%s",
                                           ...
                                           const size_t len)
{
    ...
    snprintf(buf,len, strlen("%s%s"), "%s%s", regtype_str, regnum_str);
    return buf;
...
{
    char buf[64];
    snprintf(buf, sizeof (buf), strlen("c_array_%d_%d"), "c_array_%d_%d", base, size);
    return StrDup(ctx, buf);
...
static void emit_D3D_start(Context *ctx, const char *profilestr)
{
    ...
    if (minor == 0xFF)
        strcpy_s(minor_str, strlen("sw")+1, "sw");
    else if ((major > 1) && (minor == 1))
        strcpy_s(minor_str, strlen("x")+1, "x");  // for >= SM2, apparently this is "x". Weird.
    else
        snprintf(minor_str, sizeof (minor_str), strlen("%u"), "%u", (uint) minor);
...
{
    char op[16];
    snprintf(op, sizeof (op), strlen("break%s"), "break%s", get_D3D_comparison_string(ctx));
    emit_D3D_opcode_ss(ctx, op);
} // emit_D3D_BREAKC
...
{
    char op[16];
    snprintf(op, sizeof (op), strlen("if%s"), "if%s", get_D3D_comparison_string(ctx));
    emit_D3D_opcode_ss(ctx, op);
} // emit_D3D_IFC
...
{
    char op[16];
    snprintf(op, sizeof (op), strlen("setp%s"), "setp%s", get_D3D_comparison_string(ctx));
    emit_D3D_opcode_dss(ctx, op);
} // emit_D3D_SETP
...
static void emit_D3D_DCL(Context *ctx)
{
    ...
    else
    {
        ...
        if (index != 0)
            snprintf(index_str, sizeof (index_str), strlen("%u"), "%u", (uint) index);
    } // else
...
static const char *get_BYTECODE_varname(Context *ctx, RegisterType rt, int regnum)
{
    ...
    const char *regtype_str = get_D3D_register_string(ctx, rt, regnum,
                                              regnum_str, sizeof (regnum_str));
    char buf[64];
    snprintf(buf, sizeof (buf), strlen("%s%s"), "%s%s", regtype_str, regnum_str);
    return StrDup(ctx, buf);
                                           ...
                                           const size_t len)
{
    ...
    const char *regtype_str = get_GLSL_register_string(ctx, rt, regnum,
                                              regnum_str, sizeof (regnum_str));
    snprintf(buf,len,strlen("%s_%s%s"), "%s_%s%s", ctx->shader_type_str, regtype_str, regnum_str);
    return buf;
                                                ...
                                                char *buf, const size_t buflen)
{
    ...
    snprintf(buf, buflen, strlen("%s_const_array_%d_%d"), "%s_const_array_%d_%d", type, base, size);
    return buf;
...
static inline const char *get_GLSL_input_array_varname(Context *ctx,
                                                char *buf, const size_t buflen)
{
    snprintf(buf, buflen, strlen("%s"), "%s", "vertex_input_array");
    return buf;
                                                  ...
                                                  char *buf, const size_t len)
{
    ...
    snprintf(buf, len, strlen("%s_uniforms_%s"), "%s_uniforms_%s", shadertype, type);
    return buf;
                                            ...
                                            const char *fmt, ...)
{
    ...
    if (arg->result_mod & MOD_SATURATE)
    {
        ...
        {
            snprintf(clampbuf, sizeof (clampbuf), strlen(", vec%d(0.0), vec%d(1.0))"), ", vec%d(0.0), vec%d(1.0))", vecsize, vecsize);
            clampright = clampbuf;
        } // else
    } // if
    ...
    va_list ap;
    va_start(ap, fmt);
    const int len = vsnprintf_valist(operation, sizeof (operation), fmt, ap);
    va_end(ap);
    ...
    snprintf(buf, buflen, strlen("%s_%s%s%s = %s%s%s%s%s%s;"), "%s_%s%s%s = %s%s%s%s%s%s;",
                                           ...
                                           const size_t buflen)
{
    *buf = '\0';
    ...
    if (arg->relative)
    {
        ...
        else
        {
            ...
            if (arg->relative_array->constant)
            {
                ...
                if (offset != 0)
                    snprintf(rel_offset, sizeof (rel_offset), strlen("%d + "), "%d + ", offset);
            } // if
            else
            {
                ...
                if (offset == 0)
                {
                    snprintf(rel_offset, sizeof (rel_offset),
                             strlen("ARRAYBASE_%d + "), "ARRAYBASE_%d + ", arrayidx);
                } // if
                else
                {
                    snprintf(rel_offset, sizeof (rel_offset),
                             strlen("(ARRAYBASE_%d + %d) + "), "(ARRAYBASE_%d + %d) + ", arrayidx, offset);
                } // else
            } // else
        } // else
    ...
    snprintf(buf, buflen, strlen("%s%s%s%s%s%s%s%s%s"), "%s%s%s%s%s%s%s%s%s",
                                ...
                                int flags)
{
    ...
    else if (shader_is_pixel(ctx))
    {
        ...
        if (regtype == REG_TYPE_COLOROUT)
        {
            ...
            {
                snprintf(index_str, sizeof (index_str), strlen("%u"), "%u", (uint) regnum);
                              ...
                              const char *extra)
{
    ...
    if (vecsize != 1)
    {
        snprintf(castleft, sizeof (castleft), strlen("vec%d("), "vec%d(", vecsize);
        castright = ")";
    } // if
...
static void emit_GLSL_DP2ADD(Context *ctx)
{
    ...
    char extra[64]; snprintf(extra, sizeof (extra), strlen(" + %s"), " + %s", src2);
                                                  ...
                                                  const size_t buflen)
{
    ...
    snprintf(buf, buflen, strlen("scratch%d"), "scratch%d", scratch);
    return buf;
...
static inline const char *get_ARB1_branch_label_name(Context *ctx, const int id,
                                                char *buf, const size_t buflen)
{
    snprintf(buf, buflen, strlen("branch_label%d"), "branch_label%d", id);
    return buf;
...
static inline const char *get_ARB1_const_array_varname_in_buf(Context *ctx,
                                                ...
                                                char *buf, const size_t buflen)
{
    snprintf(buf, buflen, strlen("c_array_%d_%d"), "c_array_%d_%d", base, size);
    return buf;
                                                  ...
                                                  char *buf, size_t buflen)
{
    ...
    snprintf(buf, buflen, strlen("%s%s%s%s%s%s%s"), "%s%s%s%s%s%s%s", regtype_str, regnum_str,
    ...
    snprintf(buf, buflen, strlen("%s%s%s%s%s%s%s%s%s%s"), "%s%s%s%s%s%s%s%s%s%s", premod_str,
                                            ...
                                            const size_t buflen)
{
    ...
    *buf = '\0';
    ...
    if (ctx->predicated)
    {
        ...
        //pred_left = "(";
        //pred_right = ") ";
        //make_ARB1_srcarg_string_in_buf(ctx, &ctx->predicate_arg, pred, sizeof (pred));
    } // if

    snprintf(buf, buflen, strlen("%s%s %s%s%s"), "%s%s %s%s%s", pp_str, sat_str,
             regtype_str, regnum_str, writemask_str);
    // !!! FIXME: make sure the scratch buffer was large enough.
    return buf;
...
static void emit_ARB1_REP(Context *ctx)
{
    ...
    else if (support_nv2(ctx))
    {
        ...
        char scratch[32];
        snprintf(scratch, sizeof (scratch), strlen("rep%d"), "rep%d", ctx->reps);
...
static void emit_ARB1_MOVA(Context *ctx)
{
    ...
    else
    {
        ...
        make_ARB1_srcarg_string(ctx, 0, src0, sizeof (src0));
        allocate_ARB1_scratch_reg_name(ctx, scratch, sizeof (scratch));
        snprintf(addr, sizeof (addr), strlen("addr%d"), "addr%d", ctx->dest_arg.regnum);
                                 ...
                                 const int setvariables, CtabData *ctab)
{
    ...
    if ((constantinfo + (constants * CINFO_SIZE)) >= bytes)
	{
		fail(ctx, "Shader has corrupt CTAB data");
		return;
	}
    ...
    if (!parse_ctab_string(start, bytes, target))
	{
		fail(ctx, "Shader has corrupt CTAB data");
		return;
	}
    ...
    while (opcode_count--)
    {
        ...
        while (operand_count--)
        {
            ...
            switch (SWAP32(fxlc.tokens[1]))
            {
                ...
                {
                    int i;
                    for (i = 0; i < (const int)output_map_count; i++)
                    {
                              ...
                              MOJOSHADER_malloc m, MOJOSHADER_free f, void *d)
{
    ...
    ctx->endline_len = (int)strlen(ctx->endline);
    ctx->last_address_reg_component = -1;
