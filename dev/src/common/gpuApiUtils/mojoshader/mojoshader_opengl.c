...
{
	snprintf(error_buffer, sizeof (error_buffer), strlen("%s"), "%s", str);
} // set_error
...
#pragma warning( push )
#pragma warning( disable:4267 ) // conversion from 'size_t' to 'GLsizei', possible loss of data
...
static int impl_ARB1_CompileShader(const MOJOSHADER_parseData *pd, GLuint *s)
{
	...
	{ 
		...
		snprintf(error_buffer, sizeof (error_buffer),
				  strlen( "ARB1 compile error at position %d: %s"),
										...
										void *malloc_d)
{
	...
	ctx->free_fn = f;
	ctx->malloc_data = malloc_d;
	snprintf(ctx->profile, sizeof (ctx->profile), strlen("%s"), "%s", profile);
											 ...
											 unsigned int vec4n)
{
	...
	const uint maxregs = (const uint)program->vs_preshader_reg_count;
	if (idx < maxregs)
	{
		assert(sizeof (GLfloat) == sizeof (float));
		const uint cpy = (minuint(maxregs - idx, vec4n) * sizeof (*data)) * 4;
											 ...
											 unsigned int vec4n)
{
	...
	const uint maxregs = (const uint)program->ps_preshader_reg_count;
	if (idx < maxregs)
	{
		assert(sizeof (GLfloat) == sizeof (float));
		const uint cpy = (minuint(maxregs - idx, vec4n) * sizeof (*data)) * 4;
...
void MOJOSHADER_glProgramReady(void)
{
		 ...
		 (program->generation != ctx->generation))
	{
		...
		if ((program->texbem_count) && (program->fragment))
		{
			...
			for (int _i = 0; _i < samp_count; _i++)
			{
				if (samps[_i].texbem)
				{
					assert(samps[_i].index > 0);
					assert(samps[_i].index <= MAX_TEXBEMS);
					memcpy(dstf, &ctx->texbem_state[6 * (samps[_i].index-1)],
						   sizeof (GLfloat) * 6);
					dstf[6] = 0.0f;
					dstf[7] = 0.0f;
					dstf += 8;
