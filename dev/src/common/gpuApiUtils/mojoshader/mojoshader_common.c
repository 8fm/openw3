...
int stringmap_insert(StringMap *smap, const char *key, const char *value)
{
	...
	int rc = -1;
	char *k = (char *) smap->m((int)strlen(key) + 1, smap->d);
	char *v = (char *) (value ? smap->m((int)strlen(value) + 1, smap->d) : NULL);
	int failed = ( (!k) || ((!v) && (value)) );

	if (!failed)
	{
		strcpy_s(k, strlen(key)+1, key);
		if (value != NULL)
			strcpy_s(v, strlen(value)+1, value);
...
{
	return stringcache_len(cache, str, (unsigned int)strlen(str));
} // stringcache
...
{
	const uint8 hash = (uint8)(hash_string(str, len) & (cache->table_size-1));
...
{
	return (stringcache_len_internal(cache, str, (unsigned int)strlen(str), 0) != NULL);
...
const char *stringcache_fmt(StringCache *cache, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
#ifdef __clang__
	len = vsnprintf_valist(buf, sizeof (buf), fmt, ap);
#else
	len = vsnprintf_valist(buf, sizeof (buf), strlen(fmt), fmt, ap);
#endif
	va_end(ap);
	...
	if (len > sizeof (buf))
	{
		...
		va_start(ap, fmt);
#ifdef __clang__
		vsnprintf_valist(ptr, len, fmt, ap);
#else
		vsnprintf_valist(ptr, len, strlen(fmt), fmt, ap);
#endif
		va_end(ap);
	} // if
					 ...
					 const int errpos, const char *fmt, va_list va)
{
	...
	if (_fname != NULL)
	{
		fname = (char *) list->m((int)strlen(_fname) + 1, list->d);
		if (fname == NULL)
		{
			list->f(error, list->d);
			return 0;
		} // if
		strcpy_s(fname, strlen(_fname)+1, _fname);
	} // if

	char scratch[128];
	va_list ap;
	va_copy(ap, va);
	const int len = vsnprintf_valist(scratch, sizeof (scratch), fmt, ap);
	va_end(ap);
	...
	if (len < sizeof (scratch))
		strcpy_s(failstr, strlen(scratch)+1, scratch);  // copy it over.
	else
	{
		va_copy(ap, va);
#ifdef __clang__
		vsnprintf_valist(failstr, len + 1, fmt, ap);  // rebuild it.
#else
		vsnprintf_valist(failstr, len + 1, strlen(fmt), fmt, ap);  // rebuild it.
#endif
		va_end(ap);
	} // else
...
char *buffer_reserve(Buffer *buffer, const size_t len)
{
	...
	BufferBlock *item = (BufferBlock *) buffer->m((int)malloc_len, buffer->d);
	if (item == NULL)
		return NULL;
...
int buffer_append_va(Buffer *buffer, const char *fmt, va_list va)
{
	...
	va_copy(ap, va);
#ifdef __clang__
	vsnprintf_valist(buf, len + 1, fmt, ap);  // rebuild it.
#else
	vsnprintf_valist(buf, len + 1, strlen(fmt), fmt, ap);  // rebuild it.
#endif
	va_end(ap);
...
{
	char *retval = (char *) buffer->m((int)buffer->total_bytes + 1, buffer->d);
	if (retval == NULL)
		return NULL;
	BufferBlock *item = buffer->head;
	char *ptr = retval;
...
char *buffer_merge(Buffer **buffers, const size_t n, size_t *_len)
{
	...
	char *retval = (char *) (first ? first->m((int)len + 1, first->d) : NULL);
	if (retval == NULL)
	{
		*_len = 0;
		return NULL;
