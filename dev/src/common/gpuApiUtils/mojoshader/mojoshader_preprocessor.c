...
static void failf(Context *ctx, const char *fmt, ...)
{
    ...
    va_list ap;
    va_start(ap, fmt);
    vsnprintf_valist(ctx->failstr, sizeof (ctx->failstr), fmt, ap);
    va_end(ap);
} // failf
...
#ifdef _MSC_VER
//#define WIN32_LEAN_AND_MEAN 1 // already defined in redSystem/os.h
...
#ifdef RED_PLATFORM_WINPC // mojoshader won't be used anywhere else except PC, so it's safe to only calculate file size on PC
	const DWORD fileSize = GetFileSize(handle, NULL);
#else
	const DWORD fileSize = 0;
#endif
...
static const Define *find_define(Context *ctx, const char *sym)
{
    ...
    else if ( (hash == linestrhash) && (ctx->line_macro) && (strcmp(sym, "__LINE__") == 0) )
    {
        ...
        const size_t len = snprintf(str, bufsize, strlen("%u"), "%u", state->line);
                            ...
                            MOJOSHADER_malloc m, MOJOSHADER_free f, void *d)
{
    ...
    if ((okay) && (define_count > 0))
    {
        ...
        define_include_len = (unsigned int)buffer_size(predefbuf);
...
#pragma warning( disable:4701 ) // potentially uninitialized local variable 'incltype' used
static void handle_pp_include(Context *ctx)
{
    IncludeState *state = ctx->include_stack;
    Token token = lexer(state);
    MOJOSHADER_includeType incltype = MOJOSHADER_INCLUDETYPE_LOCAL;
    char *filename = NULL;
    int bogus = 0;
...
static void handle_pp_error(Context *ctx)
{
    ...
    strcpy_s(ctx->failstr, strlen(prefix)+1, prefix);
    avail -= (int)prefixlen;
    ptr += (int)prefixlen;
                                  ...
                                  const Define *params)
{
    ...
    if (!push_source(ctx, state->filename, def->definition,
                     (unsigned int)strlen(def->definition), state->line, NULL))
    {
        buffer_destroy(buffer);
        return 0;
    } // if
    ...
    state = ctx->include_stack;
    if (!push_source(ctx, state->filename, final, (unsigned int)strlen(final), state->line,
                     close_define_include))
    {
        Free(ctx, final);
        return 0;
    ...
    while (paren > 0)
    {
        ...
        while (1)
        {
            ...
            else if (t == TOKEN_IDENTIFIER)
            {
                ...
                {
                    expr = def->definition;
                    exprlen = (unsigned int)strlen(def->definition);
                } // if
            } // else if
...
static int handle_pp_identifier(Context *ctx)
{
    ...
    return push_source(ctx, fname, def->definition, (unsigned int)deflen, line, NULL);
...
static int reduce_pp_expression(Context *ctx)
{
    ...
    int error = 0;
    const long val = interpret_rpn(output, (int)outputsize, &error);
                                             ...
                                             unsigned int *_len, Token *_token)
{
    ...
    while (1)
    {
        if (ctx->isfail)
        {
            ...
            *_len = (unsigned int)strlen(ctx->failstr);
            return ctx->failstr;
        } // if
                             ...
                             MOJOSHADER_malloc m, MOJOSHADER_free f, void *d)
{
    ...
    retval->output_len = (int)total_bytes;
    retval->malloc = m;
    retval->free = f;
