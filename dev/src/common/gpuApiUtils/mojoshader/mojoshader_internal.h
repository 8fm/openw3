...
#define snprintf _snprintf_s  // !!! FIXME: not a safe replacement!
#define vsnprintf_valist _vsnprintf_s
...
#define snprintf snprintf_wrapper  // !!! FIXME: not a safe replacement!
#define vsnprintf_valist vsnprintf_s
#define strcasecmp stricmp
...
static inline int snprintf_wrapper( char *str, size_t n, size_t dummy, const char *format, ... )
{
	...
	va_start(ap, format);
	r = vsnprintf_valist(str, n, format, ap);
	va_end(ap);
