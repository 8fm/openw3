...
#pragma warning( push )
#pragma warning( disable:4127 ) // warning C4127: conditional expression is constant in while (1)
...
static void output_comment_bytes(Context *ctx, const uint8 *buf, size_t len)
{
	...
	{
		const uint32 tokencount = ((uint32)len / 4) + (((uint32)len % 4) ? 1 : 0);
...
static int check_token_segment(Context *ctx, const char *str)
{
	...
	if ( (ctx->tokenlen < len) || (strncasecmp(ctx->token, str, len) != 0) )
		return 0;
	ctx->token += (uint32)len;
	ctx->tokenlen -= (uint32)len;
	return 1;
} // check_token_segment
...
static void parse_version_token(Context *ctx)
{
	...
	ctx->major_ver = (uint8)major;
	ctx->minor_ver = (uint8)minor;
