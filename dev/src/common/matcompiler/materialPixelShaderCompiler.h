...
class CHLSLMaterialPixelShaderCompiler : public CHLSLMaterialShaderCompiler
{
public:
	CHLSLMaterialVertexShaderCompiler*		m_vertexShader;		// Compiled vertex shader

public:
	CHLSLMaterialPixelShaderCompiler( CHLSLMaterialCompiler*	compiler, CHLSLMaterialVertexShaderCompiler* vertexShader );

	// Discard pixel fragment when value < 0
	virtual void Discard( const CodeChunk& value );
	
	// Get shader type
	virtual EMaterialShaderTarget GetShaderTarget() const { return MSH_PixelShader; }

	// Generate data
	virtual Bool GenerateData( const CodeChunk& dataType, CodeChunk& output );
