...
protected:
    ...
    int                  m_dragCount;
    CEdTimer            *m_dragTimer;
    wxTreeListItem      *m_dragItem;

    CEdTimer            *m_renameTimer;
    wxString             m_renameRes;
    wxEditTextCtrl      *m_renameControl;

    // char navigation
    CEdTimer            *m_findTimer;
    wxString             m_findStr;
...
class  wxTreeListRenameTimer: public CEdTimer
{
public:
...
void wxTreeListRenameTimer::NotifyOnce()
{
...
void wxTreeListHeaderWindow::OnMouse (wxMouseEvent &event) {
    ...
    if (m_isDragging) {
        ...
        if (event.LeftDClick() || event.LeftDown() || event.RightUp()) {
...
void wxTreeListMainWindow::Init() {
    ...
    m_dragCount = 0;
    m_isDragging = false;
    m_dragTimer = new CEdTimer(this, -1);
    ...
    m_findTimer = new CEdTimer (this, -1);
    ...
    switch (event.GetKeyCode()) {
        ...
        default:
            if (event.GetKeyCode() >= (int)' ') {
                ...
                m_findStr.Append (String::Printf( TXT( "%d" ), event.GetKeyCode() ).AsChar() );
                                   ...
                                   const wxTreeItemId& item2)
{
	...
	if ( m_comp_func )
		return (*m_comp_func)(item1, item2);
    return wxStrcmp(GetItemText(item1), GetItemText(item2));
}

void wxTreeListCtrl::SortChildren(const wxTreeItemId& item, TCompareFuncPtr ptr)
{ m_comp_func = ptr; m_main_win->SortChildren(item); }
