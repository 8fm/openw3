...
// Created:     1/08/1999
// RCS-ID:      $Id: sheet.cpp,v 1.24 2006/04/13 03:06:00 jrl1 Exp $
...
IMPLEMENT_DYNAMIC_CLASS_XTI(wxSheet, wxWindow,"sheet.h")
...
bool wxSheetDataObject::GetDataHere(void *buf) const
{
    ...
    Red::System::MemorySet( (char *)buf, 0, len );
    Red::System::MemoryCopy( buf, m_data.wc_str(), len );
    return true;
}
        ...
        for (int col_blocks = 0; col_blocks <= num_col_blocks; col_blocks++)
        {
            ...
            for (r = 0; r < n_copiedrows; r++)
            {
                ...
                for (c = 0; c < n_copiedcols; c++)
                {
                    ...
                    //wxPrintf(wxT("Pasting (%d %d), at (%d %d) '%s'\n"), r, c, pasteCell.m_row, pasteCell.m_col, value.wc_str()); fflush(stdout);
