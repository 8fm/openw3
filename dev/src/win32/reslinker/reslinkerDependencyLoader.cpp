...
CReslinkerDependencyLoader::CReslinkerDependencyLoader( IFile& file )
	: IDependencyLinker( file, 0 )
{
}

CReslinkerDependencyLoader::~CReslinkerDependencyLoader()
{
}

IFile &CReslinkerDependencyLoader::operator<<( CObject *&object )
{
	Int16 index = 0;  
	m_file << index;
...
IFile &CReslinkerDependencyLoader::operator<<( CName& name )
{
	Uint16 index = 0;  
	m_file << index;
	name = MapName( index );
...
CObject *CReslinkerDependencyLoader::MapObject( Int16 index )
{
	if ( index < 0 )
	{
...
CName CReslinkerDependencyLoader::MapName( Uint16 index )
{
	ASSERT( index < (Uint16)m_names.Size() );
	return m_names[ index ];
}

Bool CReslinkerDependencyLoader::GetImports( TDynArray< String >& importFiles )
{
	// Cleanup
	m_names.Clear();
	m_imports.Clear();
