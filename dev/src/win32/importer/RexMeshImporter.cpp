...
#include "../../common/engine/navigationObstacle.h"
...
class CREXMeshImporter : public IImporter
{
	DECLARE_ENGINE_CLASS( CREXMeshImporter, IImporter, 0 );
...
BEGIN_CLASS_RTTI( CREXMeshImporter )
	PARENT_CLASS( IImporter )
	PROPERTY_EDIT( m_autoRemoveUnusedBones, TXT("Removes unused skinned bones for imported mesh") );
END_CLASS_RTTI()

	IMPLEMENT_ENGINE_CLASS( CREXMeshImporter );

CREXMeshImporter::CREXMeshImporter()
{
	m_resourceClass = ClassID< CMesh >();
	m_formats.PushBack( CFileFormat( TXT("re"), TXT("Red Engine File") ) );
	m_autoRemoveUnusedBones = false;
	LoadObjectConfig( TXT("User") );
}

Bool CREXMeshImporter::PrepareForImport( const String& str, ImportOptions& options )
{
...
namespace
{
	...
	void ImportMaterials( const TDynArray< ReFileMesh >& meshes, CMesh::FactoryInfo& buildData, TDynArray< Int32 >& materialsid )
	{
		...
		for ( Uint32 m=0; m < meshesSize; m++ )
		{
			...
			Int32 meshChunkCount = mesh.getNumChunks();
	...
	void ImportRigs( const TDynArray< ReFileMesh >& meshes, CMesh::FactoryInfo& buildData, TDynArray< TDynArray<Int32> >& rigMapping )
	{
		Uint32 meshesSize = meshes.Size();
		rigMapping.Resize( meshesSize );
			...
			if( numRigMatrices > 0 )
			{
				...
				for ( Int32 i=0; i < numRigMatrices; i++ )
				{
					...
					if ( !alreadyAdded )
					{
						...
						xx.SetX( (Float)mesh.getRigMatrix(i).mRigMatrix[0] ); 
						xx.SetY( (Float)mesh.getRigMatrix(i).mRigMatrix[1] ); 
						xx.SetZ( (Float)mesh.getRigMatrix(i).mRigMatrix[2] );

						yy.SetX( (Float)mesh.getRigMatrix(i).mRigMatrix[3] ); 
						yy.SetY( (Float)mesh.getRigMatrix(i).mRigMatrix[4] ); 
						yy.SetZ( (Float)mesh.getRigMatrix(i).mRigMatrix[5] );

						zz.SetX( (Float)mesh.getRigMatrix(i).mRigMatrix[6] ); 
						zz.SetY( (Float)mesh.getRigMatrix(i).mRigMatrix[7] ); 
						zz.SetZ( (Float)mesh.getRigMatrix(i).mRigMatrix[8] );

						newRigMatrix->Set33( xx,yy,zz );

						newRigMatrix->SetTranslation(
							(Float)mesh.getRigMatrix(i).mRigMatrix[9]	* SCALE,
							(Float)mesh.getRigMatrix(i).mRigMatrix[10]	* SCALE,
							(Float)mesh.getRigMatrix(i).mRigMatrix[11]	* SCALE
							);
					}
	...
	void ImportCollisions( const TDynArray< ReFileCollisionConvex >& convexCollisions, CMesh* existingMesh, const IImporter::ImportOptions& options, CMesh::FactoryInfo& buildData )
	{
		Uint32 convexCollisionSize = convexCollisions.Size();
		for ( Uint32 i=0; i < convexCollisionSize; ++i )
		{
			const ReFileCollisionConvex& convexCollision = convexCollisions[i];
			StringAnsi str( convexCollision.getCollisionName().getData() );

			Int32 numv = convexCollision.getNumVertices();
			Int32 numf = convexCollision.getNumFaces();
			INT32 numMat = convexCollision.getNumMaterials();
			...
			if ( str.EndsWith( STR_NAV ) )
			{
				...
				{
					float x = convexCollision.mVertices[ (i*3)+0 ] * SCALE;
					float y = convexCollision.mVertices[ (i*3)+1 ] * SCALE;
					float z = convexCollision.mVertices[ (i*3)+2 ] * SCALE;
			...
			else
			{
				...
				if( str.EndsWith( STR_COLL ) )
				{
					...
					String materialName = ANSI_TO_UNICODE( convexCollision.mMaterials->getData() );
				...
				else
				{
					...
					for( Uint16 i = 0; i != numMat; ++i )
					{
						String materialName = ANSI_TO_UNICODE( convexCollision.mMaterials[ i ].getData() );
						materialName.MakeLower();
						newColMesh->m_physicalMaterials.PushBack( CName( materialName ) );
					}
				}
				...
				{
					float x = convexCollision.mVertices[ (v*3)+0 ];
					float y = convexCollision.mVertices[ (v*3)+1 ];
					float z = convexCollision.mVertices[ (v*3)+2 ];
				...
				{
					int x = convexCollision.mIndices[(k*4)+0];
					int y = convexCollision.mIndices[(k*4)+1];
					int z = convexCollision.mIndices[(k*4)+2];
					int mat = convexCollision.mIndices[(k*4)+3];
	...
	void ImportBoxes( const TDynArray< ReFileCollisionBox >& boxes, CMesh::FactoryInfo& buildData )
	{
		Uint32 boxesSize = boxes.Size();
		for ( Uint32 i=0; i < boxesSize; ++i )
		{
	...
	void ImportSpheres( const TDynArray< ReFileCollisionSphere >& spheres, CMesh::FactoryInfo& buildData )
	{
	...
	void ImportCapsules( const TDynArray< ReFileCollisionCapsule >& capsules, CMesh::FactoryInfo& buildData )
	{
	...
	void ImportChunks( const TDynArray< ReFileMesh >& meshes, const SLODPreset& lodPreset, const TDynArray< Int32 >& materialsid, const TDynArray< TDynArray<Int32> >& rigMapping, CMesh::FactoryInfo& buildData )
	{
		Int32 Index = 0;
		Uint32 meshesSize = meshes.Size();
		for ( Uint32 i=0; i<meshesSize; i++ )
		{
			...
			for ( Int32 c=0; c<numChunks; c++ )
			{
				...
				newChunk->m_numIndices  = mesh.getMeshChunk(c).getNumFaces() * 3;
				...
				for ( Int32 ff=0, end = mesh.getMeshChunk(c).getNumFaces(); ff < end; ++ff )
				{
					ReFileFace face = mesh.getMeshChunk(c).mFaces[ff];
					Uint32 x = face.mIndices[0];
					Uint32 y = face.mIndices[1];
					Uint32 z = face.mIndices[2];
				...
				if ( buildData.m_boneNames.Size()>0 )
				{
					...
					newChunk->m_vertexType = MVT_SkinnedMesh;
				}
				else
				{
					newChunk->m_numBonesPerVertex = 0;
...
CResource* CREXMeshImporter::DoImport( const ImportOptions& options )
{
	CMesh* existingMesh = Cast< CMesh >( options.m_existingResource );
	...
	TDynArray< ReFileCollisionConvex >			convexCollisions;
	...
	for ( int i = 0; i < reFile.mHeaders.size(); ++i )
	{
		...
		{
			convexCollisions.PushBack( ReFileCollisionConvex() );
			ReFileLoader::Read( reFile.mFile, header, convexCollisions.Back(), currentVersionPtr );
		}
		// no support for this chunk
	...
	if ( existingMesh && !meshes.Empty() )
	{
		Int32 minChunksCount = existingMesh->GetMeshLODLevels()[0].m_chunks.SizeInt();
		for ( Uint32 i = 1; i < existingMesh->GetMeshLODLevels().Size(); ++i )
		{
			Int32 currentLodChunksCount = existingMesh->GetMeshLODLevels()[i].m_chunks.SizeInt();
			if ( currentLodChunksCount < minChunksCount )
			{
				minChunksCount = currentLodChunksCount;
			}
		}

		if ( minChunksCount < meshes[0].getNumChunks() )
		{
			GFeedback->ShowWarn( TXT("Source mesh has more chunks on some LODs than existing w2mesh. Reimported mesh may contain holes. (path: %ls)"), existingMesh->GetFile()->GetDepotPath().AsChar() );
		}
	}

	CMesh::FactoryInfo buildData;
	...
	{
		ImportCollisions( convexCollisions, existingMesh, options, buildData );
	...
	CMesh* newMesh = CMesh::Create( buildData );
