...
class CGlueFilesCommandlet : public ICommandlet
{
	DECLARE_ENGINE_CLASS( CGlueFilesCommandlet, ICommandlet, 0 );
...
private:
	...
	{
	public:
		FileMapping( const String& baseDir )
			: m_baseDir( baseDir )
		{
		}
		...
		FileInfo* GetFileInfo( const String& depotPath )
		{
			...
			// does the file exist ?
			const String absolutePath = m_baseDir + depotPath;
			const Uint32 fileSize = (Uint32) GFileManager->GetFileSize( absolutePath );
			if ( 0 == fileSize )
			{
...
BEGIN_CLASS_RTTI( CGlueFilesCommandlet )
	PARENT_CLASS( ICommandlet )
END_CLASS_RTTI()

IMPLEMENT_ENGINE_CLASS( CGlueFilesCommandlet );

CGlueFilesCommandlet::CGlueFilesCommandlet()
{
	m_commandletName = CName( TXT("gluefiles") );
}
...
bool CGlueFilesCommandlet::Execute( const CommandletOptions& options )
{
	// load file list
	...
	FileMapping fileMapping( baseDirectory );
...
Uint32 CGlueFilesCommandlet::CalculateFileSizeWithNoGluedData( const CDependencyFileData& data )
{
...
void CGlueFilesCommandlet::ExtractDependencies( FileMapping& map, FileInfo* file )
{
	...
	for ( const CDependencyFileData::Import& import : tables->m_imports )
	{
		...
		// W3 Hack: don't follow files that are censored!
		{
			...
			if ( GDepot->FindFileUseLinks( filePathCensored.ToString(), 0 ) )
			{
...
Bool CGlueFilesCommandlet::ProcessFile( const FileInfo* rootFile, const GlueSettings& settings, CCookerDataBase& db )
{
	// build the glue list
...
void CGlueFilesCommandlet::BuildGlueFileList( const FileInfo* file, const Uint32 level, const GlueSettings& settings, THashMap< const FileInfo*, Uint32 >& visitedFiles, THashMap< const FileInfo*, Uint32 >& gluedFiles )
{
	// invalid file
	if ( file->m_invalid )
		return;
...
Bool CGlueFilesCommandlet::BuildGlueFlist( const FileInfo* rootFile, const GlueSettings& settings, TDynArray< const FileInfo* >& filesToGlue )
{
	Bool state = true;
...
void CGlueFilesCommandlet::PrintDependencyTree( const FileInfo* file, const Uint32 level, THashSet< const FileInfo* >& visitedFiles ) const
{
	// indent buffer
	Char tabs[ 64 ];
