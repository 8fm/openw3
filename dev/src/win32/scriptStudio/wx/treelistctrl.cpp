...
wxDEFINE_EVENT( wxEVT_TREELIST_POSTSCROLL, wxScrollWinEvent );
...
public:
	...
	bool InsertIntoEditLabel( const wxString& text );
	wxString GetEditLabelSelection() const;
	...
	virtual bool HasFocus() const override { return m_hasFocus; }
...
void wxTreeListMainWindow::ScrollTo (const wxTreeItemId &item) {
	...
	//Rewritten by Dan Neve

	int lineHeight = GetLineHeight( static_cast< wxTreeListItem* >( item.m_pItem ) );
	int windowHeight = GetClientSize().GetY();

	int numVisible = ( windowHeight / lineHeight ) - 1;

	int newScrollPosition = static_cast< wxTreeListItem* >( item.m_pItem )->GetY();
	newScrollPosition -= 2;
	newScrollPosition /= lineHeight;

	if( newScrollPosition < m_scrollPositionY )
	{
		m_dirty = true;
		m_scrollPositionY = newScrollPosition;
		SetScrollPos( wxVERTICAL, m_scrollPositionY );
	}
	else if( newScrollPosition >= m_scrollPositionY + numVisible )
	{
		m_dirty = true;
		m_scrollPositionY = newScrollPosition - numVisible;
		SetScrollPos( wxVERTICAL, m_scrollPositionY );
	}
}
...
wxTreeItemId wxTreeListMainWindow::FindItem (const wxTreeItemId& item, int column, const wxString& str, int mode) {
    ...
    if (column >= 0) { col_start = col_end = column = 0; }
...
void wxTreeListMainWindow::AdjustMyScrollbars() {
    if (m_rootItem) {
        ...
        SetScrollbars (xUnit, yUnit, x/xUnit, y/yUnit, x_pos, m_scrollPositionY);
	}else{
		m_scrollPositionY = 0;
...
// prepare edit (position)
    ...
    long style = wxTE_PROCESS_ENTER | wxTE_RICH;
...
bool wxTreeListMainWindow::InsertIntoEditLabel( const wxString& text )
{
	if( m_editControl )
	{
		m_editControl->WriteText( text );
		return true;
	}
	return false;
}

wxString wxTreeListMainWindow::GetEditLabelSelection() const
{
	if( m_editControl && m_editControl->HasSelection() )
	{
		return m_editControl->GetStringSelection();
	}

	return wxEmptyString;
}

bool wxTreeListMainWindow::IsEditing() const
{
...
*/
	...
	if( m_editControl && event.LeftDown() )
	{
		int old = flags;
		flags = 0;
		flags = old;
...
void wxTreeListMainWindow::OnScroll (wxScrollWinEvent& event) {
    ...
    if(event.GetOrientation() == wxHORIZONTAL) {
		...
		m_scrollPositionY = GetScrollPos( wxVERTICAL );
	}

	wxScrollWinEvent* postEvent = new wxScrollWinEvent( wxEVT_TREELIST_POSTSCROLL, event.GetPosition(), event.GetOrientation() );
	m_owner->QueueEvent( postEvent );
}
...
int wxTreeListCtrl::GetItemHeight(const wxTreeItemId& item) const
{ return m_main_win->GetLineHeight( (wxTreeListItem*)item.GetID() ); }
...
bool wxTreeListCtrl::InsertIntoEditLabel( const wxString& text )
{
	return m_main_win->InsertIntoEditLabel(text);
}

wxString wxTreeListCtrl::GetEditLabelSelection() const
{
	return m_main_win->GetEditLabelSelection();
}

bool wxTreeListCtrl::IsEditing() const
{
	return m_main_win->IsEditing();
}
...
void wxTreeListCtrl::Refresh(bool erase, const wxRect* rect)
{
	...
	wxControl::Refresh(erase, rect);
}

void wxTreeListCtrl::Update()
{
	m_main_win->Update();
	m_header_win->Update();
	wxControl::Update();
...
bool wxTreeListCtrl::HasFocus() const 
{ return m_main_win->HasFocus(); }
