           ...
           out float4 fcolor : SV_Target0)
{
    ...
    float4 shadowColor = gradtex.Sample(sampler_gradtex,  float2(clamp(0.0f + 0.5f * (1.0f-fcolor.r * fsize.z) , 0.0f, 0.5f), 0.0f));
    float4 shadowColor2 = gradtex.Sample(sampler_gradtex, float2(clamp(0.5f + 0.5f * fcolor.a * fsize.z, 0.5f, 1.0f), 0.0f));
