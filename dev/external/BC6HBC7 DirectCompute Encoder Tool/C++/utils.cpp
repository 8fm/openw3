...
HRESULT LoadTextureFromFile( ID3D11Device* pd3dDevice, LPCTSTR lpFileName, DXGI_FORMAT fmtLoadAs, BOOL bNoMips, DWORD dwFilter, ID3D11Texture2D** ppTextureOut )
{
    ...
    if ( info.format != fmtLoadAs )
    {
		...
		//hr = Convert( image->GetImages(), image->GetImageCount(), image->GetMetadata(), fmtLoadAs, dwFilter, 05.f, *timage );
        if ( FAILED(hr) )
        {
            delete timage;
            delete image;
         ...
         && ispow2(info.width) && ispow2(info.height) )
    {
		...
		hr = GenerateMipMaps( image->GetImages(), image->GetImageCount(), image->GetMetadata(), dwFilter, 0, *timage );        
        if ( FAILED(hr) )
        {
            delete timage;
            delete image;
    ...
    if ( !bNoMips || !ispow2(info.width) || !ispow2(info.height) )
    {
