...
HRESULT EncoderBase::GPU_EncodeAndReturn( ID3D11Texture2D* pSourceTexture, DXGI_FORMAT fmtEncode, DirectX::Image*& pOutImage )
{
	HRESULT hr = S_OK;
	...
	for ( UINT item = 0; item < desc.ArraySize; ++item )
	{
		...
		{
			ID3D11Buffer* pReadbackbuf = CreateAndCopyToCPUBuf( m_pDevice, m_pContext, buffers[item * desc.MipLevels + level] );
			if ( !pReadbackbuf )
				goto quit;

			D3D11_MAPPED_SUBRESOURCE mappedSrc;        
			m_pContext->Map( pReadbackbuf, 0, D3D11_MAP_READ, 0, &mappedSrc );
			pOutImage->slicePitch = desc.Height * desc.Width * sizeof(BufferBC6HBC7) / BLOCK_SIZE;
			pOutImage->width = desc.Width;
			pOutImage->height = desc.Height;
			pOutImage->format = DXGI_FORMAT_BC7_UNORM;
			pOutImage->pixels = (uint8_t*)malloc( pOutImage->slicePitch );
			memcpy( pOutImage->pixels, mappedSrc.pData, pOutImage->slicePitch );
