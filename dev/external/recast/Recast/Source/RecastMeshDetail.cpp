								...
								const rcHeightPatch& hp)
{
	...
	if (h == RC_UNSET_HEIGHT)
	{
		...
		auto funSampleCel =
			[&dmin, &h, &hp, ch, fy ] ( const int nx, const int nz )
			{
		...
		if ( h == RC_UNSET_HEIGHT )
		{
			// rare case not supported by original Recast library, but possible if no 'margin' (rcErodeWalkableArea) algorithm was run on data
			const int offFar[16*2] = { -2, 0, -2, -1, -2, -2, -1, -2, 0, -2, 1, -2, 2, -2, 2, -1, 2, 0, 2, 1, 2, 2, 1, 2, 0, 2, -1, 2, -2, 2, -2, 1  };

			for (int i = 0; i < 16; ++i)
			{
				const int nx = ix+offFar[i*2+0];
				const int nz = iz+offFar[i*2+1];

				funSampleCel( nx, nz );
			}

			rcAssert( h != RC_UNSET_HEIGHT );
			if ( h == RC_UNSET_HEIGHT )
			{
				h = (int)floorf(fz*ics + 0.5f);
			}
		}
	}
	return h;
