...
void PCMConvexVsMeshContactGeneration::generateLastContacts()
{
	...
	if(nbEntries)
	{
		...
		const PCMDeferredPolyData* PX_RESTRICT cd = (const PCMDeferredPolyData* PX_RESTRICT)mDeferredContacts.GetEntries();
		...
		/*	bool needsProcessing = ((triFlags & (ETD_CONVEX_EDGE_01 | ETD_CONVEX_EDGE_20)) != 0 || (mVertexCache.get(CachedVertex(ref0)) == NULL)) && 
				(((triFlags & (ETD_CONVEX_EDGE_01 | ETD_CONVEX_EDGE_12)) != 0 || mVertexCache.get(CachedVertex(ref1)) == NULL)) && 
				(((triFlags & (ETD_CONVEX_EDGE_12 | ETD_CONVEX_EDGE_20)) != 0 || mVertexCache.get(CachedVertex(ref2)) == NULL)) && 
				(((triFlags & ETD_CONVEX_EDGE_01) != 0 || mEdgeCache.get(CachedEdge(ref0, ref1)) == NULL)) && 
			...
			if(needsProcessing)
			{
				...
				generatePolyDataContactManifold(localTriangle, currentContact.mFeatureIndex, triFlags, mManifoldContacts, mNumContacts, mContactDist, patchNormal);

				/*TriangleV triangle;
				triangle.verts[0] =mConvexTransform.transform(localTriangle.verts[0]);
				triangle.verts[1] =mConvexTransform.transform(localTriangle.verts[1]);
				triangle.verts[2] =mConvexTransform.transform(localTriangle.verts[2]);

				mMultiManifold.drawLine(*gRenderOutPut, triangle.verts[0],triangle.verts[1], 0x00ff0000);
				mMultiManifold.drawLine(*gRenderOutPut, triangle.verts[1], triangle.verts[2], 0x00ff0000);
				mMultiManifold.drawLine(*gRenderOutPut, triangle.verts[2], triangle.verts[0], 0x00ff0000);*/
...
bool PCMConvexVsMeshContactGeneration::processTriangle(const PxVec3* verts, PxU32 triangleIndex, PxU8 triFlags, const PxU32* vertInds)
{
...

	{
		...
		/*static PxU32 triIndex = 1710;
		static bool debugRender = false;
		if(debugRender && triIndex == triangleIndex)
		{

			TriangleV triangle(verts);
			triangle.verts[0] =mMeshTransform.transform(triangle.verts[0]);
			triangle.verts[1] =mMeshTransform.transform(triangle.verts[1]);
			triangle.verts[2] =mMeshTransform.transform(triangle.verts[2]);
	...
	/*		if(inActiveEdge0 && inActiveEdge1)
				mVertexCache.addData(CachedVertex(vertInds[0]));
			if(inActiveEdge1 && inActiveEdge2)
				mVertexCache.addData(CachedVertex(vertInds[1]));
			if(inActiveEdge0 && inActiveEdge2)
...
bool Gu::PCMSphereVsMeshContactGeneration::processTriangle(const PxVec3* verts, PxU32 triangleIndex, PxU8 triFlags, const PxU32* vertInds)
{
	...
	const FloatV dist = FSub(V3Dot(mSphereCenter, n), d);//p.dot(n) + d;
	...
	if(FAllGrtr(mSqInflatedSphereRadius, sqDist) && (generateContact || FAllGrtr(cosTheta, tolerance)))
	{
		...
		//const FloatV pen = FSub(dist, mSphereRadius);	
		//mManifoldContacts[mNumContacts].localPointA = mMeshToConvex.transform(V3NegScaleSub(patchNormal, mSphereRadius, mSphereCenter));//in sphere's space
		//mManifoldContacts[mNumContacts].localPointB = closestP;
		//mManifoldContacts[mNumContacts++].localNormalPen = V4SetW(Vec4V_From_Vec3V(patchNormal), pen);
		//mNumContacts++;
		...
		if(mNumContactPatch > 0)
		{
			if(FAllGrtr(V3Dot(mContactPatch[mNumContactPatch-1].mPatchNormal, patchNormal), mAcceptanceEpsilon))
			{
				...
				if(FAllGrtr(patch.mPatchMaxPen, dist))
				{
					...
					//mManifoldContacts[patch.mStartIndex].mLocalPointA = mMeshToConvex.transform(V3NegScaleSub(patchNormal, mSphereRadius, mSphereCenter));//in sphere's space
		...
		if(!foundPatch)
		{
			...
			//mManifoldContacts[mNumContacts].mLocalPointA = mMeshToConvex.transform(V3NegScaleSub(patchNormal, mSphereRadius, mSphereCenter));//in sphere's space
...
void Gu::PCMCapsuleVsMeshContactGeneration::generateEE(const Ps::aos::Vec3VArg p, const Ps::aos::Vec3VArg q,
																const Ps::aos::Vec3VArg normal, const Ps::aos::Vec3VArg a, const Ps::aos::Vec3VArg b)
{
	using namespace Ps::aos;
	const FloatV zero = FZero();
	const Vec3V ab = V3Sub(b, a);
	...
	if(FAllGrtr(mSqInflatedRadius, sqDist))
	{
		...
		mManifoldContacts[mNumContacts].mLocalPointA = localPointA;
		mManifoldContacts[mNumContacts].mLocalPointB = localPointB;
		mManifoldContacts[mNumContacts++].mLocalNormalPen = V4SetW(Vec4V_From_Vec3V(normal), signedDist);
	}
}
															   ...
															   const Ps::aos::Vec3VArg c, const Ps::aos::Vec3VArg normal)
{
	...
	using namespace Ps::aos;
	const Vec3V p = mCapsule.p0;
	const Vec3V q = mCapsule.p1;
	generateEE(p, q, normal, a, b);
	generateEE(p, q, normal, b, c);
	generateEE(p, q, normal, a, c);

}

void Gu::PCMCapsuleVsMeshContactGeneration::generateContacts(const Ps::aos::Vec3VArg a, const Ps::aos::Vec3VArg b,
															   const Ps::aos::Vec3VArg c, const Ps::aos::Vec3VArg normal)
{
	...
	const BoolV tempCon0 = BAnd(con0, FIsGrtr(mInflatedRadius, dist3));
	if(BAllEq(tempCon0, bTrue))
	{
		mManifoldContacts[mNumContacts].mLocalPointA = closestP30;//transform to B space, it will get convert to A space later
		mManifoldContacts[mNumContacts].mLocalPointB = closestP31;
		mManifoldContacts[mNumContacts++].mLocalNormalPen = V4SetW(Vec4V_From_Vec3V(normal), dist3);
	...
	const BoolV tempCon1 = BAnd(con1, FIsGrtr(mInflatedRadius, dist4));
	if(BAllEq(tempCon1, bTrue))
	{
		mManifoldContacts[mNumContacts].mLocalPointA = closestP40;
		mManifoldContacts[mNumContacts].mLocalPointB = closestP41;
		mManifoldContacts[mNumContacts++].mLocalNormalPen = V4SetW(Vec4V_From_Vec3V(normal), dist4);
	}
}
													...
													Ps::aos::FloatV& t, Ps::aos::FloatV& u, Ps::aos::FloatV& v)
{
	...
	//We want to calculate the dot product of the combinations of pV20, qV20 and ab and ac. To do this quickly, we mul, transpose and then add

	/*const FloatV pD20 = V3Dot(pV20, ab);
	const FloatV pD21 = V3Dot(pV20, ac);

	const FloatV v0 = FMul(FNegMulSub(d01, pD21, FMul(d11, pD20)), bdenom);
	const FloatV w0 = FMul(FNegMulSub(d01, pD20, FMul(d00, pD21)), bdenom);
	...
	const FloatV qD20 = V3Dot(qV20, ab);
	const FloatV qD21 = V3Dot(qV20, ac);
	const FloatV v1 = FMul(FNegMulSub(d01, qD21, FMul(d11, qD20)), bdenom);
	const FloatV w1 = FMul(FNegMulSub(d01, qD20, FMul(d00, qD21)), bdenom);

	

	const BoolV cond2 = BAnd(con0, con1);*/

	/*Vec4V _pD20 = Vec4V_From_Vec3V(V3Mul(pV20, ab));
	Vec4V _pD21 = Vec4V_From_Vec3V(V3Mul(pV20, ac));
	Vec4V _qD20 = Vec4V_From_Vec3V(V3Mul(qV20, ab));
	Vec4V _qD21 = Vec4V_From_Vec3V(V3Mul(qV20, ac));

	Vec4V trnspPD0, trnspPD1, trnspPD2;

	PX_TRANSPOSE_44_34(_pD20, _pD21, _qD20, _qD21, trnspPD0, trnspPD1, trnspPD2);
	V4Add(trnspPD0, V4Add(trnspPD1, trnspPD2));*/
...
bool Gu::PCMCapsuleVsMeshContactGeneration::processTriangle(const PxVec3* verts, PxU32 triangleIndex, PxU8 triFlags, const PxU32* vertInds)
{
	...
	//static PxU32 triId = 5075;
	//if(triId != triangleIndex)
	//	return true;
	...
	{
		TriangleV triangle;
		triangle.verts[0] =mMeshTransform.transform(p0);
		triangle.verts[1] =mMeshTransform.transform(p1);
		triangle.verts[2] =mMeshTransform.transform(p2);
	...
	if(FAllGrtr(mSqInflatedRadius, sqDist))
	{
		...
		else
		{
			...
			else
			{
				...
				//const Vec3V pointOnTriangle = FAdd(FMul(w, p0), FAdd(FMul(u, p1), FMul(v, p2)));
		...
		generateEEContacts(p0, p1, p2, patchNormalInTriangle);
		generateContacts(p0, p1, p2, patchNormalInTriangle);
