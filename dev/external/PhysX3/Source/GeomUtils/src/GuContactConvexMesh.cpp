...
static PX_FORCE_INLINE void boxSupport(const float extents[3], const PxVec3& sv, float p[3])
{
	...
	IR(p[0]) = IR(extents[0])|(IR(sv.x)&SIGN_BITMASK);
	IR(p[1]) = IR(extents[1])|(IR(sv.y)&SIGN_BITMASK);
	IR(p[2]) = IR(extents[2])|(IR(sv.z)&SIGN_BITMASK);
#endif
}
...
PX_FORCE_INLINE bool testInternalObjects(	const PxVec3& localAxis0,
						...
						PxU32 id0, PxU32 /*triangleIndex*/)
{
	...
	while(numAxes--)
	{
		...
		if(!testInternalObjects(localAxis0, polyData0, triangleInHullSpace, world0, world1, dmin, contactDistance))
		{
	#ifdef PX_DEBUG
			PxReal dtest;
...
struct ConvexVsMeshContactGenerationCallback : Gu::VolumeColliderTrigCallback
{
	...
	virtual bool processResults(PxU32 count, const PxVec3* trigVertTriples, const PxU32* trigIndices, const PxU32* trigVertexIndexTriple);
};
...
enum FeatureCode
{
	...
	{
	public:
		PX_FORCE_INLINE TriangleIndices(const Gu::InternalTriangleMeshData& mesh, PxU32 triangleIndex)
		{
...
//								mThreadCache,
	...
	else
	{
		...
		SavedContactData* PX_RESTRICT cd = (SavedContactData* PX_RESTRICT)mDelayedContacts.Reserve(nb);
...
bool ConvexVsMeshContactGenerationCallback::processResults(PxU32 numTrigs, const PxVec3* trigVertTriples, const PxU32* triangleIndices, const PxU32* triVertIndexTriple)
{
	PxVec3 verts[3];
	...
	const bool idtMeshScale = mIdtMeshScale;
	...
	for(PxU32 a = 0; a < pageCount; ++a)
	{
		...
		while(trigCount--)
		{
			...
			//const PxU32 ind0 = *triVertIndexTriple++;
			//const PxU32 ind1 = *triVertIndexTriple++;
			//const PxU32 ind2 = *triVertIndexTriple++;
...
#else
			PxU8 triFlags = mExtraTrigData[triangleIndex];
#endif
		...
		mGeneration.processTriangleCache<CacheSize>(cache);// || result;
	}
	return true;
}
...
//printf("%d\n", nbEntries);
		const SavedContactData* PX_RESTRICT cd = (const SavedContactData* PX_RESTRICT)mDelayedContacts.GetEntries();
...
#pragma control %pop O
	...
	ConvexVsMeshContactGenerationCallback blockCallback(
		delayedContacts,
		t0to1, t1to0, polyData0, world0, world1, meshData->mExtraTrigData, meshScaling,
	...
	collider.SetPrimitiveTests(true);	// PT: doing full tests is faster in this case (profiled on Xbox)

//	collider.Collide(hullOBB, hmd, &blockCallback, NULL, NULL, false);
