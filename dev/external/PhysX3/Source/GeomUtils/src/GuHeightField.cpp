...
bool Gu::HeightField::loadFromDesc(const PxHeightFieldDesc& desc, bool enableTile)
{
	...
	if (nbVerts > 0) 
	{
		...
		const PxU8* src = (const PxU8*)desc.samples.data;
		PxHeightFieldSample* dst = mData.samples;
	...
	// compute extents
	PxReal minHeight = PX_MAX_REAL;
	PxReal maxHeight = -PX_MAX_REAL;
...
//		const PxU32 faceCount = getEdgeTriangleIndices(edgeIndices[i], faceIndices);
	...
	bool nbSolid = false;
	const PxU32* currentfaceIndices = faceIndices;
	for(PxU32 i=0; i<edgeCount; i++)
	{
		if(faceCounts[i] > 1) 
		{
