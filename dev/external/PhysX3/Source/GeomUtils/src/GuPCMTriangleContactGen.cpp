...
namespace physx
{
		...
		Ps::aos::FloatV& minOverlap, PxU32& feature, Ps::aos::Vec3V& faceNormal, const FeatureStatus faceStatus, FeatureStatus& status)
	{
		...
		//const PsMatTransformV& triangleToPoly = polyMap->transform.transformInv(triMap->transform);
		if(polyMap->isIdentityScale)
		{
			...
			/*	const FloatV d0 = FSub(max0, min1);
				...
				const FloatV tempOverlap1 = FMin(d0, d1);*/
	...
	//static bool testSinglePolygonEdgeNormal(const Gu::TriangleV& triangle, const Gu::HullPolygonData& polygon, PxU8* inds, const Gu::PolygonalData& polyData, Gu::SupportLocalImpl<Gu::TriangleV>* triMap, SupportLocal* polyMap, const Ps::aos::FloatVArg contactDist,
	...
	//		const Vec3V v1 =  M33TrnspsMulV3(polyMap->vertex2Shape, V3Sub(p11, p10));


	//		const Vec3V dir = V3Cross(v0, v1);
		...
		Ps::aos::FloatV& minOverlap, Ps::aos::Vec3V& minNormal, const FeatureStatus edgeStatus, FeatureStatus& status)
	{
		...
		for(PxU32 i =0; i<polyData.mNbPolygons; ++i)
		{
			...
			for(PxU32 lStart = 0, lEnd =polygon.mNbVerts-1; lStart<polygon.mNbVerts; lEnd = lStart++)
			{
				...
				const BoolV con = BOr(FIsGrtrOrEq(contactDist, signDistP10), FIsGrtrOrEq(contactDist, signDistP11));
				if(BAllEq(con, bTrue))
				{
		...
		//const Vec3V planeNormal = triangle.normal();
		//const FloatV d = V3Neg(V3Dot(planeNormal, triangle.verts[0]));
		...
		//	{
		//		
		//		const Vec3V p00 = triangle.verts[kStart];
		//		const Vec3V p01 = triangle.verts[kEnd];
		//		//change to shape space
		//		const Vec3V v0 = V3Sub(p01, p00);
	...
	static void generatedTriangleContacts(const Gu::TriangleV& triangle, const PxU8 _triFlags, const Gu::PolygonalData& polyData1, const Gu::HullPolygonData& incidentPolygon,  Gu::SupportLocal* map1, Gu::PersistentContact* manifoldContacts, PxU32& numContacts, 
		const Ps::aos::FloatVArg contactDist, const Ps::aos::Vec3VArg contactNormal)
	{
		...
		//Gu::MultiplePersistentContactManifold::drawPolygon(*gRenderOutPut, map1->transform, points1In0, incidentPolygon.mNbVerts); 
		...
		/*const FloatV d1 = V3GetZ(points0In0[1]);
		const FloatV d2 = V3GetZ(points0In0[2]);

		const FloatV ep = FloatV_From_F32(0.000001);
		const FloatV dif0 = FAbs(FSub(d, d1));
		const FloatV dif1 = FAbs(FSub(d, d2));
		PX_ASSERT(FAllGrtr(ep, dif0)==1 && FAllGrtr(ep, dif1)==1);*/
		...
		for(PxU32 i=0; i<incidentPolygon.mNbVerts; ++i)
		{
			...
			if(FAllGrtr(rd, z))
			{
				...
				if(contains(points0In0, 3, points1In0[i], rPolygonMin, rPolygonMax))
				{
					...
					//const Vec3V temp = M33MulV3(rot, points1In0[i]);
		...
		for(PxU32 i=0; i<3; ++i)
		{
			if(contains(points1In0, incidentPolygon.mNbVerts, points0In0[i], iPolygonMin, iPolygonMax))
			{
				...
				//const FloatV t = FSub(iPlaneD, V3Dot(incidentNormal, vert0));
				const FloatV t = FSub(V3Dot(incidentNormal, vert0), iPlaneD);

				/*if(FAllGrtr(FNeg(t), contactDist))
					continue;*/

				if(FAllGrtr(t, contactDist))
					continue;

				//const Vec3V projPoint = V3ScaleAdd(incidentNormal, t, vert0);
				const Vec3V projPoint = V3NegScaleSub(incidentNormal, t, vert0);

				//test whether the projPoint is inside the polyData1
				//if(contains(polyData1.mVerts, polyData1.mNbVerts, inds1, projPoint))
				{
		...
		for(PxU32 a = 0; a < 2; ++a)
		{
			...
			triFlags = triFlags ^ 0xff;
		}
	}


	static void generatedPolyContacts(const Gu::PolygonalData& polyData0, const Gu::HullPolygonData& referencePolygon, const Gu::TriangleV& triangle, const PxU8 triFlags, Gu::SupportLocal* map0, Gu::PersistentContact* manifoldContacts, PxU32& numContacts, 
		const Ps::aos::FloatVArg contactDist, const Ps::aos::Vec3VArg contactNormal)
	{
		PX_UNUSED(triFlags);
		...
		//Gu::MultiplePersistentContactManifold::drawPolygon(*gRenderOutPut, map0->transform, points0In0, referencePolygon.mNbVerts); 
		...
		for(PxU32 i=0; i<referencePolygon.mNbVerts; ++i)
		{
			const FloatV d1 = V3GetZ(points0In0[i]);
			const FloatV dif0 = FAbs(FSub(d, d1));
			PX_ASSERT(FAllGrtr(ep, dif0)==1);
		}*/
		const FloatV rd = FAdd(d, contactDist);
		...
		for (PxU32 iStart = 0, iEnd = 2; iStart < 3; iEnd = iStart++)
		{
			...
			//gManifold->drawLine(gThreadContext, map1->transform.transform(triangle.verts[iStart]), map1->transform.transform(triangle.verts[iEnd]), 0x0000ffff);
			const Vec3V ipA = points1In0[iStart];
			const Vec3V ipB = points1In0[iEnd];
	...
	bool Gu::PCMConvexVsMeshContactGeneration::generateTriangleFullContactManifold(Gu::TriangleV& localTriangle, const PxU32 triangleIndex, const PxU32* triIndices, const PxU8 triFlags, const Gu::PolygonalData& polyData,  Gu::SupportLocalImpl<Gu::TriangleV>* localTriMap, Gu::SupportLocal* polyMap, Gu::PersistentContact* manifoldContacts, PxU32& numContacts,
		const Ps::aos::FloatVArg contactDist, Ps::aos::Vec3V& patchNormal)
	{
		...
		PX_UNUSED(threshold);
		//const FloatV threshold = FloatV_From_F32(0.997f);//about 45 degree
		{
			...
			if(status == POLYDATA0)
			{
				//we should take the triangle normal
				//minNormal is in the local space of polydata0
				//const Vec3V n = transform0To1V.rotate(minNormal);
				const Vec3V n = M33TrnspsMulV3(polyMap->vertex2Shape, minNormal);
				const Gu::HullPolygonData& referencePolygon = polyData.mPolygons[getPolygonIndex(polyData, polyMap, n)];

				patchNormal = triNormal;
				generatedTriangleContacts(localTriangle, triFlags, polyData, referencePolygon, polyMap, manifoldContacts, numContacts, contactDist, triNormal);
			...
			else
			{
				...
				if(status == POLYDATA1)
				{
					...
					else
					{
						...
						generatedPolyContacts(polyData, *referencePolygon, localTriangle, triFlags, polyMap, manifoldContacts, numContacts, contactDist, contactNormal);*/
						const PxU32 nb = sizeof(PCMDeferredPolyData)/sizeof(PxU32);
						PCMDeferredPolyData* PX_RESTRICT data = (PCMDeferredPolyData* PX_RESTRICT)mDeferredContacts.Reserve(nb);
				...
				{
					const Vec3V n = M33TrnspsMulV3(polyMap->vertex2Shape, minNormal);
					feature1 = getPolygonIndex(polyData, polyMap, n);
					const Gu::HullPolygonData* referencePolygon = &polyData.mPolygons[feature1];
			
					const Vec3V contactNormal = V3Normalize(M33TrnspsMulV3(polyMap->shape2Vertex, V3LoadU(referencePolygon->mPlane.n)));
					const Vec3V nContactNormal = V3Neg(contactNormal);

					//if the SAT is edge case, we don't defer it because it is an activeEdge
					patchNormal = nContactNormal;
					generatedPolyContacts(polyData, *referencePolygon, localTriangle, triFlags, polyMap, manifoldContacts, numContacts, contactDist, contactNormal);
				}
	...
	bool Gu::PCMConvexVsMeshContactGeneration::generatePolyDataContactManifold(Gu::TriangleV& localTriangle, const PxU32 featureIndex,  const PxU8 triFlags, Gu::PersistentContact* manifoldContacts, PxU32& numContacts, const Ps::aos::FloatVArg contactDist, Ps::aos::Vec3V& patchNormal)
	{
		...
		patchNormal = nContactNormal;
		generatedPolyContacts(mPolyData, *referencePolygon, localTriangle, triFlags, mPolyMap, manifoldContacts, numContacts, contactDist, contactNormal);
