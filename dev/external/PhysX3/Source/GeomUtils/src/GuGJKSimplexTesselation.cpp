...
namespace Gu
{
	...
	PX_NOALIAS Ps::aos::Vec3V closestPtPointTriangleTesselation(Ps::aos::Vec3V* PX_RESTRICT Q, Ps::aos::Vec3V* PX_RESTRICT A, Ps::aos::Vec3V* PX_RESTRICT B, PxU32* PX_RESTRICT indices, PxU32& size, Ps::aos::Vec3V& closestA, Ps::aos::Vec3V& closestB)
	{
		...
		FloatV d1 = V3Dot(ab, ap); //  snom
		FloatV d2 = V3Dot(ac, ap); //  tnom
		FloatV d3 = V3Dot(ab, bp); // -sdenom
		FloatV d4 = V3Dot(ac, bp); //  unom = d4 - d3
		FloatV d5 = V3Dot(ab, cp); //  udenom = d5 - d6
		FloatV d6 = V3Dot(ac, cp); // -tdenom
	/*	FloatV unom = FSub(d4, d3);
		...
		d1 = V3Dot(ab, ap); //  snom
		d2 = V3Dot(ac, ap); //  tnom
		d3 = V3Dot(ab, bp); // -sdenom
		d4 = V3Dot(ac, bp); //  unom = d4 - d3
		d5 = V3Dot(ab, cp); //  udenom = d5 - d6
		d6 = V3Dot(ac, cp); // -tdenom
