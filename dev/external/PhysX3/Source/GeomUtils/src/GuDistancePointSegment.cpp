												...
												Ps::aos::Vec4V& param)
{
	...
	const Vec4V nom0(V4Scale(m0T.col0, V3GetX(ab)));
	const Vec4V nom1(V4ScaleAdd(m0T.col1, V3GetY(ab), nom0));
	const Vec4V nom(V4ScaleAdd(m0T.col2, V3GetZ(ab), nom1));
	
	const Vec4V tValue = V4Clamp(V4Scale(nom, FRecip(denom)), zero, one);

	const Vec4V t = V4Sel(V4IsEq(Vec4V_From_FloatV(denom), zero), zero, tValue);
	const Vec3V v0 =V3NegScaleSub(ab, V4GetX(t), ap0);
	const Vec3V v1 =V3NegScaleSub(ab, V4GetY(t), ap1);
	const Vec3V v2 =V3NegScaleSub(ab, V4GetZ(t), ap2);
	const Vec3V v3 =V3NegScaleSub(ab, V4GetW(t), ap3);
	param = t;
