	...
	const PxVec3& minimum, const PxVec3& maximum, const PxVec3& ro, const PxVec3& rd, float maxDist, float& tnear, float& tfar)
{
	...
	PxF32 invDx = i::recip(i::selectMax(i::abs(rd.x), 1e-9f) * i::sign(rd.x));
	...
	PxF32 invDy = i::recip(i::selectMax(i::abs(rd.y), 1e-9f) * i::sign(rd.y));
	...
	PxF32 invDz = i::recip(i::selectMax(i::abs(rd.z), 1e-9f) * i::sign(rd.z));
