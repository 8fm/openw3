...
namespace Gu
{
	...
	PxGJKStatus epaPenetration(const ConvexV& a, const ConvexV& b, EPASupportMapPair* pair, PxU8* PX_RESTRICT aInd, PxU8* PX_RESTRICT bInd, PxU8 _size, Ps::aos::Vec3V& contactA, Ps::aos::Vec3V& contactB, Ps::aos::Vec3V& /*normal*/, Ps::aos::FloatV& /*penetrationDepth*/)
	{
		...
		return epa.PenetrationDepth(a, b, pair, Q, A, B, size, contactA, contactB);
	}
	...
	void Facet::silhouette(const Ps::aos::Vec3VArg w, EdgeBuffer& edgeBuffer, EPAFacetManager& manager)
	{
		...
		for(PxU32 a = 0; a < 3; ++a)
		{
			m_adjFacets[a]->silhouette(m_adjEdges[a], w, edgeBuffer, manager);
		}
	}
	...
	bool EPA::expand(const Ps::aos::Vec3VArg q0, const Ps::aos::Vec3VArg q1, const Ps::aos::Vec3VArg q2, EPASupportMapPair* pair,  PxI32& numVerts)
	{
		...
		//if(FAllGrtrOrEq(FAbs(dist3), eps))
		{
			...
			{
				PX_ASSERT(FAllGrtr(temp3, zero) == 1);
				aBuf[3] = a3;
				bBuf[3] = b3;
				if(addInitialFacet4())
				{
		...
		//if(FAllGrtrOrEq(FAbs(dist4), eps))
		{
			...
			{
				PX_ASSERT(FAllGrtr(temp4, zero) == 1);
				aBuf[3] = a4;
				bBuf[3] = b4;
				if(addInitialFacet4())
				{	
	...
	//bool EPA::expandTriangle(const ConvexV& a, const ConvexV& b, Support aSupport, Support bSupport)
	bool EPA::expandTriangle(const ConvexV&, const ConvexV&, EPASupportMapPair* pair, PxI32& numVerts)
	{
		...
		const FloatV dist3 = FSub(V3Dot(vv, q3), d);//V3Dot(vv, V3Sub(q3, q0));
		const FloatV temp3 = FMul(dist3, dist0);
		const BoolV con3 = BAnd(FIsGrtrOrEq(FAbs(dist3), eps), FIsGrtr(temp3, zero));
		if(BAllEq(con3, bTrue))
		//if(FAllGrtrOrEq(FAbs(dist3), eps))
	...
	PxGJKStatus EPA::PenetrationDepth(const ConvexV& a, const ConvexV& b, EPASupportMapPair* pair, const Ps::aos::Vec3V* PX_RESTRICT /*Q*/, const Ps::aos::Vec3V* PX_RESTRICT A, const Ps::aos::Vec3V* PX_RESTRICT B, const PxI32 size, Ps::aos::Vec3V& pa, Ps::aos::Vec3V& pb)
	{
		...
		do 
		{
			...
			if (!facet->isObsolete()) 
			{
				...
				//const FloatV length = FSqrt(fSqDist);
				...
				/*if(FAllGrtrOrEq(eps2, FAbs(FSub(fSqDist, upper_bound2))))
				{
					facet->getClosestPoint(aBuf, bBuf, pa, pb);
					return true;
				}
				...
				const FloatV planeDist = FSub(V3Dot(facet->getPlaneNormal(), q), facet->getPlaneDist());
				if(FAllGrtrOrEq(eps2, FAbs(planeDist)))
				{
					continue;
				}*/
		...
		facet->getClosestPoint(aBuf, bBuf, pa, pb);
		return (heap.heapSize > 0) ? EPA_DEGENERATE : EPA_FAIL;
		//return false;
	}
}
