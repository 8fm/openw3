...
#include "GuDistanceSegmentBoxSIMD.h"
#include "GuDistancePointBox.h"
#include "GuDistancePointBoxSIMD.h"
#include "GuDistanceSegmentSegment.h"
#include "GuDistanceSegmentSegmentSIMD.h"
#include "GuDistancePointSegment.h"
#include "GuDistancePointSegmentSIMD.h"
												...
												Ps::aos::Vec3V& localSpaceClosestP)
{
	...
	const Vec3V eX(V3Scale(unitX, V3GetX(boxExtent)));
	const Vec3V eY(V3Scale(unitY, V3GetY(boxExtent)));
	const Vec3V eZ(V3Scale(unitZ, V3GetZ(boxExtent)));
	...
	//check for parallel
	const Vec3V absD = V3Abs(sab);
	const BoolV bParallel = V3IsGrtrOrEq(v3Eps, absD);
	const BoolV bOutOfRange = V3IsGrtr(V3Abs(sa), boxExtent);
	const BoolV bParallelAndOutOfRange(BAnd(bParallel, bOutOfRange));
	...
	{
		const Vec3V nearP = V3ScaleAdd(sab, tNear,sa);
		const Vec3V farP = V3ScaleAdd(sab, tFar, sa);
	...
	Vec4V sqDist0 = distanceSegmentSegmentSquared4(sa, sab, a, c, e, g, ab, s0, t0, closest0); //ab, cd, ef, gh
	Vec4V sqDist1 = distanceSegmentSegmentSquared4(sa, sab, a, b, e, f, ac, s1, t1, closest1); //ac, bd, eg, fh
	Vec4V sqDist2 = distanceSegmentSegmentSquared4(sa, sab, a, b, c, d, ae, s2, t2, closest2); //ae, bf, cg, dh

	Vec3V closest[3];
	FloatV sqDist[3];
	FloatV s[3];
	{
		const Vec4V sqDistv = sqDist0; 
		const BoolV con0 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetX(sqDist0))));
		const BoolV con1 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetY(sqDist0))));
		const BoolV con2 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetZ(sqDist0)))); 

		closest[0] = V3Sel(con0, closest0[0], V3Sel(con1, closest0[1], V3Sel(con2, closest0[2], closest0[3])));
		sqDist[0] =FSel(con0, V4GetX(sqDist0), FSel(con1, V4GetY(sqDist0), FSel(con2,  V4GetZ(sqDist0),  V4GetW(sqDist0))));
		s[0] = FSel(con0, V4GetX(s0), FSel(con1, V4GetY(s0), FSel(con2, V4GetZ(s0), V4GetW(s0))));
	}

	{

		const Vec4V sqDistv = sqDist1; 
		const BoolV con0 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetX(sqDist1))));
		const BoolV con1 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetY(sqDist1))));
		const BoolV con2 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetZ(sqDist1)))); 

		closest[1] = V3Sel(con0, closest1[0], V3Sel(con1, closest1[1], V3Sel(con2, closest1[2], closest1[3])));
		sqDist[1] =FSel(con0, V4GetX(sqDist1), FSel(con1, V4GetY(sqDist1), FSel(con2,  V4GetZ(sqDist1),  V4GetW(sqDist1))));
		s[1] = FSel(con0, V4GetX(s1), FSel(con1, V4GetY(s1), FSel(con2, V4GetZ(s1), V4GetW(s1))));
		
	}

	{

		const Vec4V sqDistv = sqDist2; 
		const BoolV con0 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetX(sqDist2))));
		const BoolV con1 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetY(sqDist2))));
		const BoolV con2 = BAllTrue4(V4IsGrtrOrEq(sqDistv, V4Splat(V4GetZ(sqDist2)))); 

		closest[2] = V3Sel(con0, closest2[0], V3Sel(con1, closest2[1], V3Sel(con2, closest2[2], closest2[3])));
		sqDist[2] =FSel(con0, V4GetX(sqDist2), FSel(con1, V4GetY(sqDist2), FSel(con2,  V4GetZ(sqDist2),  V4GetW(sqDist2))));
		s[2] = FSel(con0, V4GetX(s2), FSel(con1, V4GetY(s2), FSel(con2, V4GetZ(s2), V4GetW(s2))));
	}

	//compute the closest point of segment to edges
	const FloatV dist = sqDist[0];

	const BoolV con0 = FIsGrtrOrEq(dist, sqDist[1]);
	const BoolV con1 = FIsGrtrOrEq(dist, sqDist[2]);
