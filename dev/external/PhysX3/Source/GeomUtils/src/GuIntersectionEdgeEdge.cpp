...
#include "GuIntersectionEdgeEdgeSIMD.h"
...
bool Gu::intersectEdgeEdge(const Ps::aos::Vec3VArg p1, const Ps::aos::Vec3VArg p2, const Ps::aos::Vec3VArg dir, const Ps::aos::Vec3VArg p3, const Ps::aos::Vec3VArg p4, Ps::aos::FloatV& dist, Ps::aos::Vec3V& ip)
{
	...
	const Vec3V ipPlane = V3ScaleAdd(v2, t, p3);
	...
	const Vec3V tip = V3ScaleAdd(ndir, tValue, ipPlane);
							...
							Ps::aos::Vec4V& dist, Ps::aos::Vec3V* ip)
{
	...
	const Vec3V ipPlane0 = V3ScaleAdd(ab0, V4GetX(t), a0);
	const Vec3V ipPlane1 = V3ScaleAdd(ab1, V4GetY(t), a1);
	const Vec3V ipPlane2 = V3ScaleAdd(ab2, V4GetZ(t), a2);
	const Vec3V ipPlane3 = V3ScaleAdd(ab3, V4GetW(t), a3);
	...
	const Vec4V tValue = V4Neg(V4Scale(nom1, denom1));

	//const FloatV tValue = FNeg(FDiv(V3Dot(norm, qip),V3Dot(norm, ndir)));

	const Vec3V tip0 = V3ScaleAdd(ndir, V4GetX(tValue), ipPlane0);
	const Vec3V tip1 = V3ScaleAdd(ndir, V4GetY(tValue), ipPlane1);
	const Vec3V tip2 = V3ScaleAdd(ndir, V4GetZ(tValue), ipPlane2);
	const Vec3V tip3 = V3ScaleAdd(ndir, V4GetW(tValue), ipPlane3);
	...
	// check if intersection point (ip) is between edge (p1,p2) vertices
	FloatV temp[4];
	temp[0] = V3SumElems(vw0);
	temp[1] = V3SumElems(vw1);
	temp[2] = V3SumElems(vw2);
	temp[3] = V3SumElems(vw3);
