...
namespace Gu
{
	class LSSCollider : public VolumeCollider
	{
		...
		PX_FORCE_INLINE			Ps::IntBool		LSSAABBOverlap(const Vec3V center, const Vec3V extents) const;
#endif
		PX_FORCE_INLINE			Ps::IntBool		LSSAABBOverlap(const PxVec3& center, const PxVec3& extents) const;
	...
	class HybridLSSCollider : public LSSCollider
	{
		...
		PX_FORCE_INLINE							~HybridLSSCollider()									{}	
						...
						bool					Collide(ReportCapsuleCallback cb, void* userData,
														const Gu::Capsule& lss, const RTreeMidphaseData& model,
														const Cm::Matrix34* worldl=NULL, const Cm::Matrix34* worldm=NULL);
		//protected:
