...
struct PxsFluidParticleOpcodeCache
{
	...
	{	
		...
		if (isSmallMesh && mTriangleCount <= 1)
		{
			...
			//PxBounds3 bounds
			PxU8* ptr =	mData+1;
			reinterpret_cast<PxU16&>(*ptr) = (mTriangleCount > 0) ? static_cast<PxU16>(triangles[0]) : 0;
			ptr += sizeof(PxU16);
...
private:
	...
	static PX_FORCE_INLINE bool writeTriangles_BaseU16_9xU10(PxU8* data, const PxU32* triangles, const PxU32 numTriangles)
	{
		...
		if (range < (1 << 10))
		{
			...
			buffer[1] = (triCopy[0] <<  6) | (triCopy[1] >> 4);
			buffer[2] = (triCopy[1] << 12) | (triCopy[2] << 2) | (triCopy[3] >> 8);
			buffer[3] = (triCopy[3] <<  8) | (triCopy[4] >> 2);
			buffer[4] = (triCopy[4] << 14) | (triCopy[5] << 4) | (triCopy[6] >> 6);
			buffer[5] = (triCopy[6] << 10);
