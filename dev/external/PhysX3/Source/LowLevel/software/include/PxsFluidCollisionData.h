...
public:
	PX_FORCE_INLINE void init(const PxVec3& particlePos, const PxF32 particleRestOffset, const PxU32 particleIndex, const PxvParticleFlags _particleFlags)
	{
		...
		particleFlags.api = _particleFlags.api & ((~PxU16(PxParticleFlag::eCOLLISION_WITH_STATIC)) & (~PxU16(PxParticleFlag::eCOLLISION_WITH_DYNAMIC)));
		
		// Reduce cache bits
		// 11 -> 01
		// 01 -> 00
		// 00 -> 00
		PxU16 reducedCache = ((_particleFlags.low & PxvInternalParticleFlag::eGEOM_CACHE_MASK) >> 1) & PxvInternalParticleFlag::eGEOM_CACHE_MASK;
		particleFlags.low = (_particleFlags.low & ~PxU16(PxvInternalParticleFlag::eGEOM_CACHE_MASK)) | reducedCache;

 		debugParticleIndex = particleIndex;
