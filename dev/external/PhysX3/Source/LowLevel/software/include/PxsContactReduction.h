...
#define PXS_SEPARATION_TOLERANCE 0.005f
	...
	public:
		...
		void reduceContacts()
		{
			...
			PxU16 a = 1;
			for(; a < mNumOriginalContacts; ++a)
			{
				PxI32 index = -1;
				for(PxU32 b = numPatches; b > 0; --b)
				{
					PxsContactPatch& patch = mIntermediatePatches[b-1];
					if(mMaterialInfo[patch.startIndex].mMaterialIndex0 == mMaterialInfo[a].mMaterialIndex0 && mMaterialInfo[patch.startIndex].mMaterialIndex1 == mMaterialInfo[a].mMaterialIndex1 && 
						patch.rootNormal.dot(mOriginalContacts[a].normal) >= PXS_NORMAL_TOLERANCE)
					{
						index = b-1;
						break;
					}
				...
				{
					mIntermediatePatches[numPatches-1].stride = a - mIntermediatePatches[numPatches - 1].startIndex;
					//Create a new patch...
					if(numPatches == CONTACT_REDUCTION_MAX_PATCHES)
					{
						break;
					}
					mIntermediatePatches[numPatches].startIndex = a;
					mIntermediatePatches[numPatches].mNextPatch = NULL;
					if(index == -1)
					{
						mIntermediatePatches[numPatches].rootIndex = numPatches;
						mIntermediatePatches[numPatches].rootNormal = mOriginalContacts[a].normal;
						mIntermediatePatches[numPatches].maxPenetration = mOriginalContacts[a].separation;
					...
					else
					{
						...
						mIntermediatePatches[rootIndex].maxPenetration = mIntermediatePatches[numPatches].maxPenetration = PxMin(mIntermediatePatches[rootIndex].maxPenetration, mOriginalContacts[a].separation);
			...
			for(PxU32 a = 0; a < numPatches; ++a)
			{
				if(mIntermediatePatchesPtrs[a]->rootIndex == mIntermediatePatchesPtrs[a]->index)
				{
					...
					{
						//Iterate through and find the deepest contact
