...
//#define BIT_ARRAY_STACK	512
	...
	class BitArray
	{
		...
		// Data access
		PX_FORCE_INLINE	Ps::IntBool		isSet(PxU32 bitNumber)			const	{ return mBits[bitNumber>>5] & (1<<(bitNumber&31));		}
		PX_FORCE_INLINE	Ps::IntBool		isSetChecked(PxU32 bitNumber)	const
										{
											const PxU32 index = bitNumber>>5;
											if(index>=mSize)
												return 0;
											return mBits[index] & (1<<(bitNumber&31));
										}
...
///////////////////////////////////////////////////////////////////////////////
	...
	class MBP_PairManager : public Ps::UserAllocated
	{
		public:
						...
						MBP_Pair*			addPair				(PxU32 id0, PxU32 id1, const PxcBpHandle* PX_RESTRICT groups = NULL);
						bool				removePair			(PxU32 id0, PxU32 id1);
//						bool				removePairs			(const BitArray& array);
						bool				removeMarkedPairs	(MBP_Object* objects, PxsBroadPhaseMBP* mbp, const BitArray& updated, const BitArray& removed);
...
//		MBP_AABB	PX_ALIGN(128, mUpdatedDynamicBoxes_Stack[STACK_BUFFER_SIZE]);
	...
	class BoxPruner : public Ps::UserAllocated
	{
		...
		void				findOverlapsMT(MBP_PairManager& pairManager, const PxcBpHandle* PX_RESTRICT groups);
...
MBP_Pair* MBP_PairManager::addPair(PxU32 id0, PxU32 id1, const PxcBpHandle* PX_RESTRICT groups)
{
	PX_ASSERT(id0!=INVALID_ID);
	PX_ASSERT(id1!=INVALID_ID);

	if(groups && groups[DecodeHandle_Index(id0)] == groups[DecodeHandle_Index(id1)])
		return NULL;
...
void MBP_PairManager::shrinkMemory()
{
	...
	const PxU32 correctHashSize = NextPowerOfTwo(mNbActivePairs);
...
static PX_FORCE_INLINE void outputPair_DynamicDynamic(
												...
												const PxcBpHandle* PX_RESTRICT groups
												)
{
#ifdef CHECK_NB_OVERLAPS
	gNbOverlaps++;
...
//			printf("Code2: %d | %d\n", encoded0, encoded1);
	...
	pairManager.addPair(id0, id1, groups);
}
...
static PX_FORCE_INLINE void outputPair_DynamicStatic(
												...
												const MBPEntry* PX_RESTRICT objects, const PxcBpHandle* PX_RESTRICT groups
												)
{
#ifdef CHECK_NB_OVERLAPS
	gNbOverlaps++;
...
static void DoCompleteBoxPruning(MBP_PairManager* PX_RESTRICT pairManager, const BoxPruning_Input& input, const PxcBpHandle* PX_RESTRICT groups)
{
				...
				sleepingDynamicBoxes[index1].mMinX<=limit)
			{
				MBP_OVERLAP_TEST(sleepingDynamicBoxes[index1])
				{
					outputPair_DynamicStatic(
						*pairManager,
						index0, index1, inToOut_Dynamic, inToOut_Dynamic_Sleeping, objects, groups
						);
				}
...
//			const PxU32 l = sleepingDynamicBoxes[index0].mMinX;
			...
			while(updatedDynamicBoxes[index1].mMinX<=limit)
			{
				MBP_OVERLAP_TEST(updatedDynamicBoxes[index1])
				{
					outputPair_DynamicStatic(
						*pairManager,
						index1, index0, inToOut_Dynamic, inToOut_Dynamic_Sleeping, objects, groups
						);
				}
			...
			while(updatedDynamicBoxes[index1].mMinX<=limit)
			{
				MBP_OVERLAP_TEST(updatedDynamicBoxes[index1])
				{
					outputPair_DynamicDynamic(
						*pairManager,
						index0, index1, inToOut_Dynamic, objects, groups
						);
				}
#ifdef TEST2
				if(updatedDynamicBoxes[index1+1].mMinX<=limit)
...
static void DoBipartiteBoxPruning(MBP_PairManager* PX_RESTRICT pairManager, const BIP_Input& input, const PxcBpHandle* PX_RESTRICT groups)
{
...
//const PxU32 id0 = mObjects[inToOut_Dynamic[index0]].mMBPHandle;
			...
			staticBoxes[index1].mMinX<=limit)
		{
			MBP_OVERLAP_TEST(staticBoxes[index1])
			{
				outputPair_DynamicStatic(
					*pairManager,
					index0, index1, inToOut_Dynamic, inToOut_Static, mObjects, groups
					);
			}
#ifdef TWO_AT_A_TIME
			if(
				...
				staticBoxes[index1+1].mMinX<=limit)
			{
				MBP_OVERLAP_TEST(staticBoxes[index1+1])
				{
					outputPair_DynamicStatic(
						*pairManager,
						index0, index1+1, inToOut_Dynamic, inToOut_Static, mObjects, groups
						);
				}
			}
			index1+=2;
...
//		const PxU32 l = staticBoxes[index0].mMinX;
		...
		while(dynamicBoxes[index1].mMinX<=limit)
		{
			MBP_OVERLAP_TEST(dynamicBoxes[index1])
			{
				outputPair_DynamicStatic(
					*pairManager,
					index1, index0, inToOut_Dynamic, inToOut_Static, mObjects, groups
					);
			}
#ifdef TWO_AT_A_TIME
			if(dynamicBoxes[index1+1].mMinX<=limit)
			{
				MBP_OVERLAP_TEST(dynamicBoxes[index1+1])
				{
					outputPair_DynamicStatic(
						*pairManager,
						index1+1, index0, inToOut_Dynamic, inToOut_Static, mObjects, groups
						);
				}
			}
			index1+=2;
...
void BoxPruner::findOverlapsMT(MBP_PairManager& pairManager, const PxcBpHandle* PX_RESTRICT groups)
{
	PX_ASSERT(!mNeedsSorting);
	if(!mNbUpdatedBoxes)
		return;
	...
	if(mInput.mNeeded)
		DoCompleteBoxPruning(&pairManager, mInput, groups);

	if(mInput.mBIPInput.mNeeded)
		DoBipartiteBoxPruning(&pairManager, mInput.mBIPInput, groups);
...
PxU32 MBP::addBoxPruner(const PxBroadPhaseRegion& region, bool populateRegion)
{
	...
	if(mFirstFreeIndexBP!=INVALID_ID)
	{
		...
		buffer = (BoxPrunerData* PX_RESTRICT)mBoxPruners.GetEntries();
		buffer += regionHandle;
	...
	else
	{
		...
		buffer = (BoxPrunerData* PX_RESTRICT)mBoxPruners.Reserve(BoxPrunerDataSize);
	}
	...
	setupOverlapFlags(mNbBoxPruners, (BoxPrunerData* PX_RESTRICT)mBoxPruners.GetEntries());
...
bool MBP::removeBoxPruner(PxU32 handle)
{
	...
	BoxPrunerData* PX_RESTRICT boxPruner = (BoxPrunerData* PX_RESTRICT)mBoxPruners.GetEntries();
	boxPruner += handle;
...
//				printf("Object to update!\n");
	...
	PX_DELETE(bp);
	boxPruner->mBP = NULL;
	boxPruner->mUserData = reinterpret_cast<void*>(mFirstFreeIndexBP);
...
PxU32 MBP::addObject(const MBP_AABB& box, PxcBpHandle userID, bool isStatic)
{
	...
	if(mFirstFreeIndex!=INVALID_ID)
	{
		...
		flipFlop = objectMemory->getFlipFlop();
	}
	else
	{
...
bool MBP::removeObject(PxU32 handle)
{
	...
	MBP_Object* PX_RESTRICT objects = (MBP_Object* PX_RESTRICT)mMBP_Objects.GetEntries();
...
bool MBP_PairManager::removeMarkedPairs(MBP_Object* objects, PxsBroadPhaseMBP* mbp, const BitArray& updated, const BitArray& removed)
{
	PxU32 i=0;
...
void MBP::findOverlapsMT(const PxcBpHandle* PX_RESTRICT groups)
{
	...
	{
		if(boxPruners[i].mBP)
			boxPruners[i].mBP->findOverlapsMT(mPairManager, groups);
	}
}
...
{
	PxU32 nb = mNbBoxPruners;
	BoxPrunerData* PX_RESTRICT boxPruners = (BoxPrunerData* PX_RESTRICT)mBoxPruners.GetEntries();
	while(nb--)
	{
...
PxsBroadPhaseMBP::PxsBroadPhaseMBP(PxcScratchAllocator& scratchAllocator, const PxSceneDesc& desc, PxsAABBManager* manager) :
	mMBPUpdateWorkTask(scratchAllocator),
	mMBPPostUpdateWorkTask(scratchAllocator),
	mMapping	(NULL),
	mCapacity	(0),
	mGroups		(NULL)
{
...
//		mBoxPruners[i] = mbp->GetBoxPruner(i);
	...
	for(PxU32 i=0;i<nb;i++)
	{
		const BoxPruner* bp = mbp->getBoxPruner(i);
		if(bp && bp->mNbUpdatedBoxes && bp->mInput.mNeeded && bp->mInput.mUpdatedDynamicBoxes)
		{
			mBoxPruners[nbNeeded++] = bp;
		}
	}
