...
class PxsFluidCollisionTask : public PxLightCpuTask
{
public:
	PxsFluidCollisionTask(PxsFluidCollision& context, PxU32 taskDataIndex) :
	  ...
	  virtual void run()
	  {
		  CM_PROFILE_ZONE(mCollisionContext.mParticleSystem.getContext().getEventProfiler(), Cm::ProfileEventId::SimTask::GetPxsFluidCollision());
		  mCollisionContext.processShapeListWithFilter(mTaskDataIndex);
	  }

	  virtual const char* getName() const
	  {
		  return "PxsFluidCollisionTask";
	  }
										...
										PxBaseTask& continuation)
{
	...
	const PxU32 numSPUs = mParticleSystem.getContext().getSceneParamInt(PxPS3ConfigParam::eSPU_FLUID_COLLISION);
	if(numSPUs > 0)
	{
...
void PxsFluidCollision::processShapeListWithFilter(PxU32 taskDataIndex,const PxU32 skipNum)
{
	...
	while (it != taskData.packetEnd)
	{
		...
		updateFluidShapeCollision(mParticleSystem.mParticleState->getParticleBuffer(), mParticleSystem.mFluidTwoWayData, mParticleSystem.mTransientBuffer,
												  ...
												  const PxsParticleStreamShape& streamShape)
{
	...
	else
	{
		...
		packetCorner = PxVec3(packet.coords.x, packet.coords.y, packet.coords.z) * mParams.packetSize;
	}
	...
	while (numProcessedParticles < numParticles)
	{
		...
		updateSubPacket(particlesSp, fluidTwoWayData, transientBuf, constraintBufs, perParticleCacheSp, opcodeCache,  
...
{	
	...
	if (numDynamicShapes > 0)
	{
		...
		{
			PxsParticleCollData& collData = collDataSp[p];
			collisionResponse(collData, isTwoWay, mParams);			
			clampToMaxMotion(collData, mParams.maxMotionDistance, maxMotionDistanceSqr); 
	...
	for (PxU32 p=0; p < numParticlesSp; p++)
	{
		...
		collisionResponse(collData, false, mParams);
	...
	if(transientBuf)
	{
		...
		{
			PxsParticleCollData& collData = collDataSp[p];
			transientBuf[collData.debugParticleIndex] =collData.surfaceNormal;
		}
	}
	...
	if(fluidTwoWayData)
	{
		for (PxU32 p=0; p < numParticlesSp; p++)
		{
			...
			fluidTwoWayData[particleIndex].body = collData.twoWayBody;
			fluidTwoWayData[particleIndex].impulse = collData.twoWayImpulse;	
		}
	}
												   ...
												   const PxsW2STransformTemp& w2sTransform)
{
	...
	for (PxU32 p=0; p < numParticles; p++)
	{
		...
		collData.c0 = constraintBufs.constraint0Buf + collData.debugParticleIndex;
		collData.c1 = constraintBufs.constraint1Buf + collData.debugParticleIndex;	
	...
	else if (body)
	{
		for (PxU32 p=0; p < numParticles; p++)
		{
			...
			PxsFluidConstraintDynamic& c0Dynamic = constraintBufs.constraint0DynamicBuf ? constraintBufs.constraint0DynamicBuf[collData.debugParticleIndex] : cdTemp;
			PxsFluidConstraintDynamic& c1Dynamic = constraintBufs.constraint1DynamicBuf ? constraintBufs.constraint1DynamicBuf[collData.debugParticleIndex] : cdTemp;
			c0Dynamic.setEmpty();
			c1Dynamic.setEmpty();
