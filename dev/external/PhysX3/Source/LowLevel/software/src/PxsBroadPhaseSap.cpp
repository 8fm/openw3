...
public:
	...
	const char* getName() const { return "BroadPhaseSapRemoveSpuTask"; }
...
public:
	...
	const char* getName() const { return "BroadPhaseSapCreateSpuTask"; }
...
public:
	...
	const char* getName() const { return "BroadPhaseSapUpdateSpuTask"; }
...
PxsBroadPhaseContextSap::PxsBroadPhaseContextSap(PxcScratchAllocator& scratchAllocator)  
...
PxsBroadPhaseContextSap* PxsBroadPhaseContextSap::create(PxcScratchAllocator& scratchAllocator)
{	
	...
	if(bpc)
	{
		new(bpc) PxsBroadPhaseContextSap(scratchAllocator);
	}
	return bpc;
}
...
PX_FORCE_INLINE static void testPostShiftOrder(const PxcBPValType prevVal, PxcBPValType& currVal, const PxcBpHandle prevIsMax, const PxcBpHandle currIsMax)
{
	if(currVal < prevVal)
	{
		...
		const PxcBPValType shiftCorrection = (prevIsMax==currIsMax) ? 0 : 1;
...
void BroadPhaseBatchUpdateWorkTask::run()
{
	mPairsSize=0;
...
void PxsBroadPhaseContextSap::updatePPU(PxBaseTask* continuation)
{
	...
	mBatchUpdateTasks[2].setContinuation(continuation);
	mBatchUpdateTasks[1].setContinuation(&mBatchUpdateTasks[2]);
	mBatchUpdateTasks[0].setContinuation(&mBatchUpdateTasks[1]);

	mBatchUpdateTasks[2].removeReference();
	mBatchUpdateTasks[1].removeReference();
	mBatchUpdateTasks[0].removeReference();
}
