...
{
	mCells = reinterpret_cast<PxsParticleCell*>(PX_ALLOC(numHashBuckets * sizeof(PxsParticleCell), PX_DEBUG_EXP("PxsParticleCell")));
...
void PxsFluidSpatialHash::updatePacketHash(PxU32& numSorted, PxU32* sortedIndices, PxsFluidParticle* particles, const Cm::BitMap& particleMap, const PxU32 validParticleRange, PxBaseTask* continuation)
{
	...
	PxU32 packetMult = (1 << mPacketMultLog);
	...
	{		
		PxU32 numPackets = 0;
		PxU32 numParticlesRejected = 0;	// Track number of particles that where not added to the hash.
		numSorted = 0;
		...
		for (PxU32 particleIndex = particleIt.getNext(); particleIndex != Cm::BitMap::Iterator::DONE; particleIndex = particleIt.getNext())
		{
			...
			if(packet->numParticles == PX_INVALID_U32)
			{
				...
				if(numPackets >= PXS_PARTICLE_SYSTEM_PACKET_LIMIT)
				{
					...
					PX_WARN_ONCE(true, "Particle: reached maximum number of packets!");
		...
		reorderParticleIndicesToPackets(sortedIndices, numParticles, particles, particleMap, hashKeyArray);
...
*/
void PxsFluidSpatialHash::reorderParticleIndicesToPackets(PxU32* sortedIndices, PxU32 numHashedParticles, const PxsFluidParticle* particles, const Cm::BitMap& particleMap, PxU16* hashKeyArray)
{
	Cm::BitMap::Iterator particleIt(particleMap);
	for (PxU32 particleIndex = particleIt.getNext(); particleIndex != Cm::BitMap::Iterator::DONE; particleIndex = particleIt.getNext())
	{
		...
		if(particle.flags.api & PxParticleFlag::eSPATIAL_DATA_STRUCTURE_OVERFLOW)
			continue;
		...
		PX_ASSERT(index < numHashedParticles);
		PX_UNUSED(numHashedParticles);
											  ...
											  PxsFluidParticle* particles, PxU32* particleIndices)
{
	...
	PxU32 packetMult = (1 << packetMultLog);
