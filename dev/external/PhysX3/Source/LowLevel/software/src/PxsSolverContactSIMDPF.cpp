...
{   
	...
	for(PxU32 i=0;i< frictionPatchCount;i++)
	{
		...
		const FloatV restitution = FLoad(contactBase->restitution);
		...
		header->setRestitution(contactBase->restitution);
			...
			patch = c.contactPatches[patch].next)
		{
			...
			for(PxU32 j=0;j<count;j++)
			{
				...
				PxcSolverContact* PX_RESTRICT solverContact = reinterpret_cast<PxcSolverContact* PX_RESTRICT>(p);
				p += pointStride;
	...
	for(PxU32 i=0;i< frictionPatchCount;i++)
	{
		...
		const Vec3V t0 = V3Sub(vrel, V3Scale(normal, V3Dot(normal, vrel)));
		const FloatV sqDist = V3Dot(t0,t0);
		const BoolV con1 = FIsGrtr(sqDist, eps);
		const Vec3V tDir0 =V3Normalize(V3Sel(con1, t0, tFallback1));
				...
				patch = c.contactPatches[patch].next)
			{
				...
				for(PxU32 j =0; j < count; j++)
				{
					...
					for(PxU32 k = 0; k < frictionPerPointCount; ++k)
					{
						...
						PxcSolverFriction* PX_RESTRICT f0 = reinterpret_cast<PxcSolverFriction* PX_RESTRICT>(p);
						p += frictionStride;
