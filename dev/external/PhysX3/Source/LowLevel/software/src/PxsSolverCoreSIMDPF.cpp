...
void concludeContactCoulomb(const PxcSolverConstraintDesc& desc, PxcSolverContext& /*cache*/)
{
	...
	const PxcSolverContactCoulombHeader* PX_RESTRICT firstHeader = (const PxcSolverContactCoulombHeader* PX_RESTRICT)cPtr;
	PxU8* PX_RESTRICT last = desc.constraint + firstHeader->frictionOffset;//getConstraintLength(desc);
	while(cPtr < last)
	{
		const PxcSolverContactCoulombHeader* PX_RESTRICT hdr = (const PxcSolverContactCoulombHeader* PX_RESTRICT)cPtr;
		...
		for(PxU32 i=0;i<numNormalConstr;i++)
		{
			PxcSolverContact *c = reinterpret_cast<PxcSolverContact* PX_RESTRICT>(cPtr);
			cPtr += pointStride;
