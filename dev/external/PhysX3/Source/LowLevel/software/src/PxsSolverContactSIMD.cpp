							...
							PxReal invMassScale1, PxReal invInertiaScale1)	
{
	...
	PxU8 flags = n.flags & PxcNpWorkUnitFlag::eFORCE_THRESHOLD ? PxcSolverContactHeader::eHAS_FORCE_THRESHOLDS : 0;
	...
	staticFrictionX_dynamicFrictionY_dominance0Z_dominance1W=V4SetZ(staticFrictionX_dynamicFrictionY_dominance0Z_dominance1W, FLoad(n.dominance0));
	staticFrictionX_dynamicFrictionY_dominance0Z_dominance1W=V4SetW(staticFrictionX_dynamicFrictionY_dominance0Z_dominance1W, FLoad(n.dominance1));
	...
	for(PxU32 i=0;i<c.frictionPatchCount;i++)
	{
		...
		const Gu::ContactPoint* contactBase = buffer.contacts + c.contactPatches[firstPatch].start;

		const PxReal combinedRestitution = contactBase->restitution;
		
		PxcSolverContactHeader* PX_RESTRICT header = reinterpret_cast<PxcSolverContactHeader* PX_RESTRICT>(ptr);
			...
			patch = c.contactPatches[patch].next)
		{
			...
			for(PxU32 j=0;j<count;j++)
			{
				...
				PxcSolverContactPoint* PX_RESTRICT solverContact = reinterpret_cast<PxcSolverContactPoint* PX_RESTRICT>(p);
				p += pointStride;
				...
				FloatV scaledBias = FMul(FMul(velMultiplier, penetration), invDtp8);
				...
				solverContact->delAngVel1_InvMassBDom = V4SetW(Vec4V_From_Vec3V(V3Neg(delAngVel1)), invMass1_dom1fV);
				solverContact->raXnXYZ_appliedForceW = V4SetW(Vec4V_From_Vec3V(raXn), zero);
				solverContact->rbXnXYZ = Vec4V_From_Vec3V(rbXn);

				solverContact->scaledBiasX_targetVelocityY_maxImpulseZ = V3Merge(scaledBias, 
					targetVelocity, FLoad(contact.maxImpulse));
		...
		const PxReal dynamicFriction = contactBase->dynamicFriction;
		...
		if(haveFriction)
		{
			...
			for(PxU32 j = 0; j < frictionPatch.anchorCount; j++)
			{
				...
				PxcSolverContactFriction* PX_RESTRICT f0 = reinterpret_cast<PxcSolverContactFriction* PX_RESTRICT>(ptr);
				ptr += frictionStride;
				PxcSolverContactFriction* PX_RESTRICT f1 = reinterpret_cast<PxcSolverContactFriction* PX_RESTRICT>(ptr);
				ptr += frictionStride;
