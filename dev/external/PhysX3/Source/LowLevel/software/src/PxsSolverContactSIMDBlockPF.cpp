											...
											const Ps::aos::Vec4VArg invMassScale1, const Ps::aos::Vec4VArg invInertiaScale1)
{
	...
	for(PxU32 i=0;i<maxPatches;i++)
	{
		...
		PxU32 clampedFric0 = hasFinished0 ? 0 : c.frictionPatches[frictionIndex0].numConstraints;
		PxU32 clampedFric1 = hasFinished1 ? 0 : c.frictionPatches[frictionIndex1].numConstraints;
		PxU32 clampedFric2 = hasFinished2 ? 0 : c.frictionPatches[frictionIndex2].numConstraints;
		PxU32 clampedFric3 = hasFinished3 ? 0 : c.frictionPatches[frictionIndex3].numConstraints;
		...
		PxcSolverContactCoulombHeader4* PX_RESTRICT header = reinterpret_cast<PxcSolverContactCoulombHeader4* PX_RESTRICT>(ptr);

		header->frictionOffset = PxU16(ptr2 - ptr);

		ptr += sizeof(PxcSolverContactCoulombHeader4);	

		PxcSolverFrictionHeader4* PX_RESTRICT fricHeader = reinterpret_cast<PxcSolverFrictionHeader4* PX_RESTRICT>(ptr2);
		...
		fricHeader->frictionPerContact = numFrictionPerPoint == 2 ? 1 : 0;
		...
		while(finished != 0xf)
		{
			...
			PxcSolverContact4Base* PX_RESTRICT solverContact = reinterpret_cast<PxcSolverContact4Base* PX_RESTRICT>(p);
			p += constraintSize;
...

			{
				...
				if(isDynamic)
				{
					PxcSolverContact4Dynamic* PX_RESTRICT dynamicContact = reinterpret_cast<PxcSolverContact4Dynamic* PX_RESTRICT>(solverContact);
				...
				Vec4V scaledBias = V4Scale(V4Mul(velMultiplier, penetration),invDtp8);
				...
				{
					PxcSolverFriction4Base* PX_RESTRICT friction = reinterpret_cast<PxcSolverFriction4Base* PX_RESTRICT>(ptr2);
					...
					if(isDynamic)
					{
						PxcSolverFriction4Dynamic* PX_RESTRICT dFric = reinterpret_cast<PxcSolverFriction4Dynamic* PX_RESTRICT>(friction);
