...
PxsCCDContext::PxsCCDContext(PxsContext* context) : 
		mPostCCDSweepTask			(this, "PxsContext::postCCDSweepTask"),
		mPostCCDAdvanceTask			(this, "PxsContext::postCCDAdvanceTask"),
		mPostCCDDepenetrateTask		(this, "PxsContext::postCCDDepenetrateTask"),
...
PxReal PxsCCDPair::sweepFindToi(PxsThreadContext& context, PxReal dt, PxU32 pass)
{
	...
	PxReal restDistance = mCm->getWorkUnit().restDistance;
	...
	context.mContactBuffer.contact(mMinToiPoint, mMinToiNormal, 0.f, g0 == PxGeometryType::eTRIANGLEMESH ? 
		mFaceIndex : PXC_CONTACT_NO_FACE_INDEX, g1 == PxGeometryType::eTRIANGLEMESH ? mFaceIndex : PXC_CONTACT_NO_FACE_INDEX);
...
bool PxsCCDPair::sweepAdvanceToToi(PxReal dt, bool clipTrajectoryToToi)
{
	...
	{
		if(thisPair->mCm->getWorkUnit().disableCCDResponse)
		{
...
*/
class PxsCCDSweepTask : public PxLightCpuTask
{
	PxsCCDPair** 					mPairs;
	PxU32							mNumPairs;
public:
...
*/
class PxsCCDAdvanceTask : public PxLightCpuTask
{
...
public:
	...
	virtual void run()
	{
		...
		for (PxU32 iIsland = mFirstThreadIsland; iIsland < lastIsland; iIsland++)
		{
			...
			for (PxU32 iFront = islandStart; iFront < islandEnd; iFront++)
			{
				...
				{
					sweepTotalHits++;
					PxU32 islandStartIndex = iIsland == 0 ? 0 : mNumIslandBodies[iIsland - 1];
					...
					if(!mDisableResweep && !pair.mCm->getWorkUnit().disableCCDResponse)
					{
	...
	virtual const char *getName() const
	{
		return "PxsCCDAdvanceTask";
	}
};

// --------------------------------------------------------------
#if CCD_POST_DEPENETRATE_ENABLE
class PxsCCDDepenetrateTask : public PxLightCpuTask
{
	PxsCCDPair* 					mPairs;
	PxU32							mNumPairs;
	PxsContext* 					mContext;
	PxReal							mDt;
	PxU32							mCCDPass;
	const PxsBlockArray<PxsCCDBody, 64>&
									mCCDBodies;
public:
	PxsCCDDepenetrateTask(PxsCCDPair* pairs, PxU32 nPairs, const PxsBlockArray<PxsCCDBody, 64>& ccdBodies,
				PxsContext* context, PxReal dt, PxU32 ccdPass)
		:	mPairs(pairs), mNumPairs(nPairs), mContext(context), mDt(dt),
			mCCDPass(ccdPass), mCCDBodies(ccdBodies) {}
	...
	virtual void run()
	{
		...
		{
			PxsCCDPair& pair = mPairs[j];
// if we use getRestDistance() then CCD sometimes fails (for zero rest distance)
// TODO: make it so zero rest distance is allowed?
// on the other hand if we use CCD_POST_DEPENETRATE_DIST and rest distance is zero it creates jitter
// cm's restDistance is set to managerDesc.restDistance = shapes[0]->getRestOffset() + shapes[1]->getRestOffset();
// in Sc::ShapeInstancePairLL::createManager(), so it's a sum of the two rest offsets
			PxReal restDist = PxMax(CCD_POST_DEPENETRATE_DIST, pair.mCm->getRestDistance());
			bool jammed;
			pair.depenetrate(restDist, *threadContext, mDt, mCCDPass, jammed); // AP newccd handle jammed?
		}
...
void PxsCCDContext::updateCCD(PxReal dt, PxBaseTask* continuation, bool disableResweep)
{
	...
	Cm::BitMap& changedTouchManager = mContext->mChangeTouchContactManager; 
	changedTouchManager.clear();// reset the change touch bitmap to zeroes. needs to be done every frame
										// otherwise we get a crash
	mContext->mNewAndLostTouchCMCount[PXS_LOST_TOUCH_COUNT] = 0; // also have to reset these counters every frame since nobody else does...
	mContext->mNewAndLostTouchCMCount[PXS_NEW_TOUCH_COUNT] = 0;
	...
	for (PxU32 j = 0; j < ccdBodyCount; j++)
	{
		...
		//Record island size
		mIslandSizes[islandCount] = islandSize + 1;
		islandCount++;
	}
	...
	for (PxU32 j = 0, n = mCCDPtrPairs.size(); j < n; j++)
	{
		...
		PxU32 id0 = p.mBa0 ? islandLabels[p.mBa0->mCCD->getIndex()] : staticLabel;
		PxU32 id1 = p.mBa1 ? islandLabels[p.mBa1->mCCD->getIndex()] : staticLabel;
	...
	for(PxU32 a = 0; a < mCCDBodies.size(); ++a)
	{
		...
		mIslandSizes[island] = writeIndex + 1;
	...
	for (PxU32 batchBegin = 0; batchBegin < nPairs; batchBegin += mCCDPairsPerBatch)
	{
		...
		PX_ASSERT(ptr && "Failed to allocate PxsCCDSweepTask");
...
{	
	...
	PxU32 countLost = 0, countFound = 0;
	...
	for (PxU32 island = 0; island < islandCount; ++island)
	{
		...
		for(PxU32 j = index; j < islandEnd; ++j)
		{
			...
			if(p.mIsEarliestToiHit)
			{
				...
				bool oldTouch = p.mCm->getTouchStatus();
				if (!oldTouch)
				{
					mContext->mChangeTouchContactManager.growAndSet(p.mCm->getIndex());
					p.mCm->mNpUnit.touch = true;
					countFound ++;
				...
				if(createReports)
				{
					PxsMaterialInfo info[Gu::ContactBuffer::MAX_CONTACTS];
					Gu::ContactBuffer& buffer = mCCDThreadContext->mContactBuffer;

					PxU16 numContacts = 0;

					//Extract any existing contacts in the stream. These could be from the discrete collision detection or a previous CCD pass
					physx::PxContactStreamIterator iter(p.mCm->mNpUnit.compressedContacts, p.mCm->mNpUnit.compressedContactSize);
					while(iter.hasNextPatch())
					{
						...
						{
							iter.nextContact();
							Gu::ContactPoint& cp = buffer.contacts[numContacts];
							cp.dynamicFriction = iter.getDynamicFriction();
							cp.staticFriction = iter.getStaticFriction();
							cp.restitution = iter.getRestitution();
							cp.point = iter.getContactPoint();
							cp.normal = iter.getContactNormal();
							cp.targetVel = iter.getTargetVel();
							cp.maxImpulse = iter.getMaxImpulse();
							cp.internalFaceIndex0 = iter.getFaceIndex0();
							cp.internalFaceIndex1 = iter.getFaceIndex1();
							cp.separation = iter.getSeparation();
							info[numContacts].mMaterialIndex0 = iter.getMaterialIndex0();
							info[numContacts].mMaterialIndex1 = iter.getMaterialIndex1();
							numContacts++;
						}
					}

					//Make sure we still have space for our new contact
					if(numContacts < Gu::ContactBuffer::MAX_CONTACTS)
					{
						//Add our new contact to the buffer
						PxU32 origContactPoints = numContacts;
						Gu::ContactPoint& cp = buffer.contacts[numContacts];
						cp.point = p.mMinToiPoint;
						cp.normal = p.mMinToiNormal;
						...
						info[numContacts].mMaterialIndex0 = p.mMaterialIndex0;
						info[numContacts].mMaterialIndex1 = p.mMaterialIndex1;
						++numContacts;

						//Keep ahold of the old contact forces (if they exist)
						PxReal* oldContactForces = p.mCm->mNpUnit.contactForces;

						//Write a new contact stream including this new contact. This will allocate memory for the contacts and forces
						writeCompressedContact(buffer.contacts, numContacts, *mCCDThreadContext, p.mCm->mNpUnit, numContacts*sizeof(PxReal), true, info);
						
						//If we previously had forces, copy them to the new force buffer
						if(oldContactForces)
						{
							for(PxU32 a = 0; a < origContactPoints; ++a)
							{
								p.mCm->mNpUnit.contactForces[a] = oldContactForces[a];
							}
						}
						//And write this new force and the contact count
						PX_ASSERT(p.mCm->mNpUnit.contactForces != NULL);
						p.mCm->mNpUnit.contactForces[origContactPoints] = p.mAppliedForce;	
						p.mCm->mNpUnit.contactCount = numContacts;
	...
	mContext->mNewAndLostTouchCMCount[PXS_LOST_TOUCH_COUNT] += countLost;
	mContext->mNewAndLostTouchCMCount[PXS_NEW_TOUCH_COUNT] += countFound;


	// --------------------------------------------------------------------------------------
	// depenetrate postpass, this is because we pull back from CCD sweep and can potentially advance into collision slightly
#if CCD_POST_DEPENETRATE_ENABLE
	for (PxU32 batchBegin = 0, n=mCCDPairs.size(); batchBegin < n; batchBegin += mCCDPairsPerBatch )
	{
		void* ptr = mTaskPool->allocate(sizeof(PxsCCDDepenetrateTask));
		PX_ASSERT(ptr && "Failed to allocate PxsCCDDepenetrateTask");
		const PxU32 batchEnd = PxMin(n, batchBegin + mCCDPairsPerBatch );
		PX_ASSERT(batchEnd >= batchBegin);
		PxsCCDDepenetrateTask* task = PX_PLACEMENT_NEW(ptr, PxsCCDDepenetrateTask)(
			mCCDPairs.begin() + batchBegin, batchEnd - batchBegin, mCCDBodies, this, mCCDThreadContext->mDt, miCCDPass);
		if (CCD_MULTITHREADED)
		{
			task->setContinuation(*mTaskManager, continuation);
			task->removeReference(); // kick off the task
		} else
			task->run();
	}
#endif
											...
											const PxsRigidBody* PX_RESTRICT rigid0, const PxsRigidBody* PX_RESTRICT rigid1)
{
	...
	};
	{
			...
			p.transform[0] = getShapeAbsPose(shapeCore0, rigidCore0, rigid0 != NULL);
			p.transform[1] = getShapeAbsPose(shapeCore1, rigidCore1, rigid1 != NULL);
