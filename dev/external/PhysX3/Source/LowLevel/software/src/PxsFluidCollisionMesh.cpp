...
void testBoundsMesh(	
	...
	Gu::VolumeColliderTrigCallback& callback)
{
	// Find colliding triangles.
	// Setup an ICE OBB for the fluid particle cell (in local space of shape)
	...
	Gu::Box orientedBoundingBox;
	orientedBoundingBox.center	= boundsInMesh.getCenter();
	orientedBoundingBox.extents	= boundsInMesh.getExtents();
	orientedBoundingBox.rot		= PxMat33(PxIdentity);

	if(!idtScaleMesh)
		meshScaling.transformQueryBounds(orientedBoundingBox.center, orientedBoundingBox.extents);
	...
	collider.Collide<1,1,0>(orientedBoundingBox, hmd, &callback, NULL, NULL);
}
...
struct PxcContactCellMeshCallback : Gu::VolumeColliderTrigCallback
{
	...
	const PxTransform& shape2World;
		...
		PxcContactCellMeshCallback(PxsParticleCollData* collData,
									const PxU32* collDataIndices,
									PxU32 numCollDataIndices,
									const Gu::InternalTriangleMeshData& meshData,									
									const Cm::FastVertex2ShapeScaling& meshScaling,
									...
									PxsFluidParticleOpcodeCache* cache,
									const PxTransform& shape2World) :
			collData(collData), collDataIndices(collDataIndices),
			numCollDataIndices(numCollDataIndices),meshData(meshData),
			meshScaling(meshScaling),proxRadius(proxRadius),cache(cache),shape2World(shape2World)
		{
			PX_ASSERT(collData);
			PX_ASSERT(collDataIndices);
			PX_ASSERT(numCollDataIndices > 0);
		...
		virtual bool processResults(PxU32 aCount, const PxVec3* aVertTriples, const PxU32* aIndices, const PxU32*)
		{
			collideCellWithMeshTriangles(collData, collDataIndices, numCollDataIndices, meshData,
				meshScaling, aVertTriples, aCount, proxRadius, shape2World);

			if (cache)
				cache->add(aIndices,aCount);
	...
	for (PxU32 i = 0; i < numParticles; ++i)
	{
		...
		if (isCached)
		{
			...
			if (numTriangles > 0)
			{
				...
				{
					Gu::MeshInterface::GetTriangleVerts(isSmallMesh, (Cm::MemFetchPtr)meshData->mTriangles, (Cm::MemFetchPtr)meshData->mVertices, *triangleIndexIt,
		...
		else
		{
			...
			PxBounds3 cachedBounds;
			{
				...
				PxVec3 newPosPredicted = particle.newPos + 3*(particle.newPos - particle.oldPos);
