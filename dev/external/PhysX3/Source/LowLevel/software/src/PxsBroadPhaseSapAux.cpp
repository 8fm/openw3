...
{ 
	return Hash32Bits_1( PxU32(id0)|(PxU32(id1)<<16) );		
}
...
const PxcBroadPhasePair* SapPairManager::AddPair(PxcBpHandle id0, PxcBpHandle id1, const PxU8 state)
{
	...
	{
		const PxU32 newHashSize=Gu::NextPowerOfTwo(mNbActivePairs+1);
	...
	{
		// Get more entries
		mHashSize = Gu::NextPowerOfTwo(mNbActivePairs+1);
		mMask = mHashSize-1;
...
void SapPairManager::shrinkMemory()
{
	...
	const PxU32 correctHashSize = Gu::NextPowerOfTwo(mNbActivePairs);
