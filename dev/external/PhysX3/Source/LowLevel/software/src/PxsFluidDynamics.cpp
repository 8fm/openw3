...
class PxsFluidDynamicsSphTask : public PxLightCpuTask
{
public:
	...
	virtual void run()
	{
		CM_PROFILE_ZONE(mDynamicsContext.mParticleSystem.getContext().getEventProfiler(), Cm::ProfileEventId::SimTask::GetPxsFluidDynamics());
		mDynamicsContext.processPacketRange(mTaskDataIndex);
	}

	virtual const char* getName() const
	{
		return "PxsFluidDynamicsSphTask";
	}
											 ...
											 PxsFluidDynamicsTempBuffers& tempBuffers)
{
	...
	PxVec3 packetCorner = PxVec3(packet.coords.x, packet.coords.y, packet.coords.z) * mParams.packetSize;

	PxU32 particlesLeftA = packet.numParticles;
	PxsFluidParticle* particlesSpA = particles + packet.firstParticle;
	PxVec3* forceBufA = forceBuf + packet.firstParticle;
	...
	while (particlesLeftA)
	{
		...
		updateCellsSubpacket(updateType, forceBufA, particlesSpA, particleCellsSpA, particleIndicesSpA, numCellHashBucketsSpA, mParams, tempBuffers);
		...
		PxU32 particlesLeftB = particlesLeftA - numParticlesSpA;
		PxsFluidParticle* particlesSpB = particlesSpA + numParticlesSpA;
		PxVec3* forceBufB = forceBufA + numParticlesSpA;
		...
		while (particlesLeftB)
		{
			...
			updateCellsSubpacketPair(updateType, forceBufA, forceBufB, particlesSpA, particlesSpB, particleCellsSpA, particleCellsSpB,
