											...
											const Ps::aos::Vec4VArg invMassScale1, const Ps::aos::Vec4VArg invInertiaScale1)
{
	...
	for(PxU32 i=0;i<maxPatches;i++)
	{
		...
		PxcSolverContactHeader4* PX_RESTRICT header = reinterpret_cast<PxcSolverContactHeader4* PX_RESTRICT>(ptr);
	...
	/*	const Vec4V normalLenSq = V4MulAdd(normalZ, normalZ, V4MulAdd(normalY, normalY, V4Mul(normalX, normalX)));
		const Vec4V invNorLen = V4Rsqrt(normalLenSq);
		normalX = V4Mul(normalX, invNorLen);
		normalY = V4Mul(normalY, invNorLen);
		normalZ = V4Mul(normalZ, invNorLen);*/
		...
		while(finished != 0xf)
		{
			...
			PxcSolverContactBatchPointBase4* PX_RESTRICT solverContact = reinterpret_cast<PxcSolverContactBatchPointBase4* PX_RESTRICT>(p);
			p += constraintSize;
...

			{
				...
				if(isDynamic)
				{
					PxcSolverContactBatchPointDynamic4* PX_RESTRICT dynamicContact = reinterpret_cast<PxcSolverContactBatchPointDynamic4* PX_RESTRICT>(solverContact);
				...
				Vec4V scaledBias = V4Scale(V4Mul(penetration, velMultiplier),invDtp8);
				...
				solverContact->maxImpulse = maxImpulse;//V4Mul(maxImpulse, maxImpulseScale);
		...
		Vec4V maxImpulseScale = V4One();
		{
			...
			header->numNormalConstr0 = (PxU8)clampedContacts0;
			header->numNormalConstr1 = (PxU8)clampedContacts1;
			header->numNormalConstr2 = (PxU8)clampedContacts2;
			header->numNormalConstr3 = (PxU8)clampedContacts3;
			...
			for(PxU32 j = 0; j < maxAnchorCount; j++)
			{
				...
				PxcSolverContactFrictionBase4* PX_RESTRICT f0 = reinterpret_cast<PxcSolverContactFrictionBase4* PX_RESTRICT>(ptr);
				ptr += frictionSize;
				PxcSolverContactFrictionBase4* PX_RESTRICT f1 = reinterpret_cast<PxcSolverContactFrictionBase4* PX_RESTRICT>(ptr);
				ptr += frictionSize;
				...
				
				{
					...
					if(isDynamic)
					{
						PxcSolverContactFrictionDynamic4* PX_RESTRICT dynamicF0 = reinterpret_cast<PxcSolverContactFrictionDynamic4* PX_RESTRICT>(f0);
					...
					const Vec4V velMultiplier = V4Sel(V4IsGrtr(resp, zero), V4Div(p84, resp), zero);
					...
					const Vec4V targetVel = V4Mul(velMultiplier, V4MulAdd(t0Z, targetVelZ,V4MulAdd(t0Y, targetVelY, V4Mul(t0X, targetVelX))));

					const Vec4V bias = V4Mul(velMultiplier, V4Scale(V4MulAdd(t0Z, errorZ, V4MulAdd(t0Y, errorY, V4Mul(t0X, errorX))), invDt));

					f0->normalX = V4Mul(t0X, maxImpulseScale);
					f0->normalY = V4Mul(t0Y, maxImpulseScale);
					f0->normalZ = V4Mul(t0Z, maxImpulseScale);
					f0->appliedForce = zero;
					f0->raXnX = raXnX;
					f0->raXnY = raXnY;
					f0->raXnZ = raXnZ;
					...
					//f0->targetVelocity = targetVel;
					f0->scaledBias = V4Sub(bias, targetVel);
					f0->broken = zero;
					f0->bias = bias;
					f0->velMultiplier = velMultiplier;
					f0->frictionBrokenWritebackByte[0] = j < clampedAnchorCount0 ? writeback0 : NULL;
					f0->frictionBrokenWritebackByte[1] = j < clampedAnchorCount1 ? writeback1 : NULL;
					f0->frictionBrokenWritebackByte[2] = j < clampedAnchorCount2 ? writeback2 : NULL;
					f0->frictionBrokenWritebackByte[3] = j < clampedAnchorCount3 ? writeback3 : NULL;		
				}

				{
					...
					if(isDynamic)
					{
						PxcSolverContactFrictionDynamic4* PX_RESTRICT dynamicF1 = reinterpret_cast<PxcSolverContactFrictionDynamic4* PX_RESTRICT>(f1);
					...
					f1->normalX = V4Mul(t1X, maxImpulseScale);
					f1->normalY = V4Mul(t1Y, maxImpulseScale);
					f1->normalZ = V4Mul(t1Z, maxImpulseScale);
					f1->appliedForce = zero;
					f1->raXnX = raXnX;
					f1->raXnY = raXnY;
					f1->raXnZ = raXnZ;
					...
					//f1->targetVelocity = targetVel;

					f1->scaledBias = V4Mul(V4Sub(bias, targetVel), velMultiplier);
					f1->broken = zero;
					f1->bias = V4Mul(velMultiplier, bias);
					f1->velMultiplier = velMultiplier;
					f1->frictionBrokenWritebackByte[0] = j < clampedAnchorCount0 ? writeback0 : NULL;
					f1->frictionBrokenWritebackByte[1] = j < clampedAnchorCount1 ? writeback1 : NULL;
					f1->frictionBrokenWritebackByte[2] = j < clampedAnchorCount2 ? writeback2 : NULL;
					f1->frictionBrokenWritebackByte[3] = j < clampedAnchorCount3 ? writeback3 : NULL;		
				}				
			}
