...
struct MassProps
{
	...
	PX_FORCE_INLINE MassProps(const PxcSolverBodyData& bd0,
...
PX_FORCE_INLINE PxReal innerProduct(const Px1DConstraint& row0, Px1DConstraint& row1, const MassProps& m)
{
...
PxQuat diagonalize(const PxMat33& m)	// jacobi rotation using quaternions 
{
	...
	for(PxU32 i=0; i < MAX_ITERS;i++)
	{
		...
		PxU32 a = d0 > d1 && d0 > d2 ? 0 : d1 > d2 ? 1 : 2;						// rotation axis index, from largest off-diagonal element
					...
					bool diagonalizeDrive)
{
	...
	{
		const PxU32 groupMajorId = sorted[i]->solveHint>>8, start = i++;
	...
	*eaConstraintData = ea;
	...
	if(NULL == ptr || ((PxU8*)(-1))==ptr)
	{
		if(NULL==ptr)
		{
			...
			PX_WARN_ONCE(true,"Either accept joints detaching/exploding or increase buffer size allocated for constraint prep by increasing PxSceneDesc::maxNbContactDataBlocks");
			return 0;
		}
		else
		{
			PX_WARN_ONCE(true,"Attempting to allocate more than 16K of constraint data.");
			PX_WARN_ONCE(true,"Either accept joints detaching/exploding or simplify constraints");
			ptr=NULL;
			return 0;
		}
	}
	...
	preprocessRows(sorted, rows, constraintCount, *sBodyData0, *sBodyData1, ims, isExtended, (constraint.flags & PxConstraintFlag::eIMPROVED_SLERP)!=0);
