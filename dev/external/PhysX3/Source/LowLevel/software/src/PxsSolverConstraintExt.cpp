	...
	for(PxU32 i=0;i<c.frictionPatchCount;i++)
	{
			...
			patch = c.contactPatches[patch].next)
		{
			...
			for(PxU32 j=0;j<count;j++)
			{
				...
				PxcSolverContactPointExt* PX_RESTRICT solverContact = reinterpret_cast<PxcSolverContactPointExt* PX_RESTRICT>(p);
				p += pointStride;
				...
				FloatV scaledBias = FMul(penetration, FMul(velMultiplier, invDtp8));
				...
				solverContact->rbXnXYZ = Vec4V_From_Vec3V(V3LoadU(rbXn));
				solverContact->delAngVel0_InvMassADom = Vec4V_From_Vec3V(V3LoadU(deltaV0.angular));
				solverContact->delAngVel1_InvMassBDom = Vec4V_From_Vec3V(V3LoadU(deltaV1.angular));
		...
		if(haveFriction)
		{
			...
			{
				PxcSolverContactFrictionExt* PX_RESTRICT f0 = reinterpret_cast<PxcSolverContactFrictionExt* PX_RESTRICT>(ptr);
				ptr += frictionStride;
				PxcSolverContactFrictionExt* PX_RESTRICT f1 = reinterpret_cast<PxcSolverContactFrictionExt* PX_RESTRICT>(ptr);
				ptr += frictionStride;
