									...
									const PxReal contactDistance)
{
	...
	for(PxU32 i=0;i<2;i++)
	{
		...
		FloatV tnearv, tfarv;
		Vec3V minv  = V3LoadU(Min);
		Vec3V maxv  = V3LoadU(Max);
		Vec3V tmpv  = V3LoadU(tmp);
		Vec3V tmp2v  = V3LoadU(tmp2);

		int rest = Gu::intersectRayAABB(minv, maxv, tmpv, tmp2v, tnearv, tfarv);
		PxReal tnear0 = FStore(tnearv);
		PxReal tfar0 = FStore(tfarv);
#else	
...
// PT: this is actually mandatory to properly handle thin capsules...
...
bool PxcContactCapsuleBox(CONTACT_METHOD_ARGS)
{
	...
	FloatV tv;
	Vec3V onBoxv;
	Gu::SegmentV segv(worldSegment);
	const Vec3V o = V3LoadU(worldBox.center);
	const Vec3V e = V3LoadU(worldBox.extents);
	const Mat33V m = Mat33V_From_PxMat33(PxMat33(transform1.q));

	LARGE_INTEGER li;
	QueryPerformanceCounter(&li);
    PxU64 val=li.QuadPart;
	
	Vec3V dist = Gu::distanceSegmentBoxSquared(segv, o, e, m, &tv, &onBoxv);

	const PxReal squareDist = FStore(dist);
	t = FStore(tv);
	V3StoreU(onBoxv,onBox); 
