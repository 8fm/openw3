...
//				ctcPts[NbContacts++] = p.pos;
	...
	const static PxI32 indices[]={ 0,1, 1,3, 3,2, 2,0, 4,5, 5,7, 7,6, 6,4, 0,4, 1,5, 2,6, 3,7, };
...
//						ctcPts[NbContacts++] = PxVec3(0, y, z);
...

	{
		...
		const static PxI32 face[][4]={ {0,1,3,2}, {1,5,7,3}, {5,4,6,7}, {4,0,2,6}, {2,3,7,6}, {0,4,5,1} };
		PxI32 addflg=0;
									 ...
									 const Cm::Matrix34& transform0, const Cm::Matrix34& transform1, PxReal contactDistance)
{
	...
	*/
	...
	for(PxU32 i=AXIS_A0; i<6; i++)
	{
		...
		if(d>=0.0f && d<minimum)	{ minimum=d; minIndex=i; }		// >=0 !!  otherwise bug at sep = 0
	}

	collisionData = minIndex + 1;	// Leave "0" for separation
