...
PX_FORCE_INLINE void BoxSupport(const float extents[3], const PxVec3& sv, float p[3])
{
	...
	PX_IR(p[0]) = PX_IR(extents[0])|(PX_IR(sv.x)&SIGN_BITMASK);
	PX_IR(p[1]) = PX_IR(extents[1])|(PX_IR(sv.y)&SIGN_BITMASK);
	PX_IR(p[2]) = PX_IR(extents[2])|(PX_IR(sv.z)&SIGN_BITMASK);
											...
											PxReal& depth, PxVec3& sep, PxcSepAxisType& code, PxReal contactDistance)
{
	...
	PxU32* PX_RESTRICT indices0  = (PxU32* PX_RESTRICT)PxAlloca(polyData0.mNbPolygons*sizeof(PxU32));
	...
	PxU32* PX_RESTRICT indices1 = (PxU32* PX_RESTRICT)PxAlloca(polyData1.mNbPolygons*sizeof(PxU32));
