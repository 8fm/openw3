									...
									PxReal restDistance)
{
	...
	Gu::Box sweptBox;
	computeSweptBox(shape0.mExtents, shape0.mCenter, matRot, unitDir, length, sweptBox);
	...
	Gu::HybridOBBCollider obbCollider;
	obbCollider.SetPrimitiveTests(true);
	...
	shapeMesh.meshData->mOpcodeModel.getRTreeMidphaseData(hmd);

	LocalContainer(tempContainer, 128);
	VolumeColliderContainerCallback callback(tempContainer);

	obbCollider.Collide<true, true, false>(vertexSpaceBox, hmd, &callback);
									...
									PxcNpCache& cache, PxcNpThreadContext& context)
{
	...
	//queryBox.mExtents += PxVec3(1000.0f); // inflate the box to very large
	obbCollider.Collide<true, true, false>(vertexSpaceBox, hmd, &callback);
	if (tempContainer.GetNbEntries() == 0)
	{
		return PX_MAX_REAL;
	...
	//printf("sweep midphase has results\n");
...
struct EntityReportContainerCallback : public Gu::EntityReport<PxU32>
{
	...
	EntityReportContainerCallback(Gu::Container& container) : container(container)
	{
		container.Reset();
	}
									...
									PxReal restDistance)
{
	...
	PxBounds3 bounds(center - extents, center + extents);


	hfUtil.overlapAABBTriangles(transform1, bounds, PxHfQueryFlags::eWORLD_SPACE, &callback);
									...
									PxcNpCache& cache, PxcNpThreadContext& context)
{
	...
	for (PxU32 ti = 0; ti < count; ti++)
	{
		...
		if (res < minTOI)
		{
			...
			PxVec3 resultNormal = v0.cross(v1);
			resultNormal.normalize();
			//if(norDotRel > 1e-6f)
			{
				tempWorldNormal = resultNormal;
				tempWorldPoint = resultPoint;
				minTOI = res;
				context.mCCDFaceIndex = orderedList[ti];
			}
