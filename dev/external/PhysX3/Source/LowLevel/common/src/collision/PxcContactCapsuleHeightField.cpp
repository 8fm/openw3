...
bool PxcContactCapsuleHeightField(CONTACT_METHOD_ARGS)
{
	...
	PxSphereGeometry s(radius);
	PxTransform ts0(transform1.transform(verticesInHfShape[0])), ts1(transform1.transform(verticesInHfShape[1]));
	PxcContactSphereHeightFieldShared(reinterpret_cast<Gu::GeometryUnion&>(s), shape1, ts0, transform1, contactDistance, npCache, contactBuffer, true);
	PxcContactSphereHeightFieldShared(reinterpret_cast<Gu::GeometryUnion&>(s), shape1, ts1, transform1, contactDistance, npCache, contactBuffer, true);
	...
	for(PxU32 row = absMinRow; row <= absMaxRow; row++)
	{
		for(PxU32 column = absMinCol; column <= absMaxCol; column++)
		{
			...
			const PxU32 minEi = (column == absMaxCol) ? 2 : 0; 
					...
					if ((ll < radiusSquared) && (t >= 0) && (t <= 1))
					{
						...
						if (hf.isDeltaHeightOppositeExtent(d.y))
						{
							...
							for (PxU32 j = 0; j < adjFaceCount; j++)
							{
								...
								if (perp.dot(n) > 0.0f) // if the normal is in perp halfspace, clamp it to Voronoi region
								{
									n = adjNormal;
									break;
								}
