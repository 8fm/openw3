...
bool PxcPCMContactCapsuleCapsule(CONTACT_METHOD_ARGS)
{
	...
	FloatV t0, t1;
	const FloatV sqDist = pcmDistanceSegmentSegmentSquared(s0, d0, s1, d1, t0, t1);

	if(FAllGrtrOrEq(inflatedSumSquared, sqDist))
	{
		...
		const BoolV con = FIsGrtr(eps, sqDist);
		...
		const FloatV dist = FSel(con, zero, FSqrt(sqDist));
		const FloatV pen = FSub(dist, sumRadius);
