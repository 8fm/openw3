						...
						PxU32& axisConstraintCount, PxsConstraintBlockManager& constraintBlockManager)
{
	...
	if(frictionPatchByteSize >0 && (0==constraintBlockByteSize || constraintBlock))
	{
		...
		if(0==frictionPatches || ((PxcFrictionPatch*)(-1))==frictionPatches)
		{
			...
			{
				PX_WARN_ONCE(true,"Attempting to allocate more than 16K of friction data for a single contact pair in narrowphase.");
				PX_WARN_ONCE(true,"Either accept dropped contacts or simplify collision geometry");
...
//	const PxTransform& bodyFrame1 = body1 ? body1->body2World : PxTransform(PxIdentity);
			...
			while(iter.hasNextPatch())
			{
				...
				while(iter.hasNextContact())
				{
					...
					if(maxImpulse != 0.f)
					{
						...
						buffer.contacts[numContacts].internalFaceIndex0 = (iter.getMaterialIndex0() | (iter.getMaterialIndex1() << 16));
	...
	createContactPatches(threadContext, 0, PXC_SAME_NORMAL);		
	bool overflow = correlatePatches(c, buffer, *bodyFrame0, *bodyFrame1, PXC_SAME_NORMAL, 0, 0);
	PX_UNUSED(overflow);
	...
	if(successfulReserve)
	{
		...
		if(solverConstraint)
		{
			...
			Ps::invalidateCache(solverConstraint, solverConstraintByteSize);
						...
						PxU32& numFrictionPatches)
{
	...
	if(frictionPatchByteSize > 0)
	{
		...
		if(0==frictionPatches || ((PxcFrictionPatch*)(-1))==frictionPatches)
		{
			if(0==frictionPatches)
			{
				PX_WARN_ONCE(true,"Reached limit set by PxSceneDesc::maxNbContactDataBlocks - ran out of buffer space for constraint prep.");
				PX_WARN_ONCE(true,"Either accept dropped contacts or increase buffer size allocated for narrow phase by increasing PxSceneDesc::maxNbContactDataBlocks");
			}
			else
			{
				PX_WARN_ONCE(true,"Attempting to allocate more than 16K of friction data for a single contact pair in constraint prep.");
				PX_WARN_ONCE(true,"Either accept dropped contacts or simplify collision geometry");
				frictionPatches=NULL;
			}
		}
								...
								PxcNpThreadContext& threadContext)
{
	...
	for(PxU32 a = 0; a < 4; ++a)
	{
		...
		for(PxU32 i = 0; i < descs[a].numFrictionPatches; i++)
		{
			...
			if(c.frictionPatchContactCounts[ind]!=0)
			{
				...
				if(haveFriction)
				{
					const PxU32 fricCount = c.frictionPatches[ind].anchorCount * 2;
	...
	const PxU32 headerSize = sizeof(PxcSolverContactHeader4) * maxPatches;
	const PxU32 constraintSize = isStatic ? (sizeof(PxcSolverContactBatchPointBase4) * totalContacts) + ( sizeof(PxcSolverContactFrictionBase4) * totalFriction) : 
...
{	
	...
	for(PxU32 a = 0; a < 4; ++a)
	{
		...
		for(PxU32 p = c.frictionPatchCount; p > blockDesc.startFrictionPatchIndex; --p)
		{
			...
			else if(c.frictionPatches[p-1].anchorCount > 0)
				fricAnchorCount++;
		}
		...
		//If the friction patch has 0 anchors, that acts the same as if friction was disabled. In which case, we roll all the contacts into a single
		//friction patch
		if(numFricPatches != fricAnchorCount)
			return PxcSolverConstraintPrepState::eUNBATCHABLE;

		//Allocate the friction patches for this pair. We don't allocate the constraints because we do that in a big block later
	}
	...
	for(PxU32 a = 0; a < 4; ++a)
	{
		...
		n[a]->frictionDataPtr = NULL;
		n[a]->frictionPatchCount = 0;

		//KS - TODO - how can we recover if we failed to allocate this memory?
		if(successfulReserve)
		{
	#ifdef __SPU__
			n[a]->frictionDataPtr = (PxU8*)eaFrictionPatches;
	#else
			n[a]->frictionDataPtr = (PxU8*)frictionPatches;
	#endif
			n[a]->frictionPatchCount = numFrictionPatches;
...

	{
		...
		for(PxU32 a = 0; a < 4; ++a)
		{
			...
			desc[a]->writeBack	= n[a]->contactForces;
			setWritebackLength(*desc[a],  n[a]->contactForces ?  n[a]->contactCount * sizeof(PxReal) : 0);
...
PxU32 physx::writeCompressedContact(const Gu::ContactPoint* const PX_RESTRICT contactPoints, const PxU32 numContactPoints, PxcNpThreadContext& threadContext,
									...
									bool useConstraintData, PxcNpThreadContext* islandContext)
{
	...
	const bool isModifiable = !forceNoResponse && (n.flags & PxcNpWorkUnitFlag::eMODIFIABLE_CONTACT) != 0;
	...
	const PxU32 pointSize = numContactPoints * (isModifiable ? sizeof(PxModifiableContact) : hasInternalFaceIndex ? sizeof(PxFeatureContact) :	sizeof(PxSimpleContact));
	const PxU32 headerSize = isModifiable ? sizeof(PxModifyContactHeader) : sizeof(PxContactHeader);
	...
	n.compressedContacts = (PxU8* PX_RESTRICT)eaCompressedBuffer;
#else
	...
	n.compressedContactSize = requiredSize;

	/*PxContactHeader* PX_RESTRICT header = (PxContactHeader* PX_RESTRICT)data;
	header->flags = (hasInternalFaceIndex ? PxContactHeader::eHAS_FACE_INDICES : 0) | (isModifiable ? PxContactHeader::eMODIFIABLE : 0) |
		(forceNoResponse ? PxContactHeader::eFORCE_NO_RESPONSE : 0);
	header->totalContactCount = (PxU16)numContactPoints;
	PxU8* PX_RESTRICT currPtr = data + sizeof(PxContactHeader);*/
	...
	if(isModifiable)
	{
		PxModifyContactHeader* PX_RESTRICT header = (PxModifyContactHeader* PX_RESTRICT)data;
		header->flags = (hasInternalFaceIndex ? PxContactHeader::eHAS_FACE_INDICES : 0) | (isModifiable ? PxContactHeader::eMODIFIABLE : 0) |
			(forceNoResponse ? PxContactHeader::eFORCE_NO_RESPONSE : 0);
		header->totalContactCount = (PxU16)numContactPoints;
		...
		currPtr = data + sizeof(PxModifyContactHeader);
		for(PxU32 a = 0; a < numStrideHeaders; ++a)
		{
			...
			const PxsMaterialData& data0 = *n.materialManager->getMaterial(matIndex0);
			const PxsMaterialData& data1 = *n.materialManager->getMaterial(matIndex1);

			const PxReal combinedRestitution = PxsMaterialCombiner::combineRestitution(data0, data1);
			...
			const PxReal staticFriction = combinedMat.staFriction;
			const PxReal dynamicFriction = combinedMat.dynFriction;
			const PxU32 flags = combinedMat.flags;

			PxContactPatchBase* PX_RESTRICT patch = (PxContactPatchBase* PX_RESTRICT)currPtr;
			...
			PxModifiableContact* PX_RESTRICT point = (PxModifiableContact* PX_RESTRICT)currPtr;
	...
	else 
	{
		...
		header->flags = (hasInternalFaceIndex ? PxContactHeader::eHAS_FACE_INDICES : 0) | (isModifiable ? PxContactHeader::eMODIFIABLE : 0) |
			(forceNoResponse ? PxContactHeader::eFORCE_NO_RESPONSE : 0);
		header->totalContactCount = (PxU16)numContactPoints;
		currPtr = data + sizeof(PxContactHeader);
		if(hasInternalFaceIndex)
		{
			for(PxU32 a = 0; a < numStrideHeaders; ++a)
			{
				...
				PxContactPatch* PX_RESTRICT patch = (PxContactPatch* PX_RESTRICT)currPtr;
				...
				PxFeatureContact* PX_RESTRICT point = (PxFeatureContact* PX_RESTRICT)currPtr;
				for(PxU32 b = startIndex; b < endIndex; ++b)
				{
					point->contact = contactPoints[b].point;
					point->separation = contactPoints[b].separation;
		...
		else
		{
			...
			PxReal dynamicFriction, staticFriction;
			PxU32 flags;
			combiner.combineIsotropicFriction(data0, data1, dynamicFriction, staticFriction, flags);
			...
			for(PxU32 a = 0; a < numStrideHeaders; ++a)
			{
				...
				patch->materialIndex1 = matIndex1;
				patch->flags = Ps::to16(flags);
				PxSimpleContact* PX_RESTRICT point = (PxSimpleContact* PX_RESTRICT)currPtr;
				for(PxU32 b = startIndex; b < endIndex; ++b)
				{
					point->contact = contactPoints[b].point;
					point->separation = contactPoints[b].separation;
	...
	{
		PxU8* contactForces = (data + alignedRequiredSize);
#ifdef __SPU__
		PxU8* eaContactForces = ((PxU8* PX_RESTRICT)eaCompressedBuffer + alignedRequiredSize);
		n.contactForces = (PxReal*)eaContactForces;
#else
		n.contactForces = (PxReal*)contactForces;
#endif
		PxMemZero(contactForces, contactForceByteSize);
	}
...
bool physx::finishContacts(PxcNpWorkUnit& n, PxcNpThreadContext& threadContext, PxsMaterialInfo* PX_RESTRICT pMaterials)
{
	...
	n.hasSolverConstraints = ((flags & (PxcNpWorkUnitFlag::eOUTPUT_CONSTRAINTS | PxcNpWorkUnitFlag::eMODIFIABLE_CONTACT)) != 0) && buffer.count > 0;
	...
	return (writeCompressedContact(buffer.contacts, buffer.count, threadContext, n, contactForceByteSize, 0, pMaterials) != 0) || (buffer.count == 0);
}
