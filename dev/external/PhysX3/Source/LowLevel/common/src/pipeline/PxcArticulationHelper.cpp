...
namespace
{
	...
	PX_FORCE_INLINE PxcSIMDSpatial getImpulseResponseSimd(const PxcFsData& matrix, 
														  PxU32 linkID, 
														  Vec3V lZ, Vec3V aZ)
	{
		PX_ASSERT(matrix.linkCount<=PXC_ARTICULATION_MAX_SIZE);
									   ...
									   PxcFsScratchAllocator allocator)
{
	...
	Ps::invalidateCache(inertia, sizeof(PxcFsInertia)*linkCount);
	Ps::invalidateCache(contribToParent, sizeof(PxcFsInertia)*linkCount);
											 ...
											 PxU32 expectedSize)
{
	...
	Ps::invalidateCache(jointTransforms, sizeof(PxcArticulationJointTransforms)*linkCount);
	Ps::invalidateCache(poses, sizeof(PxTransform)*linkCount);
	...
	Ps::invalidateCache(jointVectors, sizeof(PxcFsJointVectors) * linkCount);
	for(PxU32 i=1;i<linkCount;i++)
	{
											 ...
											 PxReal recipDt)
{
	PxU32 linkCount = fsData.linkCount;
	PxcLtbRow* rows = getLtbRows(fsData);
	Ps::invalidateCache(rows, sizeof(PxcLtbRow) * linkCount);
										  ...
										  const PxsArticulationLink* links)
{
	...
	PX_ALIGN_PREFIX(16) PxVec4 a[] PX_ALIGN_SUFFIX(16) = { PxVec4(1,0,0,0), PxVec4(0,1,0,0), PxVec4(0,0,1,0) } ;
	const Vec3V* axes = reinterpret_cast<const Vec3V*>(a);
												   ...
												   PxReal dt)
{
	...
	PxReal normalVel = axis.dot((PxVec3&)getVelocity(fsData)[parent].angular - (PxVec3&)getVelocity(fsData)[linkIndex].angular);
	...
	s.constant = s.unbiasedConstant = 1/(a+1) * recipResponse * (b + normalVel);
	s.velMultiplier = -recipResponse;
	s.impulseMultiplier = 1;
}
													...
													PxsConstraintBlockManager& constraintBlockManager)
{
	...
	for(PxU16 i=1;i<linkCount;i++)
	{
		...
		PxVec3 twistAxis = jointTransforms[i].cB2w.rotate(PxVec3(1,0,0));
		...
		PxU8 constraintCount = swingLimited + tangentialStiffness + twistUpperLimited + twistLowerLimited;
		if(!constraintCount)
			continue;
		...
		Ps::invalidateCache(desc.constraint, constraintLength);
											   ...
											   PxReal dt)
{
	...
	for(PxU32 i=1; i<linkCount;i++)
	{
		...
		PxVec3 relQuat = Ps::log(j.targetPosition * b.cB2cA.q.getConjugate()); // as a rotation vector
		...
		PxVec3 posError = b.cA2w.rotate(relQuat); // - currentVel.angular * 0.5f * dt
