...
namespace physx
{
	namespace Sc
	{
		...
		{
		public:
			PX_FORCE_INLINE ContactReportBuffer(const PxSceneDesc& sceneDesc)
				: mBuffer(NULL)
				 ,mCurrentBufferIndex(0)
				 ,mCurrentBufferSize(sceneDesc.contactReportStreamBufferSize)
				 ,mDefaultBufferSize(sceneDesc.contactReportStreamBufferSize)
				 ,mLastBufferIndex(0)
				 ,mAllocationLocked(sceneDesc.flags & PxSceneFlag::eDISABLE_CONTACT_REPORT_BUFFER_RESIZE)
			{
				mBuffer = allocateBuffer(sceneDesc.contactReportStreamBufferSize);
				PX_ASSERT(mBuffer);
			}
