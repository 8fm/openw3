...
{
	cloth::Range<const PxVec4> particles = cloth::readCurrentParticles(*mLowLevelCloth);
	bulkData.mParticles.resize(getNbParticles(), PxClothParticle(PxVec3(0.0f), 0.0f));
	mLowLevelCloth->getFactory().copyToHost(particles.begin(), 
		particles.end(), bulkData.mParticles.begin());
...
{
	//sschirm: initialize the bulk data on heap to get correctly marked padding for serialization.
	ClothBulkData* bulkData = PX_NEW(ClothBulkData)();
...
void Sc::ClothCore::setParticles(const PxClothParticle* currentParticles, const PxClothParticle* previousParticles)
{
	if (currentParticles)
	{
		cloth::Range<PxVec4> particlesRange = mLowLevelCloth->getCurrentParticles(Cloth::HOST_MEMORY);
	...
	{
		cloth::Range<PxVec4> particlesRange = mLowLevelCloth->getPreviousParticles(Cloth::HOST_MEMORY);
...
{
	return PxU32(cloth::readCurrentParticles(*mLowLevelCloth).size());
}
...
void Sc::ClothCore::setRestPositions(const PxVec4* restPositions)
{
	if(restPositions)
	{
		cloth::Range<PxVec4> restPositionsRange = mLowLevelCloth->getRestPositions();
		mLowLevelCloth->getFactory().copyFromHost(restPositions, restPositions + 
			(PxU32)restPositionsRange.size(), (void*)restPositionsRange.begin());
	} 
	else
	{
		mLowLevelCloth->clearRestPositions();
	}
}

bool Sc::ClothCore::getRestPositions(PxVec4* restPositionsBuffer) const
{
	PxU32 nbRestPositions = mLowLevelCloth->getNumRestPositions();

	if (nbRestPositions)
	{
		cloth::Range<PxVec4> srcRange = mLowLevelCloth->getRestPositions();
		mLowLevelCloth->getFactory().copyToHost(srcRange.begin(), srcRange.end(), restPositionsBuffer);
...
void Sc::ClothCore::setClothFlag(PxClothFlag::Enum flag, bool val)
{
	...
	case PxClothFlag::eGPU:
		if (getSim() && val != (mLowLevelCloth->getFactory().getPlatform() == cloth::Factory::GPU))
			getSim()->reinsert();
		break;
...
{
	Cloth::Memory memory = data.getDataAccessFlags() & PxDataAccessFlag::eDEVICE
		? Cloth::DEVICE_MEMORY : Cloth::HOST_MEMORY;
	...
	data.particles = reinterpret_cast<PxClothParticle*>(const_cast<PxVec4*>(
		cloth::readCurrentParticles(*mLowLevelCloth, memory).begin()));
	data.previousParticles = reinterpret_cast<PxClothParticle*>(const_cast<PxVec4*>(
		cloth::readPreviousParticles(*mLowLevelCloth, memory).begin()));
}
...
void Sc::ClothCore::switchCloth(cloth::Cloth* cl)
{
	...
	{
		if(platform == cloth::Factory::GPU)
			mFabric->setLowLevelGpuFabric(NULL);
		PX_DELETE(fabric);
	}

	if(cl->getFactory().getPlatform() == cloth::Factory::GPU)
