...
void Sc::ClothSim::addCollisionSphere(const ShapeSim* shape)
{
	PxU32 startIndex = 0;
...
void Sc::ClothSim::addCollisionCapsule( const ShapeSim* shape )
{
...
void Sc::ClothSim::addCollisionPlane( const ShapeSim* shape )
{
	...
	PxU32 convexMask = 0x1 << planeIndex;
...
void Sc::ClothSim::addCollisionBox( const ShapeSim* shape )
{
	...
	PxPlane planes[6] = { 
		trafo.transform(PxPlane(PxVec3( 1,  0,  0), -halfExtents.x)),
		trafo.transform(PxPlane(PxVec3(-1,  0,  0), -halfExtents.x)),
		trafo.transform(PxPlane(PxVec3( 0,  1,  0), -halfExtents.y)),
		trafo.transform(PxPlane(PxVec3( 0, -1,  0), -halfExtents.y)),
		trafo.transform(PxPlane(PxVec3( 0,  0,  1), -halfExtents.z)),
		trafo.transform(PxPlane(PxVec3( 0,  0, -1), -halfExtents.z)),
	};
	...
	PxU32 convexMask = 0x3f << planeIndex;
	core.getLowLevelCloth()->setConvexes(createRange<const PxU32>(&convexMask, 1), convexIndex, convexIndex);
...
void Sc::ClothSim::addCollisionConvex( const ShapeSim* shape )
{
	...
	PxU32 convexMask = ((1 << numPlanes) - 1) << planeIndex;
	core.getLowLevelCloth()->setConvexes(createRange<const PxU32>(&convexMask, 1), convexIndex, convexIndex);
...
void Sc::ClothSim::addCollisionMesh( const ShapeSim* shape )
{
...
namespace
{
	void gatherHeightfieldSamples(const PxHeightField& hf, Ps::Array<PxVec3>& vertices, Ps::Array<PxHeightFieldSample>& samples)
	{
		...
		for(PxU32 i = 0; i < numRows; ++i) 
		{
			...
			{
				vertices.pushBack(PxVec3(PxReal(i), samples[j + (i*numCols)].height, PxReal(j)));
			}
		}
	}
...
void Sc::ClothSim::addCollisionHeightfield( const ShapeSim* shape )
{
...
void Sc::ClothSim::updateRigidBodyPositions()
{
	...
	for (PxU32 i = 0; i<mNumBoxes; ++i, ++shapeIt)
	{
		...
		planes.pushBack(trafo.transform(PxPlane(PxVec3( 1,  0,  0), -halfExtents.x)));
		planes.pushBack(trafo.transform(PxPlane(PxVec3(-1,  0,  0), -halfExtents.x)));
		planes.pushBack(trafo.transform(PxPlane(PxVec3( 0,  1,  0), -halfExtents.y)));
		planes.pushBack(trafo.transform(PxPlane(PxVec3( 0, -1,  0), -halfExtents.y)));
		planes.pushBack(trafo.transform(PxPlane(PxVec3( 0,  0,  1), -halfExtents.z)));
		planes.pushBack(trafo.transform(PxPlane(PxVec3( 0,  0, -1), -halfExtents.z)));
	}

	for (PxU32 i = 0; i<mNumConvexes; ++i, ++shapeIt)
	{
		const ShapeSim* shape = *shapeIt;
		const PxConvexMeshGeometry& geometry = static_cast<const PxConvexMeshGeometry&>(shape->getCore().getGeometry());
		Cm::Matrix34 trafo = core.getGlobalPose().transformInv(shape->getAbsPose()) * geometry.scale;
