...
PxReal Sc::ParticleSystemCore::getGridSize() const
{
	...
	const PxU32 packetMult =	(1 << mLLParameter.packetSizeMultiplierLog2);
...
#if PX_SUPPORT_GPU_PHYSX
void Sc::ParticleSystemCore::getReadWriteCudaBuffersGpu(PxCudaReadWriteParticleBuffers& buffers)
{
	PX_ASSERT(getSim());
	getSim()->getReadWriteCudaBuffersGpu(buffers);
}

void Sc::ParticleSystemCore::setValidParticleRangeGpu(PxU32 validParticleRange)
{
	PX_ASSERT(getSim());
	getSim()->setValidParticleRangeGpu(validParticleRange);
}

void Sc::ParticleSystemCore::setDeviceExclusiveModeFlagsGpu(PxU32 flags)
{
	PX_ASSERT(getSim());
	getSim()->setDeviceExclusiveModeFlagsGpu(flags);
}

bool Sc::ParticleSystemCore::isInDeviceExclusiveModeGpu() const
{
	PX_ASSERT(getSim());
	return getSim()->isInDeviceExclusiveModeGpu();
}
