...
void Sc::ArticulationSim::updateCachedTransforms(PxsTransformCache& cache)
{
	for(PxU32 i=0; i<mBodies.size(); i++)
	{
...
void Sc::ArticulationSim::removeBody(BodySim &body)
{
	...
	PxsArticulationLink &link = mLinks[index];

	PX_ASSERT(link.children == 0);
	PX_UNUSED(link);
...
void Sc::ArticulationSim::checkResize() const
{
	...
	if(mLinks.size()!=mSolverData.linkCount)
	{
		...
		mMotionVelocity.resize(linkCount, SpatialVector(PxVec3(0.0f), PxVec3(0.0f)));
...
void Sc::ArticulationSim::sleepCheck(PxReal dt)
{
	...
	for(PxU32 i=0;i<mLinks.size();i++)
	{
		PxReal timer = mBodies[i]->updateWakeCounter(dt, sleepThreshold);
		maxTimer = PxMax(maxTimer, timer);
		minTimer = PxMin(minTimer, timer);
	}
...
void Sc::ArticulationSim::setActive(const bool b)
{
	for(PxU32 i=0;i<mBodies.size();i++)
	{
		if (i+1 < mBodies.size())
