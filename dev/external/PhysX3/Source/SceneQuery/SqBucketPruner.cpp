...
/*
TODO:
// PT: TODO: the whole mSortedObjects/mSortedHandles buffers aren't really needed, we just need a remap table to Core (if Core is always available)
- UTS:
...
//#define DETAIL_PROFILE
//#define VERIFY_SORT
//#define BRUTE_FORCE_LIMIT	32
#define LOCAL_SIZE	256
#define USE_SIMD
#define NODE_SORT
#define NODE_SORT_MIN_COUNT	16
#ifdef PX_X360
...
static PX_FORCE_INLINE PxU32 classifyBox_x86(const BucketBox& box, const PxVec4& limits, const bool useY, const bool isCrossBucket)
{
	...
	const PxU32 Code = _mm_movemask_ps(V4IsGrtr(V4LoadA(&limits.x), comp));
...
//#define NOPS	_asm nop _asm nop _asm nop _asm nop _asm nop
#define NOPS

void BoxBucket::classifyBoxes(	float limitX, float limitYZ,
...
//			const PxU32 index = classifyBox_x86(boxes[i], limits, useY, isCrossBucket);
	...
	NOPS
	{
		for(PxU32 i=0;i<nb;i++)
		{
			const PxU32 bucketOffset = mOffsets[boxes[i].mData0]++;
...
static void processChildBuckets(PxU32 nbAllocated,
								...
								const BoxBucket& bucket, BoxBucket* PX_RESTRICT childBucket,
...
void BucketPrunerCore::release(bool releaseMemory)
{
	mDirty			= true;
	mCoreNbObjects	= 0;
...
void BucketPrunerCore::preallocate(PxU32 nb)
{
	mCoreNbObjects = 0;
	if(nb<=mCoreCapacity && (nb>=mCoreCapacity/2))
		return;
	...
	PX_FREE(mCoreRemap);
	mCoreBoxes = (PxBounds3*)PX_ALLOC(bytesNeededForBoxes, PX_DEBUG_EXP("BucketPruner"));
	mCoreObjects = (PrunerPayload*)PX_ALLOC(bytesNeededForObjects, PX_DEBUG_EXP("BucketPruner"));
	mCoreRemap = (PxU32*)PX_ALLOC(bytesNeededForRemap, PX_DEBUG_EXP("BucketPruner"));
}
...
*/
	...
	if(!mDirty)
	{
		...
		for(PxU32 i=0;i<mNbFree;i++)
		{
			...
			mCoreObjects[index] = mFreeObjects[i];
			mCoreBoxes[index] = mFreeBounds[i];
			mCoreRemap[index] = 0xffffffff;

			bool ok = mMap.insert(mFreeObjects[i], index);
			PX_UNUSED(ok);
			PX_ASSERT(ok);
		}
		mNbFree = 0;
...
{
	// If we're going to rebuild the structure anyway, don't do anything
//	if(mDirty)
//		return true;
	...
	if(removedEntry)
	{
		const PxU32 i = removedEntry->second;

		const PxU32 sortedIndex = mCoreRemap[i];
		...
		if(i!=mCoreNbObjects)
		{
			...
			mCoreObjects[i] = mCoreObjects[mCoreNbObjects];
			mCoreRemap[i] = mCoreRemap[mCoreNbObjects];

			BucketPrunerMap::Entry* movedEntry = const_cast<BucketPrunerMap::Entry*>(mMap.find(mCoreObjects[mCoreNbObjects]));
		...
		if(!mDirty)
		{
			const PxU32 i = sortedIndex;

			// PT: invalidating the box does not invalidate the sorting, since it's now captured in mData0/mData1
			PxBounds3 empty;
			empty.setEmpty();
			//mSortedWorldBoxes[i].mExtents = PxVec3(-1.0f);
			mSortedWorldBoxes[i].mCenter = empty.getCenter();
			mSortedWorldBoxes[i].mExtents = empty.getExtents();
...
/*static PX_FORCE_INLINE void precomputeBoxData(BucketBox& box)
{
	const float minX = box.mCenter.x - box.mExtents.x;
	const float maxX = box.mCenter.x + box.mExtents.x;

	box.mData0 = encodeFloat(PX_IR(minX));
	box.mData1 = encodeFloat(PX_IR(maxX));
}*/

static PxU32 sortBoxes(//Gu::RadixSortBuffered& rs,
					   PxU32 nb, const PxBounds3* PX_RESTRICT boxes, const PrunerPayload* PX_RESTRICT objects,
...
/*struct Item
{
	...
	PX_FORCE_INLINE bool operator<(const Item& p) const
	{
		return key < p.key;
	}
};
...
for(PxU32 i=0;i<nb;i++)
{
	local[i].key = boxes[i].minimum[sortAxis];
	local[i].index = i;
}
Ps::sort(local, nb);*/
...
static /*PX_FORCE_INLINE*/ PxU32 sort(const BoxBucket& parent, const PxVec3& rayDir)
{
...
static void gPrecomputeSort(BoxBucket& node, const PxVec3* PX_RESTRICT dirs)
{
	for(int i=0;i<8;i++)
...
//		printf("Order %d: %d\n", i, node.mOrder[i]);
...
#ifdef DETAIL_PROFILE
	static PX_FORCE_INLINE void	StartProfile(PxU32& val)
	{
		__asm{
			cpuid
			rdtsc
	...
	static PX_FORCE_INLINE void	EndProfile(PxU32& val)
	{
		__asm{
			cpuid
			rdtsc
...
//	printf("Nb objects: %d\n", nb);
	...
	mSortAxis = sortBoxes(/*rs,*/ nb, mCoreBoxes, mCoreObjects,
	...
	StartProfile(time);
#endif
	const PxU32 yz = mSortAxis == 1 ? 2 : 1;
...
/*	{
		precomputeBoxData(mGlobalBox);

		for(PxU32 i=0;i<5;i++)
			precomputeBoxData(mLevel1.mBucketBox[i]);

		for(PxU32 j=0;j<5;j++)
		{
			for(PxU32 i=0;i<5;i++)
				precomputeBoxData(mLevel2[j].mBucketBox[i]);
		}
		...
		for(PxU32 k=0;k<5;k++)
		{
			...
			{
				for(PxU32 i=0;i<5;i++)
					precomputeBoxData(mLevel3[k][j].mBucketBox[i]);
			}
		}
	}*/
#ifdef NODE_SORT
...
/*	struct RayAABBTest2
	{
		PX_FORCE_INLINE RayAABBTest2() {}

		PX_FORCE_INLINE RayAABBTest2(const PxVec3& origin_, const PxVec3& unitDir_, const PxReal maxDist, const PxVec3& inflation_)
		{
			Init(origin_, unitDir_, maxDist, inflation_);
		}

		PX_FORCE_INLINE void Init(const PxVec3& origin_, const PxVec3& unitDir_, const PxReal maxDist, const PxVec3& inflation_)
		{
			mOrigin = Vec3V_From_PxVec3(origin_);
			const Vec3V dirV = Vec3V_From_PxVec3(unitDir_);
			mDir = dirV;
			mInflate = Vec3V_From_PxVec3(inflation_);

			setDistance(maxDist);
		}

		PX_FORCE_INLINE void setDistance(PxReal maxDist)
		{
			const float Half = 0.5f * maxDist;
			const Vec3V DataV = V3Scale(mDir, FloatV_From_F32(Half));
			const Vec3V Data2V = V3Add(mOrigin, DataV);
			const Vec3V FDirV = V3Abs(DataV);
			mData = DataV;
			mData2 = Data2V;
			mFDir = FDirV;
			dataXZY_V = V3PermYZX(DataV);
			fdirXZY_V = V3PermYZX(FDirV);
		}

		// ### inflation
		PX_FORCE_INLINE PxU32 operator()(const Vec3V centerV, const Vec3V extentsV) const
		{
			const Vec3V DV = V3Sub(mData2, centerV);
			const BoolV maskA = V3IsGrtrOrEq(V3Add(extentsV, mFDir), V3Abs(DV));
//			if(!BAllEq(maskA, BTTTT()))
//				return 0;

			Vec3V fV = V3Mul(mData, V3PermYZX(DV));
			fV = V3NegMulSub(dataXZY_V, DV, fV);

			Vec3V fg = V3Mul(mFDir, V3PermYZX(extentsV));
			fg = V3MulAdd(extentsV, fdirXZY_V, fg);

			const BoolV maskC = BAnd(maskA, V3IsGrtrOrEq(fg, V3Abs(fV)));
//			const BoolV maskC = V3IsGrtrOrEq(fg, V3Abs(fV));
			return BAllEq(maskC, BTTTT());
		}
		Vec3V	mOrigin;
		Vec3V	mDir;
...
/*		#else
		...
		const PxU32 test = _mm_movemask_ps(absDV);
		if((test&7)!=0)
			return 0;
		...
		const PxU32 test2 = _mm_movemask_ps(absfV);
		return (test2&7)==0;
	#else
	...
	template <int inflateT>
	static PX_FORCE_INLINE void blockSegmentAABBDelayed(PxU32 res[5][4], const BoxBucket& bucket, const BPRayAABBTest& test)
	{
...
{
	return test.check<inflateT>(V3LoadU(box.mCenter), V3LoadU(box.mExtents));
}
...
Exit:
	...
	{
#ifdef __SPU__
/*		gCurrentBox = Cm::memFetch<BucketBox>(Cm::MemFetchPtr(_boxes), DMA_INPUTDATA_GET);
		gCurrentObject = Cm::memFetch<Prunable*>(Cm::MemFetchPtr(_objects), DMA_INPUTDATA_GET);

		const BucketBox& currentBox = gCurrentBox;
		Prunable* currentObject = gCurrentObject;*/
...
#endif
				{
					const BoxBucket& parent = core.mLevel3[i][j];
...
{
	PxVec3 extents = queryVolume.getInflatedWorldAABB().getExtents();
	return ::stab<1>(*this, pcb, queryVolume.getInflatedWorldAABB().getCenter(), unitDir, inOutDistance, extents);
}
											...
											PxU32 minLimitInt, PxU32 maxLimitInt)
{
	...
	{
#ifdef __SPU__
/*		gCurrentBox = Cm::memFetch<BucketBox>(Cm::MemFetchPtr(boxes), DMA_INPUTDATA_GET);
		gCurrentObject = Cm::memFetch<Prunable*>(Cm::MemFetchPtr(objects), DMA_INPUTDATA_GET);
...
struct SphereAABBTest_SIMD
{
	...
	PX_FORCE_INLINE Ps::IntBool operator()(const BucketBox& box) const		
	{
		...
		return BAllTrue3_R(FIsGrtrOrEq(mRadius2, dot));
	#else
...
PxAgain	BucketPrunerCore::overlap(const ShapeData& queryVolume, PrunerCallback& pcb) const
{
	...
	case PxGeometryType::eBOX:
		{
			...
			{	
				const Box& box = queryVolume.getOBB();
				const PxBounds3& cullBox = queryVolume.getInflatedWorldAABB();
				BucketPrunerOverlapTraversal<BucketPrunerOBBAABBTest,false> overlap;
				again = overlap(*this, BucketPrunerOBBAABBTest(box.rot, box.center, box.extents), pcb, cullBox);
			}
			else
			{
				const PxBounds3& box = queryVolume.getInflatedWorldAABB();
	...
	case PxGeometryType::eCAPSULE:
		{
			const Box& capsuleBox = queryVolume.getOBB();
			const PxBounds3& cullBox = queryVolume.getInflatedWorldAABB();
			BucketPrunerOverlapTraversal<BucketPrunerOBBAABBTest,false> overlap;
			again = overlap(*this, BucketPrunerOBBAABBTest(capsuleBox.rot, capsuleBox.center, capsuleBox.extents), pcb, cullBox);
		}
		break;
	...
	case PxGeometryType::eCONVEXMESH:
		{
			const PxBounds3& cullBox = queryVolume.getInflatedWorldAABB();
			const Gu::Box& guBox = queryVolume.getOBB();
			BucketPrunerOverlapTraversal<BucketPrunerOBBAABBTest,false> overlap;
			again = overlap(*this, BucketPrunerOBBAABBTest(guBox.rot, guBox.center, guBox.extents), pcb, cullBox);
		}
		break;
	default:
...
void BucketPruner::visualize(Cm::RenderOutput& out, PxU32 color) const { mCore.visualize(out, color); }
