...
#undef CREATE_64BIT_ADJUST_PVD_CLASS
		...
		ClassDescImpl& getOrCreateClassImpl( const NamespacedName& nm, PxI32 idx )
		{
			...
			mClasses[idx] = PVD_NEW( mAlloc, ClassDescImpl )( ClassDescription( safeName, idx), mAlloc );
			mNameToClasses.insert( nm, mClasses[idx] );
		...
		virtual bool deriveClass( const NamespacedName& parent, const NamespacedName& child )
		{
			...
			for ( ClassDescImpl* parent = &p; parent != NULL; parent = getClassImpl( parent->mBaseClass ) )
			{
				PVD_FOREACH( idx, parent->mPropImps.size() ) 
					mNameToProperties.insert( ClassPropertyName( c.mName, parent->mPropImps[idx]->mName ), parent->mPropImps[idx] );
					
				if ( parent->mBaseClass.hasValue() == false )
					break;
			}
...
DestroyNewMessage:
		...
		void read( TReaderType& stream )
		{
			...
			{
				ClassDescImpl* cls( mClasses[idx] );
				if ( cls == NULL ) continue;
				mNameToClasses.insert( cls->mName, mClasses[idx] );
