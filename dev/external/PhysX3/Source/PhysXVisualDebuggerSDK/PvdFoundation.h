...
namespace physx { namespace debugger {
	...
	struct ForwardingMemoryBuffer : public MemoryBuffer<ForwardingAllocator>
	{
		...
		inline ForwardingMemoryBuffer& operator << (  const void* inData ) { return  *this << static_cast<PxU64>(reinterpret_cast<const size_t>( inData )); }
...
#define PVD_FOREACH( varname, stop ) for( PxU32 varname = 0, end = stop; varname < end; ++varname )
