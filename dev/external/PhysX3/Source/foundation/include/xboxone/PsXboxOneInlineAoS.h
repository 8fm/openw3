...
#ifndef PS_XBOXONE_INLINE_AOS_H
#define PS_XBOXONE_INLINE_AOS_H
...
namespace _VecMathTests
{
	...
	PX_FORCE_INLINE bool allElementsNearEqualFloatV(const FloatV a, const FloatV b)
	{
		...
		static const FloatV minError=FLoad(-VECMATH_AOS_EPSILON);
		static const FloatV maxError=FLoad(VECMATH_AOS_EPSILON);
	...
	PX_FORCE_INLINE bool allElementsNearEqualVec3V(const Vec3V a, const Vec3V b)
	{
		...
		static const Vec3V minError=V3Load(-VECMATH_AOS_EPSILON);
		static const Vec3V maxError=V3Load(VECMATH_AOS_EPSILON);
		return
			(
	...
	PX_FORCE_INLINE bool allElementsNearEqualVec4V(const Vec4V a, const Vec4V b)
	{
		...
		static const Vec4V minError=V4Load(-VECMATH_AOS_EPSILON);
		static const Vec4V maxError=V4Load(VECMATH_AOS_EPSILON);
		return
			(
			_mm_comigt_ss(_mm_shuffle_ps(c, c, _MM_SHUFFLE(0,0,0,0)),minError) && 
			_mm_comilt_ss(_mm_shuffle_ps(c, c, _MM_SHUFFLE(0,0,0,0)),maxError) &&
...
PX_FORCE_INLINE Vec3V V3LoadA(const PxF32* const f)	
{
	...
	return _mm_and_ps((Vec4V&)*f, (VecI32V&)internalWindowsSimd::gMaskXYZ);
}
...
PX_FORCE_INLINE VecU32V V4ConvertToU32VSaturate(const Vec4V a, PxU32 power)
{
	...
	PX_FORCE_UNUSED(power); // prevent warning in release builds
