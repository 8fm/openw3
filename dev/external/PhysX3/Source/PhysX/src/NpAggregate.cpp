...
void NpAggregate::removeAndReinsert(NpScene* s, PxActor& actor, bool reinsert)
{
	NpActor& np = NpActor::getFromPxActor(actor);
	...
	if( s != NULL )
	{
		//Both actor and articulation remvoe goes here
	}
}
...
void NpAggregate::release()
{
	...
	*/
	for(PxU32 i=0;i<mNbActors;i++)
	{
		...
		removeAndReinsert(s, *mActors[i], true);
	}
	...
	if(s)
	{
		...
		GRB_EVENT(s, PxSceneEvent, PxSceneEvent::PxAggregateRelease, this);
	}
...
bool NpAggregate::addActor(PxActor& actor)
{
	...
	// PT: when an object is added to a compound at runtime, i.e. when the compound has already been added to the scene,
	// we need to immediately add the newcomer to the scene as well.
	if(s)
	{
		addActorInternal(actor, *s);

		GRB_EVENT(s, PxSceneEvent, PxSceneEvent::PxAggregateAddActor, this, &actor);
	}
...
bool NpAggregate::removeActorAndReinsert(PxActor& actor, bool reinsert)
{
	for(PxU32 i=0;i<mNbActors;i++)
	{
		if(mActors[i]==&actor)
		{
			...
			removeAndReinsert(getNpScene(), actor, reinsert);
			return true;
		}
	}
...
bool NpAggregate::removeActor(PxActor& actor)
{
	...
	// a) the user just wants to remove the actor from the compound, but the actor is still alive so if the compound has been added to a scene,
...
bool NpAggregate::removeArticulationAndReinsert(PxArticulation& art, bool reinsert)
{
	...
	while(idx < mNbActors)
	{
		if ((mActors[idx]->getType() == PxActorType::eARTICULATION_LINK) && (&static_cast<NpArticulationLink*>(mActors[idx])->getRoot() == npArt))
		{
			...
			removeAndReinsert(getNpScene(), *a, reinsert);
			found = true;
		}
		else
			idx++;
...
NpScene* NpAggregate::getNpScene() const
{
