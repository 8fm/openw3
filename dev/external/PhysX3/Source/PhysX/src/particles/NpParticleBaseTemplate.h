...
//~PX_SERIALIZATION
	...
	void		getReadWriteCudaBuffersGpu(struct PxCudaReadWriteParticleBuffers& buffers);
...
#if PX_SUPPORT_GPU_PHYSX
template<class APIClass, class LeafClass>
void NpParticleBaseTemplate<APIClass, LeafClass>::getReadWriteCudaBuffersGpu(PxCudaReadWriteParticleBuffers& buffers)
{
	NP_READ_CHECK(NpActor::getOwnerScene(*this));

	if (!getNpScene())
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::getReadWriteCudaBuffers() is not allowed to be called if particle system is not in scene.");
		return;
	}

	if (!(getParticleBaseFlags() & PxParticleBaseFlag::eGPU))
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::getReadWriteCudaBuffers() is not allowed for non PxParticleFlag::eGPU particle systems.");
		return;
	}

	getScbParticleSystem().getReadWriteCudaBuffersGpu(buffers);
}
...
void NpParticleBaseTemplate<APIClass, LeafClass>::setValidParticleRangeGpu(PxU32 validParticleRange)
{
	...
	if (!getNpScene())
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::setValidParticleRange() is not allowed to be called if particle system is not in scene.");
		return;
	}

	if (!(getParticleBaseFlags() & PxParticleBaseFlag::eGPU))
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::setValidParticleRange() is not allowed for non PxParticleFlag::eGPU particle systems.");
		return;
	}

	getScbParticleSystem().setValidParticleRangeGpu(validParticleRange);
}
...
void NpParticleBaseTemplate<APIClass, LeafClass>::setDeviceExclusiveModeFlagsGpu(PxU32 flags)
{
	...
	if (!getNpScene())
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::setFlags() is not allowed to be called if particle system is not in scene.");
		return;
	}

	if (!(getParticleBaseFlags() & PxParticleBaseFlag::eGPU))
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxParticleDeviceExclusive::setFlags() is not allowed for non PxParticleFlag::eGPU particle systems.");
		return;
	}

	getScbParticleSystem().setDeviceExclusiveModeFlagsGpu(flags);
}
#endif
}
