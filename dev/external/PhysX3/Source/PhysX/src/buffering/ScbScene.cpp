...
void Scb::ObjectTracker::scheduleForRemove(Scb::Base& element)
{
	...
	{
		PX_ASSERT(!"Trying to remove element not in scene.");
	}
}
...
{	
	static void createInstance(Scb::Scene& scene, Pvd::SceneVisualDebugger* d, T* v)
	{
		if(d) 
		{ 
			CM_PROFILE_ZONE_WITH_SUBSYSTEM(scene, PVD, CREATE_PVD_INSTANCE ); 
	...
	{ 
		...
		{ 
			CM_PROFILE_ZONE_WITH_SUBSYSTEM(scene,PVD, UPDATE_PVD_PROPERTIES ); 
	...
	{ 
		if(d) 
		{ 
			CM_PROFILE_ZONE_WITH_SUBSYSTEM(scene, PVD, RELEASE_PVD_INSTANCE ); 
...
#if PX_SUPPORT_VISUAL_DEBUGGER
	#define CREATE_PVD_INSTANCE(connected, obj) { if(connected) { CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,CREATE_PVD_INSTANCE ); mSceneVisualDebugger.createPvdInstance(obj); } }
	#define RELEASE_PVD_INSTANCE(connected, obj) { if(connected) { CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,RELEASE_PVD_INSTANCE ); mSceneVisualDebugger.releasePvdInstance(obj); } }
	#define UPDATE_PVD_PROPERTIES(connected, obj) { if(connected) { CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,UPDATE_PVD_PROPERTIES ); mSceneVisualDebugger.updatePvdProperties(obj); } }
	#define SEND_PVD_ARRAYS(connected, obj) { CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,SEND_PVD_ARRAYS ); if(connected) { mSceneVisualDebugger.sendArrays(obj); } }
	#define PVD_ORIGIN_SHIFT(connected, shift){ CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,ORIGIN_SHFIT ); if(connected) { mSceneVisualDebugger.originShift(shift); } }
#else
...
void Scb::Scene::switchRigidToNoSim(Scb::RigidObject& r, bool isDynamic)
{
	...
	if (r.getControlState() == ControlState::eIN_SCENE)
	{
		...
		{
			Scb::Shape& scbShape = *Ps::pointerOffset<Scb::Shape*>(shapes[i], ptrOffset);
...
void Scb::Scene::addAggregate(Scb::Aggregate& agg)
{
	...
	if (!mIsBuffering)
	{
		PxU32 compoundID = mScene.createCompound(agg.mPxAggregate, agg.getSelfCollide());
		agg.setCompoundID(compoundID);
...
void Scb::Scene::removeAggregate(Scb::Aggregate& agg)
{
	if (!mIsBuffering)
	{
		mScene.deleteCompound(agg.getCompoundID());
...
bool Scb::Scene::addMaterial(Sc::MaterialCore& material)
{
	...
	CREATE_PVD_INSTANCE(mSceneVisualDebugger.isConnected(), &material)
...
void Scb::Scene::updateMaterial(Sc::MaterialCore& material)//(PxU32 index, Sc::MaterialCore& material)
{
	...
	mEvent.mHandle = material.getMaterialIndex();
	mSceneMaterialBuffer.pushBack(mEvent);
	UPDATE_PVD_PROPERTIES(mSceneVisualDebugger.isConnected(), &material)
}
...
void Scb::Scene::removeMaterial(Sc::MaterialCore& material)//(PxU32 index, PxMaterial* nxMat)
{
	...
	RELEASE_PVD_INSTANCE(mSceneVisualDebugger.isConnected(), &material);
}
...
//#endif
	...
	for(PxU32 i=0; i< mSceneMaterialBuffer.size(); ++i)
	{
		...
		case MATERIAL_REMOVE:
			if (mEvent.mHandle < manager->getMaxSize())
...
void Scb::Scene::addShapeToPvd(Scb::Shape& shape, PxActor& owner)
{
	...
	PX_UNUSED(owner);
	 ...
	 { 
		 CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,CREATE_PVD_INSTANCE ); 
...
void Scb::Scene::removeShapeFromPvd(Scb::Shape& shape, PxActor &owner)
{
	...
	if(mSceneVisualDebugger.isConnectedAndSendingDebugInformation()) 
	{ 
		CM_PROFILE_ZONE_WITH_SUBSYSTEM( *this,PVD,RELEASE_PVD_INSTANCE ); 
...
void Scb::Scene::syncEntireScene(PxU32* error)
{
	...
	syncState();
	//
	// Process aggregates (needs to be done before adding actors because the actor's compoundID needs to get set)
	//
	...
	for(PxU32 i=0; i < mAggregateManager.getBufferedCount(); i++)
	{
		...
		{
			PxU32 compoundID = mScene.createCompound(a->mPxAggregate, a->getSelfCollide());
			a->setCompoundID(compoundID);
			a->syncState(*this);  // Necessary to set the compound ID for all actors of the aggregate
		}
...
{	
	...
	{
		T* v = static_cast<T*>(tracker.getBuffered()[i]);
		if(v->getControlState() == ControlState::eIN_SCENE)
...
void Scb::Scene::processPendingRemove()
{
	...
	for(PxU32 i=0; i < mAggregateManager.getBufferedCount(); i++)
	{
		...
		{
			a->syncState(*this);  // Clears the compound ID for all actors of the aggregate
			mScene.deleteCompound(a->getCompoundID());
		}
	}
...
#endif
		default:
			PX_ASSERT(!"scheduleForUpdate: missing type!");
			break;
	}
}
...
PxU8* Scb::Scene::getStream(ScbType::Enum type)
{
	...
	switch(type)
	{
		...
		default:
			PX_ASSERT(!"getStream: missing type!");
			return NULL;
	}
	return memory;
}
