...
struct BodyBuffer : public RigidObjectBuffer		//once RigidObject has its own buffered elements, derive from that instead
{
	...
	enum
	{
		BF_RigidBodyFlags		= 1<<10,
		BF_KinematicTarget		= 1<<11,
		BF_Acceleration			= 1<<12,
		BF_DeltaVelocity		= 1<<13,
		BF_Body2World			= 1<<14,
		BF_Body2World_CoM		= 1<<15,  // the body pose was adjusted because of a center of mass change only
		BF_LinearVelocity		= 1<<16,
		BF_AngularVelocity		= 1<<17,
		BF_WakeCounter			= 1<<18,
		BF_PutToSleep			= 1<<19,
		BF_WakeUp				= 1<<20,
	};
...
PX_INLINE void Body::setKinematicTarget(const PxTransform& p)
{
	...
	if(getControlState() == ControlState::eIN_SCENE)
	{
		...
		PX_ASSERT(scene);
		if(scene->getSceneVisualDebugger().isConnected())	
...
PX_INLINE void Body::syncState()
{
	...
	if(bufferFlags & ~(	Buf::BF_WakeCounter|Buf::BF_Body2World|Buf::BF_LinearVelocity|Buf::BF_AngularVelocity|Buf::BF_WakeUp|Buf::BF_PutToSleep ))  // Optimization to avoid all the if-statements below if possible
	{
		Buf& buffer = *getBodyBuffer();
	...
	if ((bufferFlags & (Buf::BF_WakeUp | Buf::BF_PutToSleep)) == 0)
	{
		if (getControlState() != ControlState::eREMOVE_PENDING)  // we do not want to sync the simulation sleep state if the object was removed (free standing objects have buffered state sleeping)
			mBufferedIsSleeping = isSimObjectSleeping;
		else
