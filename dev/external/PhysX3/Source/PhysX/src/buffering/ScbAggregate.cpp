...
void Scb::Aggregate::addActor(Scb::Actor& actor)
{
	...
	{
		actor.getActorCore().mCompoundID = mCompoundID;
	}
	else if ((state != ControlState::eREMOVE_PENDING) && fetchDataBuffer())  // If the aggregate is pending for deletion, adding/removing an actor should not be double buffered because the compoundID must not be set for the actors
	{
		...
		if (bufferedData->addBufferIdx != 0xffffffff)
		{
			...
			for(PxU32 i=0; i < bufferedData->addCount; i++)
			{
				if (addBuffer[i] == &actor)
				{
					...
					bufferedData->addCount--;
					return;  // It's fine to abort here because the compoundID has not been set yet
				}
			}
		}
