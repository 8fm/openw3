...
#define UPDATE_PVD_PROPERTIES_SCENE() { if(mSceneVisualDebugger.isConnected()) mSceneVisualDebugger.updatePvdProperties(); }
#else
namespace Pvd
{
...
namespace Scb
{
	...
	*/
	...
	public:
		...
		PX_INLINE void						setFrictionMode(PxFrictionType::Enum);
		PX_INLINE PxFrictionType::Enum		getFrictionMode() const;
	...
	private:
						void				syncState();
		PX_FORCE_INLINE	Ps::IntBool			isBuffered(BufferFlag f)	const	{ return mBufferFlags& f;	}
...
PX_INLINE void Scb::Scene::setFrictionMode(PxFrictionType::Enum frictionType)
{
	if (!isPhysicsBuffering())
	{
		...
		PxsFrictionModel fm = (PxsFrictionModel) frictionType;
		mScene.setFrictionModel(fm);

	}

}

PX_INLINE PxFrictionType::Enum Scb::Scene::getFrictionMode() const
{
	PxsFrictionModel fm = mScene.getFrictionModel();
	return (PxFrictionType::Enum)fm;
}
