...
namespace physx
{
	...
	#define UPDATE_PVD_PROPERTIES_OBJECT()	 {											\
		 Scb::Scene* scene = getScbSceneForAPI();												\
		 if(scene && !insertPending() && scene->getSceneVisualDebugger().isConnected())	\
			 scene->getSceneVisualDebugger().updatePvdProperties(this);	}
#else
	#define UPDATE_PVD_PROPERTIES_OBJECT() {}
#endif
