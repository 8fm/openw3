...
static void writeGeom(BatchQueryStream& stream, const PxGeometry& geom)
{
	...
	case PxGeometryType::eCAPSULE:
		stream.write<PxCapsuleGeometry>(reinterpret_cast<const PxCapsuleGeometry&>(geom));
		break;
	case PxGeometryType::eSPHERE:
		stream.write<PxSphereGeometry>(reinterpret_cast<const PxSphereGeometry&>(geom));
		break;
	case PxGeometryType::eCONVEXMESH:
		stream.write<PxConvexMeshGeometry>(reinterpret_cast<const PxConvexMeshGeometry&>(geom));
		break;
	case PxGeometryType::eBOX:
		stream.write<PxBoxGeometry>(reinterpret_cast<const PxBoxGeometry&>(geom));
		break;
	default:
		PX_ASSERT(0 && "Unsupported geometry type in writeGeom");
	}
}
...
static PxGeometry* readGeom(BatchQueryStreamReader& reader)
{
	...
	case PxGeometryType::eBOX:
		...
		break;
	default:
		PX_ASSERT(0 && "Unsupported geometry type in readGeom");
	}
...
void NpBatchQuery::execute()
{
	...
	// If PVD is connected, record the offsets for queries in pvd buffers and run the queries on PPU
	bool pvdConnected = false;
	...
	if((pvdConnected = mNpScene->getScene().getSceneVisualDebugger().isConnected()) != false && IS_PVD_SQ_ENABLED)
	{
	...
	if (PX_IS_PPU && !pvdConnected && sqm.canRunOnSPU(*this))
	{
		...
		if (!(runOnPPU[0] || runOnPPU[1] || runOnPPU[2]))
		{
			checkForSPUErrors();
			finalizeExecute();
			return;
	...
	PxI32 curQueryOffset = 0; // first query starts at 0
	...
	PxI32 hitsSpaceLeft; PX_UNUSED(hitsSpaceLeft);
	...
	do {
		...
		switch (h.hitTypeId)
		{
			...
			default:
				PX_ASSERT(0 && "Unexpected batch query type (raycast/overlap/sweep).");
		}
...
#if PX_SUPPORT_VISUAL_DEBUGGER
	if(mNpScene->getScene().getSceneVisualDebugger().isConnected() && isSqCollectorLocked && IS_PVD_SQ_ENABLED)
	{
...
///////////////////////////////////////////////////////////////////////////////
void NpBatchQuery::writeBatchHeader(const BatchStreamHeader& h) const
{
	PxU32 streamPos = mStream.getPos(); // save the stream pos before we write the header
...
void NpBatchQuery::raycast(
	...
	PxHitFlags hitFlags, const PxQueryFilterData& fd, void* userData, const PxQueryCache* cache) const
{
...
void NpBatchQuery::overlap(
	...
	const PxQueryFilterData& fd, void* userData, const PxQueryCache* cache) const
{
...
void NpBatchQuery::sweep(
	...
	PxHitFlags hitFlags, const PxQueryFilterData& fd, void* userData, const PxQueryCache* cache, const PxReal inflation) const
{
