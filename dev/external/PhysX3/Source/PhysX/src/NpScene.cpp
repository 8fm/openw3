...
NpScene::NpScene(const PxSceneDesc& desc) :
	...
	mSceneExecution			(0, "NpSceneExecution"),
	mSceneCollide			(0, "NpSceneCollide"),
	mSceneSolve				(0, "NpSceneSolve"),
...
void NpScene::setGravity(const PxVec3& g)
{
	...
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneSetGravity, g);
}
...
{
	NP_WRITE_CHECK(this);
	PX_CHECK_AND_RETURN(flag & (PxU16)(PxSceneFlag::eENABLE_CCD|PxSceneFlag::eDISABLE_CCD_RESWEEP|PxSceneFlag::eADAPTIVE_FORCE|PxSceneFlag::eENABLE_KINEMATIC_STATIC_PAIRS|PxSceneFlag::eENABLE_KINEMATIC_PAIRS),"NpScene::setFlag: This flag is not mutable - you can only set it once in PxSceneDesc at startup!");
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,addActor);
	NP_WRITE_CHECK(this);
...
void NpScene::addActorInternal(PxActor& actor)
{
	switch(actor.getConcreteType())
	{
		case PxConcreteType::eRIGID_STATIC:
		{
			...
			GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneAddRigidStatic, &npStatic);
		}
		break;
		...
		case PxConcreteType::eRIGID_DYNAMIC:
		{
			...
			GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneAddRigidDynamic, &npDynamic);
		}
		break;
...
void NpScene::addActors(PxActor*const* PX_RESTRICT actors, PxU32 nbActors)
{
	...
	scState.staticActorOffset		= size_t(&(reinterpret_cast<NpRigidStatic*>(0)->getScbRigidStaticFast().getScStatic()));
	scState.staticShapeTableOffset	= size_t(&(reinterpret_cast<NpRigidStatic*>(0)->getShapeManager().getShapeTable()));
	scState.dynamicActorOffset		= size_t(&(reinterpret_cast<NpRigidDynamic*>(0)->getScbBodyFast().getScBody()));
	scState.dynamicShapeTableOffset = size_t(&(reinterpret_cast<NpRigidDynamic*>(0)->getShapeManager().getShapeTable()));
	scState.shapeOffset				= NpShapeGetScPtrOffset();
	...
	for(actorsDone=0; actorsDone<nbActors; actorsDone++)
	{
		...
		else if(type == PxConcreteType::eRIGID_DYNAMIC)
		{
			...
			if (!(a.getScbBodyFast().getActorFlags() & PxActorFlag::eDISABLE_SIMULATION))
			{
				...
				updateScbStateAndSetupSq(a, a.getScbActorFast(), a.getShapeManager(), true, shapeBounds.begin());
				a.setRigidActorArrayIndex(mRigidActorArray.size());
				mRigidActorArray.pushBack(&a);
				a.addConstraintsToScene();
			}
	...
	if(actorsDone<nbActors)	// Everything is consistent up to the failure point, so just use removeActor to back out gracefully if necessary
	{
		for(PxU32 j=0;j<actorsDone;j++)
			removeActorInternal(*actors[j], false);
		return;
	}

	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneAddRigidActors, actors, nbActors);	
}
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,removeActor);
	NP_WRITE_CHECK(this);	
	...
	for(PxU32 actorsDone=0; actorsDone<nbActors; actorsDone++)
	{
		...
		{			
			...
			{
				CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,removeActorFromSim);
		...
		{
			removeActorInternal(*actors[actorsDone],wakeOnLostTouch);
		}
		else
		{
...
void NpScene::removeActor(PxActor& actor, bool wakeOnLostTouch)
{
	...
	{
		removeActorInternal(actor, wakeOnLostTouch);
	}
}

void NpScene::removeActorInternal(PxActor& actor, bool wakeOnLostTouch)
{
	switch(actor.getType())
	{
		...
		{
			NpRigidStatic& npStatic = static_cast<NpRigidStatic&>(actor);
			removeRigidStatic(npStatic, wakeOnLostTouch);
		}
		break;
		...
		{
			NpRigidDynamic& npDynamic = static_cast<NpRigidDynamic&>(actor);
			removeRigidDynamic(npDynamic, wakeOnLostTouch);
		}
		break;
#if PX_USE_PARTICLE_SYSTEM_API
...
void NpScene::removeFromRigidActorList(const PxU32& index)
{
	...
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneRemoveActor, mRigidActorArray[index]);
...
void NpScene::addRigidStatic(NpRigidStatic& actor)
{
	...
	{
		CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,addActorToSim);
...
void NpScene::removeRigidStatic(NpRigidStatic& actor, bool wakeOnLostTouch)
{
...
void NpScene::addRigidDynamic(NpRigidDynamic& body)
{
	...
	if(body.getMass()==0)
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxScene::addActor(): Dynamic actor with zero mass added to scene; defaulting mass to 1. Call PxRigidBodyExt::updateMassAndInertia() to set mass properties from geometry with a specific density.");
		body.setMass(1.0f);
	}
	...
	{
		Ps::getFoundation().error(PxErrorCode::eDEBUG_WARNING, __FILE__, __LINE__, "PxScene::addActor(): Dynamic actor with zero moment of inertia added to scene; defaulting inertia to (1,1,1). Call PxRigidBodyExt::updateMassAndInertia() to set mass properties from geometry with a specific density.");
...
void NpScene::removeRigidDynamic(NpRigidDynamic& body, bool wakeOnLostTouch)
{
...
void NpScene::addArticulationInternal(PxArticulation& articulation)
{
	...
	PxVec3 inertia = rootLink->getMassSpaceInertiaTensor();		
	if(inertia.x == 0.0f || inertia.y == 0.0f || inertia.z == 0.0f)
	{
...
void NpScene::removeArticulation(PxArticulation& articulation, bool wakeOnLostTouch)
{
	...
	{
		removeArticulationInternal(articulation, wakeOnLostTouch);
	}
}

void NpScene::removeArticulationInternal(PxArticulation& articulation, bool wakeOnLostTouch)
{
	NpArticulation& npa = static_cast<NpArticulation&>(articulation);
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,addAggregate);
	NP_WRITE_CHECK(this);
	...
	if ((cs == Scb::ControlState::eNOT_IN_SCENE) || ((cs == Scb::ControlState::eREMOVE_PENDING) && (agg.getScbScene()->getPxScene() == this)))
	{
		...
		GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneAddAggregate, &aggregate);
	}
	else
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,removeAggregate);
	NP_WRITE_CHECK(this);	
	...
	for(PxU32 i=0;i<nb;i++)
	{
		...
		if (a->getType() != PxActorType::eARTICULATION_LINK)
		{
			...
			np.getScbAggregate().removeActor(scb, false);  // This is only here to make sure the compoundID gets set to invalid on sync

			removeActorInternal(*a, wakeOnLostTouch);
		}
		else if (a->getScene())
		{
			...
			{
				np.getScbAggregate().removeActor(links[i]->getScbActorFast(), false);  // This is only here to make sure the compoundID gets set to invalid on sync
			}

			removeArticulationInternal(npArt, wakeOnLostTouch);
		}
	}
	...
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneRemoveAggregate, &aggregate, 1);
}
...
void NpScene::addCollection(const PxCollection& collection)
{
	...
	PxU32 nb = c.internalGetNbObjects();
	for(PxU32 i=0;i<nb;i++)
	{
...
PxU32 NpScene::addBroadPhaseRegion(const PxBroadPhaseRegion& region, bool populateRegion)
{
	...
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneAddBroadphaseRegion, &region, populateRegion);
...
{
	NP_WRITE_CHECK(this);
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneRemoveBroadphaseRegion, handle);
...
void NpScene::simulate(PxReal _elapsedTime, physx::PxTask* completionTask, void* scratchBlock, PxU32 scratchBlockSize, bool controlSimulation)
{
	{
	...
	CM_PROFILE_START_CROSSTHREAD(mScene.getEventProfiler(), Cm::ProfileEventId::SimAPI::Getsimulate());

#if PX_SUPPORT_VISUAL_DEBUGGER
	{
		CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene, SimAPI, pvdFrameStart);
	...
	{
		CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,Sim,updateShaders);
		updateDirtyShaders();
	}
#if PX_SUPPORT_VISUAL_DEBUGGER
	{
	...
	NpMaterial** materials = manager.materials;
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,checkResults);
...
{
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,Sim,firePreSyncCallbacks);
...
bool NpScene::fetchResults(bool block, PxU32* errorState)
{
	...
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,fetchResults);
	...
	mScene.getScScene().releaseConstraints(); //release constraint blocks at the end of the frame, so user can retrieve the blocks
	...
	{
		CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,Sim,firePostSyncCallbacks);
	...
	CM_PROFILE_STOP_CROSSTHREAD(mScene.getEventProfiler(), Cm::ProfileEventId::SimAPI::Getsimulate());

	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneFetchResults, eventPools, *this);
...
void NpScene::updateMaterial(NpMaterial& mat)
{
	...
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneUpdateMaterial, &mat);
}

void NpScene::removeMaterial(NpMaterial& mat)
{
	GRB_EVENT(this, PxSceneEvent, PxSceneEvent::PxSceneRemoveMaterial, &mat, 1);
...
void NpScene::releaseVolumeCache(NpVolumeCache* volumeCache)
{
	...
	PX_ASSERT(found && "volume cache not found in releaseVolumeCache");
...
void NpScene::getSpuMemBlockCounters(PxU32& numContactStreamBlocks, PxU32& numFrictionBlocks, PxU32& numConstraintBlocks)
{
	NP_READ_CHECK(this);
	return mScene.getScScene().getSpuMemBlockCounters(numContactStreamBlocks, numFrictionBlocks, numConstraintBlocks);
}
#endif
...
void NpScene::lockWrite(const char* file, PxU32 line)
{
	...
	{
		Ps::getFoundation().error(PxErrorCode::eINVALID_OPERATION, file?file:__FILE__, file?line:__LINE__, "PxScene::lockWrite() detected after a PxScene::lockRead(), lock upgrading is not supported, behaviour will be undefined.");
		return;
	}
...
void NpScene::shiftOrigin(const PxVec3& shift)
{
	...
	CM_PROFILE_ZONE_WITH_SUBSYSTEM(mScene,SimAPI,shiftOrigin);
	PX_SIMD_GUARD;
...
PxSceneEventStream * NpScene::createSceneEventStream()
{
	Array<PxSceneEvent> * eventStream = &mEventStreams.insert();
	new (eventStream) Array<PxSceneEvent>;

	static PxSceneEventDescs::SystemHeapAlloc alloc;
	mEventStreamAllocs.pushBack(new StackAllocator(mEventStreamStackAllocatorPageSize, alloc));

	PxSceneEventStream * retval = new PxSceneEventStream(this, eventStream);
	
	return retval;
}

void NpScene::releaseSceneEventStream(PxSceneEventStream * sceneEventStream)
{
	int index = (int)(sceneEventStream->getEventStream() - &mEventStreams[0]);

	mEventStreams.replaceWithLast(index);
	delete mEventStreamAllocs[index];
	mEventStreamAllocs.replaceWithLast(index);

}

PxU32 NpScene::getNumEventStreams()
{
	return mEventStreams.size();
}

void NpScene::eventStreamSend(const PxSceneEvent & sceneEvent, PxU32 stream)
{
	mEventStreams[stream].pushBack(sceneEvent);
}

StackAllocator & NpScene::getEventStreamStackAlloc(PxU32 eventStream)
{
	return *(mEventStreamAllocs[eventStream]);
}

void NpScene::clearEventStream(Array<PxSceneEvent> * eventStream)
{
	int index = (int)(eventStream - &mEventStreams[0]);
	...
	mEventStreamAllocs[index]->reset(); //reset the stack
}
#endif
///////////////////////////////////////////////////////////////////////////////
...
{
	NpBatchQuery* npsq = reinterpret_cast<NpBatchQuery*>(sq);
