		...
		PxU32 maxHits, HitType* hits, const PxReal shrunkMaxDistance, bool anyHit, PxBounds3* precomputedBounds)
	{
		if (HitTypeSupport<HitType>::IsRaycast)
		{
			...
			return PxGeometryQuery::raycast(
				input.getOrigin(), input.getDir(), sceneGeom, pose, shrunkMaxDistance, hitFlags,
				maxHits, (PxRaycastHit*)hits, anyHit);
		}
		if (HitTypeSupport<HitType>::IsSweep)
		{
			...
			const PxVec3 mn = b1.getCenter() - combExt;
			const PxVec3 mx = b1.getCenter() + combExt;

			PxF32 tnear, tfar;
 			if (!Gu::intersectRayAABB2(mn, mx, input.pose->p, input.getDir(), shrunkMaxDistance, tnear, tfar)) // returns (tnear<tfar)
			...
			PxU32 result = PxU32(PxGeometryQuery::sweep(
				input.getDir(), PxMin(tfar, shrunkMaxDistance), *input.geometry, *input.pose, sceneGeom, pose,
				(PxSweepHit&)hits[0], hitFlags, input.inflation));
			return result;
		}
		if (HitTypeSupport<HitType>::IsOverlap)
		{
			PxU32 result = PxU32(PxGeometryQuery::overlap(*input.geometry, *input.pose, sceneGeom, pose));
			return result;
		}
...
struct MultiQueryCallback : public PrunerCallback
{
	...
	MultiQueryCallback(
		const NpSceneQueries& scene, const MultiQueryInput& input, bool aAnyHit,
		PxHitCallback<HitType>& hitCall, PxHitFlags hitFlags,
		const PxQueryFilterData& filterData, PxQueryFilterCallback* filterCall,
		PxReal shrunkDistance, BatchQueryFilterData* aBfd) : 
			scene(scene), input(input), hitCall(hitCall), hitFlags(hitFlags), filterData(filterData), filterCall(filterCall),
			shrunkDistance(shrunkDistance), bfd(aBfd), reportTouchesAgain(true), farBlockFound(false), queryShapeBoundsValid(false)
	{
		isCached = false;
		anyHit = aAnyHit;
		meshAnyHit = (hitFlags.isSet(PxHitFlag::eMESH_ANY)) || anyHit;
		noBlock = (filterData.flags & PxQueryFlag::eNO_BLOCK);
	...
	virtual PxAgain invoke(PxReal& aDist, const PrunerPayload* aPayload, PxU32 aCount)
	{
		...
		for (PxU32 iShape = 0; iShape < aCount; iShape++)
		{
			...
			PxU32 nbSubHits = GeomQueryAny<HitType>::geomHit(
				...
				filteredHitFlags | (anyHit ? PxHitFlags(0) : PxHitFlags(PxHitFlag::eDISTANCE)),
			...
			for (PxU32 iSubHit = 0; iSubHit < nbSubHits; iSubHit++)
			{
				...
				#if PX_CHECKED
				if (HitTypeSupport<HitType>::IsOverlap && hitType == PxQueryHitType::eBLOCK)
				{
					static bool first = true;
					if (first)
					{
						first = false;
						Ps::getFoundation().getErrorHandler().reportError(PxErrorCode::eDEBUG_WARNING,
...
struct CapturePvdOnReturn : public PxHitCallback<HitType>
{
	...
	CapturePvdOnReturn(
		...
		BatchQueryFilterData* bfd, PxHitCallback<HitType>& parentCallback)
			: PxHitCallback<HitType>(parentCallback.touches, parentCallback.maxNbTouches), sq(sq), input(input), hitFlags(hitFlags),
			cache(cache), filterData(filterData), filterCall(filterCall), bfd(bfd), parentCallback(parentCallback)
	{}
	...
	~CapturePvdOnReturn()
	{
		if(!(sq->mScene.getSceneVisualDebugger().isConnected() && IS_PVD_SQ_ENABLED))
			return;
		...
		// all overlaps down the same path
			...
			collector.overlapMultipleWithLock(*input.geometry, *input.pose, (PxOverlapHit*)allHits.begin(), retBool, filterData.data, filterData.flags);
...
struct IssueCallbacksOnReturn
{
	...
	~IssueCallbacksOnReturn()
	{
			...
			// and the touches in touches array were already processed by the callback
		{
			...
			if (hits.nbTouches)
			{
				bool again = hits.processTouches(hits.touches, hits.nbTouches);
				if (again)
					hits.nbTouches = 0;
			}
		}
	...
	const PxQueryFilterData& filterData, PxQueryFilterCallback* filterCall, BatchQueryFilterData* bfd) const
{
	...
	if (cacheData && hits.maxNbTouches == 0) // don't use cache for queries that can return touch hits
	{
		...
		if(HitTypeSupport<HitType>::IsSweep)
		{
			...
			pcb.queryShapeBounds = sd.getInflatedWorldAABB();
			pcb.queryShapeBoundsValid = true;
		}
		PxAgain againAfterCache = pcb.invoke(dummyDist, &cachedPayload, 1);
		pcb.isCached = false;
