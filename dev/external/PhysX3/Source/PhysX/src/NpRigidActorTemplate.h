...
void NpRigidActorTemplate<APIClass>::attachShape(PxShape& shape)
{
	...
	GRB_EVENT(getScene(), PxSceneEvent, PxSceneEvent::PxActorAttachShape, static_cast<PxActor *>(this), static_cast<PxShape *>(&shape), 2);
}
...
void NpRigidActorTemplate<APIClass>::detachShape(PxShape& shape, bool wakeOnLostTouch)
{
	...
	GRB_EVENT(getScene(), PxSceneEvent, PxSceneEvent::PxActorDetachShape, static_cast<PxActor *>(this), static_cast<PxShape *>(&shape));
}
	...
	if (scbShapes)
	{
		if (shapes)
		{
			...
			for(PxU32 i=0; i < sCount; i++)
			{
				...
				if (scb.getControlState() != Scb::ControlState::eINSERT_PENDING)
				{
					scbShapes[i] = &scb;
					sAccepted++;
				}
		...
		else
		{
			...
			for(PxU32 i=0; i < sCount; i++)
			{
				...
				if ((scb.getControlState() != Scb::ControlState::eINSERT_PENDING) && (scb.getFlags() & (PxShapeFlag::eSIMULATION_SHAPE | PxShapeFlag::eTRIGGER_SHAPE)))
				{
					scbShapes[sAccepted] = &scb;
					sAccepted++;
				}
			}
			sCount = sAccepted;
		...
		if (sCount)
		{
			...
			for(PxU32 i=0; i < sCount; i++)
				GRB_EVENT(NpActor::getOwnerScene(*this), PxSceneEvent, PxSceneEvent::PxShapeResetFiltering, scbShapes[i]->getScShape().getPxShape(), (PxU8)1);
		}
	}
