...
void NpRigidBodyTemplate<APIClass>::setMass(PxReal mass)
{
	...
	PX_CHECK_AND_RETURN(mass>0, "RigidDynamic::setMass: mass must be positive!");

	getScbBodyFast().setInverseMass(1.0f/mass);
}
...
PxReal NpRigidBodyTemplate<APIClass>::getMass() const
{
	...
	return (1.0f / mBody.getInverseMass());
}
...
void NpRigidBodyTemplate<APIClass>::setMassSpaceInertiaTensor(const PxVec3& m)
{
	...
	PX_CHECK_AND_RETURN(m.x>=0 && m.y>=0 && m.z>=0, "NpRigidBody::setMassSpaceInertiaTensor: components must be positive");
...
PX_FORCE_INLINE void NpRigidBodyTemplate<APIClass>::setRigidBodyFlagsInternal(const PxRigidBodyFlags& currentFlags, const PxRigidBodyFlags& newFlags)
{
	...
	GRB_EVENT(getScene(), PxSceneEvent, PxSceneEvent::PxActorSetRigidBodyFlags, static_cast<PxActor *>(this), (PxU32)newFlags);
}
