...
//~PX_SERIALIZATION
	...
	PX_FORCE_INLINE	NpConnectorArray**			getConnectorArrayAddress()		{ return &mConnectorArray;}
					...
					static NpActor&				getFromPxActor(PxActor& actor)			{ 	return *Ps::pointerOffset<NpActor*>(&actor, sOffsets.pxActorToNpActor[actor.getConcreteType()]); }
					static const NpActor&		getFromPxActor(const PxActor& actor)	{	return *Ps::pointerOffset<const NpActor*>(&actor, sOffsets.pxActorToNpActor[actor.getConcreteType()]); }
#ifndef __SPU__ // SPU has its own implementation, see SqUtilities.spu.cpp
					static Scb::Actor&			getScbFromPxActor(PxActor& actor)		{	return *Ps::pointerOffset<Scb::Actor*>(&actor, sOffsets.pxActorToScbActor[actor.getConcreteType()]); }
#else
					static Scb::Actor&			getScbFromPxActor(PxActor& actor);
#endif
					static const Scb::Actor&	getScbFromPxActor(const PxActor& actor)	{ 	return *Ps::pointerOffset<const Scb::Actor*>(&actor, sOffsets.pxActorToScbActor[actor.getConcreteType()]); }
