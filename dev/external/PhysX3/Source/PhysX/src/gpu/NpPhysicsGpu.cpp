...
void NpPhysicsGpu::setExplicitCudaFlushCountHint(const class PxScene& scene, PxU32 cudaFlushCount)
{
	...
	if (physXGpu)
	{
		PxSceneGpu* gpuScene = ((NpScene*)&scene)->mScene.getScScene().getSceneGpu();
		PX_ASSERT(gpuScene);
...
bool NpPhysicsGpu::setTriangleMeshCacheSize(const class PxScene& scene, PxU32 size)
{
	const char* functionName = "PxParticleGpu::setTriangleMeshCacheSize()";
	PxPhysXGpu* physXGpu = getPhysXGpu(true, functionName);
	if (physXGpu)
	{
		PxSceneGpu* gpuScene = ((NpScene*)&scene)->mScene.getScScene().getSceneGpu();
		PX_ASSERT(gpuScene);
		return physXGpu->setTriangleMeshCacheSize((PxgSceneGpu&)(*gpuScene), size);
	}
	return false;
}
...
NpPhysicsGpu::NpPhysicsGpu(NpPhysics& npPhysics) : mNpPhysics(npPhysics), mMeshMirrors(PX_DEBUG_EXP("NpPhysicsGpu:mMeshMirrors"))
{}
