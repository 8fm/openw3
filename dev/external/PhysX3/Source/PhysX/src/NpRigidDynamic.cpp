...
void NpRigidDynamic::setGlobalPose(const PxTransform& pose, bool autowake)
{
	...
	PxTransform body2World = newPose * getScbBodyFast().getBody2Actor();
...
bool NpRigidDynamic::getKinematicTarget(PxTransform& target)
{
	...
	{
		return b.getKinematicTarget(target);
	}
	else
		return false;
}
...
void NpRigidDynamic::addForce(const PxVec3& force, PxForceMode::Enum mode, bool autowake)
{
	...
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "RigidDynamic::addForce: Body must be in a scene!");
...
void NpRigidDynamic::addTorque(const PxVec3& torque, PxForceMode::Enum mode, bool autowake)
{
	...
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "RigidDynamic::addTorque: Body must be in a scene!");
...
void NpRigidDynamic::clearForce(PxForceMode::Enum mode)
{
	...
	PX_UNUSED(scene);
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "RigidDynamic::clearForce: Body must be in a scene!");
...
void NpRigidDynamic::clearTorque(PxForceMode::Enum mode)
{
	...
	PX_UNUSED(scene);
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "RigidDynamic::clearTorque: Body must be in a scene!");
...
bool NpRigidDynamic::isSleeping() const
{
	...
	NP_READ_CHECK(scene);
	PX_CHECK_AND_RETURN_VAL(scene, "NpRigidDynamic::isSleeping: Body must be in a scene.", true);
...
void NpRigidDynamic::wakeUp()
{
	...
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "NpRigidDynamic::wakeUp: Body must be in a scene.");
...
void NpRigidDynamic::putToSleep()
{
	...
	NP_WRITE_CHECK(scene);
	PX_CHECK_AND_RETURN(scene, "NpRigidDynamic::putToSleep: Body must be in a scene.");
...
void NpRigidDynamic::setContactReportThreshold(PxReal threshold)
{
	...
	GRB_EVENT(getScene(), PxSceneEvent, PxSceneEvent::PxActorSetContactReportThreshold, static_cast<PxActor *>(this), threshold<0 ? 0 : threshold);
}
