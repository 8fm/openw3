...
{
	CuConstraints::CuConstraints(physx::PxCudaContextManager* ctx)
		: mHostCopy(CuHostAllocator(ctx, cudaHostAllocMapped)), mStart(ctx), mTarget(ctx)
	{}
...
public:
	...
	PxVec4* getDeviceParticles(bool markHostDirty);
	PxVec4* getHostParticles(bool markDeviceDirty);
...
public:
	...
	CuDeviceVector<CuPhaseConfig> mPhaseConfigs;
