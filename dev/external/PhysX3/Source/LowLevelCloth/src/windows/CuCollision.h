...
namespace
{
	...
	private:
		...
		template <typename CurrentT>
		__device__ void collideConvexes(CurrentT& current, float alpha);
...
namespace 
{
	...
	__device__ void generateSpheres(CuCollision::CollisionData& data, float alpha)
	{
		...
		if(threadIdx.x < gClothData.mNumSpheres * 4)
		{
			...
			uint32_t j = threadIdx.x % 4 * gClothData.mNumSpheres + threadIdx.x / 4;
			data.mSphereX[j] = start + (target - start) * alpha;
		}
...
__device__ void CuCollision::buildSphereAcceleration(const CollisionData& data)
{
	...
	volatile __int32 index = signi - __int32(floorf(
	...
	const float& position, SharedPointer<const uint32_t>::Type sphereGrid)
{
	ShapeMask result;
#if USE_ACCELERATION_STRUCTURE
	uint32_t index = __int32(floorf(position));
...
__device__ void CuCollision::collideCapsules(CurrentT& current, PreviousT& previous) const
{
	...
	float positions[3], prevPositions[3], delta[3], velocity[3];
	for(uint32_t i = threadIdx.x; i < gClothData.mNumParticles; i += blockDim.x)
	{
		positions[0] = current(i, 0);
		positions[1] = current(i, 1);
		positions[2] = current(i, 2);

		if(uint32_t numCollisions = collideCapsules(positions, delta, velocity))
		{
			float scale = __fdividef(1.0f, numCollisions);

			current(i, 0) = positions[0] + delta[0] * scale;
			current(i, 1) = positions[1] + delta[1] * scale;
			current(i, 2) = positions[2] + delta[2] * scale;

			if (frictionEnabled)
			{
				prevPositions[0] = previous(i, 0);
				prevPositions[1] = previous(i, 1);
				prevPositions[2] = previous(i, 2);

				float frictionImpulse[3];
				calcFrictionImpulse(positions, prevPositions, velocity, scale, delta, frictionImpulse);

				previous(i, 0) = prevPositions[0] - frictionImpulse[0];
				previous(i, 1) = prevPositions[1] - frictionImpulse[1];
				previous(i, 2) = prevPositions[2] - frictionImpulse[2];
			}			
...
__device__ void CuCollision::collideVirtualCapsules(CurrentT& current, PreviousT& previous) const
{
	...
	for(; setSizeIt != setSizeEnd; ++setSizeIt)
	{
		...
		for(indicesEnd += *setSizeIt * 4; indicesIt < indicesEnd; indicesIt += blockDim.x * 4)
		{
			...
			positions[0] = lerp(current[0], indices, weights);
			positions[1] = lerp(current[1], indices, weights);
			positions[2] = lerp(current[2], indices, weights);
			...
			if(uint32_t numCollisions = collideCapsules(positions, delta, velocity))
			{
				...
				if (frictionEnabled)
				{
					prevPositions[0] = lerp(previous[0], indices, weights);
					prevPositions[1] = lerp(previous[1], indices, weights);
					prevPositions[2] = lerp(previous[2], indices, weights);
...
template <typename CurrentT>
__device__ void CuCollision::collideConvexes(CurrentT& current, float alpha)
{
	...
	for(uint32_t i = threadIdx.x; i < gClothData.mNumParticles; i += blockDim.x)
	{
		...
		if(uint32_t numCollisions = collideConvexes(positions, delta))
		{
			float scale = __fdividef(1.0f, numCollisions);
