...
: CuContextLock(factory),
  ...
  mStartCollisionTriangles(getMappedAllocator<PxVec4>(mFactory.mContextManager)),
  mTargetCollisionTriangles(getMappedAllocator<PxVec4>(mFactory.mContextManager)),
...
void cloth::CuCloth::setPhaseConfig( Range<const PhaseConfig> configs )
{
	...
	for(const PhaseConfig *cIt = configs.begin(); cIt != cEnd; ++cIt)
	{
		...
		config.mNumConstaints = mFabric.mNumConstraintsInPhase[phaseIndex];
...
PxVec4* cloth::CuCloth::getDeviceParticles(bool markHostDirty)
{
	PxVec4* data = mParticles.begin().get();
	if(mDeviceParticlesDirty)
	{
		CuContextLock contextLock(mFactory);
		const PxVec4* src = mParticlesHostCopy.begin();
		mFactory.copyFromHost(src, src + 2*mNumParticles, data);
		mDeviceParticlesDirty = false;
	}
	mHostParticlesDirty |= markHostDirty;
	return data;
}

PxVec4* cloth::CuCloth::getHostParticles(bool markDeviceDirty)
{
	PxVec4* data = mParticlesHostCopy.begin();
	if(mHostParticlesDirty)
	{
		CuContextLock contextLock(mFactory);
		const PxVec4* src = mParticles.begin().get();
		mFactory.copyToHost(src, src + 2*mNumParticles, data);
		mHostParticlesDirty = false;
	}
	mDeviceParticlesDirty |= markDeviceDirty;
	return data;
}

cloth::Range<const PxVec3> cloth::CuCloth::clampTriangleCount( Range<const PxVec3> range, uint32_t replaceSize )
{
	// clamp to 500 triangles to prevent running out of shared memory
	uint32_t removedSize = mStartCollisionTriangles.size() / 3 - replaceSize;
	const PxVec3* clamp = range.begin() + 500 - removedSize;
...
template <>
Range<PxVec4> ClothImpl<CuCloth>::getCurrentParticles(Memory memory)
{
	mCloth.wakeUp();

	CuCloth& cloth = mCloth;
	PxVec4* data = memory == HOST_MEMORY ? 
		cloth.getHostParticles(true) : cloth.getDeviceParticles(true);
	return Range<PxVec4>(data, data + mCloth.mNumParticles);
}

template <>
Range<const PxVec4> ClothImpl<CuCloth>::getCurrentParticles(Memory memory) const
{
	CuCloth& cloth = const_cast<CuCloth&>(mCloth);
	const PxVec4* data = memory == HOST_MEMORY ? 
		cloth.getHostParticles(false) : cloth.getDeviceParticles(false);
	return Range<const PxVec4>(data, data + mCloth.mNumParticles);
}

template <>
Range<PxVec4> ClothImpl<CuCloth>::getPreviousParticles(Memory memory)
{
	mCloth.wakeUp();

	CuCloth& cloth = mCloth;
	PxVec4* data = mCloth.mNumParticles + (memory == HOST_MEMORY ? 
		cloth.getHostParticles(true) : cloth.getDeviceParticles(true));
	return Range<PxVec4>(data, data + mCloth.mNumParticles);
}

template <>
Range<const PxVec4> ClothImpl<CuCloth>::getPreviousParticles(Memory memory) const
{
	CuCloth& cloth = const_cast<CuCloth&>(mCloth);
	const PxVec4* data = mCloth.mNumParticles + (memory == HOST_MEMORY ? 
		cloth.getHostParticles(false) : cloth.getDeviceParticles(false));
	return Range<const PxVec4>(data, data + mCloth.mNumParticles);
}

template <>
Range<PxVec4> ClothImpl<CuCloth>::getRestPositions()
{
	if(mCloth.mRestPositions.empty())
	{
		CuContextLock contextLock(mCloth.mFactory);
		mCloth.mRestPositions.resize(mCloth.mNumParticles);
	}

	mCloth.wakeUp();

	PxVec4* data = mCloth.mRestPositions.begin().get();
	return Range<PxVec4>(data, data + mCloth.mRestPositions.size());
}

template <>
void ClothImpl<CuCloth>::clearRestPositions()
{
	CuContextLock contextLock(mCloth.mFactory);
	CuDeviceVector<PxVec4>(mCloth.mFactory.mContextManager).swap(mCloth.mRestPositions);
	mCloth.wakeUp();
}
