...
public:
	enum Memory { HOST_MEMORY, DEVICE_MEMORY };
	...
	virtual Range<PxVec4> getCurrentParticles(Memory = DEVICE_MEMORY) = 0;
	virtual Range<const PxVec4> getCurrentParticles(Memory = DEVICE_MEMORY) const = 0;
	virtual Range<PxVec4> getPreviousParticles(Memory = DEVICE_MEMORY) = 0;
	virtual Range<const PxVec4> getPreviousParticles(Memory = DEVICE_MEMORY) const = 0;
	...
	// these are host pointers for both CPU and GPU implementation.
	...
	// rest particles are currently used only during self-collision
	// these are CUDA pointers for the GPU implementation
	virtual Range<PxVec4> getRestPositions() = 0;
	virtual void clearRestPositions() = 0;
...
inline Range<const PxVec4> readCurrentParticles(const Cloth& cloth, 
	Cloth::Memory memory = Cloth::DEVICE_MEMORY) 
{ 
	return cloth.getCurrentParticles(memory); 
}
inline Range<const PxVec4> readPreviousParticles(const Cloth& cloth, 
	Cloth::Memory memory = Cloth::DEVICE_MEMORY) 
{ 
	return cloth.getPreviousParticles(memory); 
}
