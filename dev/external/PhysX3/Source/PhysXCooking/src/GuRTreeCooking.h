...
namespace physx
{
	struct RTreeCooker
	{
		...
		static void buildFromBounds(
			Gu::RTree& resultTree, const PxBounds3* allBounds, const PxU32* boundedData, const PxU32* bigBounds,
			PxU32 numBounds, PxU32 numTrisPerLeaf, Array<PxU32>& resultPermute, RemapCallback* rc, const PxVec3* verts);
		...
		static void buildFromTriangles(
			...
			PxU32 numTrisPerLeaf, Array<PxU32>& resultPermute, RemapCallback* rc);
	};
}
