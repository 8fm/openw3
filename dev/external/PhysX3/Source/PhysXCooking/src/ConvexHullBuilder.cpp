...
static bool inverseBuffer(PxU32 nbEntries, PxU8* entries)
{
	...
	for(PxU32 i=0;i<nbEntries>>1;i++)
		TSwap(entries[i], entries[nbEntries-1-i]);
...
static bool ExtractHullPolygons(PxU32& nb_polygons, Container& polygon_data, const ConvexHullBuilder& hull, Container* triangle_data, Container& rendundantVertices)
{
	...
	{			
		...
		PxU32 cleanedVerts = 0;

		for(PxU32 j=0;j<NbVerts;j++)
		{
			if(!rendundantVertices.Contains(Data[j]))
				cleanedVerts++;
		}

		if(cleanedVerts < 3)
		{
			rendundantVertices.Empty();
			break;
		}
...
static bool TestDuplicateTriangles(PxU32& nb_faces, Gu::TriangleT<PxU32>* faces, bool repair)
{
	if(!nb_faces || !faces)	return true;

	Gu::TriangleT<PxU32>* Indices32 = (Gu::TriangleT<PxU32>*)PxAlloca(nb_faces*sizeof(Gu::TriangleT<PxU32>));
	...
	// Radix-sort power...
	ReducedVertexCloud	Reducer(reinterpret_cast<PxVec3*>(Indices32), nb_faces);
	REDUCEDCLOUD rc;
	Reducer.Reduce(&rc);
	if(rc.NbRVerts<nb_faces)
	{
		if(repair)
		{
			nb_faces = rc.NbRVerts;
			for(PxU32 i=0;i<nb_faces;i++)
			{
...
static bool TestUnifiedNormals(PxU32 nb_verts, const PxVec3* verts, PxU32 nb_faces, Gu::TriangleT<PxU32>* faces, bool repair)
{
	if(!nb_verts || !verts || !nb_faces || !faces)	return false;

	// Unify normals so that all hull faces are well oriented

	// Compute geometric center - we need a vertex inside the hull
	const float Coeff = 1.0f / float(nb_verts);
...
#if 0
static bool TestZeroAreaTriangles(PxU32& nb_faces, Gu::TriangleT<PxU32>* faces, const PxVec3* verts, bool repair)
{
	if(!nb_faces || !faces || !verts)	return false;

	// Check normals for robustness... Catches the following repro case.
	/*	static PxReal v[] = {
	2.019142f, -1.345858f, 1.620000f,
	2.053024f, -1.379740f, 1.620000f,
	2.053024f, -1.583649f, 1.620000f,
	1.917324f, -1.447676f, 1.620000f,
	2.053024f, -1.583649f, 0.000000f,
	1.917324f, -1.447676f, 0.000000f,
	2.053024f, -1.379740f, 0.000000f,
	2.019142f, -1.345858f, 0.000000f,
	2.053024f, -1.583337f, 0.000000f,
	2.053024f, -1.583437f, 1.620000f,
	};*/
	...
	while(TryAgain)
	{
		...
		{
			PxVec3 Normal;
			faces[i].denormalizedNormal(verts, Normal);
			const float N = Normal.magnitude();
//			Normal.Normalize();
//			if(Normal.IsZero())
			if(N<0.0001f)
			{
				...
				// This triangle is messed up. We try to remove it by snapping two closest vertices together, hence
				// creating a real zero-area triangle (that we discard).
				const PxVec3& p0 = verts[faces[i].v[0]];
				const PxVec3& p1 = verts[faces[i].v[1]];
				const PxVec3& p2 = verts[faces[i].v[2]];
				const PxVec3 Edge01 = p0 - p1;
				const PxVec3 Edge02 = p0 - p2;
				const PxVec3 Edge12 = p1 - p2;
				const float d0 = Edge01.magnitude();
				const float d1 = Edge02.magnitude();
				const float d2 = Edge12.magnitude();

				PxU32 OldRef;
				PxU32 NewRef;
				float Smallest = d0;
				PxU32 Index=0;
				if(d1<Smallest)	{ Smallest = d1; Index = 1; }
				if(d2<Smallest)	{ Smallest = d2; Index = 2; }
				if(Index==0)
				{
					// Remove 1, replace with 0
					OldRef = faces[i].v[1];
					NewRef = faces[i].v[0];
				}
				else if(Index==1)
				{
					// Remove 2, replace with 0
					OldRef = faces[i].v[2];
					NewRef = faces[i].v[0];
				}
				else
				{
					// Remove 2, replace with 1
					OldRef = faces[i].v[2];
					NewRef = faces[i].v[1];
				}

				for(PxU32 j=0;j<nb_faces;j++)
				{
					if(faces[j].v[0]==OldRef)	faces[j].v[0] = NewRef;
					if(faces[j].v[1]==OldRef)	faces[j].v[1] = NewRef;
					if(faces[j].v[2]==OldRef)	faces[j].v[2] = NewRef;
				}
				...
				{
					if(faces[Index].isDegenerate())
					{
						faces[Index] = faces[--nb_faces];
					}
					else Index++;
				}

				if(nb_faces<=4)	return false;
				// Try again !
				TryAgain = true;
...
static bool cleanMesh(PxU32& nb_verts, PxU32& nb_tris, PxVec3* verts, Gu::TriangleT<PxU32>* faces)
{
	MBCreate create;
	create.NbVerts					= nb_verts;
	create.NbFaces					= nb_tris;
	create.NbTVerts					= 0;
	create.NbCVerts					= 0;
	create.Verts					= verts;
	create.TVerts					= NULL;
	...
	create.OptimizeVertexList		= false;

	MeshBuilder2 meshBuilder;
	if(!meshBuilder.Init(create))
		return false;

	for(PxU32 i=0;i<nb_tris;i++)
	{
		PxU32 DFaces[3];
		DFaces[0] = faces[i].v[0];
		DFaces[1] = faces[i].v[1];
		DFaces[2] = faces[i].v[2];
		...
		face.CRefs				= NULL;
		face.CCW				= false;
		bool status = meshBuilder.AddFace(face);
		PX_UNUSED(status);
	}

	MBResult result;
	if(!meshBuilder.Build(result))
		return false;

	nb_verts = result.Geometry.NbVerts;
	nb_tris = result.Topology.NbFaces;

	for(PxU32 i=0;i<nb_verts;i++)
	{
		const PxU32 vref = result.Geometry.VertsRefs[i];
		verts[i].x = result.Geometry.Verts[vref*3+0];
		verts[i].y = result.Geometry.Verts[vref*3+1];
		verts[i].z = result.Geometry.Verts[vref*3+2];
	}

	for(PxU32 i=0;i<nb_tris;i++)
	{
		faces[i].v[0] = result.Topology.VRefs[i*3+0];
		faces[i].v[1] = result.Topology.VRefs[i*3+1];
		faces[i].v[2] = result.Topology.VRefs[i*3+2];
	}
	return true;
}

static bool CleanHull(PxU32& nb_faces, Gu::TriangleT<PxU32>* faces, PxU32& nb_verts, PxVec3* verts)
{
	// Brute force mesh cleaning.
	// PT: I added this back on Feb-18-05 because it fixes bugs with hulls from QHull.
	if(!cleanMesh(nb_verts, nb_faces, verts, faces))	return false;

	// Get rid of duplicates
	TestDuplicateTriangles(nb_faces, faces, true);

	// Unify normals
	TestUnifiedNormals(nb_verts, verts, nb_faces, faces, true);

	// Remove zero-area triangles
	//	TestZeroAreaTriangles(nb_faces, faces, verts, true);
...
static bool CheckHull(PxU32 nb_faces, const Gu::TriangleT<PxU32>* faces, PxU32 nb_verts, const PxVec3* verts)
{
	...
	if(!TestDuplicateTriangles(nb_faces, f, false))	return false;

	// Test unified normals
	if(!TestUnifiedNormals(nb_verts, verts, nb_faces, f, false))	return false;

	// Test zero-area triangles
	//	if(!TestZeroAreaTriangles(nb_faces, f, verts, false))	return false;
...
bool ConvexHullBuilder::Init(const SurfaceInterface& surface, const PxU32 nb_polygons, const PxHullPolygon* hullPolygons, const bool* /*internal*/)
{
	mHullDataHullVertices			= NULL;
	mHullDataPolygons				= NULL;
	mHullDataVertexData8			= NULL;
	...
	mNbHullFaces					= surface.mNbFaces;
	mHull->mNbHullVertices			= Ps::to8(surface.mNbVerts);
	mHullDataHullVertices			= (PxVec3*)PX_ALLOC(sizeof(PxVec3) * mHull->mNbHullVertices, PX_DEBUG_EXP("PxVec3"));
	PxMemCopy(mHullDataHullVertices, surface.mVerts, mHull->mNbHullVertices*sizeof(PxVec3));
	...
	if(surface.mDFaces && !hullPolygons)
	{
		// Convert to 16 bits
		mFaces = PX_NEW(HullTriangleData)[mNbHullFaces];
		for(PxU32 i=0;i<mNbHullFaces;i++)
		{
			PX_ASSERT(surface.mDFaces[i*3+0]<=0xffff);
			PX_ASSERT(surface.mDFaces[i*3+1]<=0xffff);
			PX_ASSERT(surface.mDFaces[i*3+2]<=0xffff);
			mFaces[i].mRef[0] = surface.mDFaces[i*3+0];
			mFaces[i].mRef[1] = surface.mDFaces[i*3+1];
			mFaces[i].mRef[2] = surface.mDFaces[i*3+2];
		}
	}
	...
	{		
		for (PxU32 i = 0; i < nb_polygons; i++)
		{
			...
			{
				polygons.Add(surface.mDFaces[hullPolygon.mIndexBase + j]);
			}
		}
...
//		TestUnifiedNormals(mNbHullVertices, mHullVertices, mNbHullFaces, mFaces, true);
		...
		// CreatePolygonData
		{
			...
			const PxU32 NbPolygons = nb_polygons;
			...
			CalculateVertexMapTable(NbPolygons, true);
			...
			for(PxU32 i=0;i<NbPolygons;i++)
			{
				...
				Gu::HullPolygonData& polygon = mHullDataPolygons[i];
...
//					if(d>0.0001f)
					if(d>0.02f)
					{
						PX_ASSERT(!"Hull check failed! Please report this repro case to Pierre.");
						return false;
					}
				}
			}
		...
		for (PxU32 i = 0; i < 8; i++)
		{
			if(!foundPlane[i])
			{
				PX_ASSERT(!"Hull check failed! Hull seems to have opened volume or do (some) faces have reversed winding?");
				return false;
			}
		}
	}
...
//				if(i!=j && mHull->mHullVertices[i].distance(mHull->mHullVertices[j])<0.1f)
	...
	if(0)	// Too aggressive !!
	{
		// Aggressive & non-tolerant check
		EDGELISTCREATE create;
		create.NbFaces		= mNbHullFaces;
		create.DFaces		= (const PxU32*)mHullFaces;
		create.FacesToEdges	= true;
		create.EdgesToFaces	= true;
		create.Verts		= mHullVertices;
		EdgeList EL;
		if(!EL.Init(create))
			return false;
		...
		if(CE.GetNbConvexSubmeshes()!=1)	return false;
	}
#endif
	return true;
}
...
bool ConvexHullBuilder::Save(PxOutputStream& stream, bool platformMismatch) const
{
	...
	const_cast<ConvexHullBuilder* const>(this)->CreateEdgeList();
	...
	WriteFloatBuffer(&mHullDataHullVertices->x, mHull->mNbHullVertices*3, platformMismatch, stream);
	...
	stream.write(mHullDataFacesByEdges8, mHull->mNbEdges*2);
	stream.write(mHullDataFacesByVertices8, mHull->mNbHullVertices*3);
...
bool ConvexHullBuilder::CreatePolygonData()
{
	...
	CalculateVertexMapTable(NbPolygons);
...
void ConvexHullBuilder::CalculateVertexMapTable(PxU32 nbPolygons, bool userPolygons)
{
	mHullDataFacesByVertices8 = PX_NEW(PxU8)[mHull->mNbHullVertices*3];
	...
	if(noPlaneShift)
	{
		...
		if(!userPolygons)
			Ps::getFoundation().error(PxErrorCode::ePERF_WARNING, __FILE__, __LINE__, "ConvexHullBuilder: convex hull does not have PCM info - performance drop! Try to use different convex mesh cooking settings.");
		else
			Ps::getFoundation().error(PxErrorCode::ePERF_WARNING, __FILE__, __LINE__, "ConvexHullBuilder: convex hull does not have PCM info - performance drop! Some of the vertices have less than 3 neighbor polygons. The vertex is most likely inside a polygon or on an edge between 2 polygons, please remove those vertices.");
		for(PxU32 i=0; i< mHull->mNbHullVertices; ++i)
		{
...
bool ConvexHullBuilder::CreateEdgeList()
{
	...
	for(PxU32 i=0;i<NbPolygons;i++)
	{
		...
		for(PxU32 j=0;j<NbVerts;j++)
		{
			...
			if(VRef0>VRef1)	Swap(VRef0, VRef1);
	...
	PX_ASSERT(Run0-VRefs0==NbEdgesUnshared);
	PX_ASSERT(Run1-VRefs1==NbEdgesUnshared);
