...
//					if(fabsf(a)>0.2f)	Active = true;	// Works best for Rob Elam's ship, but fails on his scene
		...
		else
		{
			...
			PxU32 FaceInd = FBE[ED->Offset]*3;
			PxU32 VRef00, VRef01, VRef02;
			PxU32 VRef10=0, VRef11=0, VRef12=0;
			if(dfaces)
			{
				VRef00 = dfaces[FaceInd+0];
				VRef01 = dfaces[FaceInd+1];
				VRef02 = dfaces[FaceInd+2];
			}
			else //if(wfaces)
			{
				PX_ASSERT(wfaces);
				VRef00 = wfaces[FaceInd+0];
				VRef01 = wfaces[FaceInd+1];
				VRef02 = wfaces[FaceInd+2];
			}
