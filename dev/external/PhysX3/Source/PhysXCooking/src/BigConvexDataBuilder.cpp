...
{
	mSVM->mData.mSamples = PX_NEW(PxU8)[mSVM->mData.mNbSamples*2];
...
//		mNbAdjVerts = mOffsets[mNbVerts-1] + mValencies[mNbVerts-1];
		mSVM->mData.mNbAdjVerts = mSVM->mData.mValencies[mSVM->mData.mNbVerts-1].mOffset + mSVM->mData.mValencies[mSVM->mData.mNbVerts-1].mCount;
...
bool BigConvexDataBuilder::Save(PxOutputStream& stream, bool platformMismatch) const
{
...

	{
		...
		for(PxU32 i=0;i<mSVM->mData.mNbVerts;i++)	temp[i] = mSVM->mData.mValencies[i].mCount;
