...
BSP::fromMesh(const physx::Array<NxExplicitRenderTriangle>& mesh, const BSPBuildParameters& params, IProgressListener* progressListener)
{
	...
	for (physx::PxU32 i = 0; i < mesh.size(); ++i)
	{
		physx::swap(triangleOrder[i], triangleOrder[i + (physx::PxU32)(((physx::PxU64)rnd->getInt() * (physx::PxU64)(mesh.size() - i)) >> 32)]);
	}
	...
	buildTree(m_root, surfaceStack, 0, surfaceStack.size(), buildConstants, &quantityListener);
...
bool
BSP::fromConvexPolyhedron(const physx::Array<physx::PxPlane>& poly, const physx::PxMat44& internalTransform)
{
	clear();
	...
	m_planes.resize(poly.size());
	for (physx::PxU32 planeIndex = 0; planeIndex < poly.size(); ++planeIndex)
	{
		for (int i = 0; i < 3; ++i)
		{
...
BSP::getSurfaceAreaAndVolume(physx::PxF32& area, physx::PxF32& volume, bool inside, Operation::Enum operation) const
{
	...
	addLeafAreasAndVolumes(realArea, realVolume, m_root, inside, BoolOp(operation));
	area = physx::PxAbs(realArea) < MAX_REAL ? (physx::PxF32)realArea : PX_MAX_F32;
	volume =  physx::PxAbs(realVolume) < MAX_REAL ? (physx::PxF32)realVolume : PX_MAX_F32;
}
...
void
BSP::copy(const IApexBSP& ibsp, const physx::PxMat44& pxTM)
{
	const BSP& bsp = *(const BSP*)&ibsp;
	...
	// Do not transform if pxTM is the identity
	bool isIdentity = true;
	for (physx::PxU32 i = 0; i < 4 && isIdentity; ++i)
	{
		for (physx::PxU32 j = 0; j < 4 && isIdentity; ++j)
		{
			isIdentity = physx::PxAbs(pxTM(i, j) - (i == j ? 1.0f : 0.0f)) < PX_EPS_F32;
		}
	}
	if (isIdentity)
	{
	...
	// Transform
	transform(tmITM*m_internalTransformInverse);
}
...
BSP::assignLeafSide(Node* leaf, QuantityProgressListener* quantityListener)
{
	...
	{
		leaf->getLeafData()->side = sumSignedArea > 0 ? 1 : 0;
...
// Old implementation
	...
	if (combineNode->getType() != Node::Leaf)
	{
		...
		else
		{
			...
			else if (regionShapes[1].is_nonempty())
			{
...
#define CULL_PLANES_LIST	1
	...
	if (node->getType() != Node::Leaf)
	{
		...
		intersection.m_planes.pushBack(-(*(const ApexCSG::GSA::Plane<3, Real>*)(m_planes.begin() + node->getBranchData()->planeIndex)));
		intersection.m_planes.back()(3) -= tol;
...
#define CULL_PLANES_LIST	0
	...
	for (Node::It it(root); it.valid(); it.inc())
	{
		...
		if (node->getType() == BSP::Node::Leaf)
		{
			...
			{
				Real area, volume;
				calculateLeafAreaAndVolume(area, volume, planes.begin(), planes.size(), cofInternalTransform);
				totalArea = physx::PxAbs(area) < MAX_REAL ? totalArea + area : MAX_REAL;
				totalVolume = physx::PxAbs(area) < MAX_REAL ? totalVolume + volume : MAX_REAL;
			}
		}
	}
	...
	if (node->getType() != Node::Leaf)
	{
		Plane plane = -m_planes[node->getBranchData()->planeIndex];
		bool addPlane = planeIsNotRedundant(planes, plane);
		if (addPlane)
		{
			planes.pushBack(plane);
		}
		addLeafAreasAndVolumes(totalArea, totalVolume, node->getChild(0), planes, inside, op);
		if (addPlane)
		{
			planes.popBack();
		}
		plane = -plane;
		addPlane = planeIsNotRedundant(planes, plane);
		if (addPlane)
		{
			planes.pushBack(plane);
		}
		addLeafAreasAndVolumes(totalArea, totalVolume, node->getChild(1), planes, inside, op);
		if (addPlane)
		{
			planes.popBack();
		}
	...
	Real area, volume;
	calculateLeafAreaAndVolume(area, volume, &planes[0], planes.size());
	if (physx::PxAbs(area) < MAX_REAL)
	{
		totalArea += area;
	}
	else
	...
	if (physx::PxAbs(volume) < MAX_REAL)
	{
		totalVolume += volume;
	}
	else
...
BSP::buildTree(Node* node, physx::Array<Surface>& surfaceStack, physx::PxU32 stackReadStart, physx::PxU32 stackReadStop,
			   const BuildConstants& buildConstants, QuantityProgressListener* quantityListener)
{
