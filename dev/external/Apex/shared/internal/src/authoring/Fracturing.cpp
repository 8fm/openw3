...
static physx::PxMat44 randomRotationMatrix(physx::PxVec3 zAxis, physx::QDSRand& rnd)
{
	...
	physx::PxU32 maxDir =  physx::PxAbs(zAxis.x) > physx::PxAbs(zAxis.y) ?
		(physx::PxAbs(zAxis.x) > physx::PxAbs(zAxis.z) ? 0 : 2) :
		(physx::PxAbs(zAxis.y) > physx::PxAbs(zAxis.z) ? 1 : 2);
...
VoronoiCellPlaneIterator::VoronoiCellPlaneIterator(const physx::PxVec3* sites, physx::PxU32 siteCount, const physx::PxPlane* boundPlanes, physx::PxU32 boundPlaneCount, physx::PxU32 startSiteIndex)
{
	...
	for (physx::PxU32 i = 0; i < m_siteCount; ++i)
	{
		for (physx::PxU32 j = 0; j < m_siteCount; ++j)
		{
			...
			ReciprocalSitePairLink& pair = m_sitePairs[m_pairIndex];
			if (j > i)
			{
				pair.setRecip(m_sitePairs[m_pairIndex+(j-i)*(m_siteCount-2)+1]);
			}
			pair.index0 = i;
			pair.index1 = j;
			...
			if (m_pairIndex > 0)
			{
				m_sitePairs[m_pairIndex-1].setAdj(1, &pair);
			}
					   ...
					   int noiseType, int noiseDir)
{
	...
	const physx::PxU32 numModes = noiseType == FractureSliceDesc::NoiseWavePlane ? 20 : 4;
...
void physx::DisplacementMapVolume::init(const FractureSliceDesc& desc)
{
	PX_UNUSED(desc);
	...
	for (physx::PxU32 i = 0; i < 3; ++i)
	{
		...
		for (physx::PxU32 j = 0; j < desc.maxDepth; ++j)
		{
			if (desc.sliceParameters[j].noise[i].gridSize > (int)maxGridSize)
				maxGridSize = desc.sliceParameters[j].noise[i].gridSize;
		}
	}
...
static void buildSliceBSP(ApexCSG::IApexBSP& sliceBSP, physx::ExplicitHierarchicalMesh& hMesh, const physx::NoiseParameters& noise,
                          const physx::PxVec3& extent, int sliceDir, int sliceDepth, const physx::PxPlane planes[3], 
						  const physx::NxFractureMaterialDesc& materialDesc, int noiseType = FractureSliceDesc::NoiseWavePlane, bool useDisplacementMaps = false)
{
	...
	gridParameters.noise = useDisplacementMaps ? physx::NoiseParameters() : noise;
...
PX_INLINE ApexCSG::IApexBSP* createFractureBSP(physx::PxPlane slicePlanes[3], ApexCSG::IApexBSP*& sliceBSP, ApexCSG::IApexBSP& sourceBSP,
        ...
        const physx::PxF32 sliceWidths[3], const physx::PxF32 linearNoise[3], const physx::PxF32 angularNoise[3],
		const physx::NoiseParameters& noise, const physx::NxFractureMaterialDesc& materialDesc, int noiseType, bool useDisplacementMaps)
{
...
(
	...
	const physx::SliceParameters& sliceParameters,
	const physx::NxCollisionDesc& collisionDesc,
	physx::IProgressListener& progressListener,
	volatile bool* cancel
)
{
	...
	while (chunkBSP != NULL)
	{
		if (!canceled)
		{
			...
			hMesh.mChunks[newChunkIndex]->mParentIndex = chunkIndex;
			hMesh.mChunks[newChunkIndex]->mPartIndex = newPartIndex;
...
)
{
	...
	const physx::SliceParameters& sliceParameters = desc.sliceParameters[relativeSliceDepth++];
	...
	for (int sliceDir0Num = 0; sliceDir0Num < partition[sliceDir0] && !canceled; ++sliceDir0Num)
	{
		...
		if (sliceDir0Num + 1 < partition[sliceDir0])
		{
			...
			fractureBSP1 = createFractureBSP(leadingPlanes, reusedSliceBSP, *fractureBSP0, hMesh, childVolume, 0, center, extent, sliceDir0, sliceDir0Num, sliceDepth, sliceWidths,
			                                 sliceParameters.linearVariation, sliceParameters.angularVariation, sliceParameters.noise[sliceDir0],
											 desc.materialDesc[sliceDir0], desc.noiseMode, desc.useDisplacementMaps);
		...
		for (int sliceDir1Num = 0; sliceDir1Num < partition[sliceDir1] && !canceled; ++sliceDir1Num)
		{
			...
			if (sliceDir1Num + 1 < partition[sliceDir1])
			{
				...
				ApexCSG::IApexBSP*& sliceBSP = !slicingThrough ? reusedSliceBSP : sliceBSPs1[sliceDir1Num];
				fractureBSP2 = createFractureBSP(leadingPlanes, sliceBSP, *fractureBSP1, hMesh, childVolume, 0, center, extent, sliceDir1, sliceDir1Num, sliceDepth,
				                                 sliceWidths, sliceParameters.linearVariation, sliceParameters.angularVariation, sliceParameters.noise[sliceDir1],
												 desc.materialDesc[sliceDir1], desc.noiseMode, desc.useDisplacementMaps);
			...
			for (int sliceDir2Num = 0; sliceDir2Num < partition[sliceDir2] && !canceled; ++sliceDir2Num)
			{
				...
				if (sliceDir2Num + 1 < partition[sliceDir2])
				{
					...
					ApexCSG::IApexBSP*& sliceBSP = !slicingThrough ? reusedSliceBSP : sliceBSPs2[sliceDir2Num];
					fractureBSP3 = createFractureBSP(leadingPlanes, sliceBSP, *fractureBSP2, hMesh, childVolume, minChildVolume, center, extent, sliceDir2, sliceDir2Num, sliceDepth,
					                                 sliceWidths, sliceParameters.linearVariation, sliceParameters.angularVariation, sliceParameters.noise[sliceDir2],
													 desc.materialDesc[sliceDir2], desc.noiseMode, desc.useDisplacementMaps);
					if (sliceBSP == reusedSliceBSP)
					{
						reusedSliceBSP->release();
						reusedSliceBSP = NULL;
	...
	void removeTJunctions()
	{
		...
		for (physx::PxU32 i = mTrianglePartition; i < originalMeshSize; ++i)
		{
			...
			for (physx::PxU32 v = 0; v < 3; ++v)
			{
				...
				{
					splitTriangle(neighborToSplit, edgeToSplit, point);
				}
			}
		}
	}
		...
		do
		{
			...
			for (physx::PxU32 i = mTrianglePartition; i < mesh.size(); ++i)	// this array (as well as info) might grow during the loop
			{
				...
				if (edgeLengthSquared[longestEdge] > maxEdgeLength2)
				{
					...
					splitTriangle(i, longestEdge, newVertexPosition);
							...
							for (physx::PxU32 k = 0; k < triangleChildList.size(); ++k)
							{
								...
								{	
									...
									if (sharedEdge >= 0)
									{
										...
										{
											splitTriangle(neighbor, edgeToSplit, pointOnEdge);
										}
									}
								}
							}
...
private:
	...
	PX_INLINE void splitTriangle(physx::PxU32 triangleIndex, physx::PxU32 edgeIndex, const physx::PxVec3& newVertexPosition)
	{
		...
		const int nextEdge = (edgeIndex + 1)%3;
...
(
 ...
 const physx::NoiseParameters& noise,
 ...
 )
{
	...
	// Mesh and mesh size
	physx::PxF32 level0Size = hMesh.mParts[hMesh.mChunks[0]->mPartIndex]->mBounds.getExtents().magnitude();
...
(
	...
	physx::PxU32 chunkIndex,
	const ApexCSG::IApexBSP& chunkBSP,
	const physx::FractureVoronoiDesc& desc,
...
)
{
	...
	HierarchicalProgressListener progress(desc.siteCount, &progressListener);

	for (VoronoiCellPlaneIterator i(desc.sites, desc.siteCount); i.valid(); i.inc())
	{
		...
		for (physx::PxU32 planeN = 0; planeN < i.cellPlaneCount(); ++planeN)
		{
			...
			// Defer noise generation if we're using displacement maps
			gridParameters.noise = physx::NoiseParameters();
			gridParameters.level0Mesh = &hMesh.mParts[0]->mMesh;
			gridParameters.materialFrameIndex = hMesh.addMaterialFrame();
			physx::NxMaterialFrame materialFrame = hMesh.getMaterialFrame(gridParameters.materialFrameIndex);
			...
			buildIntersectMesh(grid, plane, materialFrame, desc.noiseMode, &gridParameters);
			...
			planeBSP->fromMesh(grid.m_triangles, bspBuildParams);
			...
			if (!canceled)
			{
				...
				else
				{
					...
					if (desc.faceNoise.amplitude > 0.0f){
						const physx::PxU32 parentPartIndex = *hMesh.partIndex(chunkIndex);
...
static void trimChunkHulls(physx::ExplicitHierarchicalMesh& hMesh, physx::PxU32* chunkIndexArray, physx::PxU32 chunkIndexArraySize, physx::PxF32 maxTrimFraction)
{
	...
	for (physx::PxU32 chunkNum = 0; chunkNum < chunkIndexArraySize; ++chunkNum)
	{
		...
		const physx::PxU32 partIndex = hMesh.mChunks[chunkIndex]->mPartIndex;
	...
	for (physx::PxU32 chunkNum0 = 0; chunkNum0 < chunkIndexArraySize; ++chunkNum0)
	{
		...
		const physx::PxU32 partIndex0 = hMesh.mChunks[chunkIndex0]->mPartIndex;
		...
		for (physx::PxU32 hullIndex0 = 0; hullIndex0 < hullCount0; ++hullIndex0)
		{
			...
			for (physx::PxU32 chunkNum1 = chunkNum0+1; chunkNum1 < chunkIndexArraySize; ++chunkNum1)
			{
				...
				const physx::PxU32 partIndex1 = hMesh.mChunks[chunkIndex1]->mPartIndex;
...
(
	...
	bool firstPartitionIsDepthZero
)
{
	...
	if (!havePartition)
	{
		firstPartitionIsDepthZero = false;	// This only makes sense if we have a partition
	}
	...
	const physx::PxU32 part0Size = firstPartitionIsDepthZero ? meshPartition[0] : meshTriangleCount;	// Build level 0 part out of all of the triangles if firstPartitionIsDepthZero = false
	...
	if (havePartition)
	{
		...
		const physx::PxU32 firstLevel1Part = firstPartitionIsDepthZero ? 1 : 0;
		for (physx::PxU32 i = firstLevel1Part; i < meshPartitionCount; ++i, ++partIndex)
		{
			...
			hMesh.addChunk();
			hMesh.mChunks[partIndex]->mParentIndex = 0;	// partIndex = chunkIndex here
			hMesh.mChunks[partIndex]->mPartIndex = partIndex;	// partIndex = chunkIndex here
		}
	}
	...
	{
		hMesh.mRootDepth = hMesh.depth(hMesh.mChunks.size() - 1);
	}
...
static bool buildExplicitHierarchicalMeshFromApexAssetsInternal(physx::ExplicitHierarchicalMesh& hMesh, const physx::apex::NxRenderMeshAsset& renderMeshAsset, const physx::apex::NxDestructibleAsset* destructibleAsset)
{
	...
	hMesh.mRootDepth = destructibleAsset != NULL && destructibleAsset->getDepthCount() > 0 ? destructibleAsset->getDepthCount() - 1 : 0;	// For now, we have to assume something - will make this the deepest chunk depth
	...
	for (physx::PxU32 submeshIndex = 0; submeshIndex < renderMeshAsset.getSubmeshCount(); ++submeshIndex)
	{
		...
		submeshData.mVertexFormat.mHasStaticPositions = vb.getBufferData(&positions[0], physx::NxRenderDataFormat::FLOAT3, sizeof(physx::PxVec3), positionBufferIndex, 0, submeshVertexCount);
		...
		submeshData.mVertexFormat.mHasStaticNormals = vb.getBufferData(&normals[0], physx::NxRenderDataFormat::FLOAT3, sizeof(physx::PxVec3), normalBufferIndex, 0, submeshVertexCount);
		...
		submeshData.mVertexFormat.mHasDynamicTangents = vb.getBufferData(&tangents[0], physx::NxRenderDataFormat::FLOAT3, sizeof(physx::PxVec3), tangentBufferIndex, 0, submeshVertexCount);
		...
		submeshData.mVertexFormat.mHasStaticBinormals = vb.getBufferData(&binormals[0], physx::NxRenderDataFormat::FLOAT3, sizeof(physx::PxVec3), binormalBufferIndex, 0, submeshVertexCount);
		...
		submeshData.mVertexFormat.mHasStaticColors = vb.getBufferData(&colors[0], physx::NxRenderDataFormat::B8G8R8A8, sizeof(physx::PxColorRGBA), colorBufferIndex, 0, submeshVertexCount);
		...
		for (; uvNum < NxVertexFormat::MAX_UV_COUNT; ++uvNum)
		{
			...
			if (!vb.getBufferData(&uvs[uvNum][0], physx::NxRenderDataFormat::FLOAT2, sizeof(physx::NxVertexUV), uvBufferIndex, 0, submeshVertexCount))
			{
				break;
			}
		}
		...
		bool depth0ChunkHasTrianglesWithThisSubmesh = false;
		for (physx::PxU32 partIndex = 0; partIndex < renderMeshAsset.getPartCount(); ++partIndex)
		{
			...
			if (triangleCount > 0 && destructibleAsset->getChunkCount() > 0 && destructibleAsset->getPartIndex(0) == partIndex)
			{
				// This part is in the root (depth 0) chunk.  Make sure we've accounted for all of its submeshes
				depth0ChunkHasTrianglesWithThisSubmesh = true;
			}
		...
		if (depth0ChunkHasTrianglesWithThisSubmesh)
		{
	...
	if (destructibleAsset != NULL)
	{
		for (physx::PxU32 chunkIndex = 0; chunkIndex < destructibleAsset->getChunkCount(); ++chunkIndex)
		{
			...
			chunk->mPartIndex = destructibleAsset->getPartIndex(chunkIndex);
...
bool buildExplicitHierarchicalMeshFromRenderMeshAsset(physx::IExplicitHierarchicalMesh& iHMesh, const physx::apex::NxRenderMeshAsset& renderMeshAsset)
{
	return buildExplicitHierarchicalMeshFromApexAssetsInternal(*(physx::ExplicitHierarchicalMesh*)&iHMesh, renderMeshAsset, NULL);
}

bool buildExplicitHierarchicalMeshFromDestructibleAsset(physx::IExplicitHierarchicalMesh& iHMesh, const physx::apex::NxDestructibleAsset& destructibleAsset)
{
	...
	return buildExplicitHierarchicalMeshFromApexAssetsInternal(*(physx::ExplicitHierarchicalMesh*)&iHMesh, *renderMeshAsset, &destructibleAsset);
}
...
)
{
	...
	if (hMesh.mRootDepth > 0 && hMeshCore.partCount() > 0 && exportCoreMesh)
	{
		char message[1000];
	...
	{
		void	serialize(physx::PxFileBuf& stream, IEmbedding::DataType type) const
		{
			(void)stream;
			(void)type;
		}
		void	deserialize(physx::PxFileBuf& stream, IEmbedding::DataType type, physx::PxU32 version)
		{
			(void)stream;
			(void)type;
			(void)version;
	...
	for (physx::PxU32 i = 0; i < hMesh.chunkCount(); ++i)
	{
		...
		physx::PxU32 partIndex = *hMesh.partIndex(i);
		if (hMesh.mParts[partIndex]->mMeshBSP->getType() != ApexCSG::BSPType::Nontrivial)
		{
			outputMessage("Building mesh BSP...");
			progressListener.setProgress(0);
			hMesh.calculatePartBSP(partIndex, randomSeed, meshProcessingParams.mMicrogridSize, &progressListener);
	...
	if (hMeshCore.partCount() > 0)
	{
		...
		if (exportCoreMesh)
		{
			...
			hMesh.mChunks[coreChunkIndex]->mPartIndex = corePartIndex;
		...
		for (physx::PxU32 i = 0; i < tempCoreMesh.chunkCount(); ++i)
		{
			...
			// Remap materials
			physx::PxU32 partIndex = *tempCoreMesh.partIndex(i);
			for (physx::PxU32 j = 0; j < tempCoreMesh.mParts[partIndex]->mMesh.size(); ++j)
			{
				...
				{
					tri.submeshIndex = coreMeshImprintSubmeshIndex < 0 ? submeshMap[tri.submeshIndex] : coreMeshImprintSubmeshIndex;
					if (exportCoreMesh && i == 0)
					{
						hMesh.mParts[corePartIndex]->mMesh[j].submeshIndex = submeshMap[hMesh.mParts[corePartIndex]->mMesh[j].submeshIndex];
					}
				}
				else
				{
			...
			if (tempCoreMesh.mParts[partIndex]->mMeshBSP->getType() != ApexCSG::BSPType::Nontrivial)
			{
				...
				progressListener.setProgress(0);
				tempCoreMesh.calculatePartBSP(partIndex, randomSeed, meshProcessingParams.mMicrogridSize, &progressListener);
	...
	gMicrogridSize = meshProcessingParams.mMicrogridSize;
	...
	for (physx::PxU32 chunkIndex = 0; chunkIndex < hMesh.mChunks.size() && !canceled; ++chunkIndex)
	{
		...
		physx::PxU32 partIndex = *hMesh.partIndex(chunkIndex);
		...
		for (physx::PxU32 i = 0; i < tempCoreMesh.chunkCount(); ++i)
		{
			...
			physx::PxU32 corePartIndex = *tempCoreMesh.partIndex(i);
			if (tempCoreMesh.mParts[corePartIndex]->mMeshBSP != NULL)
			{
				seedBSP->combine(*tempCoreMesh.mParts[corePartIndex]->mMeshBSP);
		...
		if (depth == hMesh.mRootDepth)
		{
			...
			progressListener.setProgress(0);
			canceled = !splitter.process(hMesh, partIndex, *seedBSP, collisionDesc, progressListener, cancel);
	...
	if (meshProcessingParams.mRemoveTJunctions && hMesh.mParts.size())
	{
...
)
{
	...
	const physx::PxU32 partIndex = *partIndexPtr;
	...
	if (meshProcessingParams.mRemoveTJunctions)
	{
		MeshProcessor meshProcessor;
...
static void createVoronoiSitesInsideMeshInternal
(
	physx::ExplicitHierarchicalMesh& hMesh,
...
)
{
	...
	for (physx::PxU32 chunkNum = 0; chunkNum < chunkCount; ++chunkNum)
	{
		...
		bounds.include(hMesh.chunkBounds(chunkIndex));
	}

	for (physx::PxU32 siteN = 0; siteN < siteCount; ++siteN)
	{
		...
		while (attemptsLeft--)
		{
			...
			physx::PxU32 chunkN = 0;
			for (; chunkN < chunkCount; ++chunkN)
			{
				physx::PxU32 chunkIndex = chunkIndices[chunkN];
				const physx::PxU32 partIndex = *hMesh.partIndex(chunkIndex);
				ApexCSG::IApexBSP* meshBSP = hMesh.mParts[partIndex]->mMeshBSP;
				if (meshBSP->pointInside(site - *hMesh.instancedPositionOffset(chunkIndex)))
				{
					*siteBuffer++ = site;
					break;
				}
			}
			if (chunkN < chunkCount)
			{
				break;
			}
		}
...
public:
	...
	bool validate(physx::ExplicitHierarchicalMesh& hMesh)
	{
		...
		{
			if (hMesh.depth(chunkIndex) != hMesh.mRootDepth)
			{
				continue;
			}
			physx::PxU32 partIndex = *hMesh.partIndex(chunkIndex);
			...
			{
				// Get parameters for this depth
				const physx::SliceParameters& sliceParameters = mDesc.sliceParameters[chunkDepth];
				int partition[3];
...
PX_INLINE physx::PxMat34Legacy createCutoutFrame(const physx::PxVec3& center, const physx::PxVec3& extents, physx::PxU32 sliceAxes[3], physx::PxU32 sliceSignNum, const FractureCutoutDesc& desc, bool& invertX)
{
...
//	}
	...
	if (hMesh.mParts[newPartIndex]->mMesh.size() > 0)
	{
		...
		hMesh.buildCollisionGeometryForPart(newPartIndex, volumeDesc);
		const physx::PxU32 newChunkIndex = hMesh.addChunk();
		hMesh.mChunks[newChunkIndex]->mParentIndex = sourceIndex;
		hMesh.mChunks[newChunkIndex]->mPartIndex = newPartIndex;
	}
	else
	{
...
static bool createCutout(
	...
	const physx::NoiseParameters& edgeNoise,
	...
	volatile bool* cancel)
{
	...
	const physx::PxU32 facePartIndex = *hMesh.partIndex(faceChunkIndex);
...
static void instanceChildren(physx::ExplicitHierarchicalMesh& hMesh, physx::PxU32 instancingChunkIndex, physx::PxU32 instancedChunkIndex)
{
	for (physx::PxU32 chunkIndex = 0; chunkIndex < hMesh.chunkCount(); ++chunkIndex)
	{
		if (hMesh.mChunks[chunkIndex]->mParentIndex == (physx::PxI32)instancingChunkIndex)
		{
			...
			hMesh.mChunks[instancedChildIndex]->mParentIndex = instancedChunkIndex;
...
{
	physx::FractureVoronoiDesc cutoutVoronoiDesc;
		...
		while (unhandledChunkCount > 0)
		{
...
#endif
					canceled = !hierarchicallySplitChunkInternal(hMesh, chunkIndex, 0, trailingPlanes, leadingPlanes, *hMesh.mParts[chunk->mPartIndex]->mMeshBSP, bspVolume, sliceDesc, collisionDesc, localProgressListener, cancel);
				}
				break;
			case FractureCutoutDesc::VoronoiFractureCutoutChunks:
				{
					// Voronoi split
					createVoronoiSitesInsideMeshInternal(hMesh, &chunkIndex, 1, cutoutVoronoiDesc.siteCount > 0 ? &perChunkSites[0] : NULL, cutoutVoronoiDesc.siteCount, NULL, &gMicrogridSize, progressListener );
					canceled = !voronoiSplitChunkInternal(hMesh, chunkIndex, *hMesh.mParts[chunk->mPartIndex]->mMeshBSP, cutoutVoronoiDesc, collisionDesc, localProgressListener, cancel);
				}
				break;
			}
			...
			if (unhandledChunkCount > 0)
			{
				...
				while (testChunkCount > 0)
				{
					...
					const physx::PxU32 testPartIndex = testChunk->mPartIndex;
					...
					combinedBSP->combine(*hMesh.mParts[chunk->mPartIndex]->mMeshBSP);
					...
					if (xorVolume <= volumeTol)
					{
						...
						hMesh.removePart(testChunk->mPartIndex);	// Remove part for this chunk, since we'll be instancing another part
...
(
	...
	const physx::FractureCutoutDesc& desc,
	const physx::NoiseParameters& backfaceNoise,
	const physx::NoiseParameters& edgeNoise,
	const physx::NxFractureMaterialDesc& materialDesc,
...
{
	physx::HierarchicalProgressListener localProgressListener(iCutoutSet.getCutoutCount(), &progressListener);
	...
	physx::PxU32 corePartIndex = hMesh.mChunks[coreChunkIndex]->mPartIndex;
	...
	if (cutoutDepth > 0.0f)	// (depth = 0) => slice all the way through
	{
		...
		hMesh.setMaterialFrame(gridParameters.materialFrameIndex, materialFrame);
		gridParameters.triangleFrame.setFlat(materialFrame.mCoordinateSystem, materialDesc.uvScale, materialDesc.uvOffset);
		buildIntersectMesh(grid, slicePlane, materialFrame, sliceDesc.noiseMode, &gridParameters);
		ApexCSG::BSPTolerances bspTolerances = ApexCSG::gDefaultTolerances;
		...
		hMesh.mChunks[faceChunkIndex]->mPartIndex = facePartIndex;
...
)
{
	...
	if (hMesh.mParts[0]->mMeshBSP->getType() != ApexCSG::BSPType::Nontrivial)
	{
		outputMessage("Building mesh BSP...");
		progressListener.setProgress(0);
		hMesh.calculateMeshBSP(randomSeed, &progressListener, &meshProcessingParams.mMicrogridSize);
		outputMessage("Mesh BSP completed.");
		userRnd.m_rnd.setSeed(randomSeed);
	}
	...
	for (physx::PxU32 dirIndex = 0; dirIndex < FractureCutoutDesc::DirectionCount; ++dirIndex)
	{
	...
	physx::HierarchicalProgressListener localProgressListener(physx::PxMax(directionCount, 1u), &progressListener);
	...
	bool canceled = false;
	bool stop = false;
	for (physx::PxU32 dirNum = 0; dirNum < FractureCutoutDesc::DirectionCount && !stop && !canceled; ++dirNum)
	{
		...
		getCutoutSliceAxisAndSign(sliceAxisNum, sliceSignNum, sliceDirIndex);
		{
			if ((desc.directions >> sliceDirIndex) & 1)
			{
				...
				canceled = !cutoutFace(hMesh, faceTrimPlanes, coreBSP, coreChunkIndex, desc, desc.cutoutParameters[sliceDirIndex].backfaceNoise, desc.cutoutParameters[sliceDirIndex].edgeNoise,
									   desc.cutoutParameters[sliceDirIndex].materialDesc, sliceDirIndex, facePlane, iCutoutSet, cutoutTM, mapXLow, mapYLow, localBounds,
...
)
{
	...
	if (chunkIndex >= hMesh.chunkCount())
	{
		...
		if (chunkList.size() > 0)
		{
			createVoronoiSitesInsideMeshInternal(hMesh, &chunkList[0], chunkList.size(), siteBuffer, siteCount, randomSeed, microgridSize, progressListener);
		}
	}
	else
	{
		createVoronoiSitesInsideMeshInternal(hMesh, &chunkIndex, 1, siteBuffer, siteCount, randomSeed, microgridSize, progressListener);
	}
		...
		if (rma != NULL)
		{
			...
			for (physx::PxU32 submeshIndex = 0; submeshIndex < rma->getSubmeshCount(); ++submeshIndex)
			{
				...
				if (vertexBuffer.getVertexCount() > 0)
				{
					...
					if (!vertexBuffer.getBufferData(&vertices[oldVertexCount], physx::NxRenderDataFormat::FLOAT3, sizeof(physx::PxVec3), posBufferIndex, 0, vertexBuffer.getVertexCount()))
					{
	...
	for (physx::PxU32 scatterMeshAssetIndex = 0; scatterMeshAssetIndex < scatterMeshAssetCount && instanceInfo.size() < scatterMeshInstancesRequested; ++scatterMeshAssetIndex)
	{
		...
		for (physx::PxU32 count = 0; success && count < counts[scatterMeshAssetIndex] && instanceInfo.size() < scatterMeshInstancesRequested; ++count)
		{
			...
			for (physx::PxU32 trial = 0; !success && trial < 1000; ++trial)
			{
				...
				// pick scale, angle, and position and buld transform
				...
				if (parentIndex >= 0)
				{
					const physx::PxU32 parentPartIndex = *hMesh.partIndex(parentIndex);
					...
					if (parentPartBSP != NULL)
					{
						...
						hullBSP->fromConvexPolyhedron(planes, parentPartBSP->getInternalTransform());
...
)
{
	...
	const physx::PxU32 partIndex = hMesh.mChunks[0]->mPartIndex;
...
void ExplicitHierarchicalMesh::deserialize(physx::PxFileBuf& stream, IEmbedding& embedding)
{
	...
	if (meshStreamVersion >= ExplicitHierarchicalMesh::InstancingData)
	{
		...
		for (physx::PxU32 i = 0; i < partCount; ++i)
		{
			...
			else
			{
				...
				userRnd.m_rnd.setSeed(0);
				mParts[i]->mMeshBSP->fromMesh(mParts[i]->mMesh, bspBuildParameters);
			}
		}
	...
	if (meshStreamVersion >= ExplicitHierarchicalMesh::SerializingMeshBSP && meshStreamVersion < ExplicitHierarchicalMesh::PerPartMeshBSPs)
	{
		...
		else
		{
			ApexCSG::BSPBuildParameters bspBuildParameters = gDefaultBuildParameters;
			bspBuildParameters.internalTransform = physx::PxMat44::createIdentity();
			mParts[0]->mMeshBSP->fromMesh(mParts[0]->mMesh, bspBuildParameters);
		}
	}
...
physx::PxI32 ExplicitHierarchicalMesh::maxDepth() const
{
	...
	while (index >= 0)
	{
		index = mChunks[index]->mParentIndex;
		++max;
	}
	return max;
}
...
physx::PxU32 ExplicitHierarchicalMesh::depth(physx::PxU32 chunkIndex) const
{
	...
	while ((index = mChunks[index]->mParentIndex) >= 0)
	{
		++depth;
	}
...
physx::PxBounds3 ExplicitHierarchicalMesh::chunkBounds(physx::PxU32 chunkIndex) const
{
	...
	{
		bounds = mParts[mChunks[chunkIndex]->mPartIndex]->mBounds;
...
const ExplicitHierarchicalMesh::IConvexHull** ExplicitHierarchicalMesh::convexHulls(physx::PxU32 partIndex) const
{
	if (partIndex < partCount())
	{
		Part* part = mParts[partIndex];
		return part->mCollision.size() > 0 ? (const ExplicitHierarchicalMesh::IConvexHull**)&part->mCollision[0] : NULL;
	}
	return NULL;
}
...
const IDisplacementMapVolume& ExplicitHierarchicalMesh::displacementMapVolume() const
{
...
void ExplicitHierarchicalMesh::sortChunks(physx::Array<physx::PxU32>* indexRemap)
{
	...
	for (physx::PxU32 i = 0; i < newChunkIndices.size(); ++i)
	{
		...
		remap[newChunkIndices[i].index] = i;
	}
	for (physx::PxU32 i = 0; i < mChunks.size(); ++i)
	{
		if (mChunks[i]->mParentIndex >= 0)
		{
			mChunks[i]->mParentIndex = remap[mChunks[i]->mParentIndex];
		}
	}
}
...
void ExplicitHierarchicalMesh::calculatePartBSP(physx::PxU32 partIndex, physx::PxU32 randomSeed, physx::PxU32 microgridSize, IProgressListener* progressListener)
{
...
void ExplicitHierarchicalMesh::calculateMeshBSP(physx::PxU32 randomSeed, IProgressListener* progressListener, const physx::PxU32* microgridSize)
{
	...
	for (physx::PxU32 chunkIndex = 0; chunkIndex < mChunks.size(); ++chunkIndex)
	{
		if (depth(chunkIndex) <= mRootDepth)
		{
			++bspCount;
		}
	}
	...
	for (physx::PxU32 chunkIndex = 0; chunkIndex < mChunks.size(); ++chunkIndex)
	{
		if (depth(chunkIndex) <= mRootDepth)
		{
			physx::PxU32 chunkPartIndex = *partIndex(chunkIndex);
...
void ExplicitHierarchicalMesh::buildCollisionGeometryForPart(physx::PxU32 partIndex, const NxCollisionVolumeDesc& desc)
{
	if (partIndex < partCount())
	{
		...
		physx::Array<physx::PxVec3> vertices(part->mMesh.size() * 3);
		...
		buildCollisionGeometry(part->mCollision, desc, vertices.begin(), vertices.size(), sizeof(physx::PxVec3), indices, vertices.size());
	}
}

void ExplicitHierarchicalMesh::buildCollisionGeometryForRootChunkParts(const NxCollisionDesc& desc)
{
	...
	for (physx::PxU32 chunkIndex = 0; chunkIndex < chunkCount(); ++chunkIndex)
	{
		if (depth(chunkIndex) <= mRootDepth)
		{
			...
			const physx::PxU32 partIndex = mChunks[chunkIndex]->mPartIndex;
			if (partIndex < mParts.size())
			{
	...
	for (physx::PxU32 chunkIndex = 0; chunkIndex < rootChunkStop; ++chunkIndex)
	{
		if (depth(chunkIndex) == mRootDepth)
		{
			...
			{
				NxCollisionVolumeDesc volumeDesc = getVolumeDesc(desc, mRootDepth);
	...
	physx::Array<bool> partHullsCopied;
	partHullsCopied.resize(mParts.size(), false);
	for (physx::PxU32 targetDepth = mRootDepth; targetDepth--;)
	{
		for (physx::PxU32 chunkIndex = 0; chunkIndex < rootChunkStop; ++chunkIndex)
		{
			if (depth(chunkIndex) == targetDepth)
			{
				const physx::PxU32 partIndex = mChunks[chunkIndex]->mPartIndex;
				if (partIndex < mParts.size() && mParts[partIndex]->mCollision.size() == 0)
				{
					for (physx::PxU32 otherChunkIndex = 0; otherChunkIndex < rootChunkStop; ++otherChunkIndex)
					{
						if (mChunks[otherChunkIndex]->mParentIndex == (physx::PxI32)chunkIndex)
						{
							const physx::PxU32 otherPartIndex = mChunks[otherChunkIndex]->mPartIndex;
							if (otherPartIndex < mParts.size() && !partHullsCopied[otherPartIndex])
							{
								const physx::PxU32 oldHullCount = mParts[partIndex]->mCollision.size();
								const physx::PxU32 newHullCount = oldHullCount + mParts[otherChunkIndex]->mCollision.size();
								resizeCollision(mParts[partIndex]->mCollision, newHullCount);
								for (physx::PxU32 newHullN = oldHullCount; newHullN < newHullCount; ++newHullN)
								{
									*mParts[partIndex]->mCollision[newHullN] = *mParts[otherPartIndex]->mCollision[newHullN-oldHullCount];
								}
								partHullsCopied[otherPartIndex] = true;
							}
						}
					}
				}
	...
	const physx::PxF32 maxTrimFraction = 0.2f;	// Should expose

	if (maxTrimFraction > 0.0f)
	{
		// Trim hulls up to root depth
		for (physx::PxU32 processDepth = 1; processDepth < mRootDepth; ++processDepth)
		{
			physx::Array<physx::PxU32> chunkIndexArray;
			for (physx::PxU32 chunkIndex = 0; chunkIndex < rootChunkStop; ++chunkIndex)
			{
				if (depth(chunkIndex) == processDepth)
				{
					chunkIndexArray.pushBack(chunkIndex);
				}
			}
			trimChunkHulls(*this, &chunkIndexArray[0], chunkIndexArray.size(), maxTrimFraction);
		}
	}
...
void ExplicitHierarchicalMesh::initializeDisplacementMapVolume(const physx::FractureSliceDesc& desc)
{
