...
public:
	...
	bool			fromMesh(const physx::Array<physx::NxExplicitRenderTriangle>& mesh, const BSPBuildParameters& params, physx::IProgressListener* progressListener = NULL);
	bool			fromConvexPolyhedron(const physx::Array<physx::PxPlane>& poly, const physx::PxMat44& internalTransform);
	bool			combine(const IApexBSP& bsp);
	bool			op(const IApexBSP& combinedBSP, Operation::Enum operation);
	bool			complement();
	BSPType::Enum	getType() const;
	void			getSurfaceAreaAndVolume(physx::PxF32& area, physx::PxF32& volume, bool inside, Operation::Enum operation = Operation::NOP) const;
	...
	void			copy(const IApexBSP& bsp, const physx::PxMat44& tm = physx::PxMat44::createIdentity());
...
private:
	...
	void		buildTree(Node* root, physx::Array<Surface>& surfaceStack, physx::PxU32 stackReadStart, physx::PxU32 stackReadStop,
	...
	void		addLeafAreasAndVolumes(Real& totalArea, Real& totalVolume, const Node* root, bool inside, const BoolOp& op) const;
