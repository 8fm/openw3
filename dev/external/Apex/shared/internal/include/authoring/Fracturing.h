...
struct DisplacementMapVolume : public IDisplacementMapVolume
{
...
class PartConvexHullProxy : public IExplicitHierarchicalMesh::IConvexHull, public physx::UserAllocated
{
public:
...
class ExplicitHierarchicalMesh : public IExplicitHierarchicalMesh, public physx::UserAllocated
{
public:
	...
	{
		Part() : mMeshBSP(NULL), mSurfaceNormal(0.0f)
		{
			mBounds.setEmpty();
		}
	...
	{
		Chunk() : mParentIndex(-1), mFlags(0), mPartIndex(-1), mInstancedPositionOffset(physx::PxVec3(0.0f)), mInstancedUVOffset(physx::PxVec2(0.0f))
		{
	...
	const IExplicitHierarchicalMesh::IConvexHull** convexHulls(physx::PxU32 partIndex) const;
	physx::PxVec3* surfaceNormal(physx::PxU32 partIndex);
	const IDisplacementMapVolume& displacementMapVolume() const;
	...
	void calculatePartBSP(physx::PxU32 partIndex, physx::PxU32 randomSeed, physx::PxU32 microgridSize, IProgressListener* progressListener = NULL);
	...
	void buildCollisionGeometryForRootChunkParts(const physx::NxCollisionDesc& desc);
	void initializeDisplacementMapVolume(const physx::FractureSliceDesc& desc);
