...
public:
		...
		for (PxU32 i=0; i<pcount; i++)
		{
			...
			switch ( p.mCommand )
			{
				...
				case DebugCommand::SET_CURRENT_USER_ID:
					{
						const DebugPrimitiveU32 &dg = static_cast< const DebugPrimitiveU32 &>(p);
						mCurrentState.mUserId = dg.mValue;
					}
					break;
	...
	{
		const PxU32 numOfSegments = 4 << PxMin(d.mSubdivision, 6u); // this maxes out at 4^6 = 256 segments
		...
		else
		{
			for( PxU32 i=0; i<numOfSegments; ++i )
			{
				...
				const PxVec3 nC = (-currentVec).getNormalized();
	...
	PX_INLINE void debugSphere(const DebugSquashedSphere &d)
	{
		for (physx::PxU32 i = 0; i < 8; i++)
		{
			...
			subdivideOnSphere(d.mSubdivision, p0, p1, p2, d.mRadius, 0.0f, 0.0f);
		}
	}
	...
	{
		physx::PxMat44 transform = physx::PxMat44::createIdentity();
		if (subdivision <= 0)
		{
			PxVec3 t[3];
			PxVec3 n[3] = { p0, p1, p2 };
	...
	PX_INLINE void debugCapsule(const DebugCapsule &d)
	{
		...
		for (physx::PxU32 i = 0; i < 8; i++)
		{
			...
			const PxVec3 p1 = simpleSpherePosition[simpleSphereIndices[i*3+1]];
			const PxVec3 p2 = simpleSpherePosition[simpleSphereIndices[i*3+2]];
			subdivideOnSphere(d.mSubdivision, p0, p1, p2, d.mRadius, height, 0.0f);
		}
	}
	...
	void debugTaperedCapsule(const DebugTaperedCapsule& d)
	{
		...
		for (PxU32 i = 0; i < 8; i++)
		{
			...
			subdivideOnSphere(subdivision, p[0], p[1], p[2], radius[i >> 2], sphereSeparation, skew);
		}
	...
	PX_INLINE void debugDetailedSphere(const DebugDetailedSphere &d)
	{
		...
		PxVec3 origTranslation = save != NULL ? save->getPosition() : PxVec3(0.0f);
