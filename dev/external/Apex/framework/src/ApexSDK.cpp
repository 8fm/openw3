...
// We require at least Visual Studio 2008 or 2010 w/ SP1 to compile
#if defined(_MSC_VER)
#	if _MSC_VER >= 1600
	PX_COMPILE_TIME_ASSERT(_MSC_FULL_VER >= 160040219);
#	elif _MSC_VER >= 1500
	PX_COMPILE_TIME_ASSERT(_MSC_FULL_VER >= 150030729);
#	endif

#	if _MSC_VER > 1600
	#pragma message("Detected compiler newer than Visual Studio 2010, please update min version checking in ApexSDK.cpp")
#	endif
#endif
...
ApexSDK::ApexSDK(NxApexCreateError* errorCode, physx::PxU32 inAPEXsdkVersion)
	...
	, cookingVersion(0)
	, APEXsdkVersion(inAPEXsdkVersion)
{
	if (errorCode)
	{
...
NiApexPhysXObjectDesc* ApexSDK::getGenericPhysXObjectInfo(const void* obj) const
{
	...
	physx::PxU16 h = ApexPhysXObjectDesc::makeHash(reinterpret_cast<size_t>(obj)) & (DescHashSize - 1);
...
NiApexPhysXObjectDesc* 	ApexSDK::createObjectDesc(const NxApexActor* apexActor, const void* nxPtr)
{
	...
	physx::PxU16 h = ApexPhysXObjectDesc::makeHash(reinterpret_cast<size_t>(nxPtr)) & (DescHashSize - 1);
	physx::PxU32 index = mPhysXObjDescHash[ h ];
...
void ApexSDK::releaseObjectDesc(void* physXObject)
{
	...
	physx::PxU16 h = ApexPhysXObjectDesc::makeHash(reinterpret_cast<size_t>(physXObject)) & (DescHashSize - 1);
	physx::PxU32 index = mPhysXObjDescHash[ h ];
...
void* ApexSDK::getTempMemory(PxU32 size)
{
	...
	if (found != -1)
	{
		...
		if ((physx::PxU32)found > mNumTempMemoriesActive)
		{
			...
			mTempMemories[mNumTempMemoriesActive] = mTempMemories[found];
			mTempMemories[found] = temp;
		}
