...
#if APEX_USE_GRB
	, mEnableGrbPhysics(sceneDesc.enableGrbPhysics)
#endif
	, mGravity(0)
{
...
static ApexStatsInfo ApexStatsData[] =
{
	...
	{"GpuParticleTime",			ApexStatDataType::FLOAT, {{0}} },
	{"GpuTurbulenceTime",			ApexStatDataType::FLOAT, {{0}} },
...
NxParameterized::Interface* ApexScene::getDebugRenderParams()
{
	return mDebugRenderParams;
}

//Module names are case sensitive:
//BasicIos, Clothing, Destructible, Emitter, Example, Explosion
//FieldBoundary, Grb, Iofx, FluidIos, Vegetation, Water, Wind
NxParameterized::Interface* ApexScene::getModuleDebugRenderParams(const char* name)
{
...
physx::PxU32 ApexScene::allocViewMatrix(ViewMatrixType::Enum viewType)
{
	...
	else
	{
		...
		{
			v = PX_NEW(ViewMatrixUserCustomized)(physx::PxMat44().createIdentity(), true, false, physx::PxVec3(0, 0, 0), physx::PxVec3(0, 0, 1), physx::PxVec3(0, 1, 0));
		}
		break;
		case ViewMatrixType::LOOK_AT_RH:
		{
			v = PX_NEW(ViewMatrixLookAt)(physx::PxMat44().createIdentity(), false, true, true);
		}
		break;
		case ViewMatrixType::LOOK_AT_LH:
		{
			v = PX_NEW(ViewMatrixLookAt)(physx::PxMat44().createIdentity(), false, true, false);
		}
		break;
		default:
			if (!mTotalElapsedMS)
...
void ApexScene::setViewParams(const physx::PxVec3& eyePosition, const physx::PxVec3& eyeDirection, const physx::PxVec3& worldUpDirection, const physx::PxU32 viewID)
{
	...
	{
		if (!mViewMatrices[viewID]->isUserCustomized)
		{
			if (!mTotalElapsedMS)
			{
				APEX_INVALID_PARAMETER("view matrix for viewID %d is not a user-customized type! see allocViewMatrix()", viewID);
			}
		}
		else
		{
			ViewMatrixUserCustomized* v = static_cast<ViewMatrixUserCustomized*>(mViewMatrices[viewID]);
			v->eyePosition = eyePosition;
			v->eyeDirection = eyeDirection;
			v->worldUpDirection = worldUpDirection;
		}
	}
}
...
#if NX_SDK_VERSION_MAJOR == 2
const NxDebugRenderable* ApexScene::getDebugRenderable()
{
	if (mSimulating)
	{
...
const PxRenderBuffer* ApexScene::getRenderBuffer()
{
	if (mSimulating)
	{
		APEX_INTERNAL_ERROR("simulation is still running");
...
void ApexScene::renderLockAllActors()
{
	...
	ApexContext::mActorListLock.unlockReader();
}

void ApexScene::renderUnLockAllActors()
{
...
void ApexScene::prepareRenderResourceContexts()
{
...
bool ApexScene::fetchResults(bool block, physx::PxU32* errorState)
{
	...
	for (PxU32 i = 0 ; i < mModuleScenes.size() ; i++)
	{
		PX_PROFILER_PERF_SCOPE("ClothingScene::fetchResultsPostRenderUnlock");

		mModuleScenes[i]->fetchResultsPostRenderUnlock();
	}

	{
		PX_PROFILER_PERF_SCOPE("stopSimulation");
		mTaskManager->stopSimulation();
	}

	{
//		PX_PROFILER_PERF_SCOPE("fetchApexStats");
//		fetchApexStats();
	}
...
# endif
	...
	dataVal.Int = nbShapes;
	setApexStatValue(NumberOfShapes, dataVal);

	dataVal.Int = nbAwakeShapes;
	setApexStatValue(NumberOfAwakeShapes, dataVal);

	dataVal.Int = nbPairs;
	setApexStatValue(NumberOfCpuShapePairs, dataVal);

	//If there are any Gpu simulated shapes, they will be added later
	dataVal.Int = 0;
	//setApexStatValue(NumberOfGpuSimulatedShapes, dataVal);
	setApexStatValue(NumberOfGpuShapePairs, dataVal);
	setApexStatValue(GpuRbSimulationTime, dataVal);
	setApexStatValue(RbThroughput, dataVal);
	setApexStatValue(GpuSceneMemoryUsed, dataVal);
	setApexStatValue(GpuTempMemoryUsed, dataVal);
	setApexStatValue(GpuRbStaticConvexMaxEdgeCount, dataVal);
	setApexStatValue(GpuRbDynamicConvexMaxEdgeCount, dataVal);
	setApexStatValue(GpuRbKinematicConvexMaxEdgeCount, dataVal);
	setApexStatValue(NumberOfGpuDynamicActorsAwake, dataVal);
	setApexStatValue(NumberOfGpuDynamicActorsSleeping, dataVal);
...
void ApexScene::fetchApexStats()
{
	...
	//ModuleScenes can also generate stats, for example a DestructibleScene may
	//contain a GRB scene. So let's collect and add those stats here.
...
bool ApexScene::checkResults(bool block)
{
...
{
	PxU32 i1 = mAddressToIndex.getIndex(actor0);
	PxU32 i2 = mAddressToIndex.getIndex(actor1);
	mPairFilter.addPair(i1,i2);
}
...
void	ApexScene::removeActorPair(PxActor *actor0,PxActor *actor1)
{
	PxU32 i1 = mAddressToIndex.getIndex(actor0);
	PxU32 i2 = mAddressToIndex.getIndex(actor1);
	mPairFilter.removePair(i1,i2);
}

bool	ApexScene::releaseActorPairIndex(PxActor *actor)
{
	return mAddressToIndex.freeIndex(actor);
}

bool	ApexScene::findActorPair(PxActor *actor0,PxActor *actor1)
{
	bool ret = false;
	PxU32 i1,i2;
	bool ok1 = mAddressToIndex.findIndex(actor0,i1);
	bool ok2 = mAddressToIndex.findIndex(actor1,i2);
	if ( ok1 && ok2 )
	{
		ret = mPairFilter.findPair(i1,i2);
	}
	return ret;
}

PxU32	ApexScene::getActorPairIndex(PxActor *actor)
{
	return mAddressToIndex.getIndex(actor);
}

bool	ApexScene::findActorPair(PxU32 id0,PxU32 id1)
{
	return mPairFilter.findPair(id0,id1);
}
#endif
