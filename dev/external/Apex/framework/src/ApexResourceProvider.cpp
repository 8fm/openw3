...
void ApexResourceProvider::setResource(const char* nameSpace, const char* name, void* resource, bool valueIsSet, bool incRefCount)
{
	PX_ASSERT(nameSpace);
	PX_ASSERT(name);
	NxResID nsID = createNameSpace(nameSpace, true);
...
NxResID ApexResourceProvider::createNameSpace(const char* nameSpace, bool releaseAtExit)
{
...
{
	setResource(nameSpace, name, (void*)id, true, incRefCount);
}
