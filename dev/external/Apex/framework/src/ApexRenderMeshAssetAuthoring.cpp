...
void ApexRenderMeshAssetAuthoring::createRenderMesh(const MeshDesc& meshDesc, bool createMappingInformation)
{
	...
	rootHandle.resizeArray(meshDesc.m_numSubmeshes);
	...
	for (PxU32 submeshNum = 0; submeshNum < meshDesc.m_numSubmeshes; ++submeshNum)
	{
		...
		handle.getChildHandle(submeshNum, elementHandle);
		...
		ibHandle.resizeArray(submeshDesc.m_numIndices);
		switch (submeshDesc.m_indexType)
		{
		case IndexType::UINT:
			for (PxU32 i = 0; i < submeshDesc.m_numIndices; ++i)
			{
				...
				submeshParams->indexBuffer.buf[i] = invMap[index];
		...
		ipHandle.resizeArray(PxMax(submeshDesc.m_numParts + 1, 2u));
		...
		vpHandle.resizeArray(lookup.size());
		vpHandle.setParamU32Array(lookup.begin(), lookup.size());
		...
		for (PxU32 i = 0; i < submeshDesc.m_numVertexBuffers; ++i)
		{
			...
			for (PxU32 semantic = 0; semantic < NxRenderVertexSemantic::NUM_SEMANTICS; ++semantic)
			{
				...
				if (vertexSemanticFormatValid(vertexSemantic, vertexFormat))
				{
					...
					format.setBufferFormat(bufferIndex, vb.getSemanticData(vertexSemantic).format);
				}
		...
		for (PxU32 i = 0; i < submeshDesc.m_numVertexBuffers; ++i)
		{
			...
			for (PxU32 index = 0; index < vb.getNumCustomSemantics(); ++index)
			{
				...
				format.setBufferFormat(bufferIndex, data.format);

				// PH: custom buffers are never serialized this way, we might need to change this!
				format.setBufferSerialize(bufferIndex, data.serialize);
			}
		}
		...
		if (createMappingInformation)
		{
			...
			format.setBufferFormat(bufferIndex, NxRenderDataFormat::UINT1);
		}
		...
		for (PxU32 i = 0; i < submeshDesc.m_numVertexBuffers; ++i)
		{
			...
			for (PxU32 semantic = 0; semantic < NxRenderVertexSemantic::NUM_SEMANTICS; ++semantic)
			{
				if (vertexSemanticFormatValid((NxRenderVertexSemantic::Enum)semantic, vb.getSemanticData((NxRenderVertexSemantic::Enum)semantic).format))
				{
					...
					void* dst = submesh.getVertexBufferWritable().getBufferAndFormatWritable(dstFormat, vf.getBufferIndexFromID(vf.getSemanticID((NxRenderVertexSemantic::Enum)semantic)));
			...
			if (numBoneIndices > 1 && numBoneWeights == numBoneIndices)
			{
				...
				for (PxU32 vi = 0; vi < submeshDesc.m_numVertices; vi++)
				{
					...
					if (dest >= 0)
					{
						...
						copyRenderVertexBuffer(dstDataWeight, dstFormatBoneWeight, 0, dest, verifyWeights, checkFormatBoneWeight, 0, 0, 1);
						copyRenderVertexBuffer(dstDataIndex, dstFormatBoneIndex, 0, dest, verifyIndices, checkFormatBoneIndex, 0, 0, 1);
					}
				}
			}
			...
			for (PxU32 index = 0; index < vb.getNumCustomSemantics(); ++index)
			{
				...
				void* dst = const_cast<void*>(submesh.getVertexBuffer().getBuffer(bufferIndex));
		...
		if (createMappingInformation)
		{
			...
			PX_ASSERT(bufferIndex >= 0);
			const void* dst = submesh.getVertexBuffer().getBuffer(bufferIndex);
	...
	mParams->resizeArray(rootHandle, partCount);
	for (PxU32 partNum = 0; partNum < partCount; ++partNum)
	{
		...
		for (PxU32 submeshNum = 0; submeshNum < meshDesc.m_numSubmeshes; ++submeshNum)
		{
			...
			const NxVertexFormat& vf = submesh.getVertexBuffer().getFormat();
			const PxVec3* positions = (const PxVec3*)submesh.getVertexBuffer().getBufferAndFormat(positionFormat, vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::POSITION)));
...
{
	size_t maxMaterials = mParams->materialNames.arraySizes[0];
	...
	if (submeshIndex < maxMaterials)
	{
		...
		mParams->getParameterHandle("materialNames", handle);
		NxParameterized::Handle elementHandle(*mParams);
		handle.getChildHandle(submeshIndex, elementHandle);
