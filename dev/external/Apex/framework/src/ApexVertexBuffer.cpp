...
void ApexVertexBuffer::applyTransformation(const physx::PxMat34Legacy& transformation)
{
	...
	// Positions
	index = getFormat().getBufferIndexFromID(getFormat().getSemanticID(NxRenderVertexSemantic::POSITION));
	buf = getBuffer(index);
	if (buf)
	{
	...
	// Normals
	index = getFormat().getBufferIndexFromID(getFormat().getSemanticID(NxRenderVertexSemantic::NORMAL));
	buf = getBuffer(index);
	if (buf)
	{
	...
	// Tangents
	index = getFormat().getBufferIndexFromID(getFormat().getSemanticID(NxRenderVertexSemantic::TANGENT));
	buf = getBuffer(index);
	if (buf)
	{
		format = getFormat().getBufferFormat(index);
	...
	// Binormals
	index = getFormat().getBufferIndexFromID(getFormat().getSemanticID(NxRenderVertexSemantic::BINORMAL));
	buf = getBuffer(index);
	if (buf)
	{
		format = getFormat().getBufferFormat(index);
...
{
	PxU32 index = getFormat().getBufferIndexFromID(getFormat().getSemanticID(NxRenderVertexSemantic::POSITION));
...
bool ApexVertexBuffer::mergeBinormalsIntoTangents()
{
	...
	if (normalBufferIndex != -1 && tangentBufferIndex != -1 && binormalBufferIndex != -1)
	{
		...
		{
			const PxU32 numElements = oldTangentsBuffer->data.arraySizes[0];
			...
			PX_ASSERT(oldTangentsBuffer->data.arraySizes[0] == oldBinormalsBuffer->data.arraySizes[0]);
			{
				...
				handle.resizeArray(numElements);
			}
			...
			// Ok, real dirty now
			mParams->buffers.buf[tangentBufferIndex] = newTangentsBuffer;
			for (PxU32 i = binormalBufferIndex + 1; i < numBuffers; i++)
			{
			...
			mParams->buffers.buf[numBuffers - 1] = NULL;
			{
				...
				PX_ASSERT(handle.isValid());
				handle.resizeArray(numBuffers - 1);
			}
...
void ApexVertexBuffer::copy(physx::PxU32 dstIndex, physx::PxU32 srcIndex, ApexVertexBuffer* srcBufferPtr)
{
	...
	for (physx::PxI32 i = 0; i < mParams->buffers.arraySizes[0]; i++)
	{
		...
		if (srcBufferIndex >= 0)
		{
			NxRenderDataFormat::Enum srcFormat = srcVF.getBufferFormat(srcBufferIndex);
			NxParameterized::Interface* dstInterface = mParams->buffers.buf[i];
			NxParameterized::Interface* srcInterface = srcVB.mParams->buffers.buf[srcBufferIndex];
...
void ApexVertexBuffer::resize(physx::PxU32 vertexCount)
{
	...
	{
		NxRenderDataFormat::Enum outFormat = mFormat.getBufferFormat(i);
		...
		if (currentReference == NULL && vertexCount > 0)
		{
			...
			if (currentReference != NULL)
			{
				...
				VERIFY_PARAM(arrayHandle.resizeArray(vertexCount));
...
{
	const PxI32 numVertices = mParams->vertexCount;
	PX_ASSERT((PxU32)numVertices == permutation.size());
	for (PxI32 i = 0; i < mParams->buffers.arraySizes[0]; i++)
	{
		...
		case NxRenderDataFormat::BYTE_SNORM1:
			{
				...
				PX_ASSERT(numVertices == byte1->data.arraySizes[0]);
		...
		case NxRenderDataFormat::HALF1:
			{
				...
				PX_ASSERT(numVertices == short1->data.arraySizes[0]);
		...
		case NxRenderDataFormat::BYTE_SNORM3:
			{
				...
				PX_ASSERT(numVertices == byte3->data.arraySizes[0]);
		...
		case NxRenderDataFormat::SHORT_SNORM4_QUATXYZW:
			{
				...
				PX_ASSERT(numVertices == int1->data.arraySizes[0]);
		...
		case NxRenderDataFormat::HALF3:
			{
				...
				PX_ASSERT(numVertices == short3->data.arraySizes[0]);
		...
		case NxRenderDataFormat::FLOAT2:
			{
				...
				PX_ASSERT(numVertices == int2->data.arraySizes[0]);
		...
		case NxRenderDataFormat::FLOAT3:
			{
				...
				PX_ASSERT(numVertices == int3->data.arraySizes[0]);
		...
		case NxRenderDataFormat::FLOAT4_QUAT:
			{
				...
				PX_ASSERT(numVertices == int4->data.arraySizes[0]);
		...
		case NxRenderDataFormat::FLOAT3x3:
			{
				...
				PX_ASSERT(numVertices == float1->data.arraySizes[0]);
