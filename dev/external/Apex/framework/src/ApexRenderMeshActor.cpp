...
void ApexRenderMeshActor::init(const NxRenderMeshActorDesc& desc, PxU16 partCount, PxU16 boneCount)
{
	...
	for (PxU32 submeshIndex = 0; submeshIndex < mRenderMeshAsset->getSubmeshCount(); submeshIndex++)
	{
		...
		{
			void* material = resourceProvider->getResource(submeshData.materialID);

			if (material != NULL)
			{
				NxUserRenderResourceManager* rrm = NiGetApexSDK()->getUserRenderResourceManager();
				submeshData.maxBonesPerMaterial = rrm->getMaxBonesForMaterial(material);

				if (submeshData.maxBonesPerMaterial == 0)
				{
					submeshData.maxBonesPerMaterial = mTransforms.size();
				}
			}
		}
...
void ApexRenderMeshActor::updateRenderResources(bool useBones, bool rewriteBuffers, void* userRenderData)
{
	...
	for (PxU32 submeshIndex = 0; submeshIndex < mSubmeshData.size(); submeshIndex++)
	{
		...
		{
			void* material = nrp->getResource(submeshData.materialID);
			submeshData.maxBonesPerMaterial = rrm->getMaxBonesForMaterial(material);
	...
	for (PxU32 submeshIndex = 0; submeshIndex < mRenderMeshAsset->getSubmeshCount(); ++submeshIndex)
	{
		...
		if (nrp != NULL)
		{
			material = nrp->getResource(submeshData.materialID);
		}
...
void ApexRenderMeshActor::getPhysicalLodRange(physx::PxF32& min, physx::PxF32& max, bool& intOnly)
{
	PX_UNUSED(min);
	PX_UNUSED(max);
	PX_UNUSED(intOnly);
...
physx::PxF32 ApexRenderMeshActor::getActivePhysicalLod()
{
	APEX_INVALID_OPERATION("NxApexRenderMeshActor does not support this operation");
	return -1.0f;
}
	...
	for (PxU32 submeshIndex = 0; submeshIndex < mRenderMeshAsset->getSubmeshCount(); ++submeshIndex)
	{
		...
		if (createAndFillSharedVertexBuffers)
		{
			...
			for (PxU32 i = 0; i < vf.getBufferCount(); ++i)
			{
				...
				{
					NxResID id = mRenderMeshAsset->mRuntimeCustomSubmeshData[submeshIndex].customBufferResIDs[staticBufDesc.numCustomBuffers];
					void* ptr = NiGetApexSDK()->getInternalResourceProvider()->getResource(id);
					mRenderMeshAsset->mRuntimeCustomSubmeshData[submeshIndex].customBufferVoidPtrs[staticBufDesc.numCustomBuffers] = ptr;
			...
			if (mTransforms.size() > 1)
			{
				...
				{
					PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::BONE_INDEX));
					boneDesc->buffersRequest[NxRenderVertexSemantic::BONE_INDEX] = vf.getBufferFormat(bufferIndex);
					bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::BONE_WEIGHT));
			...
			{
				PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID((NxRenderVertexSemantic::Enum)semantic));
				staticBufDesc.buffersRequest[ semantic ]	= vf.getBufferFormat(bufferIndex);
			}

			{
				PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::DISPLACEMENT_TEXCOORD));
				staticBufDesc.buffersRequest[ NxRenderVertexSemantic::DISPLACEMENT_TEXCOORD ] = vf.getBufferFormat(bufferIndex);
				bufferIndex       = vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::DISPLACEMENT_FLAGS));
		...
		if (fillStaticSharedVertexBuffer || fillDynamicSharedVertexBuffer || fillSkinningSharedVertexBuffer)
		{
			...
			for (PxU32 semantic = NxRenderVertexSemantic::POSITION; semantic <= NxRenderVertexSemantic::COLOR; ++semantic)
			{
				...
				{
					if (srcVB.getFormat().getBufferAccess(bufferIndex) == NxRenderDataAccess::STATIC)
					{
			...
			if (numCustom)
			{
				...
				for (physx::PxU32 i = 0; i < vf.getBufferCount(); i++)
				{
					...
					NxResID id = mRenderMeshAsset->mRuntimeCustomSubmeshData[submeshIndex].customBufferResIDs[writeIndex];
					void* ptr = NiGetApexSDK()->getInternalResourceProvider()->getResource(id);
					semanticData[writeIndex].ident = ptr;
			...
			if (mTransforms.size() > 1)
			{
				...
				if (numBonesPerVertex == 1)
				{
					...
					if (!mKeepVisibleBonesPacked)
					{
						NxRenderDataFormat::Enum format;
						const NxVertexFormat& vf = srcVB.getFormat();
						physx::PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(NxRenderVertexSemantic::BONE_INDEX));
						const void* src = srcVB.getBufferAndFormat(format, bufferIndex);
						if (format != NxRenderDataFormat::UNSPECIFIED)
						{
...
void ApexRenderMeshActor::updatePartVisibility(PxU32 submeshIndex, bool useBones, void* userRenderData)
{
	...
	{
		const physx::PxU32 partIndex = visiblePartIndexPtr[partNum++];
		...
		if (mKeepVisibleBonesPacked)
		{
			if (resourceBoneCount == submeshData.maxBonesPerMaterial || partNum == partCount)
			{
		...
		{
			submeshData.renderResources[resourceNum].boneCount = mKeepVisibleBonesPacked ? resourceBoneCount : (PxU16)mRenderMeshAsset->getBoneCount();
			...
			if (renderResource == NULL)	// Create if needed
			{
				...
				if (submeshMaterial == NULL && nrp != NULL)
				{
					submeshMaterial = nrp->getResource(submeshData.materialID);
				}

				resourceDesc.material = submeshMaterial;
			...
			if (renderResource != NULL)
			{
				...
				if (renderResource->getBoneBuffer() != NULL)
				{
					renderResource->setBoneBufferRange(0, mKeepVisibleBonesPacked ? resourceBoneCount : (PxU16)mRenderMeshAsset->getBoneCount());
...
//	if (mRenderMeshAsset->getPartCount() > 1)
	if (mRenderMeshAsset->getBoneCount() > 1)
	{
		...
		//++LAVA
		PxF32* alphas = mAlphas.begin();
		bool includeAlphas = mAlphas.size() == mTransforms.size();
		//--LAVA
		...
		for (physx::PxU32 i = 0; i < submeshData.renderResources.size(); ++i)
		{
			...
			if (renderResource && renderResource->getBoneBuffer() != NULL)
			{
				...
				if (includeAlphas)
				{
					boneWriteData.setSemanticData(NxRenderBoneSemantic::ALPHA, alphas, sizeof( physx::PxF32 ), NxRenderDataFormat::FLOAT1);
				}
				//--LAVA
...
void ApexRenderMeshActor::updateFallbackSkinning(PxU32 submeshIndex)
{
	...
	const PxU32 normalIndex = format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::NORMAL));
	...
	const PxU32 tangentIndex = format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::TANGENT));
	...
	const PxU32 boneIndexIndex = format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::BONE_INDEX));
	...
	const PxU32 boneWeightIndex = format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::BONE_WEIGHT));
...
//++LAVA
void ApexRenderMeshActor::setChunkAlpha( physx::PxF32 alpha, physx::PxU32 boneIndex /* = 0 */ )
{
	if ( mAlphas.size() != mTransforms.size() )
	{
		const PxU32 oldBoneCount = mAlphas.size();
		PxU32 boneCount =  mTransforms.size();
		mAlphas.resize( boneCount );
		for (PxU32 i = oldBoneCount; i < boneCount; ++i)
		{
			mAlphas[i] = 1.0f;
		}
	...
	PxU32 actualIndex = mKeepVisibleBonesPacked ? mVisiblePartsForAPI.getRank(boneIndex) : boneIndex;
	mAlphas[actualIndex] = alpha;
	mBonePosesDirty = true;
}
//--LAVA
...
//	if (single-weighted vertices)
	{
		...
		for (PxU32 submeshIndex = 0; submeshIndex < submeshCount; ++submeshIndex)
		{
			...
			const PxVec3* vertexPositions = (const physx::PxVec3*)vertexBuffer.getBufferAndFormat(positionFormat, vertexFormat.getBufferIndexFromID(vertexFormat.getSemanticID(physx::NxRenderVertexSemantic::POSITION)));
		...
		const bool haveNormal = vertexBuffer.getBufferData(&triangle.vertices[0].normal, physx::NxRenderDataFormat::FLOAT3, 0, normalBufferIndex, hitData.vertexIndices[0], 1);
		const bool haveTangent = vertexBuffer.getBufferData(&triangle.vertices[0].tangent, physx::NxRenderDataFormat::FLOAT3, 0, tangentBufferIndex, hitData.vertexIndices[0], 1);
		const bool haveBinormal = vertexBuffer.getBufferData(&triangle.vertices[0].binormal, physx::NxRenderDataFormat::FLOAT3, 0, binormalBufferIndex, hitData.vertexIndices[0], 1);

		physx::PxU32 fieldMask = 0;

		if (haveNormal)
		{
			vertexBuffer.getBufferData(&triangle.vertices[1].normal, physx::NxRenderDataFormat::FLOAT3, 0, normalBufferIndex, hitData.vertexIndices[1], 1);
			vertexBuffer.getBufferData(&triangle.vertices[2].normal, physx::NxRenderDataFormat::FLOAT3, 0, normalBufferIndex, hitData.vertexIndices[2], 1);
		...
		if (haveTangent)
		{
			vertexBuffer.getBufferData(&triangle.vertices[1].tangent, physx::NxRenderDataFormat::FLOAT3, 0, tangentBufferIndex, hitData.vertexIndices[1], 1);
			vertexBuffer.getBufferData(&triangle.vertices[2].tangent, physx::NxRenderDataFormat::FLOAT3, 0, tangentBufferIndex, hitData.vertexIndices[2], 1);
		...
		if (haveBinormal)
		{
			vertexBuffer.getBufferData(&triangle.vertices[1].binormal, physx::NxRenderDataFormat::FLOAT3, 0, binormalBufferIndex, hitData.vertexIndices[1], 1);
			vertexBuffer.getBufferData(&triangle.vertices[2].binormal, physx::NxRenderDataFormat::FLOAT3, 0, binormalBufferIndex, hitData.vertexIndices[2], 1);
		...
		if (fieldMask != 0)
		{
			...
			const physx::PxVec3* vertexPositions = (const physx::PxVec3*)vertexBuffer.getBuffer(vertexFormat.getBufferIndexFromID(vertexFormat.getSemanticID(physx::NxRenderVertexSemantic::POSITION)));
...
namespace
{
// These are defined for the debug visualization
typedef const physx::PxVec3* Vec3Ptr;
typedef const physx::PxU32* UIntPtr;

class CandidateProperty
{
public:
	CandidateProperty(physx::NxRenderVertexSemantic::Enum s, physx::PxI32 i, physx::NxRenderDataFormat::Enum f, const void* b)
		: vec3DataPtr(NULL)
		, semantic(s)
		, bufferIndex(i)
		, format(f)
	...
	{
		PX_ASSERT(i != -1);
		PX_ASSERT(f != physx::NxRenderDataFormat::UNSPECIFIED);
		PX_ASSERT(b != NULL);
	}

	~CandidateProperty() {};

	/*physx::NxRenderVertexSemantic::Enum getVertexSemantic() const
		{
			return semantic;
		}

		physx::PxI32 getBufferIndex() const
		{
			return bufferIndex;
		}

		physx::NxRenderDataFormat::Enum getDataFormat() const
		{
			return format;
		}

		Vec3Ptr getBufferVec3() const
		{
			PX_ASSERT(physx::NxRenderDataFormat::FLOAT3 == format);
			return static_cast<Vec3Ptr>(bufferDataPtr);
		}*/
