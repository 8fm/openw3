...
void NxParameters::initRandom(NxParameterized::Handle& handle)
{
	...
	case TYPE_REF:
		{
			...
			physx::PxU32 refIdx = RandomIdx(0, pd->numRefVariants() - 1);
	...
	case TYPE_ENUM:
		{
			physx::PxU32 enumIdx = RandomIdx(0, pd->numEnumVals()-1);
...
ErrorType NxParameters::rawSwapArrayElements(const Handle &array_handle, unsigned int firstElement, unsigned int secondElement)
{
	...
	if (elementSize > 64)
	{
		tempDataHeap = mParameterizedTraits->alloc(elementSize);
	...
	memcpy(tempData, firstPtr, elementSize);
	memcpy(firstPtr, secondPtr, elementSize);
	memcpy(secondPtr, tempData, elementSize);
...
#include "NxParameterized_types.h"
		...
		case TYPE_ARRAY:
	    {
		    physx::PxI32 arraySize1, arraySize2;
...
#define NX_PARAMETERIZED_TYPE(type_name, enum_name, c_type) \
	case TYPE_ ## enum_name: { \
		physx::PxI32 byteSize = sizeof(c_type) * arraySize1; \
		c_type *data1 = (c_type *)mParameterizedTraits->alloc(byteSize), \
			*data2 = (c_type *)mParameterizedTraits->alloc(byteSize); \
		\
		param_handle.setInterface(this); \
		NX_BOOL_RETURN(param_handle.getParam ## type_name ## Array(data1, arraySize1)); \
		\
		param_handle.setInterface(&obj); \
		NX_BOOL_RETURN(param_handle.getParam ## type_name ## Array(data2, arraySize2)); \
		\
...
#include "NxParameterized_types.h"
			...
			for (physx::PxI32 i = 0; i < arraySize1; ++i)
		    {
			    param_handle.set(i);
				if (!equals(obj, param_handle, handlesOfInequality, numHandlesOfInequality, doCompareNotSerialized))
					return(false);
													 ...
													 bool &isMemAllocated)
{
    ...
    if(buf == NULL) 
    {
        if((buf = allocAligned(parameterizedTraits, element_size * newNumElems, element_align)) == NULL) 
            return(ERROR_MEMORY_ALLOCATION_FAILURE);

		isMemAllocated = true;
        
        //initialize the array to 0's (for strings)
        memset(buf, 0, element_size * newNumElems);
    ...
    if(resize_dim == 0)
    {
        ...
        // alloc new buffer
        if((newBuf = allocAligned(parameterizedTraits, element_size * newNumElems, element_align)) == NULL) 
            return(ERROR_MEMORY_ALLOCATION_FAILURE);
		...
		if(newNumElems < currentNumElems)
			memcpy(newBuf, buf, element_size * newNumElems);
		else
			memcpy(newBuf, buf, element_size * currentNumElems);
    ...
    {
        void *newBuf = allocAligned(parameterizedTraits, element_size * newNumElems, element_align);
        if(newBuf == NULL)
            return(ERROR_MEMORY_ALLOCATION_FAILURE);
        ...
        //initialize the array to 0's (for strings)
        memset(newBuf, 0, element_size * newNumElems);
								 ...
								 physx::PxI32 level)
{
    ...
    if(level == dimension - 1)
    {
        ...
        memcpy(copy_dst, copy_src, element_size * size);
        return;
    }
