...
struct traversalState
{
	...
	void incLevel()
	{
		physx::string::strncat_s(indent, (physx::PxU32)strlen(indent) + (physx::PxU32)strlen(indentStr) + 1, indentStr, (physx::PxU32)strlen(indentStr));
		level++;
	}
...
{
	class ClassNameReader: public FAST_XML::FastXml::Callback
	{
		physx::PxU32 mDepth;
	...
	public:
			...
			physx::PxI32 /*lineno*/)
		{
			...
			mClassNames[mNumObjs] = mTraits->strdup( getAttribute("className", argc, argv) );
			++mNumObjs;

			return mNumObjs < mNumClassNames;
		}

		void *fastxml_malloc(physx::PxU32 size) { return ::malloc(size); }
		void fastxml_free(void *mem) { ::free(mem); };
	};

	ClassNameReader myReader(classNames, numClassNames, mTraits);
	FAST_XML::FastXml *xmlParser = FAST_XML::createFastXml(&myReader);

	xmlParser->processXml(stream);
...
static bool IsSimpleStruct(const Definition *pd)
{
	...
	physx::PxU32 count = pd->numChildren();
	...
	bool & /*doesNeedUpdate*/)
{
	...
	FAST_XML::FastXml *xmlParser = FAST_XML::createFastXml(d);
	if( xmlParser && !xmlParser->processXml(stream) )
	{
