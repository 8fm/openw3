...
Serializer::ErrorType BinSerializer::peekNumObjectsInplace(const void *data, physx::PxU32 dataLen, physx::PxU32 &numObjects)
{
	...
	numObjects = Canonize( ((const BinaryHeader *)data)->numObjects );
	return Serializer::ERROR_NONE;
}
...
Serializer::ErrorType BinSerializer::internalDeserialize(physx::PxFileBuf &stream, Serializer::DeserializedData &res, bool & /*doesNeedUpdate*/)
{
	...
	res.init(mTraits, hdr.numObjects);
	for(physx::PxI32 i = 0; i < hdr.numObjects; ++i)
	{
		physx::PxU32 objOff;
		...
		else
		{
			...
			{
				for(physx::PxI32 j = 0; j < i; ++j) //Last object was destroyed in readObject
					res[j]->destroy();
	...
	case TYPE_ARRAY:
		{
			...
			else
			{
				...
				//elementSize
				res.storeSimple<physx::PxI32>(res.getTargetSize(elemPd));
