...
bool ApexMeshContractor::endRegistration(physx::PxU32 subdivision, IProgressListener* progressListener)
{
	...
	mNumX = (int)((bounds.maximum.x - bounds.minimum.x) * invH) + 1;
	mNumY = (int)((bounds.maximum.y - bounds.minimum.y) * invH) + 1;
	mNumZ = (int)((bounds.maximum.z - bounds.minimum.z) * invH) + 1;
...
physx::PxU32 ApexMeshContractor::contract(physx::PxI32 steps, physx::PxF32 abortionRatio, physx::PxF32& volumeRatio, IProgressListener* progressListener)
{
	...
	{
		{
			physx::PxU32 percent = 100 * currStep / steps;
...
void ApexMeshContractor::expandBorder()
{
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		physx::PxI32 i0 = mIndices[3 * i];
		physx::PxI32 i1 = mIndices[3 * i + 1];
		physx::PxI32 i2 = mIndices[3 * i + 2];
		...
		collectNeighborhood(i, localRadius, i, localTris, localDists, triMarks);
...
void ApexMeshContractor::computeNeighbours()
{
	...
	while (i < numEdges)
	{
		...
		while (i < numEdges && edges[i] == e0)
		{
			...
			mNeighbours[e0.triNr * 3 + e0.edgeNr] = e1.triNr;
			PX_ASSERT(mNeighbours[e1.triNr * 3 + e1.edgeNr] == -1);///?
			mNeighbours[e1.triNr * 3 + e1.edgeNr] = e0.triNr;
			i++;
		}
	}
}
...
void ApexMeshContractor::computeSignedDistanceField()
{
	...
	for (physx::PxU32 xi = 0; xi < mNumX - 1; xi++)
	{
		...
		for (physx::PxU32 yi = 0; yi < mNumY - 1; yi++)
		{
			...
			{
				cc.zi = zi;
				const ContractorCell& c = cellAt(xi, yi, zi);
				if (!c.marked)
				{
					continue;
				}
	...
	while (!heap.isEmpty())
	{
		...
		ContractorCell& c = cellAt(cc.xi, cc.yi, cc.zi);
		c.marked = true;
		for (int i = 0; i < 6; i++)
		{
			CellCoord ncc = cc;
			...
			ContractorCell& cn = cellAt(ncc.xi, ncc.yi, ncc.zi);
			if (cn.marked)
			{
				continue;
			}
	...
	setInsideOutside();
	for (int i = 0; i < (int)mGrid.size(); i++)
	{
...
void ApexMeshContractor::contractionStep()
{
	...
	for (int i = 0; i < (int)mVertexCurvatures.size(); i++)
	{
	...
	for (int i = 0; i < numTris; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			int vertNr = mIndices[3 * i + j];
			if (mVertexCurvatures[vertNr] == 0.0f)
			{
				float curv = curvatureAt(i, vertNr);
...
{
	ContractorCell& ci = cellAt(xi, yi, zi);
	if (ci.marked)
	{
		return false;
	}
...
void ApexMeshContractor::interpolateGradientAt(const physx::PxVec3& pos, physx::PxVec3& grad)
{
	...
	{
		x0 = mNumX - 2;    // todo: handle boundary correctly
	}
	if (y0 < 0)
	{
		y0 = 0;
	...
	{
		x2 = mNumX - 3;    // todo: handle boundary correctly
	}
	if (y2 < 0)
	{
		y2 = 0;
...
void ApexMeshContractor::subdivide(physx::PxF32 spacing)
{
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		{
			int k = (j + 1) % 3;
			int v0 = mIndices[3 * triNr + j];
			int v1 = mIndices[3 * triNr + k];
			...
			int newTri  = mIndices.size() / 3;
			int newAdj  = -1;

			int v = getOppositeVertex(triNr, v0, v1);
			PX_ASSERT(v >= 0);
			...
			if (adj >= 0)
			{
				...
				PX_ASSERT(v >= 0);
				newAdj = mIndices.size() / 3;
				mIndices.pushBack(newVert);
				mIndices.pushBack(v);
				mIndices.pushBack(v1);
			}

			replaceVertex(triNr, v1, newVert);
			replaceVertex(adj, v1, newVert);

			replaceNeighbor(triNr, t1, newTri);
			replaceNeighbor(adj, t3, newAdj);
			replaceNeighbor(t1, triNr, newTri);
			replaceNeighbor(t3, adj, newAdj);

			mNeighbours.pushBack(newAdj);
			mNeighbours.pushBack(t1);
			mNeighbours.pushBack(triNr);
			if (adj >= 0)
			{
				mNeighbours.pushBack(adj);
				mNeighbours.pushBack(t3);
				mNeighbours.pushBack(newTri);
			}
		}
	}
}
...
void ApexMeshContractor::collapse(float spacing)
{
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		for (physx::PxU32 j = 0; j < 3; j++)
		{
			...
			for (physx::PxU32 k = 0; k < 3; k++)
			{
				if (t0 >= 0 && mNeighbours[3 * t0 + k] == triNr)
				{
					mNeighbours[3 * t0 + k] = t1;
				}
				if (t1 >= 0 && mNeighbours[3 * t1 + k] == triNr)
				{
					mNeighbours[3 * t1 + k] = t0;
				}
				if (t2 >= 0 && mNeighbours[3 * t2 + k] == adj)
				{
					mNeighbours[3 * t2 + k] = t3;
				}
				if (t3 >= 0 && mNeighbours[3 * t3 + k] == adj)
				{
					mNeighbours[3 * t3 + k] = t2;
				}
			}
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		if (old2newTris[i] != -1)
		{
			...
			for (physx::PxU32 j = 0; j < 3; j++)
			{
				mIndices[3 * triNr + j] = old2newVerts[mIndices[3 * i + j]];
...
{
	const physx::PxI32 i = 3 * t;
	if (i < 0 || i + 2 >= (physx::PxI32)mIndices.size())
	{
		return -1;
	}
...
{
	const physx::PxI32 i = 3 * t;
	if (i < 0 || i + 2 >= (physx::PxI32)mNeighbours.size())
	{
		return;
	}
	if (mNeighbours[i + 0] == nOld)
	{
		mNeighbours[i + 0] = nNew;
	}
	if (mNeighbours[i + 1] == nOld)
	{
		mNeighbours[i + 1] = nNew;
	}
	if (mNeighbours[i + 2] == nOld)
	{
		mNeighbours[i + 2] = nNew;
	}
}
	...
	while (t >= 0)
	{
		if (t != t1)
		{
			...
			while (t2 >= 0)
			{
				...
				advanceAdjTriangle(adjV, t2, prev2);
				if (t2 == t)
				{
					break;
				}
		...
		adjV = getOppositeVertex(t, v1, adjV);
		advanceAdjTriangle(v1, t, prev);
		if (t == triNr || t == adj)
		{
			break;
...
physx::PxF32 ApexMeshContractor::interpolateDistanceAt(const physx::PxVec3& pos) const
{
	...
	{
		x = mNumX - 2;    // todo: handle boundary correctly
	}
	if (y < 0)
	{
		y = 0;
...
float ApexMeshContractor::curvatureAt(int triNr, int v)
{
	...
	while (t >= 0)
	{
		advanceAdjTriangle(v, t, prev);
		if (t < 0 || t == triNr)
		{
			break;
		}

		physx::PxVec3& p0 = mVertices[mIndices[3 * prev]];
		physx::PxVec3& p1 = mVertices[mIndices[3 * prev + 1]];
		physx::PxVec3& p2 = mVertices[mIndices[3 * prev + 2]];
		n0 = (p1 - p0).cross(p2 - p0);
		n0.normalize();

		physx::PxVec3& q0 = mVertices[mIndices[3 * t]];
		physx::PxVec3& q1 = mVertices[mIndices[3 * t + 1]];
		physx::PxVec3& q2 = mVertices[mIndices[3 * t + 2]];
