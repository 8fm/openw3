...
bool ApexIsoMesh::generateMesh(IProgressListener* progressListener)
{
	...
	PxU32 maximum = mNumX * mNumY * mNumZ;
	...
	for (xi = 0; xi < mNumX; xi++)
	{
		...
		for (yi = 0; yi < mNumY; yi++)
		{
			...
			for (zi = 0; zi < mNumZ; zi++)
			{
				...
				{
					const PxU32 curr = ((xi * mNumY) + yi) * mNumZ + zi;
					const PxU32 percent = 100 * curr / maximum;
				...
				if (xi < mNumX - 1)
				{
					...
					{
						cell.vertNrX = mIsoVertices.size();
				...
				if (yi < mNumY - 1)
				{
					...
					{
						cell.vertNrY = mIsoVertices.size();
						mIsoVertices.pushBack(vPos);
					}
				}
				if (zi < mNumZ - 1)
				{
					...
					{
						cell.vertNrZ = mIsoVertices.size();
						mIsoVertices.pushBack(vPos);
					}
				}
			}
	...
	for (xi = 0; xi < mNumX - 1; xi++)
	{
		...
		for (yi = 0; yi < mNumY - 1; yi++)
		{
			...
			for (zi = 0; zi < mNumZ - 1; zi++)
			{
				...
				c.firstTriangle = mIsoTriangles.size();
...
bool ApexIsoMesh::findNeighbors(IProgressListener* progress)
{
	...
	while (i < mIsoEdges.size())
	{
		...
		if (j > i + 1)
		{
			mIsoTriangles[mIsoEdges[i  ].triangleNr].addNeighbor(mIsoEdges[j - 1].triangleNr);
			mIsoTriangles[mIsoEdges[j - 1].triangleNr].addNeighbor(mIsoEdges[i  ].triangleNr);
		}
		i = j;
	}
...
void ApexIsoMesh::removeLayers()
{
	...
	for (PxU32 i = 0; i < mIsoTriangles.size(); i++)
	{
		...
		GroupAndSize gas;
		gas.size = 0;
		gas.group = triangleGroups.size();
		gas.deleteMesh = false;

		gas.size = floodFill(i, gas.group);
	...
	if (mDiscardInnerMeshes)
	{
		...
		for (PxU32 group = 0; group < triangleGroups.size(); group++)
		{
			...
			const PxVec3 rayOrig = (mIsoVertices[mIsoTriangles[start].vertexNr[0]] + mIsoVertices[mIsoTriangles[start].vertexNr[1]] + mIsoVertices[mIsoTriangles[start].vertexNr[2]]) / 3.0f;
			...
			for (PxU32 i = start; i < numTriangles; i++)
			{
				...
				PxVec3 verts[3] =
				{
					mIsoVertices[mIsoTriangles[i].vertexNr[0]],
					mIsoVertices[mIsoTriangles[i].vertexNr[1]],
					mIsoVertices[mIsoTriangles[i].vertexNr[2]],
				};
...
PxU32 ApexIsoMesh::floodFill(PxU32 triangleNr, PxU32 groupNr)
{
	...
	while (!queue.empty())
	{
		...
		if (adj >= 0 && mIsoTriangles[adj].groupNr < 0)
		{
			queue.pushBack(adj);
		}
...
{
	for (PxI32 i = mIsoTriangles.size() - 1; i >= 0; i--)
	{
		if (mIsoTriangles[i].deleted)
		{
			mIsoTriangles.replaceWithLast(i);
		}
	}
