...
void ApexTetrahedralizer::getIndices(physx::PxU32* data)
{
	for (physx::PxU32 i = 0; i < mTetras.size(); i++)
	{
		...
		{
			data[i * 4 + j] = mTetras[i].vertexNr[j];
		}
	}
}
...
void ApexTetrahedralizer::delaunayTetrahedralization(IProgressListener* progress)
{
	...
	for (physx::PxU32 i = 0; i < mFirstFarVertex; i++)
	{
		...
		{
			const physx::PxU32 percent = 100 * i / mFirstFarVertex;
		...
		const physx::PxF32 volumeDiff = retriangulate(tNr, i);
		...
		for (physx::PxU32 j = newTetraNr; j < mTetras.size(); j++)
		{
			...
			edge.init(newTet.vertexNr[2], newTet.vertexNr[3], j, 1);
			edges.pushBack(edge);
			edge.init(newTet.vertexNr[3], newTet.vertexNr[1], j, 2);
			edges.pushBack(edge);
			edge.init(newTet.vertexNr[1], newTet.vertexNr[2], j, 3);
			edges.pushBack(edge);
		}
		...
		for (physx::PxU32 j = 0; j < edges.size(); j += 2)
		{
			...
			mTetras[edge0.tetraNr].neighborNr[edge0.neighborNr] = edge1.tetraNr;
			mTetras[edge1.tetraNr].neighborNr[edge1.neighborNr] = edge0.tetraNr;
		}
	}
}
...
physx::PxI32 ApexTetrahedralizer::findSurroundingTetra(physx::PxU32 startTetra, const physx::PxVec3& p) const
{
	...
	physx::PxI32 tetNr = startTetra;
	while (tetNr >= 0 && iterationCounter++ < iterationCounterMax)
	{
		const FullTetrahedron& tetra = mTetras[tetNr];
		PX_ASSERT(tetra.bDeleted == 0);

		const physx::PxVec3& p0 = mVertices[tetra.vertexNr[0]].pos;
		physx::PxVec3 q  = p - p0;
		physx::PxVec3 q0 = mVertices[tetra.vertexNr[1]].pos - p0;
		physx::PxVec3 q1 = mVertices[tetra.vertexNr[2]].pos - p0;
		physx::PxVec3 q2 = mVertices[tetra.vertexNr[3]].pos - p0;
...
physx::PxF32 ApexTetrahedralizer::retriangulate(const physx::PxU32 tetraNr, physx::PxU32 vertexNr)
{
	...
	for (physx::PxU32 i = 0; i < 4; i++)
	{
		...
		{
			volumeCreated += retriangulate(n, vertexNr);
		}
		else
		{
			...
			if (n >= 0)
			{
				PX_ASSERT(mTetras[n].neighborOf(tNew.vertexNr[1], tNew.vertexNr[2], tNew.vertexNr[3]) == (physx::PxI32)tetraNr);
				mTetras[n].neighborOf(tNew.vertexNr[1], tNew.vertexNr[2], tNew.vertexNr[3]) = mTetras.size();
			}
...
physx::PxU32 ApexTetrahedralizer::swapTetrahedra(physx::PxU32 startTet, IProgressListener* progress)
{
	...
	for (physx::PxU32 i = startTet; i < mTetras.size(); i++)
	{
		...
		edges.add(edge);
		edge.init(t.vertexNr[0], t.vertexNr[3], i);
		edges.add(edge);
		edge.init(t.vertexNr[1], t.vertexNr[3], i);
		edges.add(edge);
		edge.init(t.vertexNr[2], t.vertexNr[3], i);
		edges.add(edge);
	}
	edges.sort();
	...
	while (index < edges.numEdges())
	{
		...
		{
			const physx::PxU32 percent = 100 * index / edges.numEdges();
			progress->setProgress(percent);
		}
...
bool ApexTetrahedralizer::swapEdge(physx::PxU32 v0, physx::PxU32 v1)
{
	...
	for (physx::PxU32 i = 0; i < mTetras.size(); i++)
	{
		...
		nbDeleted++;
		physx::PxI32 v2, v3;
		t.get2OppositeVertices(v0, v1, v2, v3);
	...
	{
		physx::PxI32 i = 0;
		physx::PxI32 num = borderEdges.size();
		for (; i < num - 1; i += 2)
		{
	...
	{
		physx::PxI32 num = borderVertices.size();
		physx::PxI32 i0, i1, i2;
		for (i0 = 0; i0 < num; i0++)
		{
			i1 = (i0 + 1) % num;
			i2 = (i1 + 1) % num;
		...
		for (i0 = 0; i0 < num; i0++)
		{
			...
			for (physx::PxI32 i = 0; i < num; i++)
			{
				...
				physx::PxVec3& pos = mVertices[borderVertices[i]].pos;
		...
		for (physx::PxI32 i = i1; i < num - 1; i++)
		{
...
bool ApexTetrahedralizer::removeOuterTetrahedra(IProgressListener* progress)
{
	...
	for (physx::PxU32 i = 0; i < mTetras.size(); i++)
	{
		...
		{
			const physx::PxU32 percent = 100 * i / mTetras.size();
			progress->setProgress(percent);
		}
		FullTetrahedron& tetra = mTetras[i];

		if (isFarVertex(tetra.vertexNr[0]) || isFarVertex(tetra.vertexNr[1]) || isFarVertex(tetra.vertexNr[2]) || isFarVertex(tetra.vertexNr[3]))
		{
			tetra.bDeleted = 1;
		}
		else if (tetra.bDeleted == 0)
		{
			...
			orig = mVertices[tetra.vertexNr[0]].pos;
			orig += mVertices[tetra.vertexNr[1]].pos;
			orig += mVertices[tetra.vertexNr[2]].pos;
			orig += mVertices[tetra.vertexNr[3]].pos;
			orig *= 0.25f;
...
void ApexTetrahedralizer::updateCircumSphere(FullTetrahedron& tetra) const
{
	...
	const physx::PxVec3 p0 = mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3 b  = mVertices[tetra.vertexNr[1]].pos - p0;
	const physx::PxVec3 c  = mVertices[tetra.vertexNr[2]].pos - p0;
	const physx::PxVec3 d  = mVertices[tetra.vertexNr[3]].pos - p0;
...
{
	const physx::PxVec3 q  = p - mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3 q0 = mVertices[tetra.vertexNr[1]].pos - mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3 q1 = mVertices[tetra.vertexNr[2]].pos - mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3 q2 = mVertices[tetra.vertexNr[3]].pos - mVertices[tetra.vertexNr[0]].pos;
...
{
	const physx::PxVec3 v0 = mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3 v1 = mVertices[tetra.vertexNr[1]].pos - v0;
	const physx::PxVec3 v2 = mVertices[tetra.vertexNr[2]].pos - v0;
	const physx::PxVec3 v3 = mVertices[tetra.vertexNr[3]].pos - v0;
...
{
	const physx::PxVec3& v0 = mVertices[tetra.vertexNr[0]].pos;
	const physx::PxVec3& v1 = mVertices[tetra.vertexNr[1]].pos;
	const physx::PxVec3& v2 = mVertices[tetra.vertexNr[2]].pos;
	const physx::PxVec3& v3 = mVertices[tetra.vertexNr[3]].pos;
		...
		for (i = 0; i < mTetras.size(); i++)
		{
			...
			for (physx::PxU32 j = 0; j < 4; j++)
			{
				if (t.neighborNr[j] >= 0)
				{
					...
					t.neighborNr[j] = oldToNew[t.neighborNr[j]];
				}
			}
		}
	}
