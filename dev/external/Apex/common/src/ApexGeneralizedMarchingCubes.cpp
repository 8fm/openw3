		...
		for (coord[dim1] = min[dim1]; coord[dim1] <= max[dim1]; coord[dim1]++)
		{
			for (coord[dim2] = min[dim2]; coord[dim2] <= max[dim2]; coord[dim2]++)
			{
				...
				if (intersection)
				{
					...
					physx::PxI32 nr = createCube(coord[0], coord[1], coord[2]);
					...
					if (cube.vertRefs[dim0].vertNr < 0)
					{
						...
						vertPos[dim0] = pos;
						cube.vertRefs[dim0].vertNr = mVertices.size();
				...
				for (physx::PxU32 i = 0; i < 3; i++)
				{
					...
					if (q0.x != q1.x)
					{
						...
						if (0.0f <= t && t <= 1.0f)
						{
							...
							{
								GeneralizedCube& cube = mCubes[createCube(xi, yi, zi)];
...
bool ApexGeneralizedMarchingCubes::endRegistration(physx::PxU32 bubleSizeToRemove, IProgressListener* progressListener)
{
	...
	if (bubleSizeToRemove > 0)
	{
		...
		determineGroups();
		removeBubbles(bubleSizeToRemove);
		progress.completeSubtask();
	}
...
void ApexGeneralizedMarchingCubes::createTrianglesForCube(physx::PxI32 cellNr)
{
	...
	GeneralizedCube& c = mCubes[cellNr];
	...
	for (physx::PxU32 i = 0; i < 6; i++)
	{
		...
		case 13:
		{
			...
			if (nr >= 0)
			{
				faceVertNr = &mCubes[nr].sideVertexNr[0];
				b = &mCubes[nr].sideBounds[0];
			}
		}
		break;
		case 14:
		...
		case 15:
		{
			...
			if (nr >= 0)
			{
				faceVertNr = &mCubes[nr].sideVertexNr[1];
				b = &mCubes[nr].sideBounds[1];
			}
		}
		break;
		case 16:
		...
		case 17:
		{
			...
			if (nr >= 0)
			{
				faceVertNr = &mCubes[nr].sideVertexNr[2];
				b = &mCubes[nr].sideBounds[2];
			}
		}
		break;
		}
	...
	if (startFace >= 0)
	{
		...
		physx::PxI32 numTris = chainLen - 2;

		c.firstTriangle = mIndices.size() / 3;
		for (int i = 0; i < numTris; i++)
		{
			mIndices.pushBack(vertNrs[chain[0]]);
			mIndices.pushBack(vertNrs[chain[i + 1]]);
			mIndices.pushBack(vertNrs[chain[i + 2]]);
			c.numTriangles++;
		}
		return;
	}
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		mIndices.pushBack(vertNrs[mGeneralizedTriangles[3 * i + 0]]);
		mIndices.pushBack(vertNrs[mGeneralizedTriangles[3 * i + 1]]);
		mIndices.pushBack(vertNrs[mGeneralizedTriangles[3 * i + 2]]);
		c.numTriangles++;
	}
}
...
void ApexGeneralizedMarchingCubes::createNeighbourInfo()
{
	...
	while (i < numEdges)
	{
		...
		const physx::PxU32 first = mNeighbours.size();
		PX_ASSERT(mFirstNeighbour[e0.triNr * 3 + e0.edgeNr] == -1);
		mFirstNeighbour[e0.triNr * 3 + e0.edgeNr] = first;
		...
		while (i < numEdges && edges[i] == e0)
		{
			...
			PX_ASSERT(mFirstNeighbour[e1.triNr * 3 + e1.edgeNr] == -1);
			mFirstNeighbour[e1.triNr * 3 + e1.edgeNr] = first;
...
void ApexGeneralizedMarchingCubes::getCubeEdgesAndGroups(physx::PxI32 cellNr, GeneralizedVertRef* vertRefs[12], physx::PxI32 groups[8])
{
	...
	if (nr >= 0)
	{
		vertRefs[1] = &mCubes[nr].vertRefs[1];
		vertRefs[9] = &mCubes[nr].vertRefs[2];
	}
	nr = findCube(c.xi, c.yi + 1, c.zi);
	if (nr >= 0)
	{
		vertRefs[2] = &mCubes[nr].vertRefs[0];
		vertRefs[11] = &mCubes[nr].vertRefs[2];
	}
	nr = findCube(c.xi, c.yi, c.zi + 1);
	if (nr >= 0)
	{
		vertRefs[4] = &mCubes[nr].vertRefs[0];
		vertRefs[7] = &mCubes[nr].vertRefs[1];
	}
	nr = findCube(c.xi + 1, c.yi + 1, c.zi);
	if (nr >= 0)
	{
		vertRefs[10] = &mCubes[nr].vertRefs[2];
	}
	nr = findCube(c.xi, c.yi + 1, c.zi + 1);
	if (nr >= 0)
	{
		vertRefs[6] = &mCubes[nr].vertRefs[0];
	}
	nr = findCube(c.xi + 1, c.yi, c.zi + 1);
	if (nr >= 0)
	{
		vertRefs[5] = &mCubes[nr].vertRefs[1];
	}
...
void ApexGeneralizedMarchingCubes::determineGroups()
{
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		physx::PxI32 group = mGroupFirstTriangle.size();
		mGroupFirstTriangle.pushBack(mGroupTriangles.size());

		mCubeQueue.clear();
		mCubeQueue.pushBack(i);
		while (!mCubeQueue.empty())
		{
			const physx::PxI32 t = mCubeQueue[mCubeQueue.size() - 1];
			mCubeQueue.popBack();
			...
			mGroupTriangles.pushBack(t);

			for (physx::PxU32 j = 0; j < 3; j++)
			{
				physx::PxI32 first = mFirstNeighbour[3 * t + j];
				physx::PxU32 num = 0;
				physx::PxI32 n = -1;
				...
				if (num == 2 && n >= 0 && mTriangleGroup[n] < 0)
				{
					if (!mTriangleDeleted[n])
					{
...
void ApexGeneralizedMarchingCubes::removeBubbles(physx::PxI32 minGroupSize)
{
	...
	for (physx::PxU32 i = 0; i < numGroups; i++)
	{
		...
		physx::PxI32 lastTri  = i < numGroups - 1 ? mGroupFirstTriangle[i + 1] : mGroupTriangles.size();
		...
		{
			physx::PxI32 t = mGroupTriangles[j];
			for (physx::PxU32 k = 0; k < 3; k++)
			{
				physx::PxI32 first = mFirstNeighbour[3 * t + k];
				physx::PxU32 num = 0;
				physx::PxI32 n = -1;
				while (mNeighbours[first] >= 0)
				{
				...
				if (num == 2 && mTriangleGroup[n] != (physx::PxI32)i)
				{
					OK = false;
					break;
				}
		...
		for (physx::PxI32 j = firstTri; j < lastTri; j++)
		{
			...
			for (int k = 0; k < 3; k++)
			{
				...
				if (pos >= 0)
				{
					mNeighbours[pos] = mNeighbours[first - 1];
...
void ApexGeneralizedMarchingCubes::fixOrientations()
{
	...
	for (physx::PxU32 i = 0; i < numTris; i++)
	{
		...
		while (!queue.empty())
		{
			physx::PxI32 triNr = queue[queue.size() - 1];
			queue.popBack();

			for (physx::PxU32 j = 0; j < 3; j++)
			{
				physx::PxI32 first = mFirstNeighbour[3 * triNr + j];
				physx::PxU32 num = 0;
				physx::PxI32 adjNr = -1;

				while (mNeighbours[first] >= 0)
				{
					if (mNeighbours[first] != triNr)
					{
				...
				marks[adjNr] = 1;
				if (mIndices[3 * adjNr + 0] == i0 && mIndices[3 * adjNr + 1] == i1)
				{
					marks[adjNr] = 2;
				}
				if (mIndices[3 * adjNr + 1] == i0 && mIndices[3 * adjNr + 2] == i1)
				{
					marks[adjNr] = 2;
				}
				if (mIndices[3 * adjNr + 2] == i0 && mIndices[3 * adjNr + 0] == i1)
				{
					marks[adjNr] = 2;
				}
			}
