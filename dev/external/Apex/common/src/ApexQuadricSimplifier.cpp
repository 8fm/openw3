...
bool ApexQuadricSimplifier::endRegistration(bool mergeCloseVertices, IProgressListener* progressListener)
{
	...
	for (physx::PxU32 i = 0; i < mTriangles.size(); i++)
	{
		...
		qv2->q += q;
		QuadricEdge e;
		e.init(t.vertexNr[0], t.vertexNr[1]);
		mEdges.pushBack(e);
		e.init(t.vertexNr[1], t.vertexNr[2]);
		mEdges.pushBack(e);
		e.init(t.vertexNr[2], t.vertexNr[0]);
		mEdges.pushBack(e);
	}
...
physx::PxU32 ApexQuadricSimplifier::simplify(physx::PxU32 subdivision, physx::PxI32 maxSteps, physx::PxF32 maxError, IProgressListener* progressListener)
{
	...
	while (maxSteps == -1 || (maxSteps-- > 0))
	{
		...
		{
			const physx::PxU32 percent = 100 * progressCounter / maximum;
	...
	while (mNumDeletedHeapElements > 0)
	{
		...
		{
			const physx::PxU32 percent =  100 * (progressCounter - mNumDeletedHeapElements) / progressCounter;
			progress.setProgress(percent);
		}
#if TESTING
		testHeap();
...
physx::PxI32 ApexQuadricSimplifier::getTriangleNr(physx::PxU32 v0, physx::PxU32 v1, physx::PxU32 v2) const
{
	...
	for (int i = 0; i < (int)qv0->mTriangles.size(); i++)
	{
...
void ApexQuadricSimplifier::collapseEdge(QuadricEdge& edge)
{
	...
	for (physx::PxU32 i = 0; i < qv1->mEdges.size(); i++)
	{
		...
		if (found)
		{
			mVertices[vi]->removeEdge(qv1->mEdges[i]);
			ei.deleted = true;
			if (ei.heapPos >= 0)
			{
				heapRemove(ei.heapPos, false);
			}
#if TESTING
			testHeap();
#endif
	...
	for (physx::PxI32 i = qv0->mEdges.size() - 1; i >= 0; i--)
	{
		QuadricEdge& ei = mEdges[qv0->mEdges[i]];
		if (ei.otherVertex(vNr0) == vNr1)
		{
			qv0->mEdges.replaceWithLast(i);
		}
		else
		{
			computeCost(ei);
	...
	for (physx::PxI32 i = qv0->mTriangles.size() - 1; i >= 0; i--)
	{
		physx::PxU32 triangleIndex = qv0->mTriangles[i];
		...
		if (!t.deleted && t.containsVertex(vNr1))
		{
			...
			{
				mVertices[t.vertexNr[j]]->removeTriangle(triangleIndex);
				//fprintf_s(f, "  v %d\n", t.vertexNr[j]);
			}
		}
	}
...
void ApexQuadricSimplifier::quickSortVertexRefs(physx::PxI32 l, physx::PxI32 r)
{
	...
	if (l < j)
	{
		quickSortVertexRefs(l, j);
	}

	if (i < r)
	{
		quickSortVertexRefs(i, r);
	}
}
		...
		while (j < mVertexRefs.size() && fabs(mVertexRefs[j].pos.x - pos.x) < MERGE_THRESHOLD)
		{
			if ((mVertexRefs[j].pos - pos).magnitudeSquared() < d2)
			{
				...
				for (physx::PxU32 k = 0; k < vj->mTriangles.size(); k++)
				{
					...
					vi->addTriangle(vj->mTriangles[k]);
				}
...
{
	for (physx::PxI32 i = mEdges.size() - 1; i >= 0; i--)
	{
		if (mEdges[i] == (physx::PxU32) edgeNr)
		{
			mEdges.replaceWithLast(i);
		}
	}
}
...
void ApexQuadricSimplifier::QuadricVertex::addTriangle(physx::PxI32 triangleNr)
{
	...
	mTriangles.pushBack(triangleNr);
}
...
{
	physx::PxU32 found = 0;
	for (physx::PxI32 i = mTriangles.size() - 1; i >= 0; i--)
	{
		if (mTriangles[i] == (physx::PxU32) triangleNr)
		{
