...
{
	p.i0 = stream.readDword();
	p.i1 = stream.readDword();
	return stream;
}
PX_INLINE physx::PxFileBuf& operator << (physx::PxFileBuf& stream, const IntPair& p)
{
	stream.storeDword(p.i0);
	stream.storeDword(p.i1);
	return stream;
}
...
public:
	...
	{
		return mParams->vertices.arraySizes[0];
	}
	...
	{
		return mParams->uniquePlanes.arraySizes[0];
	}
	...
	{
		return mParams->widths.arraySizes[0];
	}
	...
	{
		return mParams->edges.arraySizes[0];
	}
	...
	void					applyTransformation(const physx::PxMat44& tm)	
	{
		...
		const physx::PxU32 numPlanes = mParams->uniquePlanes.arraySizes[0];
		...
		// vertices
		const physx::PxU32 numVertices = mParams->vertices.arraySizes[0];
	...
	void					applyTransformation(const physx::PxMat44& transformation, physx::PxF32 scale)
	{
		...
		// slab widths
		const physx::PxU32 numWidths = mParams->widths.arraySizes[0];
		physx::PxF32* widths = mParams->widths.buf;
		PX_ASSERT(widths);
...
#pragma warning(disable: 4127) // conditional expression is constant
		...
		{
			return (1 << (8 * physx::PxMin((int)sizeof(IndexType), (int)3))) - 1;	// Limited by data type we use for indices
		}
#pragma warning(pop)
	}
	...
	virtual	void	reserve(physx::PxU32 capacity)
	{
		...
		if (NxIndexBank<IndexType>::indices.size() > oldSize)
		{
			...
			if (nb)
			{
				...
				for (physx::PxI32 i = numIndices - 1; i >= 0; i--)
				{
...
class HierarchicalProgressListener : public IProgressListener
{
	...
	m_work(0), m_subtaskWork(1), m_totalWork(totalWork), m_taskName(NULL), m_parent(parent) {}
