...
protected:
	...
	bool convert()
	{
		...
		for (PxI32 i = 0; i < mNewData->graphicalLods.arraySizes[0]; ++i)
		{
			...
			Array<PxVec3*> submeshPositionBuffer(numSubmeshes, NULL);
			Array<PxVec4*> submeshTangentBuffer(numSubmeshes, NULL);
			Array<PxU32> submeshVertexOffsets(numSubmeshes);
			...
			for (PxI32 submeshIdx = 0; submeshIdx < numSubmeshes; ++submeshIdx)
			{
				...
				PxU32 numBuffers = vb->buffers.arraySizes[0];
				for (PxU32 bufferIdx = 0; bufferIdx < numBuffers; ++bufferIdx)
				{
					...
					if (bufferFormat.semantic == NxRenderVertexSemantic::POSITION)
					{
						if (bufferFormat.format == NxRenderDataFormat::FLOAT3)
						{
							...
							submeshPositionBuffer[submeshIdx] = posBuffer->data.buf;
						}
					}
					...
					if (bufferFormat.semantic == NxRenderVertexSemantic::TANGENT)
					{
						if (bufferFormat.format == NxRenderDataFormat::FLOAT4)
						{
							...
							submeshTangentBuffer[submeshIdx] = (PxVec4*)tangentBuffer->data.buf;
						}
					}
				}

				submeshVertexOffsets[submeshIdx] = submeshVertexOffset;
			...
			for (PxI32 j = 0; j < graphicalLod->skinClothMap.arraySizes[0]; ++j)
			{
				...
				PxI32 submeshIdx = 0;
				for (PxI32 k = submeshVertexOffsets.size()-1; k >= 0; --k)
				{
					if (submeshVertexOffsets[k] <= mapD.vertexIndexPlusOffset)
					{
						graphVertIdx = mapD.vertexIndexPlusOffset - submeshVertexOffsets[k];
						submeshIdx = k;
						break;
					}
				}

				if (submeshPositionBuffer[submeshIdx] != NULL && submeshTangentBuffer[submeshIdx] != NULL)
				{
					PxVec3 graphicalPos = submeshPositionBuffer[submeshIdx][graphVertIdx];
					PxVec3 graphicalTangent = submeshTangentBuffer[submeshIdx][graphVertIdx].getXYZ();
