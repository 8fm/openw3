...
void ClothingPhysicalMesh::setGeometry(bool tetraMesh, PxU32 numVertices, PxU32 vertexByteStride, const void* vertices, PxU32 numIndices, PxU32 indexByteStride, const void* indices)
{
	setGeometryInternal(tetraMesh, numVertices, vertexByteStride, vertices, NULL, numIndices, indexByteStride, indices);
}



void ClothingPhysicalMesh::setGeometryInternal(bool tetraMesh, PxU32 numVertices, PxU32 vertexByteStride, const void* vertices,
        const PxU32* masterFlags, PxU32 numIndices, PxU32 indexByteStride, const void* indices)
{
...
void ClothingPhysicalMesh::writeBackData()
{
	...
	while (indicesWritten < mSimplifier->getNumTriangles() * 3)
	{
		...
		if (mSimplifier->getTriangle(trianglesRead++, v0, v1, v2))
		{
			...
			mIndices[indicesWritten++] = old2new[v0];
			mIndices[indicesWritten++] = old2new[v1];
			mIndices[indicesWritten++] = old2new[v2];
		}
	}
...
{
	const PxU32 increment = mParams->physicalMesh.isTetrahedralMesh ? 4 : 3;
	const PxU32 numIndices = mParams->physicalMesh.numIndices;
...
void ClothingPhysicalMesh::applyTransformation(const PxMat34Legacy& transformation, PxF32 scale)
{
	...
	if (transformation.M.determinant() * scale < 0.0f)
	{
		...
		const PxU32 numTransDown = mParams->transitionDown.arraySizes[0];
		...
		const PxU32 numTransUp = mParams->transitionUp.arraySizes[0];
...
void ClothingPhysicalMesh::computeNeighborInformation(Array<PxI32> &neighbors)
{
	...
	while (i < edges.size())
	{
		...
		while (i < edges.size() && edges[i] == e0)
		{
			...
			neighbors[3 * e0.triNr + e0.edgeNr] = e1.triNr;
			neighbors[3 * e1.triNr + e1.edgeNr] = e0.triNr;
			i++;
		}
	}
}
...
void ClothingPhysicalMesh::fixTriangleOrientations()
{
	...
	for (PxU32 i = 0; i < numTriangles; i++)
	{
		...
		while (!queue.empty())
		{
			int triNr = queue[queue.size() - 1];
			queue.popBack();
			for (PxU32 j = 0; j < 3; j++)
			{
				int adjNr = neighbors[3 * triNr + j];
				if (adjNr < 0 || marks[adjNr] != 0)
				{
					continue;
				}
