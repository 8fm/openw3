	...
	if (strcmp(params->className(), ClothingAssetParameters::staticClassName()) == 0)
	{
		...
		if (mParams->materialLibrary != NULL && cookingInvalid)
		{
			...
			for (PxI32 i = 0; i < matLib->materials.arraySizes[0]; i++)
			{
				...
				PxF32& stretchiness = matLib->materials.buf[i].zeroStretchStiffness;

				stretchiness = PxMin(stretchiness, 0.5f); // must never be bigger than 0.5 for stability
			}
		}
...
PxF32 ClothingAsset::getMaximumSimulationBudget(PxU32 solverIterations) const
{
	...
	for (PxU32 i = 0; i < mPhysicalMeshes.size(); i++)
	{
		const PxU32 numLods = mPhysicalMeshes[i]->physicalLods.arraySizes[0];
...
bool ClothingAsset::remapBoneIndex(const char* name, PxU32 newIndex)
{
	...
	for (PxU32 i = 0; i < numBones; i++)
	{
		...
		{
			mBones[i].externalIndex = newIndex;
			found++;
		}
	}
...
void ClothingAsset::getBoneMapping(PxU32* internal2externalMap) const
{
	...
	{
		internal2externalMap[mBones[i].internalIndex] = mBones[i].externalIndex;
	}
}
...
#endif
{
	...
	for (PxI32 i = mUnusedSimulation.size() - 1; i >= 0; i--)
	{
		if (mUnusedSimulation[i]->dependsOnScene(deletedScene))
		{
			destroySimulation(mUnusedSimulation[i]);
			mUnusedSimulation.replaceWithLast(i);
		}
	}
...
ClothingAssetParametersNS::CookedEntry_Type* ClothingAsset::getCookedData(PxF32 actorScale)
{
	ClothingAssetParametersNS::CookedEntry_Type* closest = NULL;
	PxF32 closestDiff = PX_MAX_F32;

	for (PxI32 i = 0; i < mParams->cookedData.arraySizes[0]; i++)
	{
		ClothingAssetParametersNS::CookedEntry_Type* cookedEntry = mParams->cookedData.buf + i;
		NxParameterized::Interface* cookedData = cookedEntry->cookedData;
		const PxF32 cookedDataScale = cookedEntry->scale;
								  ...
								  bool localSpaceSim )
{
	simulation->initCollision(	mBoneActors.begin(), mBoneActors.size(), mBoneSpheres.begin(), mBoneSpheres.size(), mSpherePairs.begin(), mSpherePairs.size(), mBonePlanes.begin(), mBonePlanes.size(), mCollisionConvexes.begin(), mCollisionConvexes.size(), mBones.begin(), boneTansformations,
								...
								actorParam->actorDescTemplate, actorParam->shapeDescTemplate, actorParam->actorScale, globalPose, localSpaceSim );
}
...
PxU32 ClothingAsset::getNumPhysicalLods(PxU32 graphicalLodId) const
{
	...
	return mPhysicalMeshes[physicalMeshId]->physicalLods.arraySizes[0];
}
...
void ClothingAsset::visualizeBones(NiApexRenderDebug& renderDebug, const PxMat44* matrices, bool skeleton, PxF32 boneFramesScale, PxF32 boneNamesScale)
{
	...
	for (PxU32 i = 0; i < getNumUsedBones(); i++)
	{
		...
		if (skeleton && parent >= 0 && parent < (PxI32)getNumUsedBones())
		{
			...
			PxMat34Legacy absPoseParent = matrices[parent] * mBones[parent].bindPose;
			if ((mBones[parent].numMeshReferenced + mBones[parent].numRigidBodiesReferenced) == 0)
			{
...
PxU32 ClothingAsset::initializeAssetData(ClothingAssetData& assetData, const PxU32 uvChannel)
{
	...
	for (PxU32 a = 0; a < numLods; ++a)
	{
		...
		pAsset->mImmediateClothMapCount = graphicalLod->immediateClothMap.arraySizes[0];


		pAsset->mSkinClothMap = graphicalLod->skinClothMap.buf;
		pAsset->mSkinClothMapCount = graphicalLod->skinClothMap.arraySizes[0];

		pAsset->mSkinClothMapB = graphicalLod->skinClothMapB.buf;
		pAsset->mSkinClothMapBCount = graphicalLod->skinClothMapB.arraySizes[0];

		pAsset->mTetraMap = graphicalLod->tetraMap.buf;
		pAsset->mTetraMapCount = graphicalLod->tetraMap.arraySizes[0];
	...
	for (PxU32 a = 0; a < numPhysicalMeshes; ++a)
	{
		...
		pPhysicalMesh->mSkinningNormalsCount = physicalMesh->skinningNormals.arraySizes[0];
		pPhysicalMesh->mSubmeshesCount = pPhysicalMeshParams->submeshes.arraySizes[0];
		...
		pPhysicalMesh->mBoneWeightsCount = physicalMesh->boneWeights.arraySizes[0];

		pPhysicalMesh->mOptimizationData = physicalMesh->optimizationData.buf;
		pPhysicalMesh->mOptimizationDataCount = physicalMesh->optimizationData.arraySizes[0];

		pPhysicalMesh->mIndices = physicalMesh->indices.buf;
		pPhysicalMesh->mIndicesCount = physicalMesh->indices.arraySizes[0];
...
PxU32 ClothingAsset::getMeshSkinningMapSize(PxU32 lod)
{
	...
	return mGraphicalLods[lod]->skinClothMap.arraySizes[0];
}
...
void ClothingAsset::getMeshSkinningMap(PxU32 lod, ClothingMeshSkinningMap* map)
{
	...
	// copy the values
	for (PxI32 i = 0; i < mGraphicalLods[lod]->skinClothMap.arraySizes[0]; ++i)
	{
		...
		if (skinMap.vertexIndexPlusOffset != (PxU32)i)
		{
			APEX_DEBUG_WARNING("The MeshSkinningMap is not ordered correctly. Returned MeshSkinningMap is not valid.");
			return;
		}
	}
}
...
void ClothingAsset::prepareCookingJob(CookingAbstract& job, PxF32 scale, PxVec3* gravityDirection, PxVec3* morphedPhysicalMesh)
{
	...
	for (PxU32 physicalMeshId = 0; physicalMeshId < mPhysicalMeshes.size(); physicalMeshId++)
	{
		...
		PxU32 numSubMeshes = mPhysicalMeshes[physicalMeshId]->submeshes.arraySizes[0];
	...
	if (tempVerticesForScale == NULL)
	{
		...
		for (PxU32 i = 0; i < mBoneActors.size(); i++)
		{
			...
			PxU32 boneIndex = mBoneActors[i].boneIndex;
...
void ClothingAsset::getDisplacedPhysicalMeshPositions(PxVec3* morphDisplacements, NxParamArray<PxVec3> displacedMeshPositions)
{
	...
	for (PxU32 pm = 0; pm < mPhysicalMeshes.size(); pm++)
	{
		...
		if (gm < mGraphicalLods.size())
		{
			...
			if (graphicalLod->immediateClothMap.buf == NULL && graphicalLod->skinClothMapB.buf != NULL)
			{
				...
				physx::sort<SkinClothMapB, SkinClothMapBVertexPredicate>(graphicalLod->skinClothMapB.buf, graphicalLod->skinClothMapB.arraySizes[0], SkinClothMapBVertexPredicate());
			}
			...
			if (graphicalLod->immediateClothMap.buf == NULL && graphicalLod->skinClothMapB.buf != NULL)
			{
				physx::sort<SkinClothMapB, SkinClothMapFacePredicate>(graphicalLod->skinClothMapB.buf, graphicalLod->skinClothMapB.arraySizes[0], SkinClothMapFacePredicate());
			}
			...
			for (PxU32 i = 0; i < physicalMesh.numVertices; i++)
			{
				...
				else
				{
					...
					for (PxU32 submeshIndex = 0; submeshIndex < rma->getSubmeshCount() && shortestDistance > 0.0f; submeshIndex++)
					{
						...
						PxU32 positionIndex = format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::POSITION));
	...
	for (PxU32 ba = 0; ba < mBoneActors.size(); ba++)
	{
		...
		for (PxU32 bi = 0; bi < mBoneActors[ba].convexVerticesCount; bi++)
		{
			...
			const PxMat34Legacy bindPose = mBones[mBoneActors[ba].boneIndex].bindPose;
		...
		for (PxU32 lodIndex = 0; lodIndex < mGraphicalLods.size(); lodIndex++)
		{
			...
			for (PxU32 submeshIdx = 0; submeshIdx < meshAsset.getSubmeshCount(); submeshIdx++)
			{
				...
				for (PxU32 vertexIndex = 0; vertexIndex < numVertices; vertexIndex++)
				{
					...
					const PxU32 tangentWpositive = tangents[vertexIndex].w > 0.0f ? 1 : 0;
...
public:
	SortGraphicalVerts(PxU32 numVerts, PxU32 submeshOffset, const PxU32* indices, PxU32 numIndices, ClothingGraphicalLodParameters* lodParameters,
	                   ClothingPhysicalMeshParameters* physicsMesh) : mNumNotFoundVertices(0), mIndices(indices), mNumIndices(numIndices)
	{
		...
		const PxU32 numSkinC = lodParameters->skinClothMap.arraySizes[0];
		for (PxU32 i = 0; i < numSkinC; i++)
		{
		...
		const PxU32 numSkinB = lodParameters->skinClothMapB.arraySizes[0];
		...
		const PxU32 numTetras = lodParameters->tetraMap.arraySizes[0];
...
{
public:
	SortGraphicalIndices(PxU32 numIndices, PxU32* indices) : mNumIndices(numIndices), mIndices(indices)
	{
		mTriangleInfo.resize(mNumIndices / 3);
		...
		mNew2Old.resize(mNumIndices / 3);
		mOld2New.resize(mNumIndices / 3);
		for (PxU32 i = 0; i < mNew2Old.size(); i++)
		{
			mNew2Old[i] = i;
			mOld2New[i] = i;
	...
	PxI32 getTriangleSubmesh(PxU32 newTriangleIndex)
	{
...
bool ClothingAsset::reorderGraphicsVertices(PxU32 graphicalLodId, bool perfWarning)
{
	...
	const PxU32 numSkinClothMap = mGraphicalLods[graphicalLodId]->skinClothMap.arraySizes[0];
	ClothingGraphicalLodParametersNS::SkinClothMapD_Type* skinClothMap = mGraphicalLods[graphicalLodId]->skinClothMap.buf;

	const PxU32 numSkinClothMapB = mGraphicalLods[graphicalLodId]->skinClothMapB.arraySizes[0];
	...
	// allocate enough space
	{
		...
		PxI32 count = physicsMesh->submeshes.arraySizes[0] * numSubMeshes;
	...
	for (PxU32 s = 0; s < numSubMeshes; s++)
	{
		...
		SortGraphicalVerts sortedVertices(numVertices, submeshVertexOffset, submesh.getIndexBuffer(0), submesh.getIndexCount(0), mGraphicalLods[graphicalLodId], physicsMesh);

		if (sortedVertices.mNumNotFoundVertices > 0)
		{
			if (sortedVertices.mNumNotFoundVertices > numVertices / 100)
			{
				APEX_INVALID_OPERATION("Many vertices could not be matched by any means of mesh-mesh skinning. The asset <%s> seems to be broken", mName.c_str());
			}
			else
			{
				APEX_DEBUG_WARNING("A few (%d) vertices from asset <%s> could not be matched by mesh-mesh skinning. They will be CPU skinned instead", sortedVertices.mNumNotFoundVertices, mName.c_str());
			}
		}
...

		{
			...
			for (PxI32 i = 0; i < physicsMesh->submeshes.arraySizes[0]; i++)
			{
				...
				mGraphicalLods[graphicalLodId]->physicsSubmeshPartitioning.buf[submeshPartitioningIndex + i].physicalSubmesh = i;
		...
		// also sort the index buffer accordingly
		{
			...
			SortGraphicalIndices sortedIndices(submesh.getIndexCount(0), indices);

			for (PxU32 i = 0; i < sortedIndices.mNew2Old.size(); i++)
			{
			...
			sortedIndices.sort();
			ApexPermute(indices, sortedIndices.mNew2Old.begin(), sortedIndices.mNew2Old.size(), 3);

			PxU32 startTriangle = 0;
			for (PxI32 i = 0; i < physicsMesh->submeshes.arraySizes[0]; i++)
			{
				while (startTriangle < sortedIndices.mNew2Old.size() && sortedIndices.getTriangleSubmesh(startTriangle) <= i)
				{
					startTriangle++;
				}
...
{
public:
	DeformableVerticesMaxDistancePredicate(ClothingPhysicalMeshParametersNS::ConstrainCoefficient_Type* constrainCoefficients) : mConstrainCoefficients(constrainCoefficients) {}
...
private:
	ClothingPhysicalMeshParametersNS::ConstrainCoefficient_Type* mConstrainCoefficients;
};
...
bool ClothingAsset::reorderDeformableVertices(ClothingPhysicalMesh& physicalMesh)
{
	...
	for (PxI32 i = 0; i < params->submeshes.arraySizes[0]; i++)
	{
		...
		DeformableVerticesMaxDistancePredicate predicate(physicalMesh.getConstrainCoefficientsBuffer());
	...
	ClothingPhysicalMeshParametersNS::ConstrainCoefficient_Type* coeffs = physicalMesh.getConstrainCoefficientsBuffer();
	...
	if (params->transitionDown.arraySizes[0] > 0)
	{
		...
		shdfnd::sort(params->transitionDown.buf, params->transitionDown.arraySizes[0], SkinClothMapPredicate<ClothingPhysicalMeshParametersNS::SkinClothMapD_Type>());
	}
	...
	if (params->transitionUp.arraySizes[0] > 0)
	{
		...
		shdfnd::sort(params->transitionUp.buf, params->transitionUp.arraySizes[0], SkinClothMapPredicate<ClothingPhysicalMeshParametersNS::SkinClothMapD_Type>());
	}
        ...
        PxU32 submeshVertexOffset, PxU32 indices[4], PxF32 trust[4]) const
{
	...
	if (graphLod.immediateClothMap.arraySizes[0] > 0)
	{
		...
		if (indices[0] != ClothingConstants::ImmediateClothingInvalidValue)
		{
			...
			{
				const PxI32 temp = indices[0] & ClothingConstants::ImmediateClothingReadMask;
	...
	else if (graphLod.skinClothMap.arraySizes[0] > 0)
	{
		...
		PxU32 curMin = 0;
		PxU32 curMax = graphLod.skinClothMap.arraySizes[0];
...
void ClothingAsset::getNormalsAndVerticesForFace(PxVec3* vtx, PxVec3* nrm, PxU32 i,
...
bool ClothingAsset::setBoneName(PxU32 internalIndex, const char* name)
{
	...
	if (bonesHandle.isValid())
	{
		...
		bonesHandle.getChildHandle(internalIndex, boneHandle);
...
bool ClothingAsset::findTriangleForImmediateVertex(PxU32& faceIndex, PxU32& indexInTriangle, PxU32 physVertIndex, ClothingPhysicalMeshParametersNS::PhysicalMesh_Type& physicalMesh) const
{
	...
	for (physx::PxI32 physIndex = 0; physIndex < physicalMesh.indices.arraySizes[0]; physIndex++)
	{
...
bool ClothingAsset::mergeMapping(ClothingGraphicalLodParameters* graphicalLod)
{
	...
	PxU32 immediateCount = graphicalLod->immediateClothMap.arraySizes[0];
	...
	for (PxU32 submeshIndex = 0; submeshIndex < numSubmeshes; ++submeshIndex)
	{
		...
		for (PxU32 submeshVertIndex = 0; submeshVertIndex < renderMesh.getNumVertices(submeshIndex); ++submeshVertIndex, ++mapVertIndex)
		{
			...
			{
				APEX_DEBUG_WARNING("This asset (%s) is invalid! Its immediate map contains a non-mapped vertex (%i). Assigning it to physical vertex 0.", mName.c_str(), mapVertIndex);
			}
	...
	shdfnd::sort(graphicalLod->skinClothMap.buf, graphicalLod->skinClothMap.arraySizes[0], SkinClothMapPredicate<ClothingGraphicalLodParametersNS::SkinClothMapD_Type>());
...
void ClothingAsset::updateBoundingBox()
{
	...
	for (PxU32 graphicalMeshId = 0; graphicalMeshId < mGraphicalLods.size(); graphicalMeshId++)
	{
		...
		for (PxU32 submeshIndex = 0; submeshIndex < renderMeshAsset->getSubmeshCount(); submeshIndex++)
		{
			...
			physx::PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID(physx::apex::NxRenderVertexSemantic::POSITION));
