	...
	bActorCollisionChanged(0)
{
	...
	if (strcmp(descriptor.className(), ClothingActorParam::staticClassName()) == 0)
	{
		...
		bInternalLocalSpaceSim = mActorDesc->localSpaceSim ? 1 : 0;
				...
				for (PxU32 gi = 0; gi < mGraphicalMeshes.size(); ++gi)
				{
					...
					for (PxU32 si = 0; si < numSubmeshes; ++si)
					{
						...
						if (positionIndex != -1)
						{
							...
							const PxVec3* positions = reinterpret_cast<const PxVec3*>(submesh.getVertexBuffer().getBufferAndFormat(bufferFormat, positionIndex));
		...
		NxParameterized::Interface* assetCookedData = mAsset->getCookedData(mActorDesc->actorScale)->cookedData;
	...
	if (mActorDesc != NULL)
	{
		...
		for (PxI32 i = 0; i < mActorDesc->overrideMaterialNames.arraySizes[0]; ++i)
		{
...
void ClothingActor::updateState(const PxMat44& globalPose, const PxMat44* newBoneMatrices, PxU32 boneMatricesByteStride, PxU32 numBoneMatrices, ClothingTeleportMode::Enum teleportMode)
{
	...
	if (boneMatricesByteStride >= sizeof(PxMat44) && newBoneMatrices != NULL)
	{
		if (mActorDesc->boneMatrices.arraySizes[0] != (PxI32)numBoneMatrices)
		{
			...
			NxParameterized::Handle handle(mActorDesc);
			handle.getParameter("boneMatrices");
			handle.resizeArray(numBoneMatrices);
		}
...
void ClothingActor::updateMaxDistanceScale_DEPRECATED(PxF32 scale, bool multipliable)
{
...
const PxMat44& ClothingActor::getGlobalPose_DEPRECATED() const
{
	PX_ASSERT(mActorDesc != NULL);
...
void ClothingActor::setWind_DEPRECATED(PxF32 windAdaption, const PxVec3& windVelocity)
{
	APEX_DEPRECATED_ONCE();
...
void ClothingActor::setMaxDistanceBlendTime_DEPRECATED(PxF32 blendTime)
{
	APEX_DEPRECATED_ONCE();
	PX_ASSERT(mActorDesc);
...
PxF32 ClothingActor::getMaxDistanceBlendTime_DEPRECATED() const
{
	APEX_DEPRECATED_ONCE();
	PX_ASSERT(mActorDesc);
...
void ClothingActor::setLODWeights_DEPRECATED(PxF32 maxDistance, PxF32 distanceWeight, PxF32 bias, PxF32 benefitBias)
{
	PX_ASSERT(mActorDesc);
...
void ClothingActor::getPhysicalLodRange(PxF32& min, PxF32& max, bool& intOnly)
{
...
physx::PxF32 ClothingActor::getActivePhysicalLod()
{
...
void ClothingActor::forcePhysicalLod(PxF32 lod)
{
	...
	{
		mForcePhysicalLod = mAsset->getNumPhysicalLods(currGrxLOD) - 1;
	}
	else
	{
	...
	if (mClothingSimulation == NULL && mForcePhysicalLod > 0)
	{
		...
		{
			const PhysicalLod* physicalLod = mAsset->getPhysicalLodData(mCurrentGraphicalLodId, mForcePhysicalLod);
			if( physicalLod )
			{
			...
			{
				APEX_INTERNAL_ERROR("No physicalLod. This might mean an asset is broken.");
				mMaxDistReduction = mAsset->getBiggestMaxDistance();
			}
		}
	}
...
void ClothingActor::getPhysicalMeshPositions(void* buffer, PxU32 byteStride)
{
	...
	if (matrices != NULL)
	{
		...
		for (PxU32 vertexIndex = numSimulatedVertices; vertexIndex < numVertices; ++vertexIndex, ++it)
		{
			...
			const PxU8 numBones = (optimizationData[vertexIndex / 2] >> shift) & 0x7;
...
const PxVec3* ClothingActor::getSimulationPositions() const
{
	if (mClothingSimulation == NULL)
		return NULL;
...
const PxVec3* ClothingActor::getSimulationNormals() const
{
	if (mClothingSimulation == NULL)
		return NULL;
...
void ClothingActor::updateScaledGravity(PxF32 substepSize)
{
	if (mPhysXScene != NULL && mClothingScene != NULL && mClothingScene->mApexScene != NULL)
	{
		...
		bInternalScaledGravityChanged = (oldInternalScaledGravity != mInternalScaledGravity) ? 1 : 0;
	}
}
...
bool ClothingActor::clothingMaterialsEqual(ClothingMaterialLibraryParametersNS::ClothingMaterial_Type& a, ClothingMaterialLibraryParametersNS::ClothingMaterial_Type& b)
{
	...
	PX_COMPILE_TIME_ASSERT(ClothingMaterialLibraryParameters::ClassVersion == 13);
	...
	return
	    ...
	    a.zeroStretchStiffness == b.zeroStretchStiffness &&
		...
		a.horizontalStiffnessScaling.compressionRange == b.verticalStiffnessScaling.compressionRange &&
		a.horizontalStiffnessScaling.stretchRange == b.verticalStiffnessScaling.stretchRange &&
	    a.horizontalStiffnessScaling.scale == b.horizontalStiffnessScaling.scale &&
		a.bendingStiffnessScaling.compressionRange == b.verticalStiffnessScaling.compressionRange &&
		a.bendingStiffnessScaling.stretchRange == b.verticalStiffnessScaling.stretchRange &&
	    a.bendingStiffnessScaling.scale == b.bendingStiffnessScaling.scale &&
		a.shearingStiffnessScaling.compressionRange == b.verticalStiffnessScaling.compressionRange &&
		a.shearingStiffnessScaling.stretchRange == b.verticalStiffnessScaling.stretchRange &&
...
{
	const PxVec3& eyePos = mClothingScene->mApexScene->getEyePosition();
...
void ClothingActor::updatePvd()
{
	...
	if (pvdBinding != NULL)
	{
		...
		{	
			pvdBinding->lock();
			physx::debugger::comm::PvdDataStream* pvdStream = pvdBinding->getDataStream();
			physx::debugger::renderer::PvdImmediateRenderer* pvdRenderer = pvdBinding->getRenderer();
			...
			if (pvdStream != NULL && pvdRenderer != NULL)
			{
				...
				{
					mClothingSimulation->updatePvd(*pvdStream, *pvdRenderer, pvdInstance);
				}
			}
			pvdBinding->unlock();
		}
...
void ClothingActor::visualize()
{
	...
	if (mClothingSimulation != NULL)
	{
		...
		if (physicsMeshWireScale > 0.0f || physicsMeshSolidScale > 0.0f)
		{
			...
			const PxU32 numIndicesPerPrim = physicalMesh->isTetrahedralMesh ? 4 : 3;
			...
			&&	(!mAsset->getModule()->useSparseSelfCollision() || mClothingSimulation->getType() != SimulationType::CLOTH3x))
		{
			...
			{
				visualizeSpheres(renderDebug, positions, mClothingSimulation->sdkNumDeformableVertices, 0.5f*material->selfcollisionThickness, colorRed, mClothingScene->mClothingDebugRenderParams->SelfCollisionWire);
			}
		}
		/*
		...
		for (PxU32 g = 0; g < mGraphicalMeshes.size(); g++)
		{
			...
			if (graphicalLod != NULL && (graphicalLod->skinClothMapB.buf != NULL || graphicalLod->skinClothMap.buf != NULL))
			{
				...
				if (mClothingScene->mClothingDebugRenderParams->SkinMapBad)
				{
					mAsset->visualizeSkinClothMap(renderDebug, pcm,
					                              graphicalLod->skinClothMapB.buf, graphicalLod->skinClothMapB.arraySizes[0],
					                              graphicalLod->skinClothMap.buf, graphicalLod->skinClothMap.arraySizes[0], mActorDesc->actorScale, true, false);
				}
				if (mClothingScene->mClothingDebugRenderParams->SkinMapActual)
				{
					mAsset->visualizeSkinClothMap(renderDebug, pcm,
					                              graphicalLod->skinClothMapB.buf, graphicalLod->skinClothMapB.arraySizes[0],
					                              graphicalLod->skinClothMap.buf, graphicalLod->skinClothMap.arraySizes[0], mActorDesc->actorScale, false, false);
				}
				if (mClothingScene->mClothingDebugRenderParams->SkinMapInvalidBary)
				{
					mAsset->visualizeSkinClothMap(renderDebug, pcm,
						graphicalLod->skinClothMapB.buf, graphicalLod->skinClothMapB.arraySizes[0],
						graphicalLod->skinClothMap.buf, graphicalLod->skinClothMap.arraySizes[0], mActorDesc->actorScale, false, true);
				}
			...
			if (graphicalLod != NULL && mClothingScene->mClothingDebugRenderParams->RecomputeSubmeshes)
			{
				...
				for (PxU32 s = 0; s < rma->getSubmeshCount(); s++)
				{
					...
					PxU32 maxForColor = graphicalLod->physicsSubmeshPartitioning.arraySizes[0] + 2;
					{
						const PxU8 colorPart = (PxU8)((255 / maxForColor) & 0xff);
						renderDebug.setCurrentColor(colorPart | colorPart << 8 | colorPart << 16);
					}
			...
			if (graphicalLod != NULL && mClothingScene->mClothingDebugRenderParams->RecomputeVertices)
			{
				...
				for (PxU32 s = 0; s < rma->getSubmeshCount(); s++)
				{
					...
					if(mGraphicalMeshes[mCurrentGraphicalLodId].renderProxy == NULL)
						continue;
					const PxVec3* positions = mGraphicalMeshes[mCurrentGraphicalLodId].renderProxy->renderingDataPosition + submeshVertexOffset;
					const PxVec3* normals = mGraphicalMeshes[mCurrentGraphicalLodId].renderProxy->renderingDataPosition + submeshVertexOffset;
...
{
	PX_PROFILER_PERF_SCOPE("ClothingActor::updateBoneBuffer");
	if (renderProxy == NULL)
		return;
...
void ClothingActor::updateStateInternal_NoPhysX(bool prepareForSubstepping)
{
	...
	if (bHasBones)
	{
		...
		PxI32 rootNodeExternalIndex = mActorDesc->useInternalBoneOrder ? mAsset->getRootBoneIndex() : mAsset->getBoneExternalIndex(mAsset->getRootBoneIndex());
		if (rootNodeExternalIndex < mActorDesc->boneMatrices.arraySizes[0])
		{
		...
		if (mActorDesc->boneMatrices.arraySizes[0] >= (PxI32)numBones)
		{
			...
			if (mAsset->writeBoneMatrices(pose, mActorDesc->boneMatrices.buf, sizeof(PxMat44), mActorDesc->boneMatrices.arraySizes[0],
	...
	bGlobalPoseChanged = (mInternalGlobalPose != mOldInternalGlobalPose) ? 1 : 0;

	// set bBoneBufferDirty to 1 if any matrices have changed
	bBoneBufferDirty = (bGlobalPoseChanged == 1) || (bBoneMatricesChanged == 1) || (bBoneBufferDirty == 1) ? 1 : 0;
}
...
void ClothingActor::skinPhysicsMeshInternal(bool useInterpolatedMatrices, PxF32 substepFraction)
{
	...
	const PhysicalSubmesh* physicalSubMesh = mAsset->getPhysicalSubmesh(mCurrentGraphicalLodId, mCurrentPhysicsSubmesh);
	...
	invalidateCache(targetPositions, numVertices * sizeof(PxVec3));
	invalidateCache(targetNormals, numVertices * sizeof(PxVec3));
...
void ClothingActor::initializeActorData()
{
	...
	if (!mData.bIsInitialized && mClothingSimulation != NULL)
	{
		...
		mData.mMorphDisplacementBufferCount = mActorDesc->morphDisplacements.arraySizes[0];
	...
	if (mData.bIsInitialized && mClothingSimulation != NULL)
	{
		...
		for (PxU32 a = 0; a < mData.mAsset.mGraphicalLodsCount; ++a)
		{
			...
			for (PxU32 b = 0; b < pLod->mSubMeshCount; b++)
			{
				...
				const PxU32 numParts = mAsset->getGraphicalLod(a)->physicsSubmeshPartitioning.arraySizes[0];
		...
		mData.mCurrentPhysicsSubmesh = this->mCurrentPhysicsSubmesh;
...
void ClothingActor::fillWritebackData_LocksPhysX(const WriteBackInfo& writeBackInfo)
{
	...
	if (matrices != NULL)
	{
		...
		for (PxU32 i = numCopyVertices; i < numVertices; i++)
		{
			...
			const PxU8 numBones = (optimizationData[i / 2] >> shift) & 0x7;
...
void ClothingActor::applyVelocityChanges_LocksPhysX(PxF32 simulationDelta)
{
	...
	if (pressure >= 0.0f)
	{
		...
		const PhysicalSubmesh* submesh = mAsset->getPhysicalSubmesh(mCurrentGraphicalLodId, mCurrentPhysicsSubmesh);
...
void ClothingActor::getSimulation(const WriteBackInfo& writeBackInfo)
{
	...
	{
		ClothingAssetParametersNS::CookedEntry_Type* cookedEntry = mAsset->getCookedData(mActorDesc->actorScale);
		cookingInterface = cookedEntry->cookedData;
		cookedActorScale = cookedEntry->scale;
		PX_ASSERT(factory->isMatch(cookingInterface->className()));
	}

	mClothingSimulation = mAsset->getSimulation(physicalMeshId, mCurrentPhysicsSubmesh, cookingInterface, mClothingScene);
	if (mClothingSimulation != NULL)
	{
	...
	else if (cookingInterface != NULL)
	{
		...
		createSimulation(physicalMeshId, mCurrentPhysicsSubmesh, cookingInterface, cookedActorScale, writeBackInfo);
	}
...
void ClothingActor::createSimulation(PxU32 physicalMeshId, PxI32 submeshId, NxParameterized::Interface* cookedData, PxF32 cookedActorScale, const WriteBackInfo& writeBackInfo)
{
	PX_ASSERT(mClothingSimulation == NULL);
	...
	if (mClothingSimulation != NULL)
	{
		const PhysicalSubmesh* submesh = mAsset->getPhysicalSubmesh(mCurrentGraphicalLodId, submeshId);
		...
		success = mClothingSimulation->setCookedData(cookedData, cookedActorScale, mActorDesc->actorScale);
		...
		ClothingMaterialLibraryParametersNS::ClothingMaterial_Type* clothingMaterial = getCurrentClothingMaterial();
		success &= mClothingSimulation->initPhysics(mPhysXScene, physicalMeshId, mCurrentPhysicsSubmesh, indices, vertices, clothingMaterial, mInternalGlobalPose, mInternalScaledGravity, bInternalLocalSpaceSim == 1);
...
void ClothingActor::lodTick_LocksPhysX(PxF32 simulationDelta)
{
	...
	if (!mClothingScene->getNxModule()->getLODEnabled())
	{
		...
		mCurrentPhysicsLod = mForcePhysicalLod;
	}
	...
	/* original code from apex
	PxF32 maxDistReductionTarget = FLT_MAX;
	*/

	// custom fix from nv: Hermes Lanker
	...
	{
		maxDistReductionTarget = physicalLod != NULL ? physicalLod->maxDistanceReduction : FLT_MAX;
		const ClothingPhysicalMeshParametersNS::PhysicalMesh_Type* physicalMesh = mAsset->getPhysicalMeshFromLod(mCurrentGraphicalLodId);
	...
	// switch immediately when increasing physx lod or physx lod == 0 or graphical lod has changed, otherwise wait until finished blending
	if (mMaxDistReduction >= maxDistReductionTarget || graphicalLodChanged)
	{
		ClothingMaterialLibraryParametersNS::ClothingMaterial_Type* clothingMaterial = getCurrentClothingMaterial();
		const PxU32 solverIterations = clothingMaterial != NULL ? clothingMaterial->solverIterations : 5;
		...
		if (actorCooked && mCurrentSolverIterations > 0)
		{
			...
			mCurrentPhysicsSubmesh = physicalLod->submeshId;
...
PxU32 ClothingActor::getCollisionSphereCount()
{
	PxU32 result = 0;
	result += mCollisionSpheres.getSize();
	result += mCollisionCapsules.getSize() * 2;
	return result;
}

PxU32 ClothingActor::getCollisionPlaneCount()
{
	PxU32 result = 0;
	result += mCollisionPlanes.getSize();
	PxU32 collisionCount = mCollisionConvexes.getSize();
	for( PxU32 i = 0; i != collisionCount; ++i )
	{
		NxClothingConvex* resource = ( ClothingConvex* ) mCollisionConvexes.getResource( i );
		if( resource )
		{
			result += resource->getNumPlanes();
		}
	}
	return result;
}

PxU32 ClothingActor::getCollisionTriangleCount()
{
	PxU32 result = 0;
	PxU32 collisionCount = mCollisionTriangleMeshes.getSize();
	for( PxU32 i = 0; i != collisionCount; ++i )
	{
		NxClothingTriangleMesh* resource = ( ClothingTriangleMesh* ) mCollisionTriangleMeshes.getResource( i );
		if( resource )
		{
			result += resource->getNumTriangles();
		}
	}
	return result;
}
...
void ClothingActor::clearCollision(ClothingTriangleMesh& triMesh)
{
	bActorCollisionChanged = 1;
	if (mClothingSimulation != NULL)
	{
		mClothingSimulation->releaseCollision(triMesh);
	}
}
