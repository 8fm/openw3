...
void ClothingScene::simulate(PxF32 elapsedTime)
{
	for (PxU32 i = 0; i < mActorArray.size(); i++)
	{
		...
		//RED++
		if( !clothingActor->isVisibleBuffered() )
		{
			//APEX_DEBUG_INFO("task during skipped\n");
			continue;
		}
		//RED--
...
void ClothingScene::fetchResults()
{
	...
	for (PxI32 i = mActorArray.size()-1; i >= 0 ; --i)
	{
		ClothingActor* actor = static_cast<ClothingActor*>(mActorArray[i]);

		//RED++
		if( !actor->isVisibleBuffered() )
		{
			//APEX_DEBUG_INFO("fetch skipped \n");
			continue;
		}
		//RED--
	...
	if (!mModule->allowAsyncFetchResults())
	{
		for (PxU32 i = 0; i < mActorArray.size(); i++)
		{
			...
			if( !clothingActor->isVisibleBuffered() )
			{
				//APEX_DEBUG_INFO("waitForFetchResults skipped \n");
				continue;
			}
			//RED--
...
NxCompartment* ClothingScene::getClothCompartment_LocksPhysX(bool hw)
{
	...
	else
	{
		if (mClothSwCompartments.size() <= mNextClothSwCompartmentId)
		{
			...
			NxCompartmentDesc compartmentDesc;
			compartmentDesc.type = NX_SCT_CLOTH;
			compartmentDesc.deviceCode = (physx::PxU32)NX_DC_CPU;
			compartment = mPhysXScene->createCompartment(compartmentDesc);
			if (compartment != NULL)
			{
...
void ClothingScene::unregisterAsset(ClothingAsset* asset)
{
	...
	mClothingAssetsMutex.lock();
	for (PxI32 i = mClothingAssets.size() - 1; i >= 0; i--)
	{
		if (mClothingAssets[i] == asset)
		{
			mClothingAssets.replaceWithLast(i);
		}
	}
	mClothingAssetsMutex.unlock();
	...
	for (PxU32 i = 0; i < numGraphicalMeshes; ++i)
	{
		...
		for (PxI32 i = renderProxies.size()-1; i >= 0 ; --i)
		{
			ClothingRenderProxy* renderProxy = renderProxies[i];
			if (renderProxy->getTimeInPool() > 0)
			{
				PX_DELETE(renderProxies[i]);
			}
			else
			{
...
{
	for (physx::PxU32 i = 0; i < mRegisteredCompartments.size(); i++)
	{
...
void ClothingScene::destroy()
{
	...
	for(HashMap<NiApexRenderMeshAsset*, Array<ClothingRenderProxy*> >::Iterator iter = mRenderProxies.getIterator(); !iter.done(); ++iter)
	{
		...
		for (PxI32 i = renderProxies.size()-1; i >= 0 ; --i)
		{
			PxU32 timeInPool = renderProxies[i]->getTimeInPool();
			if (timeInPool > 0)
			{
				PX_DELETE(renderProxies[i]);
				renderProxies.replaceWithLast(i);
			}
			else
			{
				...
				renderProxies[i]->notifyAssetRelease();
			}
		}
...
void ClothingScene::setSceneRunning(bool on)
{
	...
	else
	{
		...
		// warn if simulation time was bigger than timestep
		PxF32 simulatedTime = 1000.0f*mApexScene->getElapsedTime();
		if (simulatedTime < dataVal.Float)
		{
//			APEX_DEBUG_WARNING("Cloth complexity in scene is too high to be simulated in real time. (Delta Time: %f ms, Simulation Time: %f ms)", simulatedTime, dataVal.Float);
		}
...
void ClothingScene::embeddedPostSim()
{
	for (PxU32 i = 0; i < mActorArray.size(); i++)
	{
		...
		if( !clothingActor->isVisibleBuffered() )
		{
			//APEX_DEBUG_INFO("startFetchTasks skipped.\n");
			continue;
		}
		//RED--
	...
	for(HashMap<NiApexRenderMeshAsset*, Array<ClothingRenderProxy*> >::Iterator iter = mRenderProxies.getIterator(); !iter.done(); ++iter)
	{
		...
		for (PxI32 i = renderProxies.size()-1; i >= 0 ; --i)
		{
			...
			if (timeInPool > 0)
			{
				...
				{
					renderProxies[i]->setTimeInPool(timeInPool+1);
				}
			}
		}
	}
