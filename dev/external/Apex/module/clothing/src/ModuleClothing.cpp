...
class DummyActor : public NxApexActor, public physx::UserAllocated
{
public:
...
class DummyAsset : public NxApexAsset, public physx::UserAllocated
{
public:
        ...
        bool mergeVertices, bool closeHoles, IProgressListener* progressListener)
{
	...
	progress.setSubtaskWork(times[0], "Creating single layered mesh");
	...
	for (PxU32 submeshNr = 0; submeshNr < renderMeshAsset->getSubmeshCount(); submeshNr++)
	{
		...
		PxU32 customIndex = vf.getBufferIndexFromID(vf.getID("USED_FOR_PHYSICS"));
		...
		customIndex = vf.getBufferIndexFromID(vf.getID("LATCH_TO_NEAREST_SLAVE"));
		...
		customIndex = vf.getBufferIndexFromID(vf.getID("LATCH_TO_NEAREST_MASTER"));
		outFormat = vf.getBufferFormat(customIndex);
		...
		for (PxU32 meshIndex = 0; meshIndex < submeshIndices; meshIndex += 3)
		{
			...
			for (PxU32 i = 0; i < 3; i++)
			{
				...
				if (old2New[index] == -1)
				{
					old2New[index] = nbVertices++;
					PxU32 master = latchToNearestMaster != NULL ? latchToNearestMaster[localIndex] : 0xffffffff;
					subdivider.registerVertex(positions[localIndex], master);
				}
			}

			const PxU32 i0 = old2New[indices[meshIndex + 0] + vertexOffset];
			const PxU32 i1 = old2New[indices[meshIndex + 1] + vertexOffset];
			const PxU32 i2 = old2New[indices[meshIndex + 2] + vertexOffset];
	...
	if (mergeVertices)
	{
		progress.setSubtaskWork(times[1], "Merging");
	...
	if (closeHoles)
	{
		progress.setSubtaskWork(times[2], "Closing holes");
	...
	{
		progress.setSubtaskWork(times[3], "Subdividing");
	...
	physicalMesh->setGeometryInternal(false, newVertices.size(), sizeof(PxVec3), newVertices.begin(), newMasterValues.begin(), newIndices.size(), sizeof(PxU32), &newIndices[0]);
