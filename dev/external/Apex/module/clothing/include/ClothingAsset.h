...
struct ClothingGraphicalMeshAssetWrapper
{
	...
	PxU32 getNumBonesPerVertex(PxU32 submeshIndex) const
	{
		...
		if (submeshIndex < meshAsset->getSubmeshCount())
		{
			...
			return vertexSemanticFormatElementCount(NxRenderVertexSemantic::BONE_INDEX, format.getBufferFormat(format.getBufferIndexFromID(format.getSemanticID(NxRenderVertexSemantic::BONE_INDEX))));
		}
		return 0;
	}
	...
	const void* getVertexBuffer(PxU32 submeshIndex, NxRenderVertexSemantic::Enum semantic, NxRenderDataFormat::Enum& outFormat) const
	{
		...
		physx::PxU32 bufferIndex = vf.getBufferIndexFromID(vf.getSemanticID((NxRenderVertexSemantic::Enum)semantic));
	...
	bool hasChannel(const char* bufferName = NULL, NxRenderVertexSemantic::Enum semantic = NxRenderVertexSemantic::NUM_SEMANTICS) const
	{
		...
		for (PxU32 i = 0; i < meshAsset->getSubmeshCount(); i++)
		{
			...
			outFormat = format.getBufferFormat(format.getBufferIndexFromID(id));
...
class ClothingAsset : public NxClothingAsset, public NxApexResource, public ApexResource, public ClothingCookingLock, public NxParameterized::SerializationCallback
{
protected:
...
public:
	...
	// cooked stuff
	ClothingAssetParametersNS::CookedEntry_Type*	getCookedData(PxF32 actorScale);
...
protected:
	...
	PxF32											getMaxMaxDistance(ClothingPhysicalMeshParametersNS::PhysicalMesh_Type& physicalMesh, PxU32 index, PxU32 numIndices) const;
	...
	bool											findTriangleForImmediateVertex(PxU32& faceIndex, PxU32& indexInTriangle, PxU32 physVertIndex, ClothingPhysicalMeshParametersNS::PhysicalMesh_Type& physicalMesh) const;
