...
namespace
{
	...
	void radixSort(const uint32_t* first, const uint32_t* last, uint16_t* out)
	{
		...
		{
			uint16_t temp0 = histograms[0][i] + sums[0];
			histograms[0][i] = sums[0], sums[0] = temp0;

			uint16_t temp1 = histograms[1][i] + sums[1];
			histograms[1][i] = sums[1], sums[1] = temp1;

			uint16_t temp2 = histograms[2][i] + sums[2];
			histograms[2][i] = sums[2], sums[2] = temp2;

			uint16_t temp3 = histograms[3][i] + sums[3];
...
void cloth::SwSelfCollision<Simd4f>::operator()()
{
	...
	for (uint32_t i=0; i < numIndices; ++i)
	{
		...
		keys[i] = ptr[sweepAxis] | (ptr[hashAxis0] << 16) | (ptr[hashAxis1] << 24);
	}
	...
	uint32_t collisionDistance = 2 + simdi::array(
		intFloor(gridScale * mCollisionDistance))[sweepAxis];
