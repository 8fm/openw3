...
#if defined(PX_VC10) && defined(PX_WINDOWS)
...
template <>
Range<PxVec4> ClothImpl<SwCloth>::getCurrentParticles(Memory)
{
	PxVec4* data = &mCloth.mCurParticles.front();
	return Range<PxVec4>(data, data + mCloth.mCurParticles.size());
}

template <>
Range<const PxVec4> ClothImpl<SwCloth>::getCurrentParticles(Memory) const
{
	const PxVec4* data = &mCloth.mCurParticles.front();
	return Range<const PxVec4>(data, data + mCloth.mCurParticles.size());
}

template <>
Range<PxVec4> ClothImpl<SwCloth>::getPreviousParticles(Memory)
{
	PxVec4* data = &mCloth.mPrevParticles.front();
	return Range<PxVec4>(data, data + mCloth.mPrevParticles.size());
}

template <>
Range<const PxVec4> ClothImpl<SwCloth>::getPreviousParticles(Memory) const
{
	const PxVec4* data = &mCloth.mPrevParticles.front();
	return Range<const PxVec4>(data, data + mCloth.mPrevParticles.size());
}
...
template <>
Range<PxVec4> ClothImpl<SwCloth>::getRestPositions()
{
	if(mCloth.mRestPositions.empty())
	{
		uint32_t n = mCloth.mCurParticles.size();
		mCloth.mRestPositions.resize(n, PxVec4(0.0f));
	}

	mCloth.wakeUp();

	PxVec4* data = &mCloth.mRestPositions.front();
	return Range<PxVec4>(data, data + mCloth.mRestPositions.size());
}

template <>
void ClothImpl<SwCloth>::clearRestPositions()
{
	Vec4fAlignedVector().swap(mCloth.mRestPositions);
	mCloth.wakeUp();
}
...
void ClothImpl<SwCloth>::setVirtualParticles(Range<const uint32_t[4]> indices, Range<const PxVec3> weights)
{
	...
	Vec4us dummy(numParticles, numParticles+1, numParticles+2, 0); 
