...
public:
	...
	virtual Range<PxVec4> getCurrentParticles(Memory = DEVICE_MEMORY);
	virtual Range<const PxVec4> getCurrentParticles(Memory = DEVICE_MEMORY) const;
	virtual Range<PxVec4> getPreviousParticles(Memory = DEVICE_MEMORY);
	virtual Range<const PxVec4> getPreviousParticles(Memory = DEVICE_MEMORY) const;
	...
	virtual Range<PxVec4> getRestPositions();
	virtual void clearRestPositions();
...
template <typename T>
void ClothImpl<T>::teleport(const PxVec3& delta)
{
...
{
	PxVec3 value = safeLog2(PxVec3(1) - damping);
...
{
	return PxVec3(1) - safeExp2(mCloth.mLogDamping);
}

template <typename T>
inline void ClothImpl<T>::setLinearDrag(const PxVec3& drag)
{
	PxVec3 value = safeLog2(PxVec3(1) - drag);
...
{
	return PxVec3(1) - safeExp2(mCloth.mLinearLogDrag);
}
...
{
	return PxVec3(1) - safeExp2(mCloth.mAngularLogDrag);
}
...
inline void ClothImpl<T>::setSpheres( Range<const PxVec4> spheres, uint32_t first, uint32_t last )
{
	...
	else
	{
		...
		for(uint32_t i=mCloth.mTargetCollisionSpheres.size(); i<oldSize; ++i)
			mCloth.mTargetCollisionSpheres.pushBack(mCloth.mStartCollisionSpheres[i]);

		// resize to larger of oldSize and newSize
		mCloth.mStartCollisionSpheres.resize(PxMax(oldSize, newSize),PxVec4(0.0f));
		mCloth.mTargetCollisionSpheres.resize(PxMax(oldSize, newSize),PxVec4(0.0f));

		if(int32_t delta = newSize-oldSize)
		{
			// move past-range elements to new place
			move(mCloth.mStartCollisionSpheres.begin(), last, oldSize, last + delta);
			move(mCloth.mTargetCollisionSpheres.begin(), last, oldSize, last + delta);

			// fill new elements from spheres
			for(uint32_t i=last; i<last+delta; ++i)
				mCloth.mStartCollisionSpheres[i] = spheres[i-first];

			// adjust capsule indices
			Vector<IndexPair>::Type::Iterator cIt, cEnd = mCloth.mCapsuleIndices.end();
			for(cIt = mCloth.mCapsuleIndices.begin(); cIt != cEnd;)
			{
				bool removed = false;
				removed |= updateIndex(cIt->first, last + PxMin(0, delta), delta);
				removed |= updateIndex(cIt->second, last + PxMin(0, delta), delta);
				if(!removed)
					++cIt;
				else
				{
					mCloth.mCapsuleIndices.replaceWithLast(cIt);
					cEnd = mCloth.mCapsuleIndices.end();
				}
			}

			mCloth.mStartCollisionSpheres.resize(newSize);
			mCloth.mTargetCollisionSpheres.resize(newSize);
		...
		for(uint32_t i=0; i<spheres.size(); ++i)
			mCloth.mTargetCollisionSpheres[first+i] = spheres[i];
	}
...
template <typename T>
void ClothImpl<T>::setCapsules( Range<const uint32_t> capsules, uint32_t first, uint32_t last )
{
	...
	const IndexPair* indices = reinterpret_cast<const IndexPair*>(capsules.begin());
	...
	{
		// move past-range elements to new place
		move(mCloth.mCapsuleIndices.begin(), last, oldSize, last + delta);

		// fill new elements from capsules
		for(uint32_t i=last; i<last+delta; ++i)
			mCloth.mCapsuleIndices[i] = indices[i-first];

		mCloth.mCapsuleIndices.resize(newSize);
		mCloth.notifyChanged();
	}
...
inline void ClothImpl<T>::setTriangles(Range<const PxVec3> startTriangles, Range<const PxVec3> targetTriangles, uint32_t first)
{
	...
	uint32_t last = uint32_t(mCloth.mStartCollisionTriangles.size() / 3);
...
template <typename T>
void* ClothImpl<T>::getUserData() const
{
