...
#if _MSC_VER >= 1600 && defined(PX_WINDOWS)
#define PX_AVX
...
#if _MSC_FULL_VER < 160040219 // need at least VC10 SP1
		return 0;
#else
		int cpuInfo[4];
		__cpuid(cpuInfo, 1);
...
namespace
{
	...
	const Simd4fConstant sSignXYW = simd4f(simd4i(0x80000000, 0x80000000, 0, 0x80000000)); 
		...
		const Simd4f* __restrict curEnd, const ConstraintIterator& spheres)
	{
		...
		for(; curIt < curEnd; curIt += 4)
		{
			...
			Simd4f slack = simd4f(_1) - deltaW * rsqrt<1>(sqrLength);
		...
		const float* __restrict rEnd, const uint16_t* __restrict iIt, Simd4f stiffness)
	{
		...
		for(; rIt != rEnd; rIt += 4, iIt += 8)
		{
			...
			Simd4f erij = simd4f(_1) - rij * rsqrt(e2ij) & rij > sEpsilon;
...
void cloth::SwSolverKernel<Simd4f>::solveFabric()
{
	...
	for(; cIt != cEnd; ++cIt)
	{
		PX_ASSERT(cIt->mPhaseIndex < mClothData.mNumPhases);
		const uint32_t* sIt = sBegin + pBegin[cIt->mPhaseIndex];
		PX_ASSERT(sIt < sBegin + mClothData.mNumSets);
