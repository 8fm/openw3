...
typedef shdfnd::HashMap<PxU32, cloth::Fabric*> FabricMap;
...
PxU32 SimulationPhysX3::getNumSolverIterations() const
{
	...
	if (mCloth != NULL)
	{
		numSolverIterations = PxMax(1, int(mLastTimestep * mCloth->getSolverFrequency() + 0.5f));
	}
...
bool SimulationPhysX3::setCookedData(NxParameterized::Interface* cookedData, PxF32 cookedActorScale, PxF32 actorScale)
{
	PX_ASSERT(cookedData != NULL);

	mActorScale = actorScale;
	PX_ASSERT(mActorScale > 0.0f);
	mCookedActorScale = cookedActorScale;
...
#endif
{
	...
	if (mCookedData != NULL)
	{
		...
		PxF32 scaleRatio = mActorScale / mCookedActorScale;
		float restLengthScale = simulation.restLengthScale * scaleRatio;

		void*& fabricMapPtr = factory.factory->getPlatform() == cloth::Factory::CPU ? mCookedData->fabricCPU : mCookedData->fabricGPU;
		if(fabricMapPtr == NULL)
		{
			fabricMapPtr = new(PX_ALLOC(sizeof(FabricMap), "FabricMap")) FabricMap;
		}
		cloth::Fabric*& fabricPtr = (*(FabricMap*)fabricMapPtr)[PxU32(restLengthScale * 100)];
		if(fabricPtr == NULL)
		{
			shdfnd::Array<PxU32> phases(mCookedData->deformablePhaseDescs.arraySizes[0]);
			for (PxU32 i = 0; i < phases.size(); i++)
				phases[i] = mCookedData->deformablePhaseDescs.buf[i].setIndex;
			shdfnd::Array<PxU32> sets(mCookedData->deformableSets.arraySizes[0]);
			...
			fabricPtr = factory.factory->createFabric(
			...
			if (restLengthScale != 1.0f && fabricPtr != NULL)
			{
				fabricPtr->scaleRestvalues( restLengthScale );
				fabricPtr->scaleTetherLengths( restLengthScale );
			}
		}

		if (fabricPtr != NULL && mCloth == NULL)
		{
			...
			mCloth = factory.factory->createCloth(startPos, *fabricPtr);
		}
		...
		if (mCloth != NULL)
		{
			...
			const PxU32 numVirtualParticleIndices = mCookedData->virtualParticleIndices.arraySizes[0];
			const PxU32 numVirtualParticleWeights = mCookedData->virtualParticleWeights.arraySizes[0];
			...
			const PxU32 numSelfcollisionIndices = mCookedData->selfCollisionIndices.arraySizes[0];
	...
	const PxU32 numPhaseDescs = mCookedData->deformablePhaseDescs.arraySizes[0];
...
{
	// nVidia hotfix Hermes Lanker 
	// cdp implementation Lukasz Horzelski
	// case: When simulating clothing in local space and if in this case we will pass
	// open trimesh geometry to the clothing (like terrain) then during graphical LOD switching
	// clothing is strechted becasue collision wasnt initialize correctly
	// It works well for cloesed trimeshes/boxes/spheres/capsules etc but not for opened trimeshes.

	mLocalSpaceSim = localSpaceSim;
	setGlobalPose(globalPose); // initialize current frame
	setGlobalPose(globalPose); // initialize previous frame

	// end of nVidia hotfix
									   ...
									   bool /*teleport*/)
{
	...
	for (PxU32 i = 0; i < actorCapsules.getSize(); ++i)
	{
		...
		PxI32 s0 = spheres[0]->getId();
		PxI32 s1 = spheres[1]->getId();
		if (s0 > 32 || s1 > 32)
		{
	...
	for (PxU32 i = 0; i < actorPlanes.getSize(); ++i)
	{
		...
		{
			mCollisionConvexes.pushBack(1 << planeId);
		}
	}
	...
	// triangles
	if (mReleasedTriangleMeshes.size() > 0)
	{
		// make sure the order of id's doesn't change
		CollisionCompare compare;
		actorTriangleMeshes.sort(compare);
	}
	...
	for (PxU32 i = 0; i < actorTriangleMeshes.getSize(); ++i)
	{
		...
		PxU32 numTriangles = mesh->getNumTriangles();
		if (numTriangles == 0)
			continue;

		PX_ASSERT(mCollisionTriangles.size() % 3 == 0);
		mesh->setId(mCollisionTriangles.size() / 3);

		const PxVec3* triangles = mesh->getTriangles();
		...
		for (PxU32 j = 0; j < 3*numTriangles; ++j)
		{
 			mCollisionTriangles.pushBack(tm.transform(triangles[j]));
		}
	}
}
...
void SimulationPhysX3::releaseCollision(ClothingCollision& collision)
{
	...
	if (sphere != NULL)
	{
		...
		if (id != -1)
		{
			mReleasedSphereIds.pushBack(id);
		}
		return;
	}
	...
	if (plane != NULL)
	{
		...
		if (id != -1)
		{
			mReleasedPlaneIds.pushBack(id);
		}
		return;
	}

	ClothingTriangleMesh* mesh = DYNAMIC_CAST(ClothingTriangleMesh*)(collision.isTriangleMesh());
	if (mesh != NULL)
	{
		PxI32 id = mesh->getId();
		PxU32 numTriangles = mesh->getNumTriangles();
		if (id != -1 && numTriangles > 0)
		{
			mReleasedTriangleMeshes.pushBack(TriangleMeshId(id, numTriangles));
		}
		return;
	}
}
...
void SimulationPhysX3::fetchResults(bool computePhysicsMeshNormals)
{
	...
	{
		{
			cloth::Range<PxVec4> particles = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
...
void SimulationPhysX3::attachVertexToGlobalPosition(PxU32 vertexIndex, const PxVec3& globalPosition)
{
	...
	cloth::Range<PxVec4> curParticles = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
	cloth::Range<PxVec4> prevParticles = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
...
void SimulationPhysX3::freeVertex(PxU32 vertexIndex)
{
	...
	cloth::Range<PxVec4> curParticles = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
	cloth::Range<PxVec4> prevParticles = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
...
void SimulationPhysX3::visualize(NiApexRenderDebug& renderDebug, ClothingDebugRenderParams& clothingDebugParams)
{
	        ...
	        clothingDebugParams.ShearingFibers)
	{
		...
		for (PxU32 pc = 0; pc < mPhaseConfigs.size(); ++pc)
		{
			...
			PxU32 rangeColor = brightness | (brightness << 8) | (brightness << 16);
	...
	if (clothingDebugParams.TethersActive || clothingDebugParams.TethersInactive)
	{
		...
		const PxU32 numTetherAnchors = mCookedData->tetherAnchors.arraySizes[0];
	...
	{
		cloth::Range<const PxVec4> curParticles = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
		cloth::Range<const PxVec4> prevParticles = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
	...
	if (clothingDebugParams.SelfCollision && module->useSparseSelfCollision())
	{
		...
		PxU32 numIndices = mCookedData->selfCollisionIndices.arraySizes[0];
	...
	if (clothingDebugParams.SelfCollisionAttenuation > 0.0f)
	{
		...
		for (PxU32 i = 0; i < mSelfCollisionAttenuationPairs.size(); i += 2)
		{
			...
			{
				renderDebug.setCurrentColor(c << 16 | c << 8 | c);
			}
...
#ifndef WITHOUT_PVD
void SimulationPhysX3::updatePvd(physx::debugger::comm::PvdDataStream& /*pvdStream*/, PvdImmediateRenderer& pvdRenderer, NxApexResource* clothingActor)
{
	// update rendering
...
void SimulationPhysX3::getVelocities(PxVec3* velocities) const
{
	...
	const PxReal invTimeStep = 1.0f / mCloth->getPreviousIterationDt();

	const cloth::Range<PxVec4> newPositions = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
	const cloth::Range<PxVec4> oldPositions = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
...
void SimulationPhysX3::setVelocities(PxVec3* velocities)
{
	...
	cloth::Range<PxVec4> newPositions = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
	cloth::Range<PxVec4> oldPositions = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY); // read the data, the .w is vital!
	...
	if (adaption > 0.0f)
	{
		...
		for (PxU32 i = 0; i < sdkNumDeformableVertices; i++)
		{
			...
			if (coeffs[i].maxDistance > 0.0f && !dv.isZero())
			{
				...
				normalizedDv *= NxClothingUserRecompute::invSqrt(normalizedDv.magnitudeSquared());
...
{
	if (mCloth != NULL && weight > 0.0f)
	{
		mTeleported = true;

		if (reset)
		{
			cloth::Range<PxVec4> curPos = mCloth->getCurrentParticles(cloth::Cloth::HOST_MEMORY);
			cloth::Range<PxVec4> prevPos = mCloth->getPreviousParticles(cloth::Cloth::HOST_MEMORY);
...
void SimulationPhysX3::setRestPositions()
{
	if (mCloth == NULL)
		return;

	cloth::Range<PxVec4> restPositions = mCloth->getRestPositions(); // need to be called before getNumRestPositions
	PxU32 numRestPositions = mCloth->getNumRestPositions();

	PxVec4* restPositionsHost = NULL;
	if (mUseCuda)
	{
		restPositionsHost = (PxVec4*)NiGetApexSDK()->getTempMemory(numRestPositions * sizeof(PxVec4));
	}
	else
	{
		restPositionsHost = restPositions.begin();
	}

	for (PxU32 i = 0; i < numRestPositions; ++i)
	{
		restPositionsHost[i] = PxVec4(mRestPositions[i], 0.0f);
	}
	...
	if (mUseCuda)
	{
		...
		PxVec4* restPositionsDevice = restPositions.begin();
		factory.factory->copyFromHost(restPositionsHost, restPositionsHost+numRestPositions, restPositionsDevice);
		
		NiGetApexSDK()->releaseTempMemory(restPositionsHost);
	}
}
...
void SimulationPhysX3::setInterCollisionChannels(PxU32 channels)
{
	if (mCloth != NULL)
	{
		mCloth->setUserData((void*)channels);
	}
}
...
namespace
{
	void releaseFabricMap(FabricMap* map)
	{
		FabricMap::Iterator it = map->getIterator();
		for(; !it.done(); ++it)
			delete it->second;
		map->~FabricMap();
		PX_FREE(map);
...
void SimulationPhysX3::releaseFabric(NxParameterized::Interface* _cookedData)
{
	if (strcmp(_cookedData->className(), ClothingCookedPhysX3Param::staticClassName()) == 0)
	{
		...
		while (cookedData != NULL)
		{
			...
			{
				releaseFabricMap(static_cast<FabricMap*>(cookedData->fabricCPU));
				cookedData->fabricCPU = NULL;
			}

			if (cookedData->fabricGPU != NULL)
			{
				releaseFabricMap(static_cast<FabricMap*>(cookedData->fabricGPU));
				cookedData->fabricGPU = NULL;
			}
...
void SimulationPhysX3::applyCollision()
{
	if (mCloth != NULL && !mIsStatic)
	{
		...
		if (numReleased > 0)
		{
			...
			for (PxI32 i = numReleased-1; i >= 0; --i)
			{
				PxU32 id = mReleasedSphereIds[i];
				if(id < 32)
				{
		...
		// triangle meshes
		numReleased = mReleasedTriangleMeshes.size();
		if (numReleased > 0)
		{
			// remove all deleted triangles
			// biggest id's first, such that we don't
			// invalidate remaining id's
			physx::sort<TriangleMeshId>(&mReleasedTriangleMeshes[0], numReleased);
			for (PxI32 meshIdx = numReleased-1; meshIdx >= 0; --meshIdx)
			{
				PxU32 triangleIdx = mReleasedTriangleMeshes[meshIdx].id;
				mCloth->setTriangles(cloth::Range<const PxVec3>(), triangleIdx, triangleIdx + mReleasedTriangleMeshes[meshIdx].numTriangles);
			}
			mReleasedTriangleMeshes.clear();
		}

		cloth::Range<const PxVec3> triangles(mCollisionTriangles.begin(), mCollisionTriangles.end());
		mCloth->setTriangles(triangles, 0, mCloth->getNumTriangles());
...
#ifdef PX_WINDOWS
	if (factory.factory->getPlatform() == cloth::Factory::GPU)
	{
		if (mSolverGPU == NULL)
		{
