...
ClothingCookedPhysX3Param* CookingPhysX3::fiberCooker(PxU32 subMeshIndex) const
{
	...
	PxU32 nbConstraints = fabric.sets[fabric.nbSets - 1];
	...
	if (success)
	{
		...
		arrayHandle.resizeArray(fabric.nbTethers);
		arrayHandle.setParamU32Array(fabric.tetherAnchors, fabric.nbTethers);

		arrayHandle.getParameter("tetherLengths");
		arrayHandle.resizeArray(fabric.nbTethers);
		arrayHandle.setParamF32Array(fabric.tetherLengths, fabric.nbTethers);
...
void CookingPhysX3::computeVertexWeights(ClothingCookedPhysX3Param* cookedData, PxU32 subMeshIndex) const
{
	...
	if (handle.resizeArray(numVertices) == NxParameterized::ERROR_NONE)
	{
		for (PxU32 i = 0; i < numVertices; i++)
		{
...
void CookingPhysX3::createVirtualParticles(ClothingCookedPhysX3Param* cookedData, PxU32 subMeshIndex)
{
	...
	if (!particles.empty())
	{
		...
		handle.resizeArray(particles.size() * 4);
...
void CookingPhysX3::createSelfcollisionIndices(ClothingCookedPhysX3Param* cookedData, PxU32 subMeshIndex) const
{
	...
	arrayHandle.resizeArray(selfCollisionIndices.size());
	arrayHandle.setParamU32Array(selfCollisionIndices.begin(), selfCollisionIndices.size());
}
...
bool CookingPhysX3::verifyValidity(const ClothingCookedPhysX3Param* cookedData, PxU32 subMeshIndex)
{
	...
	const PxU32 numSetsDescs			= cookedData->deformableSets.arraySizes[0];
