	...
	float* g_benefit, InplaceHandle<SimulationParams> simParamsHandle, FieldAccessor& fieldAccessor)
{
	...
	const SimulationParams* simParams = simParamsHandle.resolve( KERNEL_CONST_MEM(simulateConstMem) );
	const InjectorParamsArray* injectorParamsArray = injectorParamsArrayHandle.resolve( KERNEL_CONST_MEM(simulateConstMem) );
	const InjectorParams* injectorParamsElems = injectorParamsArray->getElems( KERNEL_CONST_MEM(simulateConstMem) );
	...
	for (unsigned int dstIdx = BlockSize*blockIdx.x + threadIdx.x; dstIdx < targetCount; dstIdx += BlockSize*gridDim.x)
	{
		...
		float benefit = simulateParticle(
			simParams, KERNEL_CONST_MEM(simulateConstMem), injectorParamsElems,
...
BOUND_KERNEL_BEG(SIMULATE_WARPS_PER_BLOCK, simulateKernel,
	unsigned int lastCount, float deltaTime, physx::PxVec3 gravity, physx::PxVec3 eyePos,
	InplaceHandle<InjectorParamsArray> injectorParamsArrayHandle, unsigned int injectorCount, 
	unsigned int* g_holeScanSum, unsigned int* g_moveCount, unsigned int* g_tmpHistogram, unsigned int* g_InjectorsCounters,  
	float4* g_positionMass, float4* g_velocityLife, float4* g_collisionNormalFlags, unsigned int* g_userData,
...
BOUND_KERNEL_BEG(SIMULATE_WARPS_PER_BLOCK, simulateApplyFieldKernel,
	unsigned int lastCount, float deltaTime, physx::PxVec3 gravity, physx::PxVec3 eyePos,
	InplaceHandle<InjectorParamsArray> injectorParamsArrayHandle, unsigned int injectorCount, 
	unsigned int* g_holeScanSum, unsigned int* g_moveCount, unsigned int* g_tmpHistogram, unsigned int* g_InjectorsCounters,
	float4* g_positionMass, float4* g_velocityLife, float4* g_collisionNormalFlags, unsigned int* g_userData,
...
BOUND_KERNEL_BEG(STATE_WARPS_PER_BLOCK, stateKernel,
