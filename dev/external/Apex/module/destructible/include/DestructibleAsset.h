...
public:
	...
	void								setAssetName( const char * assetName ); // ctremblay +- Hack to override name
...
private:
	...
	physx::Array< physx::Array<NxConvexMesh*>	>	mConvexMeshSets;	// [scale#][part#]
};
...
//		OFFSET_FN(ScatterInstanceBufferDataElement, alpha)
	...
	{
		ScatterMeshInstanceInfo() : m_actor(NULL), m_instanceBuffer(NULL) {}
	...
	physx::PxU32						getInstancedChunkCount() const
	{
		return m_instancedChunkRenderMeshActors.size();
	}

	physx::PxU32						getChunkCount() const
	{
		return mParams->chunks.arraySizes[0];
	}
	...
	physx::PxI32 getChunkChild(physx::PxU32 chunkIndex, physx::PxU32 childIndex) const
	{
		...
		return (physx::PxU32)mParams->chunks.buf[chunkIndex].firstChildIndex + childIndex;
	}
	...
	CachedOverlapsNS::IntPair_DynamicArray1D_Type*	getOverlapsAtDepth(physx::PxU32 depth, bool create = true);
	...
	void								setChunkOverlapsCacheDepth(bool enabled, physx::PxI32 depth = -1);
	...
	*/
	...
	{
		return mParams->actorTransforms.arraySizes[0];
	}
...
protected:
	...
	void calculateChunkOverlaps(physx::Array<IntPair>& overlaps, physx::PxU32 depth);
...
public:
	...
	void	cookChunks(const NxDestructibleAssetCookingDesc&);
	...
	void	serializeFractureToolState(physx::PxFileBuf& stream, physx::IExplicitHierarchicalMesh::IEmbedding& embedding) const;
	void	deserializeFractureToolState(physx::PxFileBuf& stream, physx::IExplicitHierarchicalMesh::IEmbedding& embedding);
