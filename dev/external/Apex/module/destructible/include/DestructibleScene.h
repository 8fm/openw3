...
#endif
#if APEX_USE_GRB
class DestructibleGrbContactReport : public GrbUserContactReport
{
public:
	DestructibleGrbContactReport() : destructibleScene(NULL) {}

	void onContactNotify(NxU32 arraySizes, void** shape0Array, void** shape1Array, void** actor0Array, void** actor1Array, float* positionArray, float* normalArray);
...
struct ActorFIFOEntry
{
	...
	//++LAVA -- initializer for fadeAlpha
	ActorFIFOEntry() : actor(NULL), origin(0.0f), age(0.0f), benefitCache(PX_MAX_F32), maxSpeed(0.0f), averagedLinearVelocity(0.0f), averagedAngularVelocity(0.0f), unscaledMass(0.0f), flags(0), fadeAlpha(1.0f) {}
	ActorFIFOEntry(NxActor* inActor, physx::PxF32 inUnscaledMass, physx::PxU32 inFlags) : actor(inActor), age(0.0f), benefitCache(PX_MAX_F32), maxSpeed(0.0f),
		averagedLinearVelocity(0.0f), averagedAngularVelocity(0.0f), unscaledMass(inUnscaledMass), flags(inFlags), fadeAlpha(1.0f)
	{
...
#if APEX_USE_GRB
	virtual void				createGRBScene();
	virtual void				releaseGRBScene();
	virtual void				simulate(PxF32 deltaTime);
	virtual void				postPhysX(physx::PxF32 maxTimestep, physx::PxU32 maxIter);
	virtual bool				isGrbSimulationEnabled() const;
	virtual void				setGrbSimulationEnabled(bool enabled);
#else
	        bool				isGrbSimulationEnabled() const { return false; }
#endif
...
private:
...
#if APEX_USE_GRB
	bool						mGrbSimulationEnabled;
	DestructibleGrbContactReport mGrbContactReport;
	...
	PxScene*					mGrbScene;
#endif
	DestructiblePostPhysX*		mPostPhysXTask;
	bool						mGrbSimulating;
	bool						mGrbInteropEnabled;
#endif
	...
	physx::PxU64				mGRBSimulateTickCount;
...
public:
	...
	void						addActor(NiApexPhysXObjectDesc& desc, NxActor& actor, physx::PxF32 unscaledMass, bool isDebris, bool isGRB);
	...
	PX_INLINE void					createChunkReportDataForFractureEvent(const FractureEvent& fractureEvent, DestructibleStructure::Chunk& chunk)
	{
		PX_ASSERT(fractureEvent.damageEventIndex < mDamageEventReportData.size());
		if (fractureEvent.damageEventIndex < mDamageEventReportData.size())
		{
			ApexDamageEventReportData& damageReportData = mDamageEventReportData[fractureEvent.damageEventIndex];
			...
			handle.set((physx::PxI32)fractureEvent.damageEventIndex, (physx::PxU32)fractureEventIndex);
		}
	}
	...
	physx::Array<physx::PxU32>				mDamageEventReportIndices;	// For each destructible actor in the scene.  This is temporary data.
	...
	public:
		VisibleChunkSetDescendents(physx::PxU32 chunkIndex, bool inIsGRB, bool makeDynamic) 
		...
		bool execute(DestructibleStructure* structure, DestructibleStructure::Chunk& chunk)
		{
			...
			chunk.visibleAncestorIndex = visibleChunkIndex;
			return true;
		}
	};
...
PX_INLINE void DestructibleScene::forSubtree(DestructibleStructure::Chunk& chunk, ChunkOpClass chunkOp, bool skipRoot)
{
	...
	for (;;)
	{
		...
		{
			for (indexInAsset = source.firstChildIndex + source.numChildren; --indexInAsset > source.firstChildIndex;)
			{
...
PX_INLINE physx::PxF32 DestructibleScene::calculateBenefit(const DestructibleActorParamNS::LODWeights_Type& weights, const physx::PxVec3& center, physx::PxF32 radiusSquared, const physx::PxVec3& eyePos, physx::PxF32 age) const
{
	...
	return weights.distanceWeight * distanceBenefit + weights.ageWeight * ageBenefit;
}
