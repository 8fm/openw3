...
{
	val = setFlag ? (val | (T)flag) : (val & ~(T)flag);
}
			...
			AssignType assignType = Default)
{
	...
	assignFlags(descA.contactReportFlags, PxPairFlag::eRESOLVE_CONTACTS,                   descB0.contactReportFlags.eRESOLVE_CONTACTS,                assignType);
	...
	assignFlags(descA.contactReportFlags, PxPairFlag::eCCD_LINEAR,						   descB0.contactReportFlags.eCCD_LINEAR,                      assignType);
						 ...
						 NxParameterized::Interface& nxOut)
{
	if (isType(&nxOut, NxDestructibleParameterizedType::Params))
	{
		...
		nxOutParams.resizeArray(handle, nxIn.depthParametersCount);
						   ...
						   NxDestructibleParameters& nxOut)
{
	...
	nxOut.depthParametersCount = nxIn2.arraySizes[0];
						 ...
						 DestructibleActorChunks& nxOut)
{
	...
	nxOut.resizeArray(handle, nxInCount);
