...
public:
	...
	bool updateMassFromShapes(NxReal density, NxReal totalMass)
	{
		...
		if (rigidBody && density == 0.0f)
		{
			...
			PxRigidBodyExt::setMassAndUpdateInertia(*rigidBody, totalMass, NULL);
	...
	bool isSleeping() const
	{
		...
		if (rigidDynamic)
		{
			...
			if( !rigidDynamic->getScene() ) return true;
	...
	void setToBeDestroyed(bool destroyFlag)
	{
		setActorFlag( PxActorFlag::eVISUALIZATION, !destroyFlag );
		setActorFlag( PxActorFlag::eDISABLE_SIMULATION, destroyFlag );
		mForceDestroy = destroyFlag;
	}

	bool getIsToBeDestroyed()
	{
		return mForceDestroy;
	}
};
