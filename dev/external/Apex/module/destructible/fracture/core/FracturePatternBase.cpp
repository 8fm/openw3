...
{
	clear();
	shdfnd::Array<PxVec3> points(numCells);
	...
	d->tetrahedralize(&points[0], points.size(), sizeof(PxVec3), true);
...
void FracturePattern::createCrack(const PxVec3 &dims, float spacing, float zRand)
{
	...
	d->triangulate(&points[0], points.size(), sizeof(PxVec3), true);
	d->computeVoronoiMesh();
	...
	for (int side = 0; side < 2; side++) {
		for (int i = 0; i < (int)convexes.size(); i++) {
			...
			for (int j = 0; j < c2.numNeighbors; j++) {
				int n = neighbors[c2.firstNeighbor + j] + 1;
				if (side == 0) 
			...
			if (side == 0) {
				mGeom.neighbors.pushBack(1 + numConvexes + i);
				mGeom.neighbors.pushBack(0);
			}
			else {
				mGeom.neighbors.pushBack(1 + i);
...
void FracturePattern::create2dVoronoi(const PxVec3 &dims, int numCells, float biasExp, int numRays)
{
	...
	for (int i = 0; i < (int)convexes.size(); i++) {
		...
		c3.firstVert = mGeom.vertices.size();
		c3.numVerts = 2 * c2.numVerts;
		for (int j = 0; j < c2.numVerts; j++) {
			PxVec3 p = verts[c2.firstVert+j];
		...
		c3.firstIndex = mGeom.indices.size();
		c3.numFaces = c2.numVerts + 2;
		mGeom.indices.pushBack(c2.numVerts);
...
void FracturePattern::createGlass(float radius, float thickness, int numSectors, float sectorRand, float firstSegmentSize, float segmentScale, float segmentRand)
{
	...
	shdfnd::Array<float> secAngles(numSectors);
	...
	for (int i = 0; i < numSectors; i++) {
		...
		for (int j = 0; j < numSegs; j++) {
			...
			mGeom.neighbors.pushBack(i > 0 ? (i-1) * numSegs + j : (numSectors-1) * numSegs + j);
			mGeom.neighbors.pushBack(j < numSegs-1 ? i * numSegs + j+1 : -1);
			mGeom.neighbors.pushBack(i < numSectors-1 ? (i+1) * numSegs + j : j);
			mGeom.neighbors.pushBack(j > 0 ? i * numSegs + j-1 : -1);
	...
	for (int zi = 0; zi < numZ; zi++) {
		for (int yi = 0; yi < numY; yi++) {
			for (int xi = 0; xi < numX; xi += 5) {
				...
				mFirstConvexOfCell.pushBack(mGeom.convexes.size());
...
void FracturePattern::getConvexIntersection(const Convex *convex, const PxMat44 &trans, float minConvexSize, int /*numFitDirections*/) const
{
	...
	for (int i = 0; i < (int)convexPlanes.size(); i++) {
		...
		for (int j = 0; j < (int)candidates.size(); j++) {
			const CompoundGeometry::Convex &c = mGeom.convexes[candidates[j]];
			bool inside = false;
			for (int k = 0; k < c.numVerts; k++) {
				PxVec3 p = trans.transform(mGeom.vertices[c.firstVert + k]);
	...
	for (int i = 0; i < (int)candidates.size(); i++) {
		...
		bool empty = true;
		c->intersectWithConvex(&mGeom.planes[gc.firstPlane], gc.numFaces, trans, empty);
		...
		for (int i = 0; i < (int)newConvexes.size(); i++) {
			...
			mFirstPiece[convexNr] = mPieces.size();
...
//		}
	...
	int  numCells = complexCells ? mFirstConvexOfCell.size()-1 : mGeom.convexes.size();

	for (int i = 0; i < numCells; i++) {
		int numConvexes = complexCells ? mFirstConvexOfCell[i+1] - mFirstConvexOfCell[i] : 1;
		...
		for (int k = 0; k < numConvexes; k++) {
			int convexNr = complexCells ? mFirstConvexOfCell[i] + k : i;
		...
		if (!hasGhosts) {
			int num = 0;
			for (int j = 0; j < (int)cellConvexes.size(); j++) {
		...
		if (splitCell) {			// each convex piece within the pattern piece becomes a separate compound
			...
			// create single piece
			Convex *c = mScene->createConvex();
			c->createFromGeometry(mGeom, i, &trans);
			newConvexes.pushBack(c);
			...
			for (int j = 0; j < (int)islands.size(); j++) {
				...
				for (int k = 0; k < is.size; k++) 
					newConvexes.pushBack(cellConvexes[islandConvexes[is.firstNr+k]]);
	...
	if (radius > 0.0f) {
		...
		int oldConvexNr = 0;
		int newConvexNr = 0;
		int numNewCompounds = 0;

		for (int i = 0; i < (int)compoundSizes.size(); i++) {
			int oldCompoundSize = compoundSizes[i];
			int newCompoundSize = 0;
			bool onlyGhosts = true;
			for (int j = 0; j < oldCompoundSize; j++) {
			...
			if (newCompoundSize > 0) {
				if (onlyGhosts) {		// remove compounds that contain only ghost convexes
					for (int j = 0; j < newCompoundSize; j++) {
						newConvexNr--;
						PX_DELETE(newConvexes[newConvexNr]);
					}
				}
				else {
					compoundSizes[numNewCompounds] = newCompoundSize;
					numNewCompounds++;
				}
			}
	...
	if (farConvexes.size() > 0) {
		...
		for (int i = 0; i < (int)islands.size(); i++) {
			...
			for (int j = 0; j < island.size; j++) {
				int nr = islandConvexes[island.firstNr + j];
				newConvexes.pushBack(farConvexes[nr]);
			}
		}
