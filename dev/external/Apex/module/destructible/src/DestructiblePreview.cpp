	...
	m_api(_api)
{
	...
	renderableMeshDesc.overrideMaterialCount	= destructiblePreviewParams->overrideSkinnedMaterialNames.arraySizes[0];
	...
	if (m_drawUnexpandedChunksStatically)
	{
		...
		renderableMeshDesc.overrideMaterialCount	= destructiblePreviewParams->overrideStaticMaterialNames.arraySizes[0];
	...
	tempPartToActorMap.resize(m_asset->mParams->chunks.arraySizes[0], 0xFFFF);

	physx::PxU16 instancedChunkCount = 0;

	m_instancedChunkActorMap.resize(m_asset->mParams->chunkInstanceInfo.arraySizes[0]);
	...
	m_instancedActorVisiblePart.resize(instancedChunkCount);
		...
		while (indexPtr < indexPtrStop)
		{
			...
			if (index < m_asset->getChunkCount())
			{
				...
				else
				{
					...
					extents = physx::PxVec3(abs(instanceDataElement.scaledRotation(0, 0) * extents.x) + abs(instanceDataElement.scaledRotation(0, 1) * extents.y) + abs(instanceDataElement.scaledRotation(0, 2) * extents.z),
											abs(instanceDataElement.scaledRotation(1, 0) * extents.x) + abs(instanceDataElement.scaledRotation(1, 1) * extents.y) + abs(instanceDataElement.scaledRotation(1, 2) * extents.z),
											abs(instanceDataElement.scaledRotation(2, 0) * extents.x) + abs(instanceDataElement.scaledRotation(2, 1) * extents.y) + abs(instanceDataElement.scaledRotation(2, 2) * extents.z));
