...
void DestructibleActor::initializeCommon()
{
	...
	mDescOverrideSkinnedMaterialCount = getParams()->overrideSkinnedMaterialNames.arraySizes[0];
	mDescOverrideStaticMaterialCount  = getParams()->overrideStaticMaterialNames.arraySizes[0];
	...
	if(mDestructibleScene->mGrbScene)
	{
		for(Array<PxMaterial *>::ConstIterator i = p3Desc.materials.begin(); i < p3Desc.materials.end(); ++i)
		{
			PxMaterial *pxmaterial = *i;
			PxMaterial *grbMaterial = mDestructibleScene->mGrbScene->getPhysics().createMaterial(pxmaterial->getStaticFriction(), pxmaterial->getDynamicFriction(), pxmaterial->getRestitution());
			...
			grbMaterials.pushBack(grbMaterial);
		}
	}
#endif
#endif
...
void DestructibleActor::initializeActor(void)
{
	...
	if (drawStaticChunksInSeparateMesh() && !(getFlags() & Dynamic))
	{
		...
		else
		{
			...
			renderableMeshDesc.overrideMaterialCount = mAsset->mParams->staticMaterialNames.arraySizes[0];
	...
	//#else
	mRenderable = PX_NEW(DestructibleRenderable)(renderMeshActors,getAsset(),m_listIndex);
	//#endif
	...
	if (mUseDamageColoring)
	{
		...
		for (physx::PxU32 submeshIndex = 0; submeshIndex < rma->getSubmeshCount(); ++submeshIndex)
		{
			...
			if (vb.getVertexCount() != 0)
			{
				...
				{
					vb.getBufferData(&damageColorArray[0], NxRenderDataFormat::R8G8B8A8, 0, colorBufferIndex, 0, vb.getVertexCount());
				}
...
void DestructibleActor::decrementWakeCount(void)
{
	...
	// this value is compeltely out of sync and that can result in bad things (performance loss
 ...
 * NxScene reference changes.  nxScene can be NULL if the NxApexScene is losing it's NxScene reference.
 */
...
void DestructibleActor::destroy()
{
	...
	// BRG_GRB
	for(Array<PxMaterial *>::Iterator i = grbMaterials.begin(); i < grbMaterials.end(); ++i)
	{
		(*i)->release();
	}
	grbMaterials.clear();
...
void DestructibleActor::initializeChunk(physx::PxU32 index, DestructibleStructure::Chunk& target) const
{
	...
	{
		target.state               = isInitiallyDynamic() ? ChunkDynamic : 0;
		target.flags               = 0;
		target.damage              = 0;
...
{
#if NX_SDK_VERSION_MAJOR == 3
	SCOPED_PHYSX_LOCK_READ(*mStructure->dscene->mApexScene);
#endif
...
//		computeOverlapLineSegmentAABBCache(segmentCache, worldDisplacement);
	...
	IndexedReal* chunkProjectedMinima = (IndexedReal*)NxAlloca(totalVisibleChunkCount * sizeof(IndexedReal));
	physx::PxU32* chunkIndices = (physx::PxU32*)NxAlloca(totalVisibleChunkCount * sizeof(physx::PxU32));
	physx::PxMat34Legacy* chunkTMs = (physx::PxMat34Legacy*)NxAlloca(totalVisibleChunkCount * sizeof(physx::PxMat34Legacy));
...

	{
		...
		while (parentChunkIndex != NxModuleDestructibleConst::INVALID_CHUNK_INDEX)
		{
			...
			{
				const physx::PxU16 childIndexInAsset = source.firstChildIndex + childNum;
        ...
        NxDestructibleActorRaycastFlags::Enum flags, physx::PxI32 parentChunkIndex) const
{
	...
	if (accurateRaycasts)
	{
		const DestructibleStructure::Chunk& chunkFound = getStructure()->chunks[chunkFoundIndex];
...
void DestructibleActor::applyDamage_immediate(DamageEvent& damageEvent)
{
	...
	if (damageEvent.isFromImpact() && chunkIndexInAsset != NxModuleDestructibleConst::INVALID_CHUNK_INDEX)
	{
		DestructibleStructure::Chunk& chunk = mStructure->chunks[chunkIndexInAsset + mFirstChunkIndex];
		if ((chunk.state & ChunkVisible) != 0)
		{
			physx::PxF32 time = 0;
			NxRay worldRay(NXFROMPXVEC3(damageEvent.position).isFinite() ? NXFROMPXVEC3(damageEvent.position) : NxVec3(0.0f), 
			...
			physx::PxVec3 n;
			if (rayCast(time, n, worldRay, NxDestructibleActorRaycastFlags::AllChunks, chunkIndexInAsset) != NxModuleDestructibleConst::INVALID_CHUNK_INDEX)
			{
				damageEvent.position += time * damageEvent.direction;
			}
		}
	}
	physx::PxF32 time = 0;
	...
	chunkIndexInAsset = rayCast(time, fractureNormal, worldRay, NxDestructibleActorRaycastFlags::AllChunks, chunkIndexInAsset);
	...
	physx::Array<DestructibleStructure::Chunk*> chunkList;
	...
	for (physx::PxU32 i = 0; i < chunkList.size(); ++i)
	{
		DestructibleStructure::Chunk& chunk = *chunkList[i];
		if ((chunk.state & ChunkVisible) != 0)
		{
			// Damage coloring:
...
void DestructibleActor::applyRadiusDamage_immediate(DamageEvent& damageEvent)
{
	...
	while (chunkIndexPtr < chunkIndexPtrStop)
	{
		...
		if (!getStructure()->dscene->getModule()->getUseLegacyDamageRadiusSpread())
		{
			// New behavior
...
void DestructibleActor::updateRenderResources(bool rewriteBuffers, void* userRenderData)
{
	...
	if (mRenderable->getReferenceCount() == 1)
	{
...
void DestructibleActor::fillInstanceBuffers()
{
	...
	while (indexPtr < indexPtrStop)
	{
		...
		if (index < mAsset->getChunkCount())
		{
			...
			// Scatter meshes
			const physx::PxU16 scatterMeshStop = sourceChunk.firstScatterMesh + sourceChunk.scatterMeshCount;
...
//				instanceDataElement.alpha = 1.0f;	// alpha = 1 for now
				instanceBufferData.pushBack(instanceDataElement);
			...
			if (index < mAsset->getChunkCount())
			{
				...
				if ((sourceChunk.flags & DestructibleAsset::Instanced) == 0)
				{
					if (getDynamic(index) || !drawStaticChunksInSeparateMesh() || initializedFromState())
					{
						...
						{
							// chunkPose = controlledChunkPose ? controlledChunkPose : calculatedChunkPose
							DestructibleStructure::Chunk & chunk = mStructure->chunks[mFirstChunkIndex + index];
							if(mSyncParams.isSyncFlagSet(NxDestructibleActorSyncFlags::CopyChunkTransform) && (NULL != mSyncParams.getChunkSyncState()))
							{
								const NxDestructibleChunkSyncState & chunkSyncState = *(mSyncParams.getChunkSyncState());
								PX_ASSERT(NULL != &chunkSyncState);
								if( getDynamic(index) &&
									!chunkSyncState.disableTransformBuffering &&
#if NX_SDK_VERSION_MAJOR == 2
							...
							if(mSyncParams.isSyncFlagSet(NxDestructibleActorSyncFlags::ReadChunkTransform) && (NULL != chunk.controlledChunk))
							{
						...
						//++LAVA
						{
							...
							NiApexPhysXObjectDesc* actorObjDesc = (NiApexPhysXObjectDesc*)mStructure->dscene->mModule->mSdk->getPhysXObjectInfo(actor);
							const uintptr_t cindex = (uintptr_t)actorObjDesc->userData;
							physx::PxF32 alpha = 1.0f;
							if (cindex != 0)
							{
								if (!actor->readBodyFlag(NX_BF_KINEMATIC))
								{
									PX_ASSERT(mStructure->dscene->mActorFIFO[(physx::PxU32)~cindex].actor == actor);
									ActorFIFOEntry& FIFOEntry = mStructure->dscene->mActorFIFO[(physx::PxU32)~cindex];
									alpha = FIFOEntry.fadeAlpha;
								}
							}
							skinnedRMA->setChunkAlpha(alpha, sourceChunk.meshPartIndex);
						}
						//--LAVA
					}
				}
...
{
	//++LAVA -- reduced, from nvidia suggestion
	pose = mRelTM * mTM;
	return true;
	//--LAVA
}
...
void DestructibleActor::getPhysicalLodRange(physx::PxF32& min, physx::PxF32& max, bool& intOnly)
{
	min = 0.0f;
...
physx::PxF32 DestructibleActor::getActivePhysicalLod()
{
...
void DestructibleActor::setDynamic(physx::PxI32 chunkIndex)
{
	if (chunkIndex == NxModuleDestructibleConst::INVALID_CHUNK_INDEX)
	{
		...
		while (indexPtr < indexPtrStop)
		{
			...
			if ((chunk.state & ChunkDynamic)==0)
			{
				FractureEvent& fractureEvent = mStructure->dscene->mFractureBuffer.pushBack();
	...
	{
		FractureEvent& fractureEvent = mStructure->dscene->mFractureBuffer.pushBack();
		fractureEvent.chunkIndexInAsset = chunkIndex;
		fractureEvent.destructibleID = mID;
		fractureEvent.impulse = physx::PxVec3(0.0f);
		fractureEvent.position = physx::PxVec3(0.0f);
		fractureEvent.flags = FractureEvent::Forced | FractureEvent::Silent;
...
#endif
{
	...
	for (physx::PxU32 chunkNum = 0; chunkNum < mVisibleChunks.usedCount(); ++chunkNum)
	{
		DestructibleStructure::Chunk& chunk = mStructure->chunks[(physx::PxU32)mVisibleChunks.usedIndices()[chunkNum] + mFirstChunkIndex];
		NxActor* actor = (NxActor*)mStructure->getChunkActor(chunk);
		...
		if (eliminateRedundantActors)
		{
			if (objDesc->userData != &chunk)	// This ensures we only get the actor once
			{
				continue;
			}
		}
		...
		{
			// Chunk is static
			if (flags & NxDestructiblePhysXActorQueryFlags::Static)
			{
#if NX_SDK_VERSION_MAJOR == 2
		...
		else
		{
			...
			if (((flags & NxDestructiblePhysXActorQueryFlags::Dormant) && isKinematic) || ((flags & NxDestructiblePhysXActorQueryFlags::Dynamic) && !isKinematic))
			{
#if NX_SDK_VERSION_MAJOR == 2
				mPhysXActorBuffer.pushBack(actor);
#elif NX_SDK_VERSION_MAJOR == 3
...
//	const physx::PxF32 minRadiusSquared = behaviorGroup.damageColorSpread.minimumRadius*behaviorGroup.damageColorSpread.minimumRadius;
	...
	for (physx::PxU32 submeshIndex = 0; submeshIndex < rma->getSubmeshCount(); ++submeshIndex)
	{
		...
		{
			NxRenderDataFormat::Enum positionBufferFormat = vf.getBufferFormat(positionBufferIndex);
			PX_ASSERT(positionBufferFormat == NxRenderDataFormat::FLOAT3);
			if (positionBufferFormat != NxRenderDataFormat::FLOAT3)
			{
				continue;
			}
			const physx::PxVec3* positions = (const physx::PxVec3*)vb.getBuffer(positionBufferIndex);
	...
	// Recurse to children
	const physx::PxU32 stopIndex = source.firstChildIndex + source.numChildren;
...
void DestructibleActor::spawnParticles(NxApexEmitterActor* emitter, NxUserChunkParticleReport* report, DestructibleStructure::Chunk& chunk, physx::Array<physx::PxVec3>& positions, bool deriveVelocitiesFromChunk, const physx::PxVec3* overrideVelocity)
{
	...
	else
	{
		...
		if (report != NULL)
		{
			...
			reportData.velocityCount = overrideVelocity != NULL ? 1 : 0;
			report->onParticleEmission(reportData);
		}
	}
#else
...
{
	PX_ASSERT(NULL != actorSyncState ? (isSyncFlagSet(NxDestructibleActorSyncFlags::CopyDamageEvents) || isSyncFlagSet(NxDestructibleActorSyncFlags::CopyFractureEvents)) : true);
	return actorSyncState;
}

const NxDestructibleChunkSyncState * SyncParams::getChunkSyncState() const
{
	PX_ASSERT(NULL != chunkSyncState ? isSyncFlagSet(NxDestructibleActorSyncFlags::CopyChunkTransform) : true);
	return chunkSyncState;
}
...
// ++ LAVA ++
physx::PxF32 DestructibleActor::getActorDistanceSquaredFromViewport( physx::PxU32 index )
{
	if( !mStructure ) return -1.0f;
	return mStructure->getActorDistanceSquaredFromViewport( index );
}

bool DestructibleActor::setActorChunksToBeDestroyed(NxActor& actor)
{
	DestructibleScene* scene = getDestructibleScene();
	if( !scene ) return false;
	actor.setToBeDestroyed( true );
	return true;
}
