...
NxActor* DestructibleScene::createRoot(DestructibleStructure::Chunk& chunk, const physx::PxMat34Legacy& pose, bool dynamic, physx::PxMat34Legacy* relTM, bool fromInitialData)
{
	...
	NxConvexShapeDesc* convexShapeDescs = (NxConvexShapeDesc*)NxAlloca(sizeof(NxConvexShapeDesc) * hullCount);
	...
	for (physx::PxU32 hullIndex = 0; hullIndex < hullCount; ++hullIndex)
	{
		...
		convexShapeDesc.meshData = collisionSet->getConvexMesh(hullIndex + firstHullIndex, destructible->getScale());
	...
	if (usingGRB)
	{
		...
		// The mass is set explicitly
		ractorDesc.density = 0.0;
		ractorDesc.body = &bodyDesc;
		...
		newActor = mModule->createGrbActor(ractorDesc, *mApexScene);
	...
	if (newActor)
	{
		...
		if (!newActor->readBodyFlag(NX_BF_KINEMATIC))
		{
			...
			addActor(*actorObjDesc, *newActor, actorMass, isDebris, usingGRB);
		}
