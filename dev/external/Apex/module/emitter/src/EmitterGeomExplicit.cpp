...
EmitterGeomExplicit::EmitterGeomExplicit(NxParameterized::Interface* params)
{
	...
	// get points
	mPoints.resize(mGeomParams->points.positions.arraySizes[0]);
	if (!mPoints.empty())
	{
	...
	for (physx::PxI32 i = 0; i < mGeomParams->points.velocities.arraySizes[0]; i++)
	{
		mVelocities.pushBack(mGeomParams->points.velocities.buf[i]);
	}

	// get spheres
	mSpheres.resize(mGeomParams->spheres.positions.arraySizes[0]);
	if (!mSpheres.empty())
	{
	...
	// get ellipsoids
	mEllipsoids.resize(mGeomParams->ellipsoids.positions.arraySizes[0]);
...
void EmitterGeomExplicit::updateAssetPoints()
{
	...
	pError = h.resizeArray(mPoints.size());
	PX_ASSERT(pError == NxParameterized::ERROR_NONE);
	...
	{
		pError = mGeomParams->resizeArray(h, mVelocities.size());
		PX_ASSERT(pError == NxParameterized::ERROR_NONE);

		pError = h.setParamVec3Array(&mVelocities[0], mVelocities.size());
		PX_ASSERT(pError == NxParameterized::ERROR_NONE);
	}
}
    ...
    const physx::PxVec3* velocities)
{
	...
	pError = h.resizeArray(mSpheres.size());
	PX_ASSERT(pError == NxParameterized::ERROR_NONE);
	...
	if (pError == NxParameterized::ERROR_NONE)
	{
		pError = mGeomParams->resizeArray(h, mSphereVelocities.size());
		PX_ASSERT(pError == NxParameterized::ERROR_NONE);

		h.setParamVec3Array(&mSphereVelocities[0], mSphereVelocities.size());
	}
}
    ...
    const physx::PxVec3* velocities)
{
	...
	pError = h.resizeArray(mEllipsoids.size());
	PX_ASSERT(pError == NxParameterized::ERROR_NONE);
	...
	if (pError == NxParameterized::ERROR_NONE)
	{
		pError = mGeomParams->resizeArray(h, mEllipsoidVelocities.size());
		PX_ASSERT(pError == NxParameterized::ERROR_NONE);

		h.setParamVec3Array(&mEllipsoidVelocities[0], mEllipsoidVelocities.size());
	}
}
    ...
    QDSRand& rand) const
{
	...
	{
		physx::PxVec3 posedPos;
		pose.multiply(randPos, posedPos);
		positions.pushBack(posedPos);
	}
