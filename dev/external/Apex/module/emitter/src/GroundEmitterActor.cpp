...
void GroundEmitterActor::getPhysicalLodRange(physx::PxF32& min, physx::PxF32& max, bool& intOnly)
{
	PX_UNUSED(min);
	PX_UNUSED(max);
	PX_UNUSED(intOnly);
...
physx::PxF32 GroundEmitterActor::getActivePhysicalLod()
{
    ...
    const NxEmitterLodParamDesc& lodDesc)
{
	...
	if (injectorIndex >= mInjectorList.size())
	{
		...
		NxIosAsset* iosAsset = mAsset->mIosAssetTracker.getIosAssetFromName(desc.instancedObjectSimulationTypeName,
		...
		NxApexActor* nxActor = iosAsset->createIosActor(*mScene->mApexScene, desc.instancedObjectEffectsAssetName);
		...
		data->mInjector = ios->allocateInjector(desc.instancedObjectEffectsAssetName);
		if (!data->mInjector)
		{
			delete data;
			return false;
...
#define MIN_CELL_REFRESH_STEPS 12
	...
	physx::PxU32 gridDim = physx::PxMax(yrange.maximum - yrange.minimum, xrange.maximum - xrange.minimum);
...
//		PxMaterial*	materail	= hit.shape->getMaterialFromInternalFaceIndex(hit.faceIndex);
		physx::PxU16 matIndex = matRequest ? (physx::PxU16) matRequest[i].outMaterialID : 0;//materail;	// apan, fixme, hard code to 0
		if (mPerMaterialData.isValid(matIndex) &&
