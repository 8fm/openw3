...

{
	...
	NxApexActor* nxActor = iosAsset->createIosActor(*(mOwner->getApexScene()), eventPtr->parameters().iofxAssetName->name());
	...
	mParticleInjector = iosActor->allocateInjector(eventPtr->parameters().iofxAssetName->name());
...
void ImpactEmitterActor::getPhysicalLodRange(physx::PxF32& min, physx::PxF32& max, bool& intOnly)
{
	PX_UNUSED(min);
	PX_UNUSED(max);
	PX_UNUSED(intOnly);
...
physx::PxF32 ImpactEmitterActor::getActivePhysicalLod()
{
