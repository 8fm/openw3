...
typedef enum {
  ...
  /**
   * Get theoretical instructions per cycle. Value is a
   * uint32_t.
   */
  CUPTI_DEVICE_ATTR_INSTRUCTION_PER_CYCLE = 4,
  /**
   * Get theoretical number of single precision instructions that
   * can be executed per second. Value is a uint64_t.
   */
...
typedef enum {
   ...
   * completes. If multiple kernel executions occur between the
 ...
 * groups created in the \p context.
