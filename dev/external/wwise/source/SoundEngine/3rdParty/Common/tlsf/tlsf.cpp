...
#define CLEAN_MEMORY (0xCD)
#define DEAD_MEMORY (0xDD)
...
#ifdef _DEBUG
		memset(p, CLEAN_MEMORY, block_size(block));
#endif
...
{
	// Don't check the NULL pointer.
	if (ptr == NULL)
		return 0;

	const pool_t* pool = tlsf_cast(pool_t*, tlsf);
	const block_header_t* block_first =
		offset_to_block(pool, sizeof(pool_t) - block_header_overhead);
	const block_header_t* block = block_from_ptr(ptr);
	const block_header_t* block_loop = block;

	int status = 0;
	int i;

	// check the heap in both directions

	const int prev_check_count = 4;
	const int next_check_count = 4;

	for (i = 0; i < prev_check_count; i++)
	{
		if (!block_is_prev_free(block_loop))
			break;

		const block_header_t* prev = block_prev(block_loop);

		tlsf_insist(prev >= block_first && "prev block out of range");

		tlsf_insist(block_next(prev) == block_loop && "prev's next is wrong");
		tlsf_insist(block_is_free(prev) && "prev is free, but block not marked as such");
	...
	for (i = 0; i < next_check_count; i++)
	{
		tlsf_insist(block_loop >= block_first && "block out of range");

		if (block_is_last(block_loop))
			break;

		const block_header_t* next = block_next(block_loop);

		if (block_is_free(block_loop))
		{
			tlsf_insist(block_is_prev_free(next) && "block is free, but next's prev not marked as such");

			if (block_loop->next_free != &pool->block_null)
			{
				tlsf_insist(block_loop->next_free >= block_first && "next free out of range");
			}

			if (block_loop->prev_free != &pool->block_null)
			{
				tlsf_insist(block_loop->prev_free >= block_first && "prev free out of range");
			}
		}

		if (block_is_prev_free(next))
		{
			tlsf_insist(block_is_free(block_loop) && "next's prev is free, but block not marked as such");

			tlsf_insist(block_prev(next) == block_loop && "next's prev is wrong");
		}
...
{
	//This function is called for big alignments and big allocations.  
	//And it's called only if the ordinary tlsf allocation failed.

	//TLSF will try to allocate a block that is in_ulSize + 2*in_ulAlign + 4 bytes.  
	//This is to avoid searching for a block with the proper characteristics and do the allocation in one shot.
	//For big alignments it is obvious that this can be wasteful.  So when the regular allocation fails,
	//let's try to find a suitable block the "long" way.

	//The algorithm is simple, find a block of the exact size we want and check if 
	//a) it has an aligned portion of the right size.
	//b) the remaining block before the aligned portion is either 0 or larger than 16 bytes (minimum block size)
	//c) the remaining block after either 0 or larger than 16 bytes.

	//Anatomy of a block 
	//-Header-|-Memory------------------------------------------------------|
	//Anatomy of a block with aligned memory
	//-Header-|-Unaligned-|-Header-|-Aligned mem--------|-Header-|-Residual-|
	//Each block must have a header.  The unaligned gap at the beginning might or might not be there.
	//The residual memory at the end might or might not be the there either.

	const ptrdiff_t gap_minimum = tlsf_cast(ptrdiff_t, sizeof(block_header_t));
	pool_t* pool = tlsf_cast(pool_t*, in_pPool);
	size_t adjustedSize = adjust_request_size(in_ulSize, ALIGN_SIZE);
	void *pValidBlock = NULL;

	void *aTries[TLSF_TRIES];	//Let's try TLSF_TRIES times
	int i = -1;
	block_header_t* block = block_locate_free(pool, adjustedSize);
	while(i < TLSF_TRIES && block != NULL && pValidBlock == NULL)
	{
		//Find a block of the right size
		char *p = (char*)block_to_ptr(block);

		//Compute the nearest aligned address
		char *pAligned = (char*)align_ptr(p, in_ulAlign);

		//Compute the gap before the aligned block.
		ptrdiff_t gapBefore = tlsf_cast(ptrdiff_t, tlsf_cast(tlsfptr_t, pAligned) - tlsf_cast(tlsfptr_t, p));

		//Check if there is enough space in the block, from the aligned address.
		if (pAligned + adjustedSize > p + block_size(block) || (gapBefore != 0 && gapBefore < gap_minimum))
		{
			//Not enough space.  
			//Mark the block as used so we don't pick it on the next loop.
			i++;
			block_mark_as_used(block);	
			aTries[i] = p;
			block = block_locate_free(pool, adjustedSize);
		}
		else
		{
			//Check the block before, it must be equal or larger than gap_minimum
			if (gapBefore != 0)
			{
				//If we reached this point, we have a valid block.  But we need to split it.
				block = block_trim_free_leading(pool, block, gapBefore);
			}

			//Check the block after the aligned block, it must be larger than gap_minimum.	
			//If not, include that small block in the returned block anyway.
			ptrdiff_t gapAfter = block_size(block) - adjustedSize;
			if (gapAfter != 0 && gapAfter <= gap_minimum)
				adjustedSize = block_size(block);

			pValidBlock = block_prepare_used(pool, block, adjustedSize);
		}
	}

	//Free all the tries we did
	for(; i >= 0; i--)
		tlsf_free(in_pPool, aTries[i]);
...
void tlsf_free(tlsf_pool tlsf, void* ptr)
{
	...
	if (ptr)
	{
		...
		block_header_t* block = block_from_ptr(ptr);
#ifdef _DEBUG
		memset(ptr, DEAD_MEMORY, block_size(block)); 
#endif 
