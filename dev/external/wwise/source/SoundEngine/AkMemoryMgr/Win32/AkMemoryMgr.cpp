...
static void DeallocatePool( AkMemPool & in_pool )
{
	...
	{
		AK::VirtualFreeHook(in_pool.pcAllocAddress,0,MEM_RELEASE);
	}
	else
	{
...
//----------------------------------------------------------------------------------------------------
		...
		if(in_pvMemAddress == NULL)
		{
			...
			if((in_eAttributes&AkAllocMask) == AkVirtualAlloc)
			{
				...
				{
					pThisPool->pcAllocAddress = static_cast<char*>(AK::VirtualAllocHook(NULL, 
																	ulAdjustedSize + in_ulBlockAlign,
																	MEM_COMMIT,
																	PAGE_READWRITE));
				}
			}
