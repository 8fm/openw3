	...
	else if( operation == OperationID_Opened && m_pMissingFiles )
	{
		...
		if( pAction && pClientFile )
		{
				...
				csAction == L"move/delete" )
			{
				...
				{
					// remove the first 2 slash
					csClientFile.Delete( 0, 2 ); 

					// Replace the client by the root path
					csClientFile.Replace( csClient, itFound->second );
...
int PerforceSourceControl::IsAlive()
{
	...
	else if( m_pOperationDialog == NULL && dwElapsed > 3000 && IsMainThread() && !m_currentOperation.empty() )
	{
