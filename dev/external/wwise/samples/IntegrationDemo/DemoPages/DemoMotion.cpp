...
void DemoMotion::Release()
{
	...
	for ( int i = 0; i < MAX_PLAYERS; i++ )
	{
		if ( m_bPlayerConnected[i] )
		{
			...
			AK::MotionEngine::RemovePlayerMotionDevice( i, AKCOMPANYID_AUDIOKINETIC, AKMOTIONDEVICEID_RUMBLE );
...
{
	const UniversalGamepad* gp = m_pParentMenu->Input()->GetGamepad( in_iPlayerIndex + 1 ); // UniversalInput is 1-based.
...
#endif //AK_WIIU
#else // AK_WII_FAMILY
	if ( AK::MotionEngine::AddPlayerMotionDevice( in_iPlayerIndex, AKCOMPANYID_AUDIOKINETIC, AKMOTIONDEVICEID_RUMBLE, gp->GetExtraParam() ) != AK_Success )
	{
		SetErrorMessage( "Could not set up the motion device for new player" );
	}
#endif	// AK_WII_FAMILY
	...
	AK::MotionEngine::SetPlayerListener( in_iPlayerIndex, in_iPlayerIndex+1 );
...
void DemoMotion::OnPlayerDisconnect( int in_iPlayerIndex )
{
	...
	// Remove the motion device for the player
	AK::MotionEngine::RemovePlayerMotionDevice( in_iPlayerIndex, AKCOMPANYID_AUDIOKINETIC, AKMOTIONDEVICEID_RUMBLE );
...
bool DemoMotion::SetupMotionDevices()
{
	...
	{
		const UniversalGamepad* gp = m_pParentMenu->Input()->GetGamepad( i + 1 ); // UniversalInput is 1-based.
...
#else
		if ( AK::MotionEngine::AddPlayerMotionDevice( i, AKCOMPANYID_AUDIOKINETIC, 
		...
		AK::MotionEngine::SetPlayerListener( i, i+1 );
