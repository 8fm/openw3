                           ...
                           )
{
    ...
    {
        m_pfnGetFormatCallback( in_pSourceFXContext->GetPlayingID(), io_rFormat );
    }
...
AkReal32 CAkFXSrcAudioInput::GetGain()
{
    ...
    if( m_pfnGetGainCallback )
        gain *= m_pfnGetGainCallback( m_pSourceFXContext->GetPlayingID() );
									...
									)
{
    ...
    m_pfnExecCallback( m_pSourceFXContext->GetPlayingID(), io_pBufferOut);

    // Change target gain if necessary (RTPC value)

    AkUInt32 uNumFrames = io_pBufferOut->uValidFrames;
    AkUInt32 uNumChannels = AK::GetNumChannels( io_pBufferOut->GetChannelMask() );
