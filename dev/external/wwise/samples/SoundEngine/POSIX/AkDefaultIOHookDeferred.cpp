...
void* AioFuncRead(void* lpParameter)
{
	...
	{
		// Read the data
		int byteTransferred =  read( fileno(fileHandle), pInfo->pBuffer, pInfo->uRequestedSize );
...
void* AioFuncWrite(void* lpParameter)
{
	...
	{
		// Read the data
		int byteTransferred =  write( fileno(fileHandle), pInfo->pBuffer, pInfo->uRequestedSize );
